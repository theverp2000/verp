  ...
  try {
    res = s.replace(/[\r\n\s]+/g, ' ')
      ...
      .map(arg => {
        ...
        value = value
          ? value.trim() // JSON.parse(`${value.trim().replace(/(?:['`])([^@\."`][^"'`]*)(?:['`])/g, '"$1"')}`)
          : value;
...
export const controllersPerModule = new Map2(); //list
...
export class WebRequest {
  ...
  set params(val: any) {
    this.httpRequest.params = MultiDict.from(val);
  }
  ...
  async _callFunction(...args: any[]) {
    ...
    if (bool(this.endpointArgs)) {
      ...
      update(args[args.length - 1], this.endpointArgs);
    }
    ...
    // @service_model.check
    async function checkedCall(dbName, ...args) {
      // nonlocal firstTime
      ...
      firstTime = false;
      const result = await self.endpoint.run(...args);
...
export class HttpRequest extends WebRequest {
  ...
  async dispatch(res: http.ServerResponse): Promise<WebResponse> {
    ...
    if (!['GET', 'HEAD', 'OPTIONS', 'TRACE'].includes(this.httpRequest.method) && this.endpoint.routing.get('csrf', true)) {// csrf checked by default
      const token = this.params.pop('csrfToken', null);
    ...
    let r: any = await this._callFunction(res, this.params.toDict());
    if (!r) {
...
export class JsonRequest extends WebRequest {
  ...
  async dispatch(res: http.ServerResponse): Promise<WebResponse> {
    const result = await this._callFunction(res, this.params.toDict());
...
export function contentDisposition(filename) {
  ...
  const escaped = encodeURIComponent(filename);//, { safe: '', unsafe: '()<>@,;:"/[]?={}\\*\'%' });  // RFC6266
