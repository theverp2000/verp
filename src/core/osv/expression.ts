...
export class Expression {
  ...
  async parse() {
    ...
    while (stack.length) {
      ...
      if (!field) {
        throw new ValueError("Invalid field %s.%s in leaf %s", model._name, path[0], leaf);
      }
      else if (field.inherited) {
        const parentModel = model.env.items(field.relatedField.modelName);
        const parentFname = model.cls._inherits[parentModel._name];
      ...
      // Applying recursivity on field(one2many)
        ...
        let dom;
        if (field.comodelName != model._name) {
        ...
        if (operator in HIERARCHY_FUNCS) {
          ...
          if (comodel === model) {
            ...
            const params = ids2.length ? ids2 : [null];
            const termId2 = params.map(x => '%s').join(',');
            pushResult(`
              EXISTS (
          ...
          let ids2, termId2, params, subquery;
          // determine ids2 in comodel
          if (typeof right === 'string') {
            const domain = await field.getDomainList(model);
            const op2 = NEGATIVE_TERM_OPERATORS.includes(operator) ? TERM_OPERATORS_NEGATION[operator] : operator;
          ...
          if (isInstance(ids2, Query)) {
            // rewrite condition in terms of ids2
            [termId2, params] = ids2.subselect();
          }
          else {
            // rewrite condition in terms of ids2
            ids2 = ids2.filter(it => it);
            params = ids2.length ? ids2 : [null];
            termId2 = params.map(x => '%s').join(',');
          }
        ...
        if (operator in HIERARCHY_FUNCS) {
          ...
          if (typeof right === 'string' || (Array.isArray(right) && bool(right) && right.every((item) => typeof item === 'string'))) {
            ...
            // right == [] or right == false and all other cases are handled by __leafToSql()
            const [expr, params] = await this.__leafToSql(leaf, model, alias);
            pushResult(expr, params);
          }
        }
  ...
  async __leafToSql(leaf, model, alias): Promise<[string, any[]]> {
    ...
    if (isTrueLeaf(leaf)) {
      ...
      if (typeof right === 'boolean') {
        ...
        if (params?.length) {
          if (left === 'id') {
            instr = params.map(x => '%s').join(',');
          }
          else {
            const field = model._fields[left] as Field;
            instr = params.map(x => field.columnFormat).join(',');
            const _params = [];
