...
export class Query {
   ...
   * Perform a join between a table already present in the current Query object and
        ...
        @param {string} lhsAlias alias of a table already defined in the current Query object.
        @param {string} lhsColumn column of `lhsAlias` to be used for the join's ON condition.
        @param {string} rhsTable name of the table to join to `lhs_alias`.
        @param {string} rhsColumn column of `rhsAlias` to be used for the join's ON condition.
        @param {string} link used to generate the alias for the joined table, this string should
            represent the relationship (the link) between both tables.
        @param {string} extra: an sql string of a predicate or series of predicates to append to the
            join's ON condition, `lhsAlias` and `rhsAlias` can be injected if the string uses
            the `lhs` and `rhs` variables with the `util.format` syntax. e.g.::

                query.join(..., extra="{lhs}.name != {rhs}.name OR ...", ...)

        @param {Array} extraParams a tuple of values to be interpolated into `extra`.
   ...
   * @returns 
   */
  join(lhsAlias: string, lhsColumn: string, rhsTable: string, rhsColumn: string, link: string, options?: {extra?: string, extraParams?: string[]}): string {
    return this._join('JOIN', lhsAlias, lhsColumn, rhsTable, rhsColumn, link, options);
  }

  /**
   * Add a LEFT JOIN to the current table (if necessary), and return the
        alias corresponding to ``rhsTable``.

        See the documentation of method `osv.query.Query.join` for a better overview of the
  ...
  /**
   * Return the SELECT query as a pair ``[queryString, queryParams]``.
   * @param args 
   * @returns 
   */
