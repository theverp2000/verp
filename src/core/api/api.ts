...
export class Environment {
  ...
  private _user: any;
  private _cacheKey: Map2<any, any>;
  ...
  async init(cr: Cursor, uid: any, context: {} = {}, su = false, req?: WebRequest): Promise<Environment> {
    ...
    this._cacheKey = new Map2<any, any>();
    this.req = req;
   ...
   * @returns record.id
   */
  async refId(xmlid: string, raiseIfNotFound = true) {
    const record = await this.ref(xmlid, raiseIfNotFound);
  ...
  async protecting(what: any, records: any, func?: Function) {
    ...
    try {
      ...
      global.logDebug && console.debug('>> Protecting alert', e.stack ?? e.message); // todo hide
      throw e;
    } finally {
      _protected.pop();
    }
...
export class Cache {
  private _data: Map2<any, any>; // easy debug using MapKey<any, any>;

  constructor() {
    this._data = new Map2();     // MapKey(key => key.repr());
  }
  ...
  *getFields(record: any) {
  ...
  getRecords(model: ModelRecords, field: Field): ModelRecords {
    ...
    return model.browse(fieldCache.keys());
  }
  ...
  *getMissingIds(records: ModelRecords, field: Field): Generator<any, any, number> {
   ...
   * @returns 
   */
  _getFieldCache(model: ModelRecords, field: Field): Map2<any, any> {
    let fieldCache: Map2<any, any> = this._data.get(field) ?? new Map2<any, any>();
    if (fieldCache.size && model.pool.fieldDependsContext.get(field)?.length) {
      const key = model.env.cacheKey(field);
      fieldCache = fieldCache.get(key, new Map2<any, any>());
    }
    return fieldCache;
  }
  ...
  _setFieldCache(model: ModelRecords, field: Field) {
    let fieldCache: Map2<any, any> = this._data.setdefault(field, new Map2<any, any>());
    if (model.pool.fieldDependsContext.get(field)?.length) {
      const key = model.env.cacheKey(field);
      fieldCache = fieldCache.setdefault(key, new Map2<any, any>());
    }
    return fieldCache;
  }
  ...
  invalidate(spec?: any) {
    if (spec == null) {
      ...
      for (const [field, ids] of spec) {
        ...
        const cache: Map2<any, any> = this._data.get(field);
        if (!bool(cache)) {
          continue;
        }
  ...
  async check(env: Environment) {
    ...
    const dump = Map2.fromEntries(this._data.items());
    this.invalidate();
    ...
    async function _check(model: ModelRecords, field: Field, fieldDump: Map2<any, any>) {
