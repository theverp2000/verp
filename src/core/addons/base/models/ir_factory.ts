...
class IrFactory extends AbstractModel {
  ...
  async _create(dataList = []) {
    ...
    for (const data of dataList) {
      ...
      const columns = [];
      for (const [name, val] of sorted(Object.entries(stored), ([name]) => name)) {
      ...
      // if (this._name == 'account.move' || this._name == 'account.move.line') {
      //   cr._printQuery = true;
      // }
      ...
      ids.push(res[0]['id']);
      // if (this._name == 'account.move' || this._name == 'account.move.line') {
      //   console.debug('>>> Inserted %s(%s)\n', this._name, String(ids));
      //   cr._printQuery = false;
      // }
    }
    const cacheToClear = [];
    const records = this.browse(ids);
    const inversesUpdate = new Map2<any, any>();
  ...
  async create(valsList): Promise<any> {
    ...
    for (let vals of valsList) {
      ...
      const inherited = data['inherited'] = new Map2<any, any>();
  ...
  async _write(values) {
    ...
    for (const [name, val] of sorted(Object.entries(values), ([name]) => name)) {
    ...
    if (columns.length) {
      ...
      for (const subIds of cr.splitForInConditions(this.ids)) {
        try {
          ...
          // if (this._name == 'account.move' || this._name == 'account.move.line') {
          //   console.debug('>>> Updated %s(%s)\n', this._name, String(subIds));
          //   cr._printQuery = false;
          // }
        } catch (e) {
          throw e;
  ...
  async write(values = {}): Promise<boolean> {
    ...
    const determineInverses = new Map(); // {inverse: [fields]}
    // const recordsToInverse = new Map();               // {field: records}
    ...
    for (const [fname, value] of vals.items()) {
      ...
      if (field.inverse) {
        ...
        if (!determineInverses.has(field.inverse)) {
          determineInverses.set(field.inverse, []);
        }
        determineInverses.get(field.inverse).push(field);
        // recordsToInverse.set(field, await this.filtered('id'));
      }
    ...
    const tocompute = [..._protected].filter(field => field.compute && !(field.name in vals)).map((field) => field.name);
    if (tocompute.length) {
  ...
  async unlink(): Promise<boolean> {
    ...
    // with (this.env.norecompute()) 
    {
      ...
      for (const subIds of cr.splitForInConditions(this.ids)) {
        ...
        // records)
        query = `SELECT id FROM "irAttachment" WHERE "resModel"='${this._name}' AND "resId" IN (${subIds})`;
  ...
  async _nameSearch(name: string = '', args?: any, operator: string = 'ilike', opts: { limit?: number, nameGetUid?: any } = {}): Promise<number | any[] | Query> {
    ...
    if (!this.cls._recName) {
      console.warn("Cannot execute nameSearch, no _recName defined on %s", this._name);
    }
  ...
  async nameGet(): Promise<any[]> {
    ...
    if (name in this._fields) {
      ...
      for (const record of this) {
        result.push([record.id, `${record._name},${record.id}`]);
      }
    }
    return result;
  }
  ...
  async nameCreate(name): Promise<any> {
    if (this.cls._recName) {
      ...
      console.warn("Cannot execute nameCreate, no _recName defined on %s", this._name);
      return false
    }
  }
  ...
  async read(fields, load = '_classicRead') {
    ...
    for (const name of fields) {
      const field = this._fields.get(name);
      if (!field) {
        throw new ValueError("Invalid field %s on model %s", name, this._name);
      }
      if (field.store) {
  ...
  async _readGroupRaw(domain, fields, groupby, options: { offset?: number, limit?: number, orderby?: string, lazy?: boolean } = {}) {
    ...
    for (const fspec of fields) {
      ...
      if (func) {
        ...
        const field = this._fields[fname];
        if (!field) {
          throw new ValueError("Invalid field %s on model %s", fname, this._name);
        }
  ...
  async defaultGet(fieldsList: any[]): Promise<Dict<any>> {
    ...
    const irDefaults = await this.env.items('ir.default').getModelDefaults(this._name);
    let field: Field;
  ...
  async *_convertRecords(records, log: Function = (x) => null) {
    ...
    if (await this.env.lang) {
      fieldNames.updateFrom(await this.env.items('ir.translation').getFieldString(this._name));
    }
  ...
  async load(fields, data) {
    ...
    const creatableModels = new Set([self._name]);
  ...
  async flush(fnames?: string[], records?: any): Promise<void> {
    ...
    if (!bool(fnames)) {
      ...
      if (records != null) {
        ...
        const towrite = this.env.all.towrite[this._name];
        if (!len(towrite)) {
          return;
        }
        let all = true;
