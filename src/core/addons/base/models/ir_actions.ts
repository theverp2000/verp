...
import { AbstractModel, MetaModel, Model, _super, api, tools, Command, Fields } from "../../..";
    ...
    for (const cls of this.cls.mro()) {
      for (const symbol of Object.keys(cls.prototype)) {
        if (symbol.startsWith('runAction')) {
          console.warn(
            "RPC-public action methods are deprecated, found %s (in class %s.%s)",
...
class IrActionsTodo extends Model {
  ...
  static type = Fields.Char({ string: 'Type' });
  ...
  async actionLaunch() {
    ...
    // Load action
    const [action, type] = await this('actionId', 'type');
    const result = (await this.env.items(type).searchRead([['id', '=', action.id]]))[0];
    if (type !== 'ir.actions.actwindow') {
      return result;
    }
