...
class View extends Model {
      ...
      if (node.tagName === 'xpath') {
        ...
        for (const attr of TRANSLATED_ATTRS.keys()) {
          if (node.getAttribute(attr)) {
            const message = f("View inheritance may not use attribute %s as a selector.", attr);
   ...
   */
  async _combine(hierarchy: Map2<any, any>) {
    this.ensureOne();
      ...
      if (!e.hasAttribute('t-raw')) {
        ...
        for (const child of iterchildren(e, (n) => isElement(n) || isProcessingInstruction(n))) {
          if (child.getAttribute('data-oe-xpath')) {
            ...
            this.distributeBranding(child, distributedBranding, nodePath, indexes);
          }
        }
      }
    }
