...
class AvatarMixin extends AbstractModel {
  ...
  async _computeAvatar(avatarField, imageField) {
    for (const record of this) {
      ...
      if (!bool(avatar)) {
        if (bool(record.id) && bool(await record[record._avatarNameField])) {
