...
class IrFieldsConverter extends AbstractModel {
  ...
  async _getTranslations(types: string[], src): Promise<any[]> {
    ...
    const tnxCache = this._cr.cache[this._name] = this._cr.cache[this._name] ?? new Map();
  ...
  async _xmlidToRecordId(xmlid: string, model) {
    ...
    if (result) {
      ...
      if (resModel !== model._name) {
        const msg = `Invalid external ID ${xmlid}: expected model ${model._name}, found ${resModel}`;
