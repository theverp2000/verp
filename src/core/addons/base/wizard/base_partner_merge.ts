...
class MergePartnerAutomatic extends TransientModel {
    ...
    async _updateReferenceFields(srcPartners, dstPartner) {
        console.debug('_updateReferenceFields for dstPartner: %s for srcPartners: %s', dstPartner.id, srcPartners.ids);
    ...
    async _updateValues(srcPartners, dstPartner) {
        console.debug('_updateValues for dstPartner: %s for srcPartners: %s', dstPartner.id, String(srcPartners.ids));
        ...
        const valuesByCompany = new DefaultDict(() => new Dict<any>());   // {company: vals}
     ...
     * @param aggrIds stringified list of partner ids separated with a comma (sql array_agg)
