...
import { PyDate, PyTimeDelta } from "@web/core/ast/py_date";
...
QUnit.module("vj", {}, () => {
    QUnit.module("date stuff", () => {
        ...
        QUnit.test("to_utc", (assert) => {
            ...
            const expr =
                "datetime.datetime.combine(contextToday(), datetime.time(0,0,0)).to_utc()";
        ...
        QUnit.test("datetime.datetime.combine", (assert) => {
            const expr =
                "datetime.datetime.combine(contextToday(), datetime.time(23,59,59)).strftime('%Y-%m-%d %H:%M:%S')";
            assert.ok(
                check(expr, (d) => {
        ...
        QUnit.test("contextToday", (assert) => {
            assert.ok(check("contextToday().strftime('%Y-%m-%d')", formatDate));
        });
        ...
        QUnit.test("currentDate", (assert) => {
            patchDate(2021, 8, 20, 10, 0, 0);
            assert.deepEqual(evaluateExpr("currentDate"), "2021-09-20");
        });
    });
});
