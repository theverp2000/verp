verp.define('web.list_tests', function (require) {
"use strict";
...
QUnit.module('Views', {
        ...
        this.data = {
            foo: {
                fields: {
                    ...
                    int_field: {string: "int_field", type: "integer", sortable: true, groupOperator: "sum"},
                    ...
                    currencyId: {string: "Currency", type: "many2one",
            ...
            bar: {
                fields: {},
                records: [
                    {id: 1, displayName: "Value 1"},
                    {id: 2, displayName: "Value 2"},
                    {id: 3, displayName: "Value 3"},
                ]
            },
            res_currency: {
                fields: {
    ...
    QUnit.test('simple readonly list', async function (assert) {
        ...
        assert.containsOnce(list, 'th.o-column-sortable', "should have 1 sortable column");
        ...
        assert.isVisible(list.$buttons.find('.o-list-button-add'));
        assert.isNotVisible(list.$buttons.find('.o-list-button-save'));
        assert.isNotVisible(list.$buttons.find('.o-list-button-discard'));
        list.destroy();
    });

    QUnit.test('onAttachCallback is properly called', async function (assert) {
        assert.expect(3);
        ...
        const list = await createView({
            ...
            model: 'foo',
            data: this.data,
            arch: '<tree><field name="displayName"/></tree>',
        });
    ...
    QUnit.test('list with create="0"', async function (assert) {
        ...
        assert.containsNone(list.$buttons, '.o-list-button-add',
    ...
    QUnit.test('list with delete="0"', async function (assert) {
        ...
        assert.containsNone(list.el, 'div.o-control-panel .o-cp-action-menus');
        assert.ok(list.$('tbody td.o-list-record-selector').length, 'should have at least one record');

        await testUtils.dom.click(list.$('tbody td.o-list-record-selector:first input'));
        assert.containsNone(list.el, 'div.o-control-panel .o-cp-action-menus .dropdown-menu');
    ...
    QUnit.test('editable list with edit="0"', async function (assert) {
        ...
        await testUtils.dom.click(list.$('tr td:not(.o-list-record-selector)').first());
        assert.containsNone(list, 'tbody tr.o-selected-row', "should not have editable row");
    ...
    QUnit.test('export feature in list for users not in base.group_allow_export', async function (assert) {
        ...
        assert.containsNone(list.el, 'div.o-control-panel .o-cp-action-menus');
        assert.ok(list.$('tbody td.o-list-record-selector').length, 'should have at least one record');
        assert.containsNone(list.el, 'div.o-control-panel .o-cp-buttons .o-list-export_xlsx');

        await testUtils.dom.click(list.$('tbody td.o-list-record-selector:first input'));
        assert.containsOnce(list.el, 'div.o-control-panel .o-cp-action-menus');
    ...
    QUnit.test('list with export button', async function (assert) {
        ...
        assert.containsNone(list.el, 'div.o-control-panel .o-cp-action-menus');
        assert.ok(list.$('tbody td.o-list-record-selector').length, 'should have at least one record');
        assert.containsOnce(list.el, 'div.o-control-panel .o-cp-buttons .o-list-export_xlsx');
    ...
    QUnit.test('export button in list view', async function (assert) {
        ...
        assert.isVisible(list.$('.o-list-export_xlsx'));

        await testUtils.dom.click(list.$('tbody td.o-list-record-selector:first input'));

        assert.isNotVisible(list.$('.o-list-export_xlsx'));
        assert.containsOnce(list.$('.o-cp-buttons'), '.o-list-selection_box');
    ...
    QUnit.test('export button in empty list view', async function (assert) {
        ...
        assert.isNotVisible(list.el.querySelector('.o-list-export_xlsx'));

        await list.reload({ domain: [['id', '>', 0]] });
        assert.isVisible(list.el.querySelector('.o-list-export_xlsx'));
    ...
    QUnit.test('list view with adjacent buttons', async function (assert) {
        ...
        assert.containsN(list.$('.o-data-row:first'), 'td.o-list-button', 2);
    ...
    QUnit.test('list view with adjacent buttons with invisible modifier', async function (assert) {
        ...
        assert.containsN(list, 'th', 3,
            "adjacent buttons in the arch must be grouped in a single column");
        assert.containsOnce(list.$('.o-data-row:first'), 'td.o-list-button');
        assert.strictEqual(list.$('.o-field-cell').text(), 'yopblipgnapblip');
    ...
    QUnit.test('list view: action button in controlPanel basic rendering', async function (assert) {
        ...
        assert.containsNone(cpButtons[0], '.o-list-selection_box');
        assert.containsNone(cpButtons[0], 'button[name="y"]');

        await testUtils.dom.click(
            list.el.querySelector('.o-data-row .o-list-record-selector input[type="checkbox"]')
        );
        ...
        assert.containsOnce(cpButtons[0], '.o-list-selection_box');
        assert.strictEqual(
            cpButtons[0].querySelector('button[name="x"]').nextElementSibling,
            cpButtons[0].querySelector('.o-list-selection_box')
        );
        assert.containsNone(cpButtons[0], 'button[name="y"]');
    ...
    QUnit.test('list view: buttons handler is called once on double click', async function (assert) {
        ...
        const list = await createView({
            ...
            <tree>
                <field name="foo" />
                <button name="x" type="object" class="doSomething" string="Do Something"/>
            </tree>`,
            intercepts: {
                async execute_action(ev) {
        ...
        await testUtils.dom.click(list.$('tbody .o-list-button:first > button'));
        await testUtils.dom.click(list.$('tbody .o-list-button:first > button'));
    ...
    QUnit.test('list view: action button executes action on click with domain selected: correct parameters', async function (assert) {
        ...
        await testUtils.dom.click(cpButtons[0].querySelector('.o-list-select_domain'));
        assert.verifySteps([]);
    ...
    QUnit.test('column names (noLabel, label, string and default)', async function (assert) {
        ...
        const list = await createView({
            ...
            arch: `
                <tree>
                    <field name="displayName" widget="nolabel_char"/>
        ...
        assert.strictEqual(list.$('thead th[data-name=displayName]').text(), '');
    ...
    QUnit.test('simple editable rendering', async function (assert) {
        ...
        assert.containsN(list, '.o-list-record-selector input:enabled', 5);
        ...
        await testUtils.dom.click(list.$('td:not(.o-list-record-selector)').first());

        assert.isNotVisible(list.$buttons.find('.o-list-button-add'),
            "should not have a visible Create button");
        assert.isVisible(list.$buttons.find('.o-list-button-save'),
            "should have a visible save button");
        assert.isVisible(list.$buttons.find('.o-list-button-discard'),
            "should have a visible discard button");
        assert.containsNone(list, '.o-list-record-selector input:enabled');
    ...
    QUnit.test('editable rendering with handle and no data', async function (assert) {
        ...
        assert.hasClass(list.$('thead th:eq(0)'), 'o-list-record-selector');
        assert.hasClass(list.$('thead th:eq(1)'), 'o-handle-cell');
    ...
    QUnit.test('field titles are not escaped', async function (assert) {
        ...
        assert.strictEqual(list.$('tbody tr:first .o-data-cell').text(), "<div>Hello</div>");
        assert.strictEqual(list.$('tbody tr:first .o-data-cell').attr('title'), "<div>Hello</div>");
    ...
    QUnit.test('record-depending invisible lines are correctly aligned', async function (assert) {
        ...
        assert.containsOnce(list, 'tbody td.o-invisible-modifier',
            "there should be 1 invisible bar cell");
        assert.hasClass(list.$('tbody tr:first td:eq(2)'),'o-invisible-modifier',
    ...
    QUnit.test('do not perform extra RPC to read invisible many2one fields', async function (assert) {
        ...
        assert.verifySteps(['searchRead', 'onchange'], "no nameget should be done");
    ...
    QUnit.test('editable list datetimepicker destroy widget (edition)', async function (assert) {
        ...
        list.$el.on({
            'show.datetimepicker': async function () {
                ...
                await testUtils.fields.triggerKeydown(list.$('.o-datepicker-input'), 'escape');
                ...
                assert.containsNone(list, '.o-selected-row');
        ...
        await testUtils.dom.openDatepicker(list.$('.o-datepicker'));
    ...
    QUnit.test('editable list datetimepicker destroy widget (new line)', async function (assert) {
        ...
        list.$el.on({
            'show.datetimepicker': async function () {
                assert.containsOnce($('body'), '.bootstrap-datetimepicker-widget');
                assert.containsN(list, '.o-data-row', 5);
                assert.containsOnce(list, '.o-selected-row');
        ...
        assert.equal(list.$('.o-data-row').length, 4,
            'There should be 4 rows');

        assert.equal(list.$('.o-selected-row').length, 0,
    ...
    QUnit.test('discard a new record in editable="top" list with less than 4 records', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-list-button-add'));

        assert.containsN(list, '.o-data-row', 4);
        assert.hasClass(list.$('tbody tr:first'), 'o-selected-row');

        await testUtils.dom.click(list.$('.o-list-button-discard'));

        assert.containsN(list, '.o-data-row', 3);
        assert.containsN(list, 'tbody tr', 4);
        assert.hasClass(list.$('tbody tr:first'), 'o-data-row');
    ...
    QUnit.test('basic grouped list rendering', async function (assert) {
        ...
        assert.containsN(list, 'tr.o-group-header', 2, "should have 2 .o-group-header");
        assert.containsN(list, 'th.o-group-name', 2, "should have 2 .o-group-name");
        list.destroy();
    });
    ...
    QUnit.test('basic grouped list rendering 1 col without selector', async function (assert) {
        ...
        assert.strictEqual(list.$('.o-group-header:first').children().length, 1,
            "group header should have exactly 1 column");
        assert.strictEqual(list.$('.o-group-header:first th').attr('colspan'), "1",
    ...
    QUnit.test('basic grouped list rendering 1 col with selector', async function (assert) {
        ...
        assert.strictEqual(list.$('.o-group-header:first').children().length, 1,
            "group header should have exactly 1 column");
        assert.strictEqual(list.$('.o-group-header:first th').attr('colspan'), "2",
            "the header should span the whole table");
        list.destroy();
    });
    ...
    QUnit.test('basic grouped list rendering 2 cols without selector', async function (assert) {
        ...
        assert.strictEqual(list.$('.o-group-header:first').children().length, 2,
    ...
    QUnit.test('basic grouped list rendering 3 cols with selector', async function (assert) {
        ...
        assert.strictEqual(list.$('.o-group-header:first').children().length, 2,
            "group header should have exactly 2 columns");
        assert.strictEqual(list.$('.o-group-header:first th').attr('colspan'), "3",
            "the header should not span the whole table");
        list.destroy();
    });
    ...
    QUnit.test('basic grouped list rendering 7 cols with aggregates and selector', async function (assert) {
        ...
        assert.strictEqual(list.$('.o-group-header:first').children().length, 5,
        ...
        assert.containsN(list, '.o-group-header:first td', 3,
            "there should be 3 tds (aggregates + fields in between)");
        assert.strictEqual(list.$('.o-group-header:first th:last').attr('colspan'), "2",
    ...
    QUnit.test('basic grouped list rendering with groupby m2m field', async function (assert) {
        ...
        assert.containsN(list, ".o-group-header", 4, "should contain 4 open groups");
        assert.containsNone(list, ".o-group-open", "no group is open");
        assert.deepEqual(
            [...list.el.querySelectorAll(".o-group-header .o-group-name")].map((el) => el.innerText),
            ["Value 1 (3)", "Value 2 (2)", "Value 3 (1)", "Undefined (1)"],
            "should have those group headers",
        );

        // Open all groups
        await testUtils.dom.click(list.el.querySelectorAll(".o-group-name")[0]);
        await testUtils.dom.click(list.el.querySelectorAll(".o-group-name")[1]);
        await testUtils.dom.click(list.el.querySelectorAll(".o-group-name")[2]);
        await testUtils.dom.click(list.el.querySelectorAll(".o-group-name")[3]);
        assert.containsN(list, ".o-group-open", 4, "all groups are open");
    ...
    QUnit.test('grouped list rendering with groupby m2o and m2m field', async function (assert) {
        ...
        await testUtils.dom.click(list.$('th.o-group-name').first());
        ...
        await testUtils.dom.click(list.$('tbody:eq(2) th.o-group-name'));
    ...
    QUnit.test('deletion of record is disabled when groupby m2m field', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-header:first')); // open first group
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-list-record-selector input'));
        assert.containsOnce(list.el, 'div.o-control-panel .o-cp-action-menus');
        assert.containsNone(list.el, 'div.o-control-panel .o-cp-action-menus .dropdown',
        ...
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-list-record-selector input'));
        assert.containsOnce(list.el, 'div.o-control-panel .o-cp-action-menus');
        assert.containsOnce(list.el, 'div.o-control-panel .o-cp-action-menus .dropdown');
        await testUtils.dom.click(list.$('div.o-control-panel .o-cp-action-menus .dropdown button'));
        assert.deepEqual(
            [...list.el.querySelectorAll('.o-cp-action-menus .o-menu-item')].map((el) => el.innerText),
            ["Delete"]
        );
    ...
    QUnit.test('editing a record should change same record in other groups when grouped by m2m field', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-header:first')); // open first group
        await testUtils.dom.click(list.$('.o-group-header:eq(1)')); // open second group
        assert.containsOnce(list, 'tbody:eq(1) .o-list-char:contains("yop")',
            "should have yop record in first group");
        assert.containsOnce(list, 'tbody:eq(3) .o-list-char:contains("yop")',
            "should have yop record in second group");

        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-list-record-selector input'));
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell:first'));
        await testUtils.fields.editInput(list.$('.o-data-row:eq(0) .o-data-cell:first .o-input'), 'xyz');
        await testUtils.dom.click(list.$el);
        assert.containsOnce(list, 'tbody:eq(1) .o-list-char:contains("xyz")',
            "should have yop renamed in xyz in first group");
        assert.containsOnce(list, 'tbody:eq(3) .o-list-char:contains("xyz")',
    ...
    QUnit.test('change a record field in readonly should change same record in other groups when grouped by m2m field', async function (assert) {
        ...
        assert.containsNone(list, ".o-priority-star.fa-star", "should not have any starred records");
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-priority-star.fa-star-o'));
        assert.containsN(list, ".o-priority-star.fa-star", 2, "both 'yop' records should have been starred");
    ...
    QUnit.test('ordered list, sort attribute in context', async function (assert) {
        ...
        await testUtils.dom.click(list.$('th.o-column-sortable:contains("Foo")'));
        await testUtils.dom.click(list.$('th.o-column-sortable:contains("Foo")'));

        // Ascending order on Date
        await testUtils.dom.click(list.$('th.o-column-sortable:contains("Date")'));
                ...
                if (route === '/web/dataset/searchRead') {
                    if (searchReads === 0) {
                        assert.strictEqual(args.sort, 'date ASC, foo DESC',
                            'The sort attribute of the filter should be used by the initial searchRead');
                    } else if (searchReads === 1) {
                        assert.strictEqual(args.sort, 'date DESC, foo ASC',
                            'The sort attribute of the filter should be used by the next searchRead');
                    }
                    searchReads += 1;
                }
                return this._super.apply(this,arguments);
    ...
    QUnit.test('many2one field rendering', async function (assert) {
        ...
        assert.ok(list.$('td:contains(Value 1)').length,
            "should have the displayName of the many2one");
        list.destroy();
    });
    ...
    QUnit.test('grouped list view, with 1 open group', async function (assert) {
        ...
        await testUtils.dom.click(list.$('th.o-group-name:nth(1)'));
        await testUtils.nextTick();
        ...
        assert.containsOnce(list, 'tr.o-group-header td:contains(10)', "but 10 should be in a header");
        list.destroy();
    });
    ...
    QUnit.test('opening records when clicking on record', async function (assert) {
        ...
        assert.containsN(list, 'tr.o-group-header', 3, "list should be grouped");
        await testUtils.dom.click(list.$('th.o-group-name').first());

        testUtils.dom.click(list.$('tr:not(.o-group-header) td:not(.o-list-record-selector)').first());
        list.destroy();
    });
    ...
    QUnit.test('editable list view: readonly fields cannot be edited', async function (assert) {
        ...
        var $td = list.$('td:not(.o-list-record-selector)').first();
        var $second_td = list.$('td:not(.o-list-record-selector)').eq(1);
        var $third_td = list.$('td:not(.o-list-record-selector)').eq(2);
        await testUtils.dom.click($td);
        assert.hasClass($td.parent(),'o-selected-row',
            "row should be in edit mode");
        assert.hasClass($td,'o-readonly-modifier',
            "foo cell should be readonly in edit mode");
        assert.doesNotHaveClass($second_td, 'o-readonly-modifier',
            "bar cell should be editable");
        assert.hasClass($third_td,'o-readonly-modifier',
    ...
    QUnit.test('editable list view: line with no active element', async function (assert) {
            ...
            arch: '<form>'+
                    '<field name="o2m">'+
                        '<tree editable="top">'+
                            ...
                            '<field name="grosminet" widget="booleanToggle"/>'+
                        '</tree>'+
                    '</field>'+
                '</form>',
            mockRPC: function (route, args) {
        ...
        var $td = form.$('.o-data-cell').first();
        var $td2 = form.$('.o-data-cell').eq(1);
        assert.hasClass($td, 'o-readonly-modifier');
        assert.hasClass($td2, 'o-boolean-toggle-cell');
        await testUtils.dom.click($td);
        await testUtils.dom.click($td2.find('.o-boolean-toggle input'));
        await testUtils.nextTick();
    ...
    QUnit.test('editable list view: click on last element after creation empty new line', async function (assert) {
        ...
        await testUtils.dom.click(form.$('.o-data-row:last() > td.o-list-char'));
        // This test ensure that they aren't traceback when clicking on the last row.
        assert.containsN(form, '.o-data-row', 2, "list should have exactly 2 rows");
        form.destroy();
    });

    QUnit.test('edit field in editable field without editing the row', async function (assert) {
        // some widgets are editable in readonly (e.g. priority, booleanToggle...) and they
        ...
        const list = await createView({
            ...
            arch: `
                <tree editable="top">
                    <field name="foo"/>
                    <field name="bar" widget="booleanToggle"/>
                </tree>`,
            mockRPC(route, args) {
                if (args.method === 'write') {
        ...
        assert.ok(list.$('.o-data-row:first .o-boolean-toggle input')[0].checked);
        assert.containsNone(list, '.o-selected-row');
        await testUtils.dom.click(list.$('.o-data-row:first .o-boolean-toggle'));
        assert.notOk(list.$('.o-data-row:first .o-boolean-toggle input')[0].checked);
        assert.containsNone(list, '.o-selected-row');
        ...
        assert.containsOnce(list, '.o-selected-row');
        await testUtils.dom.click(list.$('.o-selected-row .o-boolean-toggle'));
        assert.containsOnce(list, '.o-selected-row');
        assert.verifySteps([]);

        // save
        await testUtils.dom.click(list.$('.o-list-button-save'));
        assert.containsNone(list, '.o-selected-row');
    ...
    QUnit.test('basic operations for editable list renderer', async function (assert) {
        ...
        var $td = list.$('td:not(.o-list-record-selector)').first();
        assert.doesNotHaveClass($td.parent(), 'o-selected-row', "td should not be in edit mode");
        await testUtils.dom.click($td);
        assert.hasClass($td.parent(),'o-selected-row', "td should be in edit mode");
        list.destroy();
    });
    ...
    QUnit.test('editable list: add a line and discard', async function (assert) {
        ...
        assert.containsOnce(list, '.o-data-row',
    ...
    QUnit.test('field changes are triggered correctly', async function (assert) {
        ...
        var n = 0;
        testUtils.mock.intercept(list, "fieldChanged", function () {
            n += 1;
        });
        await testUtils.dom.click($td);
        await testUtils.fields.editInput($td.find('input'), 'abc');
        assert.strictEqual(n, 1, "fieldChanged should have been triggered");
        await testUtils.dom.click(list.$('td:not(.o-list-record-selector)').eq(2));
        assert.strictEqual(n, 1, "fieldChanged should not have been triggered");
        list.destroy();
    });
    ...
    QUnit.test('editable list view: basic char field edition', async function (assert) {
        ...
        var $next_row_td = list.$('tbody tr:eq(1) td:not(.o-list-record-selector)').first();
        await testUtils.dom.click($next_row_td);
        assert.strictEqual(list.$('td:not(.o-list-record-selector)').first().text(), 'abc',
            'changes should be saved correctly');
        assert.doesNotHaveClass(list.$('tbody tr').first(), 'o-selected-row',
    ...
    QUnit.test('editable list view: save data when list sorting in edit mode', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-column-sortable'));

        assert.hasClass(list.$('.o-data-row:first'),'o-selected-row',
    ...
    QUnit.test('editable list view: check that controlpanel buttons are updating when groupby applied', async function (assert) {
        ...
        serverData.actions = {
            11: {
                ...
                views: [[3, 'list']],
                searchViewId: [9, 'search'],
            },
        };
        serverData.views = {
            'foo,3,list': '<tree editable="top"><field name="displayName"/><field name="foo"/></tree>',
        ...
        assert.isNotVisible($(webClient.el).find('.o-list-button-add'),
            "create button should be invisible");
        assert.isVisible($(webClient.el).find('.o-list-button-save'), "save button should be visible");

        await testUtils.dom.click($(webClient.el).find('.dropdown-toggle:contains("Group By")'));
        await testUtils.dom.click($(webClient.el).find('.o-group-by-menu .o-menu-item:contains("candle")'));

        assert.isNotVisible($(webClient.el).find('.o-list-button-add'), "create button should be invisible");
        assert.isNotVisible($(webClient.el).find('.o-list-button-save'),
    ...
    QUnit.test('list view not groupable', async function (assert) {
        ...
        const list = await createView({
            ...
            archs: {
                'foo,false,search': `
                    <search>
                        <filter context="{'groupby': 'foo'}" name="foo"/>
                    </search>
                `,
            },
            mockRPC: function (route, args) {
        ...
        assert.containsNone(list, '.o-control-panel div.o-search-options div.o-group-by-menu',
    ...
    QUnit.test('selection changes are triggered correctly', async function (assert) {
        ...
        var $tbody_selector = list.$('tbody .o-list-record-selector input').first();
        var $thead_selector = list.$('thead .o-list-record-selector input');
        ...
        assert.containsN(list, 'tbody .o-list-record-selector input:checked',
        ...
        assert.containsNone(list, 'tbody .o-list-record-selector input:checked',
    ...
    QUnit.test('Row selection checkbox can be toggled by clicking on the cell', async function (assert) {
        ...
        testUtils.dom.click(list.$('tbody .o-list-record-selector:first'));
        assert.containsOnce(list, 'tbody .o-list-record-selector input:checked');
        testUtils.dom.click(list.$('tbody .o-list-record-selector:first'));
        assert.containsNone(list, '.o-list-record-selector input:checked');

        testUtils.dom.click(list.$('thead .o-list-record-selector'));
        assert.containsN(list, '.o-list-record-selector input:checked', 5);
        testUtils.dom.click(list.$('thead .o-list-record-selector'));
        assert.containsNone(list, '.o-list-record-selector input:checked');
    ...
    QUnit.test('head selector is toggled by the other selectors', async function (assert) {
        ...
        assert.ok(!list.$('thead .o-list-record-selector input')[0].checked,
            "Head selector should be unchecked");

        await testUtils.dom.click(list.$('.o-group-header:first()'));
        await testUtils.dom.click(list.$('thead .o-list-record-selector input'));

        assert.containsN(list, 'tbody .o-list-record-selector input:checked',
            3, "All visible checkboxes should be checked");

        await testUtils.dom.click(list.$('.o-group-header:last()'));

        assert.ok(!list.$('thead .o-list-record-selector input')[0].checked,
            "Head selector should be unchecked");

        await testUtils.dom.click(list.$('tbody .o-list-record-selector input:last()'));

        assert.ok(list.$('thead .o-list-record-selector input')[0].checked,
            "Head selector should be checked");

        await testUtils.dom.click(list.$('tbody .o-list-record-selector:first() input'));
    ...
    QUnit.test('selection box is properly displayed (single page)', async function (assert) {
        ...
        assert.containsN(list, '.o-data-row', 4);
        assert.containsNone(list.$('.o-cp-buttons'), '.o-list-selection_box');

        // select a record
        await testUtils.dom.click(list.$('.o-data-row:first .o-list-record-selector input'));
        assert.containsOnce(list.$('.o-cp-buttons'), '.o-list-selection_box');
        assert.containsNone(list.$('.o-list-selection_box'), '.o-list-select_domain');
        assert.strictEqual(list.$('.o-list-selection_box').text().trim(), '1 selected');

        // select all records of first page
        await testUtils.dom.click(list.$('thead .o-list-record-selector input'));
        assert.containsOnce(list.$('.o-cp-buttons'), '.o-list-selection_box');
        assert.containsNone(list.$('.o-list-selection_box'), '.o-list-select_domain');
        assert.strictEqual(list.$('.o-list-selection_box').text().trim(), '4 selected');

        // unselect a record
        await testUtils.dom.click(list.$('.o-data-row:nth(1) .o-list-record-selector input'));
        assert.containsOnce(list.$('.o-cp-buttons'), '.o-list-selection_box');
        assert.containsNone(list.$('.o-list-selection_box'), '.o-list-select_domain');
        assert.strictEqual(list.$('.o-list-selection_box').text().trim(), '3 selected');
    ...
    QUnit.test('selection box is properly displayed (multi pages)', async function (assert) {
        ...
        await testUtils.dom.click(list.$('thead .o-list-record-selector input'));
        assert.containsOnce(list.$('.o-cp-buttons'), '.o-list-selection_box');
        assert.containsOnce(list.$('.o-list-selection_box'), '.o-list-select_domain');
        assert.strictEqual(list.$('.o-list-selection_box').text().replace(/\s+/g, ' ').trim(),
            '3 selected Select all 4');

        // select all domain
        await testUtils.dom.click(list.$('.o-list-selection_box .o-list-select_domain'));
        assert.containsOnce(list.$('.o-cp-buttons'), '.o-list-selection_box');
        assert.strictEqual(list.$('.o-list-selection_box').text().trim(), 'All 4 selected');
    ...
    QUnit.test('selection box is displayed after header buttons', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:first .o-list-record-selector input'));
        assert.containsOnce(list.$('.o-cp-buttons'), '.o-list-selection_box');
        const lastElement = list.$('.o-cp-buttons .o-list-buttons').children(":last")[0];
        assert.strictEqual(lastElement, list.$('.o-cp-buttons .o-list-selection_box')[0],
    ...
    QUnit.test('selection box is removed after multi record edition', async function (assert) {
        ...
        const list = await createView({
            ...
            model: 'foo',
            data: this.data,
            arch: '<tree multiEdit="1"><field name="foo"/><field name="bar"/></tree>',
        });
        ...
        assert.containsN(list, '.o-data-row .o-list-record-selector input:checked', 4,
            "all 4 records should be selected");

        // edit selected records
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell:eq(0)'));
        await testUtils.fields.editInput(list.$('.o-field-widget[name=foo]'), 'legion');
        await testUtils.dom.click($('.modal-dialog button.btn-primary'));
        assert.containsNone(list.$('.o-cp-buttons'), '.o-list-selection_box',
            "list selection box should not be displayed");
        assert.containsNone(list, '.o-data-row .o-list-record-selector input:checked',
    ...
    QUnit.test('selection is reset on reload', async function (assert) {
        ...
        // select first record
        var $firstRowSelector = list.$('tbody .o-list-record-selector input').first();
        ...
        // reload
        await list.reload();
        $firstRowSelector = list.$('tbody .o-list-record-selector input').first();
    ...
    QUnit.test('selection is kept on render without reload', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-header:contains("blip (2)")'));
        await testUtils.dom.click(list.$('.o-data-row:first input'));
        assert.containsOnce(list.el, 'div.o-control-panel .o-cp-action-menus');
        assert.containsOnce(list.$('.o-cp-buttons'), '.o-list-selection_box');

        // open yop grouping and verify blip are still checked
        await testUtils.dom.click(list.$('.o-group-header:contains("yop (1)")'));
        assert.containsOnce(list, '.o-data-row input:checked',
        ...
        await testUtils.dom.click(list.$('.o-group-header:contains("blip (2)")'));
        await testUtils.dom.click(list.$('.o-group-header:contains("blip (2)")'));
        assert.containsNone(list, '.o-data-row input:checked',
    ...
    QUnit.test('aggregates are computed correctly', async function (assert) {
        ...
        var $tbody_selectors = list.$('tbody .o-list-record-selector input');
        var $thead_selector = list.$('thead .o-list-record-selector input');
    ...
    QUnit.test('aggregates are computed correctly in grouped lists', async function (assert) {
        ...
        var $groupHeader1 = list.$('.o-group-header').filter(function (index, el) {
            return $(el).data('group').resId === 1;
        });
        var $groupHeader2 = list.$('.o-group-header').filter(function (index, el) {
            return $(el).data('group').resId === 2;
        });
        ...
        await testUtils.dom.click(list.$('tbody .o-list-record-selector input').first());
    ...
    QUnit.test('hide aggregated value in grouped lists when no data provided by RPC call', async function(assert) {
        ...
        var list = await createView({
            ...
            data: this.data,
            groupBy: ['bar'],
            arch: '<tree editable="bottom"><field name="foo"/><field name="qux" widget="floatTime" sum="Sum"/></tree>',
            mockRPC: function (route, args) {
                return this._super.apply(this, arguments).then(result => {
                    // On PY-side, a readGroup won't provide any aggregate value
                    // if the field does not have a `groupOperator` attribute.
                    // (see 5691b126f06/verp/models.js#L2521-L2528)
    ...
    QUnit.test('aggregates are updated when a line is edited', async function (assert) {
        ...
        await testUtils.dom.click(list.$('tr.o-data-row td.o-data-cell').first());
        await testUtils.fields.editInput(list.$('td.o-data-cell input'), "15");
    ...
    QUnit.test('aggregates are formatted according to field widget', async function (assert) {
        ...
        var list = await createView({
            ...
            arch: '<tree>' +
                    '<field name="foo"/>' +
                    '<field name="qux" widget="floatTime" sum="Sum"/>' +
                '</tree>',
        });

        assert.strictEqual(list.$('tfoot td:nth(2)').text(), '19:24',
            "total should be formatted as a floatTime");
    ...
    QUnit.test('aggregates digits can be set with digits field attribute', async function (assert) {
        ...
        assert.strictEqual(list.$('.o-data-row td:nth(1)').text(), '1200.00',
    ...
    QUnit.test('groups can be sorted on aggregates', async function (assert) {
        ...
        assert.strictEqual(list.$('tbody .o-list-number').text(), '10517',
            "initial order should be 10, 5, 17");
        assert.strictEqual(list.$('tfoot td:last()').text(), '32', "total should be 32");

        await testUtils.dom.click(list.$('.o-column-sortable'));
        assert.strictEqual(list.$('tfoot td:last()').text(), '32', "total should still be 32");
        assert.strictEqual(list.$('tbody .o-list-number').text(), '51017',
            "order should be 5, 10, 17");

        await testUtils.dom.click(list.$('.o-column-sortable'));
        assert.strictEqual(list.$('tbody .o-list-number').text(), '17105',
    ...
    QUnit.test('groups cannot be sorted on non-aggregable fields', async function (assert) {
        ...
        //we cannot sort by sort_field since it doesn't have a groupOperator
        await testUtils.dom.click(list.$('.o-column-sortable:eq(2)'));
        //we can sort by int_field since it has a groupOperator
        await testUtils.dom.click(list.$('.o-column-sortable:eq(1)'));
        //we keep previous order
        await testUtils.dom.click(list.$('.o-column-sortable:eq(2)'));
        //we can sort on foo since we are groupped by foo + previous order
        await testUtils.dom.click(list.$('.o-column-sortable:eq(0)'));
    ...
    QUnit.test('properly apply onchange in simple case', async function (assert) {
        ...
        var $foo_td = list.$('td:not(.o-list-record-selector)').first();
        var $int_field_td = list.$('td:not(.o-list-record-selector)').eq(1);
    ...
    QUnit.test('width of some of the fields should be hardcoded if no data', async function (assert) {
        ...
        var list = await createView({
            ...
            arch: '<tree editable="top">' +
                        ...
                        '<field name="datetime"/>' +
                        '<field name="amount"/>' +
                        '<field name="currencyId" width="25px"/>' +
                    '</tree>',
        });

        assert.containsNone(list, '.o-resize', "There shouldn't be any resize handle if no data");
        ...
        assert.strictEqual(list.$('th[data-name="currencyId"]')[0].offsetWidth, 25,
    ...
    QUnit.test('width of some fields should be hardcoded if no data, and list initially invisible', async function (assert) {
        ...
        assert.isNotVisible(form.$('.o-field-one2many'));

        await testUtils.dom.click(form.$('.nav-item:last-child .nav-link'));

        assert.isVisible(form.$('.o-field-one2many'));

        assert.containsNone(form, '.o-field-one2many .o-resize',
            "There shouldn't be any resize handle if no data");
        assertions.forEach(a => {
            assert.strictEqual(form.$(`.o-field-one2many th[data-name="${a.field}"]`)[0].offsetWidth, a.expected,
                `Field ${a.type} should have a fixed width of ${a.expected} pixels`);
        });
        assert.strictEqual(form.$('.o-field-one2many th[data-name="foo"]')[0].style.width, '100%',
            "Char field should occupy the remaining space");
        assert.strictEqual(form.$('th[data-name="currencyId"]')[0].offsetWidth, 25,
            'Currency field should have a fixed width of 25px (see arch)');
        assert.strictEqual(form.el.querySelector('.o-list-record-remove-header').style.width, '32px');
    ...
    QUnit.test('empty editable list with the handle widget and no content help', async function (assert) {
        ...
        assert.containsNone(list, '.o-list-table', " there should not be any records in the view.");
        assert.containsOnce(list, '.o-view-nocontent', "should have no content help");

        // click on create button
        await testUtils.dom.click(list.$('.o-list-button-add'));
        const handleWidgetMinWidth = "33px";
        const handleWidgetHeader = list.$('thead > tr > th.o-handle-cell');
        ...
        // creating one record
        await testUtils.fields.editInput(list.$("tr.o-selected-row input[name='foo']"), 'test_foo');
        await testUtils.dom.click(list.$('.o-list-button-save'));
        assert.strictEqual(handleWidgetHeader.css('min-width'), handleWidgetMinWidth,
    ...
    QUnit.test('editable list: overflowing table', async function (assert) {
        ...
        assert.strictEqual(list.$('table').width(), list.$('.o-list-view').width(),
    ...
    QUnit.test('editable list: overflowing table (3 columns)', async function (assert) {
        ...
        assert.strictEqual(list.$('table').width(), list.$('.o-list-view').width());
        const largeCells = list.$('.o-data-cell.large');
        ...
        assert.ok(list.$('.o-data-cell:not(.large)')[0].offsetWidth < largeCells[0].offsetWidth);
    ...
    QUnit.test('editable list: list view in an initially unselected notebook page', async function (assert) {
        ...
        const one2many = form.el.querySelector('.o-field-one2many');
    ...
    QUnit.test('editable list: list view hidden by an invisible modifier', async function (assert) {
        ...
        await testUtils.dom.click(form.el.querySelector('.o-field-boolean input'));
    ...
    QUnit.test('editable list: updating list state while invisible', async function (assert) {
        ...
        this.data.foo.onchanges = {
            bar: function (obj) {
                obj.o2m = [[5], [0, null, { displayName: "Whatever" }]];
            },
        };
        var form = await createView({
            View: FormView,
    ...
    QUnit.test('editable list: unnamed columns cannot be resized', async function (assert) {
        ...
        this.data.foo.records = [{ id: 1, o2m: [1] }];
        this.data.bar.records = [{ id: 1, displayName: "Oui" }];
        var form = await createView({
            View: FormView,
            model: 'foo',
            data: this.data,
        ...
        const [charTh, buttonTh] = form.$('.o-field-one2many th');
        const thRect = charTh.getBoundingClientRect();
        const resizeRect = charTh.getElementsByClassName('o-resize')[0].getBoundingClientRect();
        ...
        assert.containsNone(buttonTh, '.o-resize',
    ...
    QUnit.test('editable list view, click on m2o dropdown do not close editable row', async function (assert) {
        ...
        await testUtils.dom.click(list.$buttons.find('.o-list-button-add'));
        await testUtils.dom.click(list.$('.o-selected-row .o-data-cell .o-field-many2one input'));
        const $dropdown = list.$('.o-selected-row .o-data-cell .o-field-many2one input').autocomplete('widget');
        await testUtils.dom.click($dropdown);
        assert.containsOnce(list, '.o-selected-row', "should still have editable row");
    ...
    QUnit.test('column widths are kept when editing a record', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell:eq(1)'));
        assert.containsOnce(list, '.o-selected-row');
        ...
        var longVal = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed blandit, ' +
            ...
            'bibendum est.';
        await testUtils.fields.editInput(list.$('.o-field-widget[name=text]'), longVal);
        await testUtils.dom.click(list.$buttons.find('.o-list-button-save'));
    ...
    QUnit.test('column widths are kept when switching records in edition', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:nth(1) .o-data-cell:first'));

        assert.hasClass(list.$('.o-data-row:nth(1)'), 'o-selected-row');
        assert.strictEqual(list.$('th[data-name="m2o"]')[0].offsetWidth, width);
    ...
    QUnit.test('column widths are re-computed on window resize', async function (assert) {
        ...
        const selectorWidth = list.$('th.o-list-record-selector')[0].offsetWidth;
        ...
        const postResizeSelectorWidth = list.$('th.o-list-record-selector')[0].offsetWidth;
    ...
    QUnit.test('list view with data: text columns are not crushed', async function (assert) {
        ...
        const firstRowHeight = list.$('.o-data-row:nth(0)')[0].offsetHeight;
        const secondRowHeight = list.$('.o-data-row:nth(1)')[0].offsetHeight;
    ...
    QUnit.test("button in a list view with a default relative width", async function (assert) {
        ...
        assert.strictEqual(list.el.querySelector('.o-data-cell button').style.width, "",
    ...
    QUnit.test('row height and width should not change when switching mode', async function (assert) {
        ...
        var startHeight = list.$('.o-data-row:first').outerHeight();
        var startWidth = list.$('.o-data-row:first').outerWidth();

        // start edition of first row
        await testUtils.dom.click(list.$('.o-data-row:first > td:not(.o-list-record-selector)').first());
        assert.hasClass(list.$('.o-data-row:first'), 'o-selected-row');
        var editionHeight = list.$('.o-data-row:first').outerHeight();
        var editionWidth = list.$('.o-data-row:first').outerWidth();

        // leave edition
        await testUtils.dom.click(list.$buttons.find('.o-list-button-save'));
        var readonlyHeight = list.$('.o-data-row:first').outerHeight();
        var readonlyWidth = list.$('.o-data-row:first').outerWidth();
    ...
    QUnit.test('fields are translatable in list view', async function (assert) {
            ...
            mockRPC: function (route, args) {
                if (route === "/web/dataset/call_button" && args.method === 'translateFields') {
                    return Promise.resolve({
                        domain: [],
                        context: {searchDefault_label: 'foo,foo'},
                    });
                }
        ...
        await testUtils.dom.click(list.$('input.o-field-translate+span.o-field-translate'));
        await testUtils.nextTick();

        assert.containsOnce($('body'), '.o-translation-dialog');
        assert.containsN($('.o-translation-dialog'), '.translation>input.o-field-char', 2,
    ...
    QUnit.test('long words in text cells should break into smaller lines', async function (assert) {
        ...
        var shortText = list.$('.o-data-row:eq(0) td:last')[0].clientHeight;
        var longText = list.$('.o-data-row:eq(1) td:last')[0].clientHeight;
        var emptyText = list.$('.o-data-row:eq(2) td:last')[0].clientHeight;
    ...
    QUnit.test('deleting one record', async function (assert) {
        ...
        assert.containsNone(list.el, 'div.o-control-panel .o-cp-action-menus');
        assert.containsN(list, 'tbody td.o-list-record-selector', 4, "should have 4 records");
        ...
        assert.containsN(list, 'tbody td.o-list-record-selector', 3, "should have 3 records");
        list.destroy();
    });
        ...
        const list = await createView({
            ...
            mockRPC: function (route, args) {
                if (args.method === 'unlink') {
                    return Promise.reject({ message: "Verp Server Error" });
                }
                return this._super(...arguments);
            },
        });
    ...
    QUnit.test('delete all records matching the domain', async function (assert) {
        ...
        assert.containsNone(list, 'div.o-control-panel .o-cp-action-menus');
        assert.containsN(list, 'tbody td.o-list-record-selector', 2, "should have 2 records");

        await testUtils.dom.click(list.$('thead .o-list-record-selector input'));

        assert.containsOnce(list, 'div.o-control-panel .o-cp-action-menus');
        assert.containsOnce(list, '.o-list-selection_box .o-list-select_domain');
    ...
    QUnit.test('archiving one record', async function (assert) {
        ...
        var list = await createView({
            ...
            mockRPC: function (route) {
                assert.step(route);
                if (route === '/web/dataset/callKw/foo/actionArchive') {
        ...
        assert.containsN(list, 'tbody td.o-list-record-selector', 4, "still should have 4 records");
        ...
        await testUtils.dom.click($('.modal-footer .btn-primary'));
        assert.containsN(list, 'tbody td.o-list-record-selector', 3, "should have 3 records");
        assert.verifySteps(['/web/dataset/callKw/foo/actionArchive', '/web/dataset/searchRead']);
        list.destroy();
    });
    ...
    QUnit.test('archive/unarchive handles returned action', async function (assert) {
        ...
        serverData.views = {
            ...
            'bar,false,form': '<form><field name="displayName"/></form>',
        };
        ...
        assert.containsNone(webClient, '.o-cp-action-menus', 'sidebar should be invisible');
        assert.containsN(webClient, 'tbody td.o-list-record-selector', 4, "should have 4 records");

        await click(webClient.el.querySelector('tbody td.o-list-record-selector input'));
        assert.containsOnce(webClient, '.o-cp-action-menus', 'sidebar should be visible');

        await click(webClient.el.querySelector('.o-cp-action-menus .dropdown-toggle'));
        const archiveItem = [...webClient.el.querySelectorAll('.o-cp-action-menus .dropdown-menu li > a')]
    ...
    QUnit.test('pager (ungrouped and grouped mode), default limit', async function (assert) {
        ...
        assert.containsOnce(list.el, 'div.o-control-panel .o-cp-pager');
    ...
    QUnit.test('can sort records when clicking on header', async function (assert) {
        ...
        assert.strictEqual(nbSearchRead, 1, "should have done one searchRead");
    ...
    QUnit.test('do not sort records when clicking on header with nolabel', async function (assert) {
        ...
        assert.strictEqual(nbSearchRead, 1, "should have done one searchRead");
        assert.strictEqual(list.$('.o-data-cell').text(), "yop10blip9gnap17blip-4");

        await testUtils.dom.click(list.$('thead th[data-name="int_field"]'));
        assert.strictEqual(nbSearchRead, 2, "should have done one other searchRead");
        assert.strictEqual(list.$('.o-data-cell').text(), "blip-4blip9yop10gnap17");

        await testUtils.dom.click(list.$('thead th[data-name="foo"]'));
        assert.strictEqual(nbSearchRead, 2, "shouldn't have done anymore searchRead");
        assert.strictEqual(list.$('.o-data-cell').text(), "blip-4blip9yop10gnap17");

        list.destroy();
    });

    QUnit.test('use defaultOrder', async function (assert) {
        assert.expect(3);
        ...
        var list = await createView({
            ...
            model: 'foo',
            data: this.data,
            arch: '<tree defaultOrder="foo"><field name="foo"/><field name="bar"/></tree>',
            mockRPC: function (route, args) {
                if (route === '/web/dataset/searchRead') {
    ...
    QUnit.test('use more complex defaultOrder', async function (assert) {
        assert.expect(3);
        ...
        var list = await createView({
            ...
            model: 'foo',
            data: this.data,
            arch: '<tree defaultOrder="foo, bar desc, int_field">' +
    ...
    QUnit.test('use defaultOrder on editable tree: sort on save', async function (assert) {
        assert.expect(8);
        ...
        var form = await createView({
            ...
            arch: '<form>' +
                    '<sheet>' +
                        '<field name="o2m">' +
                            '<tree editable="bottom" defaultOrder="displayName">' +
                                '<field name="displayName"/>' +
                            '</tree>' +
                        '</field>' +
                    '</sheet>' +
        ...
        var $o2m = form.$('.o-field-widget[name=o2m]');
        await testUtils.dom.click(form.$('.o-field-x2many-list-row-add a'));
        await testUtils.fields.editInput($o2m.find('.o-field-widget'), "Value 2");
        assert.ok(form.$('tbody tr:first td:contains(Value 1)').length,
            "Value 1 should be first");
        assert.ok(form.$('tbody tr:eq(1) td:contains(Value 3)').length,
            "Value 3 should be second");
    ...
    QUnit.test('use defaultOrder on editable tree: sort on demand', async function (assert) {
        assert.expect(11);
        ...
        var form = await createView({
            ...
            arch: '<form>' +
                    '<sheet>' +
                        '<field name="o2m">' +
                            '<tree editable="bottom" defaultOrder="name">' +
                                '<field name="label"/>' +
                            '</tree>' +
                        '</field>' +
                    '</sheet>' +
        ...
        await testUtils.dom.click(form.$('.o-form-sheet-bg'));

        await testUtils.dom.click($o2m.find('.o-column-sortable'));
    ...
    QUnit.test('use defaultOrder on editable tree: sort on demand in page', async function (assert) {
        assert.expect(4);
        ...
        await testUtils.controlPanel.pagerNext('.o-field-widget[name=o2m]');
        ...
        await testUtils.dom.click(form.$('.o-column-sortable'));
    ...
    QUnit.test('can display a list with a many2many field', async function (assert) {
        ...
        assert.verifySteps(['/web/dataset/searchRead'], "should have done 1 searchRead");
        assert.ok(list.$('td:contains(3 records)').length,
            "should have a td with correct formatted value");
        list.destroy();
    });

    QUnit.test('list with groupby_noLeaf flag in context', async function (assert) {
        assert.expect(1);
        ...
        assert.containsNone(list, '.o-list-buttons', "should not have any buttons");
        list.destroy();
    });

    QUnit.test('list with groupby_noLeaf and group by', async function (assert) {
        assert.expect(4);
        ...
        const list = await createView({
            ...
            groupBy: ['currencyId'],
            context: { groupby_noLeaf: true },
        });
        const groups = list.el.querySelectorAll(".o-group-name");
        const groupsRecords = [...list.el.querySelectorAll(".o-data-row .o-data-cell")];
    ...
    QUnit.test('display a tooltip on a field', async function (assert) {
        ...
        assert.strictEqual($('.tooltip .oe-tooltip-string').length, 0, "should not have rendered a tooltip");
        ...
        list.$('th[data-name=foo]').tooltip('show', false);
        list.$('th[data-name=foo]').trigger($.Event('mouseenter'));
        assert.strictEqual($('.tooltip .oe-tooltip-string').length, 1, "should have rendered a tooltip");

        await list.reload();
        list.$('th[data-name=bar]').tooltip('show', false);
        list.$('th[data-name=bar]').trigger($.Event('mouseenter'));
        assert.containsOnce($, '.oe-tooltip-technical>li[data-item="widget"]',
    ...
    QUnit.test('support row decoration (with unset numeric values)', async function (assert) {
        ...
        assert.containsNone(list, 'tr.o-data-row.text-danger',
        ...
        assert.containsOnce(list, 'tr.o-data-row.text-danger',
    ...
    QUnit.test('support field decoration', async function (assert) {
        ...
        assert.containsN(list, 'tbody tr', 4, "should have 4 rows");
        assert.containsN(list, 'tbody td.o-list-char.text-danger', 3);
        assert.containsNone(list, 'tbody td.o-list-number.text-danger');
    ...
    QUnit.test('bounce create button when no data and click on empty area', async function (assert) {
        ...
        assert.containsNone(list, '.o-view-nocontent');
        await testUtils.dom.click(list.$('.o-list-view'));
        assert.doesNotHaveClass(list.$('.o-list-button-add'), 'o-catch-attention');

        await list.reload({ domain: [['id', '<', 0]] });
        assert.containsOnce(list, '.o-view-nocontent');
        await testUtils.dom.click(list.$('.o-view-nocontent'));
        assert.hasClass(list.$('.o-list-button-add'), 'o-catch-attention');
        list.destroy();
    });
    ...
    QUnit.test('no content helper when no data', async function (assert) {
        ...
        assert.strictEqual(list.$('.o-view-nocontent p.hello:contains(add a partner)').length, 1,
    ...
    QUnit.test('no nocontent helper when no data and no help', async function (assert) {
        ...
        assert.containsNone(list, 'tr.o-data-row',
    ...
    QUnit.test("empty list with sample data", async function (assert) {
        ...
        assert.hasClass(list.$el, 'o-view-sample-data');
        assert.containsOnce(list, '.o-list-table');
        assert.containsN(list, '.o-data-row', 10);
        assert.containsOnce(list, '.o-nocontent-help .hello');

        // Check list sample data
        const firstRow = list.el.querySelector('.o-data-row');
        const cells = firstRow.querySelectorAll(':scope > .o-data-cell');
        ...
        assert.doesNotHaveClass(list.$el, 'o-view-sample-data');
        assert.containsNone(list, '.o-list-table');
        assert.containsOnce(list, '.o-nocontent-help .hello');
        ...
        assert.containsOnce(list, '.o-list-table');
        assert.containsN(list, '.o-data-row', 4);
        assert.containsNone(list, '.o-nocontent-help .hello');
    ...
    QUnit.test("empty list with sample data: toggle optional field", async function (assert) {
        ...
        assert.hasClass(list.$el, 'o-view-sample-data');
        assert.ok(list.$('.o-data-row').length > 0);
        assert.hasClass(list.el.querySelector('.o-data-row'), 'o-sample-data-disabled');
        assert.containsN(list, 'th', 2, "should have 2 th, 1 for selector and 1 for foo");
        assert.containsOnce(list.$('table'), '.o-optional-columns-dropdown-toggle');

        await testUtils.dom.click(list.$('table .o-optional-columns-dropdown-toggle'));
        await testUtils.dom.click(list.$('div.o-optional-columns div.dropdown-item:first input'));
    ...
    QUnit.test("empty list with sample data: keyboard navigation", async function (assert) {
        ...
        assert.hasClass(
            list.el.querySelector('.o-data-row'),
            'o-sample-data-disabled'
        );
        assert.hasClass(
            list.el.querySelector('.o-list-table > tfoot'),
            'o-sample-data-disabled'
        );
        assert.hasClass(
            list.el.querySelector('.o-list-table > thead .o-list-record-selector'),
            'o-sample-data-disabled'
        );
        ...
        // From 'Create' button
        document.querySelector('.btn.o-list-button-add').focus();

        assert.hasClass(document.activeElement, 'o-list-button-add');
        ...
        assert.containsNone(document.body, '.oe-tooltip-string');
    ...
    QUnit.test("non empty list with sample data", async function (assert) {
        ...
        assert.containsOnce(list, '.o-list-table');
        assert.containsNone(list, '.o-data-row');
        assert.doesNotHaveClass(list.$el, 'o-view-sample-data');
    ...
    QUnit.test('click on header in empty list with sample data', async function (assert) {
        ...
        assert.hasClass(list, 'o-view-sample-data');
        assert.containsOnce(list, '.o-list-table');
        assert.containsN(list, '.o-data-row', 10);

        const content = list.$el.text();
        await testUtils.dom.click(list.$('tr:first .o-column-sortable:first'));
    ...
    QUnit.test("non empty editable list with sample data: delete all records", async function (assert) {
        ...
        assert.doesNotHaveClass(list, 'o-view-sample-data');
        assert.containsOnce(list, '.o-list-table');
        assert.containsN(list, '.o-data-row', 4);
        assert.containsNone(list, '.o-nocontent-help');

        // Delete all records
        await testUtils.dom.click(list.el.querySelector('thead .o-list-record-selector input'));
        await testUtils.controlPanel.toggleActionMenu(list);
        await testUtils.controlPanel.toggleMenuItem(list, "Delete");
        await testUtils.dom.click($('.modal-footer .btn-primary'));
        ...
        assert.doesNotHaveClass(list, 'o-view-sample-data');
        assert.containsNone(list, '.o-list-table');
        assert.containsOnce(list, '.o-nocontent-help');
    ...
    QUnit.test("empty editable list with sample data: start create record and cancel", async function (assert) {
        ...
        await testUtils.dom.click(list.el.querySelector('.btn.o-list-button-add'));

        assert.doesNotHaveClass(list, 'o-view-sample-data');
        assert.containsOnce(list, '.o-data-row');

        // Discard temporary record
        await testUtils.dom.click(list.el.querySelector('.btn.o-list-button-discard'));
    ...
    QUnit.test("empty editable list with sample data: create and delete record", async function (assert) {
        ...
        await testUtils.dom.click(list.el.querySelector('.btn.o-list-button-save'));
        ...
        await testUtils.dom.click(list.el.querySelector('.o-data-row input'));
        await testUtils.controlPanel.toggleActionMenu(list);
        await testUtils.controlPanel.toggleMenuItem(list, "Delete");
        await testUtils.dom.click($('.modal-footer .btn-primary'));
    ...
    QUnit.test('groupby node with a button', async function (assert) {
        ...
        var list = await createView({
            ...
            intercepts: {
                execute_action: function (ev) {
                    ...
                    assert.strictEqual(ev.data.actionData.name, 'button_method',
                        "should call correct method");
                    assert.strictEqual(ev.data.actionData.type, 'object',
        ...
        assert.verifySteps(['/web/dataset/searchRead']);
        assert.containsOnce(list, 'thead th:not(.o-list-record-selector)',
            "there should be only one column");

        await list.update({groupBy: ['currencyId']});

        assert.verifySteps(['webReadGroup']);
        assert.containsN(list, '.o-group-header', 2,
            "there should be 2 group headers");
        assert.containsNone(list, '.o-group-header button', 0,
            "there should be no button in the header");

        await testUtils.dom.click(list.$('.o-group-header:eq(0)'));
        assert.verifySteps(['/web/dataset/searchRead']);
        assert.containsOnce(list, '.o-group-header button');

        await testUtils.dom.click(list.$('.o-group-header:eq(0) button'));
    ...
    QUnit.test('groupby node with a button in inner groupbys', async function (assert) {
        ...
        var list = await createView({
            ...
            arch: '<tree>' +
                ...
                '</groupby>' +
            '</tree>',
            groupBy: ['bar', 'currencyId'],
        });

        assert.containsN(list, '.o-group-header', 2,
            "there should be 2 group headers");
        assert.containsNone(list, '.o-group-header button',
            "there should be no button in the header");

        await testUtils.dom.click(list.$('.o-group-header:eq(0)'));

        assert.containsN(list, 'tbody:eq(1) .o-group-header', 2,
            "there should be 2 inner groups header");
        assert.containsNone(list, 'tbody:eq(1) .o-group-header button',
            "there should be no button in the header");

        await testUtils.dom.click(list.$('tbody:eq(1) .o-group-header:eq(0)'));
    ...
    QUnit.test('groupby node with a button with modifiers', async function (assert) {
        ...
        assert.verifySteps(['webReadGroup', 'read']);

        await testUtils.dom.click(list.$('.o-group-header:eq(0)'));

        assert.verifySteps(['/web/dataset/searchRead']);
        assert.containsOnce(list, '.o-group-header button.o-invisible-modifier',
            "the first group (EUR) should have an invisible button");

        await testUtils.dom.click(list.$('.o-group-header:eq(1)'));

        assert.verifySteps(['/web/dataset/searchRead']);
        assert.containsN(list, '.o-group-header button', 2,
            "there should be two buttons (one by header)");
        assert.doesNotHaveClass(list, '.o-group-header:eq(1) button', 'o-invisible-modifier',
    ...
    QUnit.test('groupby node with a button with modifiers using a many2one', async function (assert) {
        ...
        assert.containsOnce(list, '.o-group-header:eq(0) button.o-invisible-modifier');
        assert.containsOnce(list, '.o-group-header:eq(1) button:not(.o-invisible-modifier)');
    ...
    QUnit.test('reload list view with groupby node', async function (assert) {
        ...
        assert.containsOnce(list, '.o-group-header button:not(.o-invisible-modifier)',
    ...
    QUnit.test('editable list view with groupby node and modifiers', async function (assert) {
        ...
        assert.doesNotHaveClass(list.$('.o-data-row:first'), 'o-selected-row',
            "first row should be in readonly mode");

        await testUtils.dom.click(list.$('.o-data-row:first .o-data-cell'));
        assert.hasClass(list.$('.o-data-row:first'), 'o-selected-row',
    ...
    QUnit.test('groupby node with subfields, and onchange', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:first .o-data-cell:first'));
        await testUtils.fields.editInput(list.$('.o-field-widget[name=foo]'), "new value");
    ...
    QUnit.test('list view, editable, without data', async function (assert) {
        ...
        assert.hasClass(list.$('tbody tr:eq(0)'), 'o-selected-row',
        ...
        assert.strictEqual(list.$('tr.o-selected-row .o-list-record-selector input').prop('disabled'), true,
            "record selector checkbox should be disabled while the record is not yet created");
        assert.strictEqual(list.$('.o-list-button button').prop('disabled'), true,
            "buttons should be disabled while the record is not yet created");

        await testUtils.dom.click(list.$buttons.find('.o-list-button-save'));

        assert.strictEqual(list.$('tbody tr:eq(0) .o-list-record-selector input').prop('disabled'), false,
            "record selector checkbox should not be disabled once the record is created");
        assert.strictEqual(list.$('.o-list-button button').prop('disabled'), false,
    ...
    QUnit.test('list view, editable, with a button', async function (assert) {
        ...
        assert.containsOnce(list, 'table button.o-icon-button i.fa-phone',
    ...
    QUnit.test('list view, editable, can discard', async function (assert) {
        ...
        assert.strictEqual(list.$('td:not(.o-list-record-selector) input').length, 0, "no input should be in the table");

        await testUtils.dom.click(list.$('tbody td:not(.o-list-record-selector):first'));
        assert.strictEqual(list.$('td:not(.o-list-record-selector) input').length, 1, "first cell should be editable");

        assert.ok(list.$buttons.find('.o-list-button-discard').is(':visible'),
    ...
    QUnit.test('editable list view, click on the list to save', async function (assert) {
        ...
        await testUtils.dom.click(list.$buttons.find('.o-list-button-add'));
        await testUtils.fields.editInput(list.$('tr.o-selected-row input[name="foo"]'), 'new value');
        await testUtils.dom.click(list.$('.o-list-view'));
    ...
    QUnit.test('click on a button in a list view', async function (assert) {
            ...
            intercepts: {
                execute_action: function (event) {
                    ...
                    assert.strictEqual(event.data.env.model, 'foo',
                        'should call with correct model');
                    assert.strictEqual(event.data.actionData.name, 'button_action',
                        "should call correct method");
                    assert.strictEqual(event.data.actionData.type, 'object',
                        'should have correct type');
        ...
        assert.containsN(list, 'tbody .o-list-button', 4,
            "there should be one button per row");
        assert.containsOnce(list, 'tbody .o-list-button:first .o-icon-button .fa.fa-car',
            'buttons should have correct icon');

        await testUtils.dom.click(list.$('tbody .o-list-button:first > button'));
        assert.verifySteps(['/web/dataset/searchRead', '/web/dataset/searchRead'],
    ...
    QUnit.test('invisible attrs in readonly and editable list', async function (assert) {
        ...
        assert.hasClass(list.$('tbody tr:nth(0) td:nth(1) button'), "o-invisible-modifier",
            "button with invisible attrs should be properly hidden");

        // edit first row
        await testUtils.dom.click(list.$('tbody tr:nth(0) td:nth(2)'));
        assert.strictEqual(list.$('tbody tr:nth(0) td:nth(4) input.o-invisible-modifier').length, 1,
        ...
        assert.hasClass(list.$('tbody tr:nth(0)'),'o-selected-row',
    ...
    QUnit.test('monetary fields are properly rendered', async function (assert) {
        ...
        assert.containsN(list, 'tbody tr:first td', 3,
            "currencyId column should not be in the table");
        assert.strictEqual(list.$('tbody tr:first td:nth(2)').text().replace(/\s/g, ' '),
            '1200.00 €', "currencyId column should not be in the table");
        assert.strictEqual(list.$('tbody tr:nth(1) td:nth(2)').text().replace(/\s/g, ' '),
            '$ 500.00', "currencyId column should not be in the table");
    ...
    QUnit.test('edit a row by clicking on a readonly field', async function (assert) {
        ...
        assert.hasClass(list.$('.o-data-row:first td:nth(1)'),'o-readonly-modifier',
            "foo field cells should have class 'o-readonly-modifier'");

        // edit the first row
        await testUtils.dom.click(list.$('.o-data-row:first td:nth(1)'));
        assert.hasClass(list.$('.o-data-row:first'),'o-selected-row',
            "first row should be selected");
        var $cell = list.$('.o-data-row:first td:nth(1)');
        // review
        assert.hasClass($cell, 'o-readonly-modifier');
        assert.hasClass($cell.parent(),'o-selected-row');
        assert.strictEqual(list.$('.o-data-row:first td:nth(1) span').text(), 'yop',
            "a widget should have been rendered for readonly fields");
        assert.hasClass(list.$('.o-data-row:first td:nth(2)').parent(),'o-selected-row',
            "field 'int_field' should be in edition");
        assert.strictEqual(list.$('.o-data-row:first td:nth(2) input').length, 1,
    ...
    QUnit.test('list view with nested groups', async function (assert) {
        ...
        var list = await createView({
            ...
            mockRPC: function (route, args) {
                if (args.method === 'webReadGroup') {
                    if (args.kwargs.groupby[0] === 'foo') { // nested readGroup
                        // called twice (once when opening the group, once when sorting)
                        assert.deepEqual(args.kwargs.domain, [['m2o', '=', 1]],
                            "nested readGroup should be called with correct domain");
                    }
                    nbRPCs.readGroup++;
                } else if (route === '/web/dataset/searchRead') {
                    // called twice (once when opening the group, once when sorting)
                    assert.deepEqual(args.domain, [['foo', '=', 'blip'], ['m2o', '=', 1]],
                        "nested searchRead should be called with correct domain");
                    nbRPCs.searchRead++;
                }
                return this._super.apply(this, arguments);
            },
            intercepts: {
                switchView: function (event) {
                    assert.strictEqual(event.data.resId, 4,
                        "'switchView' event has been triggered");
                },
            },
        });

        assert.strictEqual(nbRPCs.readGroup, 1, "should have done one readGroup");
        assert.strictEqual(nbRPCs.searchRead, 0, "should have done no searchRead");
        ...
        assert.strictEqual(list.$('.o-group-name:first').text(), 'Value 1 (4)',
            "group should have correct name and count");
        assert.containsN(list, '.o-group-name .fa-caret-right', 2,
            "the carret of closed groups should be right");
        assert.strictEqual(list.$('.o-group-name:first span').css('padding-left'),
            '2px', "groups of level 1 should have a 2px padding-left");
        assert.strictEqual(list.$('.o-group-header:first td:last').text(), '16',
        ...
        assert.containsOnce(list, '.o-group-name:first .fa-caret-down',
            "the carret of open groups should be down");
        assert.strictEqual($openGroup.find('.o-group-header').length, 3,
            "open group should contain 3 groups");
        assert.strictEqual($openGroup.find('.o-group-name:nth(2)').text(), 'blip (2)',
            "group should have correct name and count");
        assert.strictEqual($openGroup.find('.o-group-name:nth(2) span').css('padding-left'),
            '22px', "groups of level 2 should have a 22px padding-left");
        assert.strictEqual($openGroup.find('.o-group-header:nth(2) td:last').text(), '-11',
            "inner group aggregates are correctly displayed");

        // open subgroup
        nbRPCs = {readGroup: 0, searchRead: 0};
        envIDs = [4, 5]; // the opened subgroup contains these two records
        await testUtils.dom.click($openGroup.find('.o-group-header:nth(2)'));
        assert.strictEqual(nbRPCs.readGroup, 0, "should have done no readGroup");
        assert.strictEqual(nbRPCs.searchRead, 1, "should have done one searchRead");
        assert.deepEqual(list.exportState().resIds, envIDs);

        var $openSubGroup = list.$('tbody:nth(2)');
        assert.containsN(list, 'tbody', 4, "there should be 4 tbodys");
        assert.strictEqual($openSubGroup.find('.o-data-row').length, 2,
            "open subgroup should contain 2 data rows");
        assert.strictEqual($openSubGroup.find('.o-data-row:first td:last').text(), '-4',
            "first record in open subgroup should be resId 4 (with int_field -4)");

        // open a record (should trigger event 'open_record')
        await testUtils.dom.click($openSubGroup.find('.o-data-row:first'));
        ...
        assert.strictEqual($openSubGroup.find('.o-data-row').length, 2,
            "open subgroup should contain 2 data rows");
        assert.strictEqual($openSubGroup.find('.o-data-row:first td:last').text(), '-7',
            "first record in open subgroup should be resId 5 (with int_field -7)");

        // close first level group
        nbRPCs = {readGroup: 0, searchRead: 0};
        envIDs = []; // the group being closed, there is no more record in the environment
        await testUtils.dom.click(list.$('.o-group-header:nth(1)'));
        assert.strictEqual(nbRPCs.readGroup, 0, "should have done no readGroup");
        assert.strictEqual(nbRPCs.searchRead, 0, "should have done no searchRead");
        assert.deepEqual(list.exportState().resIds, envIDs);
    ...
    QUnit.test('grouped list on selection field at level 2', async function (assert) {
        ...
        assert.strictEqual($openGroup.find('tr').length, 3,
            "should have 3 subgroups");
        assert.strictEqual($openGroup.find('.o-group-name:first').text(), 'Low (3)',
    ...
    QUnit.test('grouped list with a pager in a group', async function (assert) {
        ...
        var headerHeight = list.$('.o-group-header').css('height');

        // basic rendering checks
        await testUtils.dom.click(list.$('.o-group-header'));
        assert.strictEqual(list.$('.o-group-header').css('height'), headerHeight,
            "height of group header shouldn't have changed");
        assert.hasClass(list.$('.o-group-header th:eq(1) > nav'), 'o-pager',
            "last cell of open group header should have classname 'o-pager'");

        assert.strictEqual(testUtils.controlPanel.getPagerValue('.o-group-header'), '1-3',
            "pager's value should be correct");
        assert.containsN(list, '.o-data-row', 3,
            "open group should display 3 records");

        // go to next page
        await testUtils.controlPanel.pagerNext('.o-group-header');
        assert.strictEqual(testUtils.controlPanel.getPagerValue('.o-group-header'), '4-4',
            "pager's value should be correct");
        assert.containsOnce(list, '.o-data-row',
    ...
    QUnit.test('edition: create new line, then discard', async function (assert) {
        ...
        assert.containsN(list, 'tr.o-data-row', 4,
            "should have 4 records");
        assert.strictEqual(list.$buttons.find('.o-list-button-add:visible').length, 1,
            "create button should be visible");
        assert.strictEqual(list.$buttons.find('.o-list-button-discard:visible').length, 0,
            "discard button should be hidden");
        assert.containsN(list, '.o-list-record-selector input:enabled', 5);
        await testUtils.dom.click(list.$buttons.find('.o-list-button-add'));
        assert.strictEqual(list.$buttons.find('.o-list-button-add:visible').length, 0,
            "create button should be hidden");
        assert.strictEqual(list.$buttons.find('.o-list-button-discard:visible').length, 1,
            "discard button should be visible");
        assert.containsNone(list, '.o-list-record-selector input:enabled');
        await testUtils.dom.click(list.$buttons.find('.o-list-button-discard'));
        assert.containsN(list, 'tr.o-data-row', 4,
    ...
    QUnit.test('invisible attrs on fields are re-evaluated on field change', async function (assert) {
        ...
        assert.containsN(list, 'tbody td.o-invisible-modifier', 3,
        ...
        assert.strictEqual(list.$('tbody tr:nth(0) td:nth(1) > input[name="foo"].o-invisible-modifier').length, 1,
            "the foo field widget should have been rendered as invisible");

        await testUtils.dom.click(list.$('tbody tr:nth(0) td:nth(2) input'));
        assert.strictEqual(list.$('tbody tr:nth(0) td:nth(1) > input[name="foo"]:not(.o-invisible-modifier)').length, 1,
            "the foo field widget should have been marked as non-invisible");
        assert.containsN(list, 'tbody td.o-invisible-modifier', 2,
    ...
    QUnit.test('readonly attrs on fields are re-evaluated on field change', async function (assert) {
        ...
        assert.containsN(list, 'tbody td.o-readonly-modifier', 3,
        ...
        assert.containsN(list, 'tbody td.o-readonly-modifier', 2,
    ...
    QUnit.test('required attrs on fields are re-evaluated on field change', async function (assert) {
        ...
        assert.containsN(list, 'tbody td.o-required-modifier', 3,
            "there should be 3 required foo cells in readonly mode");

        // Make first line editable
        await testUtils.dom.click(list.$('tbody tr:nth(0) td:nth(1)'));

        assert.strictEqual(list.$('tbody tr:nth(0) td:nth(1) > input[name="foo"].o-required-modifier').length, 1,
            "the foo field widget should have been rendered as required");

        await testUtils.dom.click(list.$('tbody tr:nth(0) td:nth(2) input'));
        assert.strictEqual(list.$('tbody tr:nth(0) td:nth(1) > input[name="foo"]:not(.o-required-modifier)').length, 1,
            "the foo field widget should have been marked as non-required");
        assert.containsN(list, 'tbody td.o-required-modifier', 2,
    ...
    QUnit.test('modifiers of other x2many rows a re-evaluated when a subrecord is updated', async function (assert) {
        ...
        this.data.foo.onchanges = {
            o2m: function (obj) {
                obj.o2m = [
                    [5],
                    [1, 1, { displayName: 'Value 1', stage: 'open' }],
                    [1, 2, { displayName: 'Value 2', stage: 'draft' }],
                ];
            },
        };
        ...
        const form = await createView({
            ...
            arch:
                `<form>
                    <field name="o2m">
                        <tree editable="top">
                            <field name="displayName" attrs="{'invisible': [('stage', '=', 'open')]}"/>
                            <field name="stage"/>
                        </tree>
                    </field>
                </form>`,
        ...
        assert.hasClass(form.$('.o-field-widget[name=o2m] tbody tr:nth(1) td:nth(0)'), 'o-invisible-modifier',
            "first cell of second row should have o-invisible-modifier class");
        assert.doesNotHaveClass(form.$('.o-field-widget[name=o2m] tbody tr:nth(0) td:nth(0)'), 'o-invisible-modifier',
            "first cell of first row should not have o-invisible-modifier class");

        // Make a change in the list to trigger the onchange
        await testUtils.dom.click(form.$('.o-field-widget[name=o2m] tbody tr:nth(0) td:nth(1)'));
        await testUtils.fields.editSelect(form.$('.o-field-widget[name=o2m] tbody select[name="stage"]'), '"open"');

        assert.strictEqual(form.$('.o-data-row:nth(1)').text(), 'Value 2Draft',
            "the onchange should have been applied");
        assert.doesNotHaveClass(form.$('.o-field-widget[name=o2m] tbody tr:nth(1) td:nth(0)'), 'o-invisible-modifier',
            "first cell of second row should not have o-invisible-modifier class");
        assert.hasClass(form.$('.o-field-widget[name=o2m] tbody tr:nth(0) td:nth(0)'), 'o-invisible-modifier',
            "first cell of first row should have o-invisible-modifier class");
    ...
    QUnit.test('leaving unvalid rows in edition', async function (assert) {
        ...
        assert.strictEqual($firstFooTd.parent('.o-selected-row').length, 1,
            "first line should still be in edition as invalid");
        assert.containsOnce(list, 'tbody tr.o-selected-row',
            "no other line should be in edition");
        assert.strictEqual($firstFooTd.find('input.o-field-invalid').length, 1,
    ...
    QUnit.test('open a virtual id', async function (assert) {
        ...
        var list = await createView({
            ...
            model: 'event',
            data: this.data,
            arch: '<tree><field name="label"/></tree>',
        });
        ...
        testUtils.mock.intercept(list, 'switchView', function (event) {
            assert.deepEqual(_.pick(event.data, 'mode', 'model', 'resId', 'viewType'), {
                mode: 'readonly',
                model: 'event',
                resId: '2-20170808020000',
                viewType: 'form',
            }, "should trigger a switchView event to the form view for the record virtual id");
        });
    ...
    QUnit.test('pressing enter on last line of editable list view', async function (assert) {
        ...
        await testUtils.fields.triggerKeydown(list.$('tr.o-selected-row input[name="foo"]'), 'enter');
        assert.hasClass(list.$('tr.o-data-row:eq(3)'),'o-selected-row',
            "4rd row should be selected");
        assert.doesNotHaveClass(list.$('tr.o-data-row:eq(2)'), 'o-selected-row',
            "3rd row should no longer be selected");

        // press enter on last row
        await testUtils.fields.triggerKeydown(list.$('tr.o-selected-row input[name="foo"]'), 'enter');
        assert.containsN(list, 'tr.o-data-row', 5, "should have created a 5th row");

        assert.verifySteps(['/web/dataset/searchRead', '/web/dataset/callKw/foo/onchange']);
        list.destroy();
    });
    ...
    QUnit.test('pressing tab on last cell of editable list view', async function (assert) {
        ...
        await testUtils.fields.triggerKeydown(list.$('tr.o-selected-row input[name="foo"]'), 'tab');
        assert.strictEqual(document.activeElement.name, "int_field",
            "focus should be on an input with name = int_field");

        await testUtils.fields.triggerKeydown(list.$('tr.o-selected-row input[name="foo"]'), 'tab');
        assert.hasClass(list.$('tr.o-data-row:eq(4)'),'o-selected-row',
            "5th row should be selected");
        assert.strictEqual(document.activeElement.name, "foo",
            "focus should be on an input with name = foo");

        assert.verifySteps(['/web/dataset/searchRead',
            '/web/dataset/callKw/foo/write',
            '/web/dataset/callKw/foo/read',
            '/web/dataset/callKw/foo/onchange']);
        list.destroy();
    });
    ...
    QUnit.test('navigation with tab and read completes after default_get', async function (assert) {
        ...
        await testUtils.fields.editInput(list.$('tr.o-selected-row input[name="int_field"]'), '1234');
        await testUtils.fields.triggerKeydown(list.$('tr.o-selected-row input[name="int_field"]'), 'tab');

        onchangeGetPromise.resolve();
        assert.containsN(list, 'tbody tr.o-data-row', 4,
            "should have 4 data rows");

        readPromise.resolve();
        await testUtils.nextTick();
        assert.containsN(list, 'tbody tr.o-data-row', 5,
    ...
    QUnit.test('display toolbar', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-list-record-selector:first input'));
    ...
    QUnit.test('execute ActionMenus actions with correct params (single page)', async function (assert) {
        ...
        await testUtils.dom.click(list.$('tbody .o-list-record-selector:first input'));
        assert.containsN(list, '.o-list-record-selector input:checked', 3);
        await testUtils.controlPanel.toggleActionMenu(list);
        await testUtils.controlPanel.toggleMenuItem(list, "Custom Action");
        ...
        await testUtils.dom.click(list.$('tbody .o-list-record-selector:nth(0) input'));
        await testUtils.dom.click(list.$('tbody .o-list-record-selector:nth(1) input'));
        assert.containsN(list, '.o-list-record-selector input:checked', 2);

        await testUtils.controlPanel.toggleActionMenu(list);
        await testUtils.controlPanel.toggleMenuItem(list, "Custom Action");

        assert.verifySteps([
            '{"actionId":44,"context":{"activeId":1,"activeIds":[1,2,3,4],"activeModel":"foo","activeDomain":[]}}',
            '{"actionId":44,"context":{"activeId":2,"activeIds":[2,3,4],"activeModel":"foo","activeDomain":[]}}',
            '{"actionId":44,"context":{"activeId":1,"activeIds":[1,2],"activeModel":"foo","activeDomain":[["bar","=",true]]}}',
        ]);
    ...
    QUnit.test('execute ActionMenus actions with correct params (multi pages)', async function (assert) {
        ...
        assert.verifySteps([
            '{"actionId":44,"context":{"activeId":1,"activeIds":[1,2],"activeModel":"foo","activeDomain":[]}}',
            '{"actionId":44,"context":{"activeId":1,"activeIds":[1,2,3,4],"activeModel":"foo","activeDomain":[]}}',
            '{"actionId":44,"context":{"activeId":1,"activeIds":[1,2,3],"activeModel":"foo","activeDomain":[["bar","=",true]]}}',
        ]);
    ...
    QUnit.test('edit list line after line deletion', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:nth(2) > td:not(.o-list-record-selector)').first());
        assert.ok(list.$('.o-data-row:nth(2)').is('.o-selected-row'),
            "third row should be in edition");
        await testUtils.dom.click(list.$buttons.find('.o-list-button-discard'));
        await testUtils.dom.click(list.$buttons.find('.o-list-button-add'));
        assert.ok(list.$('.o-data-row:nth(0)').is('.o-selected-row'),
    ...
    QUnit.test('pressing TAB in editable list with several fields [REQUIRE FOCUS]', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-cell:first'));
        assert.hasClass(list.$('.o-data-row:first'), 'o-selected-row');
        assert.strictEqual(document.activeElement, list.$('.o-data-row:first .o-data-cell:first input')[0]);

        // // Press 'Tab' -> should go to next cell (still in first row)
        await testUtils.fields.triggerKeydown(list.$('.o-selected-row input[name="foo"]'), 'tab');
        assert.hasClass(list.$('.o-data-row:first'), 'o-selected-row');
        assert.strictEqual(document.activeElement, list.$('.o-data-row:first .o-data-cell:last input')[0]);

        // // Press 'Tab' -> should go to next line (first cell)
        await testUtils.fields.triggerKeydown(list.$('.o-selected-row input[name="int_field"]'), 'tab');
        assert.hasClass(list.$('.o-data-row:nth(1)'), 'o-selected-row');
        assert.strictEqual(document.activeElement, list.$('.o-data-row:nth(1) .o-data-cell:first input')[0]);
    ...
    QUnit.test('pressing SHIFT-TAB in editable list with several fields [REQUIRE FOCUS]', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:nth(2) .o-data-cell:nth(1)'));
        assert.hasClass(list.$('.o-data-row:nth(2)'), 'o-selected-row');
        assert.strictEqual(document.activeElement, list.$('.o-data-row:nth(2) .o-data-cell:last input')[0]);

        // Press 'shift-Tab' -> should go to previous line (last cell)
        list.$('tr.o-selected-row input').trigger($.Event('keydown', {which: $.ui.keyCode.TAB, shiftKey: true}));
        await testUtils.nextTick();
        assert.hasClass(list.$('.o-data-row:nth(2)'), 'o-selected-row');
        assert.strictEqual(document.activeElement, list.$('.o-data-row:nth(2) .o-data-cell:first input')[0]);

        // Press 'shift-Tab' -> should go to previous cell
        list.$('tr.o-selected-row input').trigger($.Event('keydown', {which: $.ui.keyCode.TAB, shiftKey: true}));
        await testUtils.nextTick();
        assert.hasClass(list.$('.o-data-row:nth(1)'), 'o-selected-row');
        assert.strictEqual(document.activeElement, list.$('.o-data-row:nth(1) .o-data-cell:last input')[0]);
    ...
    QUnit.test('navigation with tab on a list with create="0"', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:nth(2) .o-data-cell:first'));
        assert.hasClass(list.$('.o-data-row:nth(2)'),'o-selected-row',
            "third row should be in edition");
        ...
        await testUtils.fields.editInput(list.$('.o-selected-row input').eq(1), 11);
        await testUtils.fields.triggerKeydown(list.$('.o-selected-row input[name="displayName"]'), 'tab');
        assert.hasClass(list.$('.o-data-row:nth(3)'),'o-selected-row',
    ...
    QUnit.test('navigation with tab on a one2many list with create="0"', async function (assert) {
        ...
        assert.containsN(form, '.o-field-widget[name=o2m] .o-data-row', 2,
            "there should be two records in the many2many");

        await testUtils.dom.click(form.$('.o-field-widget[name=o2m] .o-data-cell:first'));
        assert.hasClass(form.$('.o-field-widget[name=o2m] .o-data-row:first'),'o-selected-row',
            "first row should be in edition");

        // Press 'Tab' -> should go to next line
        await testUtils.fields.triggerKeydown(form.$('.o-field-widget[name=o2m] .o-selected-row input'), 'tab');
        await testUtils.nextTick(); // wait for discard
        assert.hasClass(form.$('.o-field-widget[name=o2m] .o-data-row:nth(1)'),'o-selected-row',
    ...
    QUnit.test('edition, then navigation with tab (with a readonly field)', async function (assert) {
        ...
        // click on first td and press TAB
        await testUtils.dom.click(list.$('td:contains(yop)'));
        await testUtils.fields.editSelect(list.$('tr.o-selected-row input[name="foo"]'), 'new value');
        await testUtils.fields.triggerKeydown(list.$('tr.o-selected-row input[name="foo"]'), 'tab');
    ...
    QUnit.test('edition, then navigation with tab (with a readonly field and onchange)', async function (assert) {
        ...
        await testUtils.dom.click(form.$('.o-data-cell:contains(yop)'));
        assert.strictEqual(document.activeElement, form.$('tr.o-selected-row input[name="foo"]')[0],
            "focus should be on an input with name = foo");
        ...
        await testUtils.dom.triggerEvents(form.$('tr.o-selected-row input[name="foo"]'), [tabEvent]);
        assert.strictEqual(document.activeElement, form.$('tr.o-selected-row input[name="int_field"]')[0],
            "focus should be on an input with name = int_field");
    ...
    QUnit.test('pressing SHIFT-TAB in editable list with a readonly field [REQUIRE FOCUS]', async function (assert) {
        ...
        // start on 'qux', line 3
        await testUtils.dom.click(list.$('.o-data-row:nth(2) .o-data-cell:nth(2)'));
        assert.hasClass(list.$('.o-data-row:nth(2)'), 'o-selected-row');
        assert.strictEqual(document.activeElement, list.$('.o-data-row:nth(2) .o-data-cell input[name=qux]')[0]);
        ...
        await testUtils.nextTick();
        assert.hasClass(list.$('.o-data-row:nth(2)'), 'o-selected-row');
        assert.strictEqual(document.activeElement, list.$('.o-data-row:nth(2) .o-data-cell input[name=foo]')[0]);
    ...
    QUnit.test('pressing SHIFT-TAB in editable list with a readonly field in first column [REQUIRE FOCUS]', async function (assert) {
        ...
        await testUtils.nextTick();
        assert.hasClass(list.$('.o-data-row:nth(1)'), 'o-selected-row');
        assert.strictEqual(document.activeElement, list.$('.o-data-row:nth(1) .o-data-cell input[name=qux]')[0]);
    ...
    QUnit.test('pressing SHIFT-TAB in editable list with a readonly field in last column [REQUIRE FOCUS]', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:nth(2) .o-data-cell:first'));
        assert.hasClass(list.$('.o-data-row:nth(2)'), 'o-selected-row');
        assert.strictEqual(document.activeElement, list.$('.o-data-row:nth(2) .o-data-cell input[name=int_field]')[0]);

        // Press 'shift-Tab' -> should go to previous line ('foo' field)
        $(document.activeElement).trigger({type: 'keydown', which: $.ui.keyCode.TAB, shiftKey: true});
        await testUtils.nextTick();
        assert.hasClass(list.$('.o-data-row:nth(1)'), 'o-selected-row');
        assert.strictEqual(document.activeElement, list.$('.o-data-row:nth(1) .o-data-cell input[name=foo]')[0]);
    ...
    QUnit.test('navigation: not moving down with keydown', async function (assert) {
        ...
        await testUtils.dom.click(list.$('td:contains(yop)'));
        assert.hasClass(list.$('tr.o-data-row:eq(0)'),'o-selected-row',
            "1st row should be selected");
        await testUtils.fields.triggerKeydown(list.$('tr.o-selected-row input[name="foo"]'), 'down');
        assert.hasClass(list.$('tr.o-data-row:eq(0)'),'o-selected-row',
    ...
    QUnit.test('discarding changes in a row properly updates the rendering', async function (assert) {
        ...
        assert.strictEqual(list.$('.o-data-cell:first').text(), "yop",
    ...
    QUnit.test('numbers in list are right-aligned', async function (assert) {
        ...
        var nbCellRight = _.filter(list.$('.o-data-row:first > .o-data-cell'), function (el) {
        ...
        var nbInputRight = _.filter(list.$('.o-data-row:first > .o-data-cell input'), function (el) {
            var style = window.getComputedStyle(el);
            return style.textAlign === 'right';
        }).length;
    ...
    QUnit.test('grouped list with another grouped list parent, click unfold', async function (assert) {
        ...
        var list = await createView({
            ...
            archs: {
                ...
                'bar,false,search': '<search><filter context="{\'groupby\': \'cornichon\'}" string="cornichon"/></search>',
            },
        });

        await list.update({groupBy: []});

        await testUtils.dom.clickFirst(list.$('.o-data-cell'));

        await testUtils.fields.many2one.searchAndClickItem('m2o', { item: 'Search More' });

        assert.containsOnce($('body'), '.modal-content');

        assert.containsNone($('body'), '.modal-content .o-group-name', 'list in modal not grouped');

        await testUtils.dom.click($('body .modal-content button:contains(Group By)'));

        await testUtils.dom.click($('body .modal-content .o-menu-item:contains(cornichon)'));

        await testUtils.dom.click($('body .modal-content .o-group-header'));

        assert.containsOnce($('body'), '.modal-content .o-group-open');
    ...
    QUnit.test('field values are escaped', async function (assert) {
        ...
        assert.strictEqual(list.$('.o-data-cell:first').text(), value,
    ...
    QUnit.test('pressing ESC discard the current line changes', async function (assert) {
        ...
        await testUtils.dom.click(list.$buttons.find('.o-list-button-add'));
        assert.containsN(list, 'tr.o-data-row', 5,
        ...
        assert.containsNone(list, 'tr.o-data-row.o-selected-row',
            "no rows should be selected");
        assert.ok(!list.$buttons.find('.o-list-button-save').is(':visible'),
            "should not have a visible save button");
        list.destroy();
    });
    ...
    QUnit.test('field with password attribute', async function (assert) {
        ...
        assert.strictEqual(list.$('td.o-data-cell:eq(0)').text(), '***',
            "should display string as password");
        assert.strictEqual(list.$('td.o-data-cell:eq(1)').text(), '****',
            "should display string as password");
    ...
    QUnit.test('result of consecutive resequences is correctly sorted', async function (assert) {
        ...
        assert.strictEqual(list.$('tbody tr td.o-list-number').text(), '1234',
        ...
        assert.strictEqual(list.$('tbody tr td.o-list-number').text(), '1243',
        ...
        assert.deepEqual(list.$('tbody tr td.o-list-number').text(), '1423',
            "the int_field (sequence) should have been correctly updated");
        ...
        assert.deepEqual(list.$('tbody tr td.o-list-number').text(), '1243',
            "the int_field (sequence) should have been correctly updated");
    ...
    QUnit.test('editable list with handle widget', async function (assert) {
        ...
        var list = await createView({
            ...
            model: 'foo',
            data: this.data,
            arch: '<tree editable="top" defaultOrder="int_field">' +
                    '<field name="int_field" widget="handle"/>' +
                    '<field name="amount" widget="float" digits="[5,0]"/>' +
                  '</tree>',
            mockRPC: function (route, args) {
    ...
    QUnit.test('list with handle widget, create and move should keep empty lines at last', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(
            ...
            list.$('tbody tr.o-data-row').eq(1),
            { position: 'bottom' }
        );
        assert.containsN(list, '.o-data-row', 2);
        assert.hasClass(list.$('.o-data-row:first'), 'o-selected-row');
        assert.doesNotHaveClass(list.$('.o-data-row:nth(1)'), 'o-selected-row');
    ...
    QUnit.test('reference field rendering', async function (assert) {
        ...
        assert.strictEqual(list.$('tbody td:not(.o-list-record-selector)').text(), "Value 1USDEUREUR",
            "should have the displayName of the reference");
        list.destroy();
    });
    ...
    QUnit.test('reference field batched in grouped list', async function (assert) {
        ...
        assert.containsN(list, '.o-group-header', 2);
        const allNames = Array.from(list.el.querySelectorAll('.o-data-cell'), node => node.textContent);
    ...
    QUnit.test('multi edit in view grouped by field not in view', async function (assert) {
        ...
        const list = await createView({
            ...
            model: 'foo',
            data: this.data,
            arch: `<tree expand="1" multiEdit="1">
                   <field name="foo"/>
               </tree>`,
            groupBy: ['m2o'],
        });
        ...
        await testUtils.dom.click(list.$('.o-data-row .o-list-record-selector input:eq(0)'));
        await testUtils.dom.click(list.$('.o-data-row .o-list-record-selector input:eq(1)'));

        await testUtils.dom.click(list.$('.o-list-char:eq(0)'));

        await testUtils.fields.editInput(list.$('.o-field-widget[name=foo]'), 'test');

        assert.containsOnce(document.body, '.modal');
        await testUtils.dom.click($('.modal .modal-footer .btn-primary'));
        assert.containsNone(document.body, '.modal');

        const allNames = [...document.querySelectorAll('.o-data-cell')].map(n => n.textContent);
        assert.deepEqual(allNames, [
            'test',
            'test',
            '1',
    ...
    QUnit.test('multi edit reference field batched in grouped list', async function (assert) {
        ...
        // Field booleanToggle just to simplify the test flow
        let nameGetCount = 0;
        const list = await createView({
            View: ListView,
            model: 'foo',
        ...
        await testUtils.dom.click(list.$('.o-data-row .o-list-record-selector input')[0]);
        await testUtils.dom.click(list.$('.o-data-row .o-list-record-selector input')[1]);
        await testUtils.dom.click(list.$('.o-data-row .o-list-record-selector input')[2]);
        await testUtils.dom.click(list.$('.o-data-row .o-field-boolean')[0]);
        assert.containsOnce(document.body, '.modal');
        await testUtils.dom.click($('.modal .modal-footer .btn-primary'));
        assert.containsNone(document.body, '.modal');
        assert.verifySteps([
        ...
        const allNames = Array.from(list.el.querySelectorAll('.o-data-cell'))
    ...
    QUnit.test('multi edit field with daterange widget', async function (assert) {
        ...
        this.data.daterange = {
            fields: {
                dateStart: {string: "Date Start", type: "date"},
                dateEnd: { string: "Date End", type: 'date' },
            },
            records: [{
                id: 1,
        ...
        const list = await createView({
            ...
            arch: `
                <tree multiEdit="1">
                    <field name="dateStart" widget="daterange" options="{'related_end_date': 'dateEnd'}" />
                    <field name="dateEnd" widget="daterange" options="{'related_start_date': 'dateStart'}"/>
                </tree>`,
            mockRPC(route, args) {
                if (args.method === 'write') {
                    assert.deepEqual(args.args, [
                        [1, 2],
                        { dateStart: "2017-01-16", dateEnd: "2017-02-12" },
                    ]);
                }
                return this._super(...arguments);
            },
        ...
        await testUtils.dom.click(list.$('thead .o-list-record-selector:first input'));

        await testUtils.dom.click(list.$('.o-data-row:first .o-data-cell:eq(0)')); // edit first row
        await testUtils.dom.click(list.$('.o-data-row:first .o-data-cell:eq(0) input.o-field-date-range'));
        ...
        const changesTable = document.querySelector('.modal-body .o-modal-changes');
    ...
    QUnit.test('multi edit field with daterange widget (edition without using the picker)', async function (assert) {
        ...
        await testUtils.dom.click(list.$('thead .o-list-record-selector:first input'));
        await testUtils.dom.click(list.$('.o-data-row:first .o-data-cell:eq(0)'));

        // Change the date in the first datetime
        await testUtils.fields.editInput(list.$('.o-data-row:first .o-data-cell:eq(0) .o-field-dateRange'), "2021-04-01 11:00:00");
    ...
    QUnit.test('editable list view: contexts are correctly sent', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-cell:first'));
        await testUtils.fields.editInput(list.$('.o-field-widget[name=foo]'), 'abc');
        await testUtils.dom.click(list.$buttons.find('.o-list-button-save'));
    ...
    QUnit.test('editable list view: contexts with multiple edit', async function (assert) {
        ...
        var list = await createView({
            ...
            arch: '<tree multiEdit="1">' +
                    ...
                    '</tree>',
            mockRPC: function (route, args) {
                if (route === '/web/dataset/callKw/foo/write' ||
                    route === '/web/dataset/callKw/foo/read') {
        ...
        await testUtils.dom.click(list.$('.o-content input:first'));
        await testUtils.dom.click(list.$('.o-data-cell:first'));
    ...
    QUnit.test('editable list view: single edition with selected records', async function (assert) {
        ...
        const list = await createView({
            arch: `<tree editable="top" multiEdit="1"><field name="foo"/></tree>`,
            data: this.data,
            model: 'foo',
            View: ListView,
        });
        ...
        // Edit the second
        await testUtils.dom.click(list.$('.o-data-row:eq(1) .o-data-cell:first()'));
        await testUtils.fields.editInput(list.$('.o-data-row:eq(1) .o-data-cell:first() input'), "oui");
        await testUtils.dom.click($('.o-list-button-save'));

        assert.strictEqual(list.$('.o-data-row:eq(0) .o-data-cell:first()').text(), "yop",
            "First row should remain unchanged");
        assert.strictEqual(list.$('.o-data-row:eq(1) .o-data-cell:first()').text(), "oui",
    ...
    QUnit.test('editable list view: non dirty record with required fields', async function (assert) {
        ...
        assert.strictEqual(list.$('.o-selected-row').data('id'), 'foo_7',
            "foo_7 record should be currently selected");
        await testUtils.dom.click(list.$('.o-data-row:eq(1) .o-data-cell:eq(0)'));
        assert.strictEqual(list.$('.o-selected-row').data('id'), 'foo_2',
        ...
        await testUtils.fields.triggerKeydown(list.$('tr.o-selected-row input.o-field-widget[name="foo"]'), 'enter');
        assert.containsOnce(list, '.o-selected-row');

        // discard row and create new record and keep required field empty and click anywhere
        await testUtils.dom.click(list.$buttons.find('.o-list-button-discard'));
        await testUtils.dom.click($('.o-list-button-add'));
        assert.containsOnce(list, '.o-selected-row', "row should be selected");
        await testUtils.fields.editInput(list.$('.o-selected-row .o-field-widget[name=int_field]'), 123);
        await testUtils.dom.click('body');
        assert.containsOnce(list, '.o-selected-row', "row should still be selected");
    ...
    QUnit.test('editable list view: do not commit changes twice', async function (assert) {
        ...
        testUtils.mock.intercept(list, "fieldChanged", function (ev) {
            assert.deepEqual(ev.data.changes, { bar: false });
        }, true);

        assert.strictEqual(list.$('.o-field-cell[name="foo"]')[0].textContent, 'yop');

        await testUtils.dom.click(list.$('.o-field-cell[name="bar"]')[0]);
        await testUtils.dom.click(list.$('.o-field-cell[name="bar"] label')[0]);

        await testUtils.dom.click(list.$('.o-list-button-save')[0]);
        assert.strictEqual(list.$('.o-field-cell[name="foo"]')[0].textContent, 'changed');
    ...
    QUnit.test('editable list view: multi edition', async function (assert) {
        ...
        var list = await createView({
            ...
            model: 'foo',
            data: this.data,
            arch: '<tree editable="bottom" multiEdit="1">' +
                        '<field name="foo"/>' +
                        '<field name="int_field"/>' +
                    '</tree>',
            mockRPC: function (route, args) {
        ...
        await testUtils.dom.click('body');
        assert.containsNone(list, '.o-selected-row', "no row should be selected");
        ...
        // edit a field
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell:eq(1)'));
        await testUtils.fields.editInput(list.$('.o-field-widget[name=int_field]'), 666);
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell:eq(0)'));
        ...
        assert.strictEqual(list.$('.o-data-row:eq(0) .o-data-cell').text(), 'yop10',
            "changes have been discarded and row is back to readonly");
        assert.strictEqual(document.activeElement, list.$('.o-data-row:eq(0) .o-data-cell:eq(1)')[0],
        ...
        await testUtils.dom.click($('.modal .btn-primary'));
        assert.containsNone(list, '.o-data-cell input.o-field-widget', "no field should be editable anymore");
        assert.containsNone(list, '.o-list-record-selector input:checked', "no record should be selected anymore");
        assert.verifySteps(['write', 'read']);
        assert.strictEqual(list.$('.o-data-row:eq(0) .o-data-cell').text(), "yop666",
            "the first row should be updated");
        assert.strictEqual(list.$('.o-data-row:eq(1) .o-data-cell').text(), "blip666",
        ...
        var list = await createView({
            ...
            intercepts: {
                switchView: function (ev) {
                    assert.strictEqual(ev.data.viewType, 'form');
                },
            },
        });

        // click on CREATE (should trigger a switchView)
        await testUtils.dom.click($('.o-list-button-add'));
    ...
    QUnit.test('editable list view: multi edition cannot call onchanges', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-list-record-selector input'));
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell:eq(0)'));
        await testUtils.fields.editInput(list.$('.o-field-widget[name=foo]'), 'hi');

        assert.containsNone(document.body, '.modal');
        assert.strictEqual(list.$('.o-data-row:eq(0) .o-data-cell').text(), "hi2");
        assert.strictEqual(list.$('.o-data-row:eq(1) .o-data-cell').text(), "blip9");
        ...
        // edit foo, first row
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell:eq(0)'));
        await testUtils.fields.editInput(list.$('.o-field-widget[name=foo]'), 'hello');
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell:eq(1)'));

        assert.containsOnce(document.body, '.modal'); // save dialog
        await testUtils.dom.click($('.modal .btn-primary'));

        assert.strictEqual(list.$('.o-data-row:eq(0) .o-data-cell').text(), "hello5");
        assert.strictEqual(list.$('.o-data-row:eq(1) .o-data-cell').text(), "hello5");
    ...
    QUnit.test('editable list view: multi edition error and cancellation handling', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell:eq(0)'));
        assert.containsNone(list, '.o-list-record-selector input:enabled');
        await testUtils.fields.editInput(list.$('.o-selected-row .o-field-widget[name=foo]'), "abc");
        await testUtils.dom.click($('.modal .btn:contains("Cancel")'));
        assert.strictEqual(list.$('.o-data-row:eq(0) .o-data-cell').text(), 'yop10', "first cell should have discarded any change");
        assert.containsN(list, '.o-list-record-selector input:enabled', 5);

        // edit a line with an invalid format type
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell:eq(1)'));
        assert.containsNone(list, '.o-list-record-selector input:enabled');
        await testUtils.fields.editInput(list.$('.o-selected-row .o-field-widget[name=int_field]'), "hahaha");
        assert.containsOnce(document.body, '.modal', "there should be an opened modal");
        await testUtils.dom.click($('.modal .btn-primary'));
        assert.strictEqual(list.$('.o-data-row:eq(0) .o-data-cell').text(), 'yop10', "changes should be discarded");
        assert.containsN(list, '.o-list-record-selector input:enabled', 5);

        // edit a line with an invalid value
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell:eq(0)'));
        assert.containsNone(list, '.o-list-record-selector input:enabled');
        await testUtils.fields.editInput(list.$('.o-selected-row .o-field-widget[name=foo]'), "");
        assert.containsOnce(document.body, '.modal', "there should be an opened modal");
        await testUtils.dom.click($('.modal .btn-primary'));
        assert.strictEqual(list.$('.o-data-row:eq(0) .o-data-cell').text(), 'yop10', "changes should be discarded");
        assert.containsN(list, '.o-list-record-selector input:enabled', 5);
    ...
    QUnit.test('multi edition: many2manyTags in many2many field', async function (assert) {
        assert.expect(5);

        for (let i = 4; i <= 10; i++) {
            this.data.bar.records.push({ id: i, displayName: "Value" + i});
        }
        ...
        const list = await createView({
            ...
            model: 'foo',
            data: this.data,
            arch: '<tree multiEdit="1"><field name="m2m" widget="many2manyTags"/></tree>',
            archs: {
                'bar,false,list': '<tree><field name="label"/></tree>',
        ...
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-list-record-selector input'));
        await testUtils.dom.click(list.$('.o-data-row:eq(1) .o-list-record-selector input'));
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell'));

        await testUtils.fields.many2one.clickOpenDropdown("m2m");
        await testUtils.fields.many2one.clickItem("m2m", "Search More");
        assert.containsOnce(document.body, '.modal .o-list-view', "should have open the modal");

        await testUtils.dom.click($('.modal .o-list-view .o-data-row:first'));

        assert.containsOnce(document.body, ".modal [role='alert']", "should have open the confirmation modal");
        assert.containsN(document.body, ".modal .o-field-many2manytags .badge", 3);
        assert.strictEqual($(".modal .o-field-many2manytags .badge:last").text().trim(), "Value 3",
            "should have displayName in badge");
    ...
    QUnit.test('multi edition: many2many field in grouped list', async function (assert) {
        ...
        assert.strictEqual(list.$('tbody:eq(1) .o-data-row:first .o-data-cell:eq(1)').text().replace(/\s/g, ''),
            'Value1Value2Value3',
            "should have a right value in many2many field");
        assert.strictEqual(list.$('tbody:eq(3) .o-data-row:first .o-data-cell:eq(1)').text().replace(/\s/g, ''),
            'Value1Value2Value3',
            "should have same value in many2many field on all other records with same resId");
    ...
    QUnit.test('editable list view: multi edition of many2one: set same value', async function (assert) {
        ...
        assert.strictEqual(list.$('.o-list-many2one').text(), "Value 1Value 2Value 1Value 1");
        ...
        assert.strictEqual(list.$('.o-list-many2one').text(), "Value 1Value 1Value 1Value 1");
    ...
    QUnit.test('editable list view: clicking on "Discard changes" in multi edition', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:first() .o-data-cell:first()'));
        list.$('.o-data-row:first() .o-data-cell:first() input').val("oof");

        const $discardButton = list.$buttons.find('.o-list-button-discard');
        ...
        await testUtils.dom.triggerEvents(list.$('.o-data-row:first() .o-data-cell:first() input'),
        ...
        assert.strictEqual(list.$('.o-data-row:first() .o-data-cell:first()').text(), "yop");
    ...
    QUnit.test('editable list view (multi edition): mousedown on "Discard", but mouseup somewhere else', async function (assert) {
        ...
        await testUtils.dom.triggerEvents(list.$buttons.find('.o-list-button-discard'), ['mousedown']);
        await testUtils.dom.triggerEvents(list.$('.o-data-row:first() .o-data-cell:first() input'),
            ['change', 'blur', 'focusout']);
    ...
    QUnit.test('editable list view (multi edition): writable fields in readonly (force save)', async function (assert) {
        ...
        // select two records
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-list-record-selector input'));
        await testUtils.dom.click(list.$('.o-data-row:eq(2) .o-list-record-selector input'));
    ...
    QUnit.test('editable list view: multi edition with readonly modifiers', async function (assert) {
        ...
        // select all records
        await testUtils.dom.click(list.$('th.o-list-record-selector input'));
        ...
        assert.strictEqual(document.querySelector('.modal .o-modal-changes .o-field-widget').style.pointerEvents, 'none',
            "pointer events should be deactivated on the demo widget");

        await testUtils.dom.click($('.modal .btn-primary'));
        assert.strictEqual(list.$('.o-data-row:eq(0) .o-data-cell').text(), "1yop666",
            "the first row should be updated");
        assert.strictEqual(list.$('.o-data-row:eq(1) .o-data-cell').text(), "2blip666",
            "the second row should be updated");
        list.destroy();
    });
    ...
    QUnit.test('editable list view: many2one with readonly modifier', async function (assert) {
        ...
        assert.containsOnce(list, '.o-data-row:eq(0) .o-data-cell:eq(0) a[name="m2o"]');
        assert.strictEqual(document.activeElement, list.$('.o-data-row:eq(0) .o-data-cell:eq(1) input')[0],
    ...
    QUnit.test('editable list view: multi edition server error handling', async function (assert) {
        ...
        assert.strictEqual(list.$('.o-data-row:eq(0) .o-data-cell').text(), 'yop',
            "first cell should have discarded any change");
        assert.strictEqual(list.$('.o-data-row:eq(1) .o-data-cell').text(), 'blip',
            "second selected record should not have changed");
        assert.containsNone(list, '.o-data-cell input.o-field-widget',
    ...
    QUnit.test('editable readonly list view: navigation', async function (assert) {
        ...
        const list = await createView({
            ...
            intercepts: {
                switchView: function (event) {
                    assert.strictEqual(event.data.resId, 3,
                        "'switchView' event has been triggered");
                },
            },
            model: 'foo',
        ...
        // select 2 records
        await testUtils.dom.click(list.$('.o-data-row:eq(1) .o-list-record-selector input'));
        await testUtils.dom.click(list.$('.o-data-row:eq(3) .o-list-record-selector input'));

        // toggle a row mode
        await testUtils.dom.click(list.$('.o-data-row:eq(1) .o-data-cell:eq(1)'));
        assert.hasClass(list.$('.o-data-row:eq(1)'), 'o-selected-row',
            "the second row should be selected");

        // Keyboard navigation only interracts with selected elements
        await testUtils.fields.triggerKeydown(list.$('tr.o-selected-row input.o-field-widget[name="int_field"]'), 'enter');
        assert.hasClass(list.$('.o-data-row:eq(3)'), 'o-selected-row',
        ...
        await testUtils.dom.click(list.$('.o-data-row:eq(2) .o-data-cell:eq(0)'));
        assert.containsNone(list, '.o-data-cell input.o-field-widget',
            "no row should be editable anymore");
        // Clicking on an unselected record while no row is being edited will open the record (switchView)
        await testUtils.dom.click(list.$('.o-data-row:eq(2) .o-data-cell:eq(0)'));
    ...
    QUnit.test('editable list view: multi edition: edit and validate last row', async function (assert) {
        ...
        // select all records
        await testUtils.dom.click(list.$('.o-list-view thead .o-list-record-selector input'));

        // edit last cell of last line
        await testUtils.dom.click(list.$('.o-data-row:last .o-data-cell:last'));
        testUtils.fields.editInput(list.$('.o-field-widget[name=int_field]'), '666');
        await testUtils.fields.triggerKeydown(list.$('tr.o-selected-row .o-data-cell:last input'), 'enter');
    ...
    QUnit.test('editable readonly list view: navigation in grouped list', async function (assert) {
        ...
        // Keyboard navigation only interracts with selected elements
        await testUtils.fields.triggerKeydown(list.$('tr.o-selected-row input.o-field-widget'), 'enter');
        assert.hasClass(list.$('.o-data-row:eq(3)'), 'o-selected-row',
            "the fourth row should be selected");
        ...
        await testUtils.dom.click(list.$('.o-data-row:eq(2) .o-data-cell:eq(0)'));
        assert.containsNone(list, '.o-data-cell input.o-field-widget', "no row should be editable anymore");
        await testUtils.dom.click(list.$('.o-data-row:eq(2) .o-data-cell:eq(0)'));
    ...
    QUnit.test('editable readonly list view: single edition does not behave like a multi-edition', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell:eq(0)'));
        await testUtils.fields.editInput(list.$('.o-field-widget[name=foo]'), "");
        ...
        // edit a field
        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-data-cell:eq(0)'));
        await testUtils.fields.editInput(list.$('.o-field-widget[name=foo]'), "bar");

        assert.containsNone($('body'),'.modal', "should not have a modal");
        assert.strictEqual(list.$('.o-data-row:eq(0) .o-data-cell').text(), "bar",
            "the first row should be updated");
    ...
    QUnit.test('editable list view: m2m tags in grouped list', async function (assert) {
        ...
        assert.notEqual(list.$('.o-data-row:first').text(), list.$('.o-data-row:last').text(),
            "First row and last row should have different values");

        await testUtils.dom.click(list.$('thead .o-list-record-selector:first input'));
        await testUtils.dom.click(list.$('.o-data-row:first .o-data-cell:eq(1)'));
        await testUtils.dom.click(list.$('.o-selected-row .o-field-many2manytags .o-delete:first'));
        await testUtils.dom.click($('.modal .btn-primary'));

        assert.strictEqual(list.$('.o-data-row:first').text(), list.$('.o-data-row:last').text(),
    ...
    QUnit.test('editable list: edit many2one from external link', async function (assert) {
        ...
        const list = await createView({
            ...
            archs: {
                'bar,false,form': '<form string="Bar"><field name="displayName"/></form>',
            },
            data: this.data,
            mockRPC: function (route, args) {
        ...
        await testUtils.dom.click(list.$('.o-external-button:first'));
        ...
        assert.strictEqual($('.modal .o-field-widget[name=m2o]').text(), "OOF",
            "Value of the m2o should be updated in the confirmation dialog");

        // Close the confirmation dialog
        await testUtils.dom.click($('.modal .btn-primary'));

        assert.strictEqual(list.$('.o-data-cell:first').text(), "OOF",
    ...
    QUnit.test('editable list with fields with readonly modifier', async function (assert) {
        ...
        assert.containsOnce(list, '.o-selected-row');
        assert.notOk(list.$('.o-selected-row .o-field-boolean input').is(':checked'));
        assert.doesNotHaveClass(list.$('.o-selected-row .o-list-char'), 'o-readonly-modifier');
        assert.hasClass(list.$('.o-selected-row .o-list-many2one'), 'o-readonly-modifier');

        await testUtils.dom.click(list.$('.o-selected-row .o-field-boolean input'));

        assert.ok(list.$('.o-selected-row .o-field-boolean input').is(':checked'));
        assert.hasClass(list.$('.o-selected-row .o-list-char'), 'o-readonly-modifier');
        assert.doesNotHaveClass(list.$('.o-selected-row .o-list-many2one'), 'o-readonly-modifier');

        await testUtils.dom.click(list.$('.o-selected-row .o-field-many2one input'));

        assert.strictEqual(document.activeElement, list.$('.o-selected-row .o-field-many2one input')[0]);
    ...
    QUnit.test('editable list with many2one: click out does not discard the row', async function (assert) {
        ...
        form.$('.o-field-many2one input').focus().val('abcdef').trigger('keyup');
        await testUtils.nextTick();

        // then simulate a mousedown outside
        form.$('.o-field-widget[name="displayName"]').focus().trigger('mousedown');
        await testUtils.nextTick();
        assert.containsOnce(document.body, '.modal', "should ask confirmation to create a record");

        // trigger the mouseup and the click
        form.$('.o-field-widget[name="displayName"]').trigger('mouseup').trigger('click');
        await testUtils.nextTick();

        assert.containsOnce(document.body, '.modal', "modal should still be displayed");
        assert.containsOnce(form, '.o-data-row', "the row should still be there");
    ...
    QUnit.test('editable list alongside html field: click out to unselect the row', async function (assert) {
        ...
        await testUtils.fields.editInput(form.$('.o-field-x2many[name="o2m"] .o-selected-row input[name="displayName"]'), 'new value');

        // click outside to unselect the row
        await testUtils.dom.click(document.body);
        assert.containsOnce(form, '.o-data-row');
        assert.doesNotHaveClass(form.$('.o-data-row'), 'o-selected-row');
    ...
    QUnit.test('grouped list edition with toggle_button widget', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-header:first'));
        assert.containsOnce(list, '.o-data-row:first .o-toggle-button-success',
            "boolean value of the first record should be true");
        await testUtils.dom.click(list.$('.o-data-row:first .o-icon-button'));
        assert.strictEqual(list.$('.o-data-row:first .text-muted:not(.o-toggle-button-success)').length, 1,
    ...
    QUnit.test('grouped list view, indentation for empty group', async function (assert) {
        ...
        var list = await createView({
            ...
            mockRPC: function (route, args) {
                // Override of the readGroup to display the row even if there is no record in it,
                // to mock the behavihour of some fields e.g stageId on the sale order.
                if (args.method === 'webReadGroup' && args.kwargs.groupby[0] === "m2o") {
                    return Promise.resolve({
                        groups: [{
                            id: 8,
                            m2o: [1, "Value 1"],
        ...
        // open the first group
        await testUtils.dom.click(list.$('.o-group-header:first'));
        assert.strictEqual(list.$('th.o-group-name').eq(1).children().length, 1,
            "There should be an empty element creating the indentation for the subgroup.");
        assert.hasClass(list.$('th.o-group-name').eq(1).children().eq(0), 'fa',
            "The first element of the row name should have the fa class");
        assert.strictEqual(list.$('th.o-group-name').eq(1).children().eq(0).is('span'), true,
    ...
    QUnit.test('concurrent reloads finishing in inverse order', async function (assert) {
        ...
        var list = await createView({
            ...
            mockRPC: function (route) {
                var result = this._super.apply(this, arguments);
                if (route === '/web/dataset/searchRead' && blockSearchRead) {
        ...
        assert.containsN(list, '.o-list-view .o-data-row', 4,
        ...
        assert.containsN(list, '.o-list-view .o-data-row', 4,
            "list view should still contain 4 records (searchRead being blocked)");
    ...
    QUnit.test('list view on a "noCache" model', async function (assert) {
        ...
        core.bus.on('clearCache', list, assert.step.bind(assert, 'clearCache'));

        // create a new record
        await testUtils.dom.click(list.$buttons.find('.o-list-button-add'));
        await testUtils.fields.editInput(list.$('.o-selected-row .o-field-widget'), 'some value');
        await testUtils.dom.click(list.$buttons.find('.o-list-button-save'));

        // edit an existing record
        await testUtils.dom.click(list.$('.o-data-cell:first'));
        await testUtils.fields.editInput(list.$('.o-selected-row .o-field-widget'), 'new value');
        await testUtils.dom.click(list.$buttons.find('.o-list-button-save'));
    ...
    QUnit.test('list view move to previous page when all records from last page deleted', async function (assert) {
        ...
        const list = await createView({
            ...
            arch: '<tree limit="3">' +
                ...
                '</tree>',
            mockRPC: function (route, args) {
                if (checkSearchRead && route === '/web/dataset/searchRead') {
        ...
        assert.strictEqual(list.$('.o-pager-counter').text().trim(), '1-3 / 4',
        "should have 2 pages and current page should be first page");

        // move to next page
        await testUtils.dom.click(list.$('.o-pager-next'));
        assert.strictEqual(list.$('.o-pager-counter').text().trim(), '4-4 / 4',
        "should be on second page");

        // delete a record
        await testUtils.dom.click(list.$('tbody .o-data-row:first td.o-list-record-selector:first input'));
        ...
        await testUtils.dom.click($('body .modal button span:contains(Ok)'));
        assert.strictEqual(list.$('.o-pager-counter').text().trim(), '1-3 / 3',
    ...
    QUnit.test('grouped list view move to previous page of group when all records from last page deleted', async function (assert) {
        ...
        await testUtils.dom.click(list.$('th.o-group-name:nth(0)'));
        assert.strictEqual(list.$('th.o-group-name:eq(0) .o-pager-counter').text().trim(), '1-2 / 3',
            "should have 2 pages and current page should be first page");

        // move to next page
        await testUtils.dom.click(list.$('.o-group-header .o-pager-next'));
        assert.strictEqual(list.$('th.o-group-name:eq(0) .o-pager-counter').text().trim(), '3-3 / 3',
            "should be on second page");
        ...
        assert.strictEqual(list.$('th.o-group-name:eq(0) .o-pager-counter').text().trim(), '',
    ...
    QUnit.test('list view move to previous page when all records from last page archive/unarchived', async function (assert) {
        ...
        const list = await createView({
            ...
            model: 'foo',
            data: this.data,
            arch: '<tree limit="3"><field name="displayName"/></tree>',
            viewOptions: {
                hasActionMenus: true,
            },
            mockRPC: function (route) {
        ...
        assert.strictEqual(list.$('.o-pager-counter').text().trim(), '1-3 / 4',
            "should have 2 pages and current page should be first page");
        assert.strictEqual(list.$('tbody td.o-list-record-selector').length, 3,
        ...
        assert.strictEqual(list.$('.o-pager-counter').text().trim(), '4-4 / 4',
            "should be on second page");
        assert.strictEqual(list.$('tbody td.o-list-record-selector').length, 1,
            "should have 1 records");
        assert.containsNone(list, '.o-cp-action-menus', 'sidebar should not be available');

        await testUtils.dom.click(list.$('tbody .o-data-row:first td.o-list-record-selector:first input'));
        assert.containsOnce(list, '.o-cp-action-menus', 'sidebar should be available');

        // archive all records of current page
        await testUtils.dom.click(list.$('.o-cp-action-menus .dropdown-toggle:contains(Action)'));
    ...
    QUnit.test('list with handle field, override default_get, bottom when inline', async function (assert) {
        ...
        var list = await createView({
            ...
            data: this.data,
            arch:
                '<tree editable="bottom" defaultOrder="int_field">'
                    + '<field name="int_field" widget="handle"/>'
                    + '<field name="foo"/>'
                +'</tree>',
        });
        ...
        // starting condition
        assert.strictEqual($('.o-data-cell').text(), "blipblipyopgnap");
        ...
        var inputText = 'ninja';
        await testUtils.dom.click($('.o-list-button-add'));
        await testUtils.fields.editInput(list.$('.o-input[name="foo"]'), inputText);
        await testUtils.dom.click($('.o-list-button-save'));
        await testUtils.dom.click($('.o-list-button-add'));

        assert.strictEqual($('.o-data-cell').text(), "blipblipyopgnap" + inputText);
    ...
    QUnit.test('create record on list with modifiers depending on id', async function (assert) {
        ...
        assert.containsOnce(list, '.o-selected-row');
        assert.doesNotHaveClass(list.$('.o-selected-row .o-data-cell:first'), 'o-readonly-modifier');
        assert.doesNotHaveClass(list.$('.o-selected-row .o-data-cell:nth(1)'), 'o-invisible-modifier');

        // set a value and save
        await testUtils.fields.editInput(list.$('.o-selected-row input[name=foo]'), 'some value');
        await testUtils.dom.click(list.$buttons.find('.o-list-button-save'));

        // modifiers should be evaluted to true
        assert.hasClass(list.$('.o-data-row:first .o-data-cell:first'), 'o-readonly-modifier');
        assert.hasClass(list.$('.o-data-row:first .o-data-cell:nth(1)'), 'o-invisible-modifier');
        ...
        // modifiers should be evaluted to true
        assert.containsOnce(list, '.o-selected-row');
        assert.hasClass(list.$('.o-selected-row .o-data-cell:first'), 'o-readonly-modifier');
        assert.hasClass(list.$('.o-selected-row .o-data-cell:nth(1)'), 'o-invisible-modifier');
    ...
    QUnit.test('readonly boolean in editable list is readonly', async function (assert) {
        ...
        var $disabledCell = list.$('.o-data-row:eq(1) .o-data-cell:last-child');
        ...
        var $enabledCell = list.$('.o-data-row:eq(0) .o-data-cell:last-child');
    ...
    QUnit.test('grouped list with async widget', async function (assert) {
        ...
        assert.containsNone(list, '.o-data-row', "no group should be open");
        ...
        assert.containsN(list, '.o-data-row', 1, "group should be open");
        assert.strictEqual(list.$('.o-data-row .o-data-cell').text(), 'ready',
    ...
    QUnit.test('grouped lists with groups_limit attribute', async function (assert) {
        ...
        assert.containsN(list, '.o-group-header', 3); // page 1
        assert.containsNone(list, '.o-data-row');
        assert.containsOnce(list, '.o-pager'); // has a pager

        await testUtils.controlPanel.pagerNext(list); // switch to page 2

        assert.containsN(list, '.o-group-header', 1); // page 2
        assert.containsNone(list, '.o-data-row');

        assert.verifySteps([
            'webReadGroup', // readGroup page 1
            'webReadGroup', // readGroup page 2
        ]);
    ...
    QUnit.test('grouped list with expand attribute', async function (assert) {
        ...
        assert.containsN(list, '.o-group-header', 2);
        assert.containsN(list, '.o-data-row', 4);
        assert.strictEqual(list.$('.o-data-cell').text(), 'yopblipgnapblip');

        assert.verifySteps([
            'webReadGroup', // records are fetched alongside groups
        ]);
    ...
    QUnit.test('grouped list (two levels) with expand attribute', async function (assert) {
        ...
        assert.containsN(list, '.o-group-header', 6);

        assert.verifySteps([
            'webReadGroup', // global
            'webReadGroup', // first group
            'webReadGroup', // second group
        ]);
    ...
    QUnit.test('grouped lists with expand attribute and a lot of groups', async function (assert) {
        ...
        assert.containsN(list, '.o-group-header', 10); // page 1
        assert.containsN(list, '.o-data-row', 12); // two groups contains two records
        assert.containsOnce(list, '.o-pager'); // has a pager

        assert.deepEqual(
            [...list.el.querySelectorAll(".o-group-name")].map((el) => el.innerText),
            [
                "10 (2)",
                "9 (2)",
                "17 (1)",
        ...
        assert.containsN(list, '.o-group-header', 7); // page 2
        assert.containsN(list, '.o-data-row', 7);
    ...
    QUnit.test('add filter in a grouped list with a pager', async function (assert) {
        ...
        assert.strictEqual($(webClient.el).find('.o-pager-counter').text().trim(), '1-3 / 4');
        assert.containsN(webClient, '.o-group-header', 3); // page 1

        await testUtils.dom.click($(webClient.el).find('.o-pager-next')); // switch to page 2
        await legacyExtraNextTick();

        assert.strictEqual($(webClient.el).find('.o-pager-counter').text().trim(), '4-4 / 4');
        assert.containsN(webClient, '.o-group-header', 1); // page 2
        ...
        assert.strictEqual($(webClient.el).find('.o-pager-counter').text().trim(), '1-1 / 1');
        assert.containsN(webClient, '.o-group-header', 1); // page 1
    ...
    QUnit.test('editable grouped lists', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-cell:first'));
        assert.containsOnce(list, '.o-selected-row .o-data-cell:first');
    ...
    QUnit.test('char field edition in editable grouped list', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-header:first')); // open first group
        await testUtils.dom.click(list.$('.o-data-cell:first'));
        await testUtils.fields.editAndTrigger(list.$('tr.o-selected-row .o-data-cell:first input[name="foo"]'), 'pla', 'input');
        await testUtils.dom.click(list.$buttons.find('.o-list-button-save'));

        assert.strictEqual(this.data.foo.records[0].foo, 'pla',
            "the edition should have been properly saved");
        assert.containsOnce(list, '.o-data-row:first:contains(pla)');
    ...
    QUnit.test('control panel buttons in multi editable grouped list views', async function (assert) {
        ...
        assert.containsNone(list, '.o-data-row', "all groups should be closed");
        assert.isVisible(list.$buttons.find('.o-list-button-add'),
            "should have a visible Create button");

        await testUtils.dom.click(list.$('.o-group-header:first'));
        assert.containsN(list, '.o-data-row', 1, "first group should be opened");
        assert.isVisible(list.$buttons.find('.o-list-button-add'),
            "should have a visible Create button");

        await testUtils.dom.click(list.$('.o-data-row:eq(0) .o-list-record-selector input'));
        assert.containsOnce(list, '.o-data-row:eq(0) .o-list-record-selector input:enabled',
            "should have selected first record");
        assert.isVisible(list.$buttons.find('.o-list-button-add'),
            "should have a visible Create button");

        await testUtils.dom.click(list.$('.o-group-header:last'));
        assert.containsN(list, '.o-data-row', 2, "two groups should be opened");
        assert.isVisible(list.$buttons.find('.o-list-button-add'),
            "should have a visible Create button");
    ...
    QUnit.test('edit a line and discard it in grouped editable', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-header:first'));
        await testUtils.dom.click(list.$('.o-data-row:nth(2) > td:contains(gnap)'));
        assert.ok(list.$('.o-data-row:nth(2)').is('.o-selected-row'),
            "third group row should be in edition");

        await testUtils.dom.click(list.$buttons.find('.o-list-button-discard'));
        await testUtils.dom.click(list.$('.o-data-row:nth(0) > td:contains(yop)'));
        assert.ok(list.$('.o-data-row:eq(0)').is('.o-selected-row'),
    ...
    QUnit.test('add and discard a record in a multi-level grouped list view', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-header:first'));
        await testUtils.dom.click(list.$('.o-group-header:eq(1)'));
        assert.hasClass(list.$('.o-group-header:first'), 'o-group-open');
        assert.hasClass(list.$('.o-group-header:eq(1)'), 'o-group-open');
        assert.containsOnce(list, '.o-data-row');

        // add a record to first subgroup
        await testUtils.dom.click(list.$('.o-group-field-row-add a'));
        assert.containsN(list, '.o-data-row', 2);
    ...
    QUnit.test('inputs are disabled when unselecting rows in grouped editable', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-header:first'));
        await testUtils.dom.click(list.$('td:contains(yop)'));
        $input = list.$('tr.o-selected-row input[name="foo"]');
    ...
    QUnit.test('pressing ESC in editable grouped list should discard the current line changes', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-header:first')); // open first group
        assert.containsN(list, 'tr.o-data-row', 3);

        await testUtils.dom.click(list.$('.o-data-cell:first'));

        // update name by "foo"
        await testUtils.fields.editAndTrigger(list.$('tr.o-selected-row .o-data-cell:first input[name="foo"]'), 'new_value', 'input');
    ...
    QUnit.test('pressing TAB in editable="bottom" grouped list', async function (assert) {
        ...
        // open two groups
        await testUtils.dom.click(list.$('.o-group-header:first'));
        assert.containsN(list, '.o-data-row', 3, 'first group contains 3 rows');
        await testUtils.dom.click(list.$('.o-group-header:nth(1)'));
        assert.containsN(list, '.o-data-row', 4, 'first group contains 1 row');

        await testUtils.dom.click(list.$('.o-data-cell:first'));
        assert.hasClass(list.$('.o-data-row:first'), 'o-selected-row');

        // Press 'Tab' -> should go to next line (still in first group)
        await testUtils.fields.triggerKeydown(list.$('.o-selected-row .o-input'), 'tab');
        assert.hasClass(list.$('.o-data-row:nth(1)'), 'o-selected-row');
    ...
    QUnit.test('pressing SHIFT-TAB in editable="bottom" grouped list', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:eq(1) .o-data-cell')); // select second row of first group
        assert.hasClass(list.$('tr.o-data-row:eq(1)'), 'o-selected-row');

        // press Shft+tab
        list.$('tr.o-selected-row input').trigger($.Event('keydown', {which: $.ui.keyCode.TAB, shiftKey: true}));
        await testUtils.nextTick();
        assert.hasClass(list.$('tr.o-data-row:first'), 'o-selected-row');
        assert.doesNotHaveClass(list.$('tr.o-data-row:eq(1)'), 'o-selected-row');

        // navigate between groups
        await testUtils.dom.click(list.$('.o-data-cell:eq(3)')); // select row of second group
    ...
    QUnit.test('editing then pressing TAB in editable grouped list', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:nth(2) .o-data-cell'));
        assert.hasClass(list.$('.o-data-row:nth(2)'), 'o-selected-row');
        await testUtils.fields.editInput(list.$('.o-selected-row input[name="foo"]'), 'new value');
        ...
        await testUtils.fields.editInput(list.$('.o-selected-row input[name="foo"]'), 'new record');
        await testUtils.fields.triggerKeydown(list.$('.o-selected-row .o-input'), 'tab');

        assert.containsN(list, '.o-data-row', 6);
        assert.hasClass(list.$('.o-data-row:nth(4)'), 'o-selected-row');
    ...
    QUnit.test('editing then pressing TAB (with a readonly field) in grouped list', async function (assert) {
        ...
        // click on first td and press TAB
        await testUtils.dom.click(list.$('td:contains(yop)'));
        await testUtils.fields.editAndTrigger(list.$('tr.o-selected-row input[name="foo"]'), 'new value', 'input');
        await testUtils.fields.triggerKeydown(list.$('tr.o-selected-row input[name="foo"]'), 'tab');
    ...
    QUnit.test('pressing ENTER in editable="bottom" grouped list view', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-header:first')); // open first group
        await testUtils.dom.click(list.$('.o-group-header:nth(1)')); // open second group
        assert.containsN(list, 'tr.o-data-row', 4);
        await testUtils.dom.click(list.$('.o-data-row:nth(1) .o-data-cell')); // click on second line
        assert.hasClass(list.$('tr.o-data-row:eq(1)'), 'o-selected-row');

        // press enter in input should move to next record
        await testUtils.fields.triggerKeydown(list.$('tr.o-selected-row .o-input'), 'enter');
    ...
    QUnit.test('cell-level keyboard navigation in non-editable list', async function (assert) {
        ...
        assert.ok(document.activeElement.classList.contains('o-searchview-input'),
    ...
    QUnit.test('removing a groupby while adding a line from list', async function (assert) {
        ...
        const list = await createView({
            ...
            data: this.data,
            arch: `
                <tree multiEdit="1" editable="bottom">
                    <field name="displayName"/>
                    <field name="foo"/>
                </tree>
            `,
                ...
                'foo,false,search': `
                    <search>
                        ...
                        <group expand="1" string="Group By">
                            <filter name="groupby_foo" context="{'groupby': 'foo'}"/>
                        </group>
                    </search>
                `,
            },
        ...
        // expand group
        await testUtils.dom.click(list.el.querySelector('th.o-group-name'));
        await testUtils.dom.click(list.el.querySelector('td.o-group-field-row-add a'));
        checkUnselectRow = true;
        await testUtils.dom.click($('.o-searchview-facet .o-facet-remove'));
        assert.verifySteps([]);
        testUtils.mock.unpatch(ListRenderer);
        list.destroy();
    });
    ...
    QUnit.test('cell-level keyboard navigation in editable grouped list', async function (assert) {
        ...
        var $secondRowInput = list.$('tr.o-data-row:eq(1) td:eq(1) input');
        ...
        assert.strictEqual(list.$('tr.o-data-row:eq(1) td:eq(1)').text(), 'blip',
        ...
        await testUtils.fields.triggerKeydown($(document.activeElement), 'enter');
        var $firstRowInput = list.$('tr.o-data-row:eq(0) td:eq(1) input');
        assert.hasClass(list.$('tr.o-data-row:eq(0)'), 'o-selected-row',
            'first row should be selected');
        ...
        await testUtils.fields.triggerKeydown($(document.activeElement), 'enter');
        assert.strictEqual(list.$('tr.o-data-row:eq(0) td:eq(1)').text(), 'Zipadeedoodah',
            'first record should be saved');
        assert.doesNotHaveClass(list.$('tr.o-data-row:eq(0)'), 'o-selected-row',
            'first row should be closed');
        assert.hasClass(list.$('tr.o-data-row:eq(1)'), 'o-selected-row',
            'second row should be opened');
        assert.strictEqual(list.$('tr.o-data-row:eq(1) td:eq(1) input').val(), 'blip',
            'second record should be in edit mode');
        ...
        assert.strictEqual(list.$('tr.o-data-row').length, 3,
            'should have 3 rows displayed');
        await testUtils.fields.triggerKeydown($(document.activeElement), 'enter');
        assert.strictEqual(list.$('tr.o-data-row').length, 4,
        ...
        await testUtils.fields.triggerKeydown($(document.activeElement), 'enter');
        await testUtils.fields.editAndTrigger($('tr.o-data-row:eq(4) td:eq(1) input'),
            'cheateur arrete de cheater', 'input');
        await testUtils.fields.triggerKeydown($(document.activeElement), 'enter');
        assert.strictEqual(list.$('tr.o-data-row').length, 6,
        ...
        await testUtils.fields.triggerKeydown($(document.activeElement), 'enter');
        assert.strictEqual(list.$('tr.o-data-row').length, 2,
        ...
        await testUtils.fields.triggerKeydown($(document.activeElement), 'right');
        assert.strictEqual(list.$('tr.o-data-row').length, 5,
    ...
    QUnit.test('execute group header button with keyboard navigation', async function (assert) {
        ...
        var list = await createView({
            ...
            intercepts: {
                execute_action: function (ev) {
                    assert.strictEqual(ev.data.actionData.name, 'some_method');
                },
            },
        });
        ...
        list.$('.o-list-button-add').focus();
        assert.ok(document.activeElement.classList.contains('o-list-button-add'));
        await testUtils.fields.triggerKeydown($(document.activeElement), 'down');
        assert.strictEqual(document.activeElement.tagName, 'INPUT',
        ...
        // unfold first group
        await testUtils.fields.triggerKeydown($(document.activeElement), 'enter');
        assert.containsN(list, '.o-data-row', 3, "first group should be open");
        ...
        await testUtils.fields.triggerKeydown($(document.activeElement), 'up');
        assert.ok(document.activeElement.classList.contains('o-group-name'),
        ...
        assert.ok(document.activeElement.classList.contains('o-group-name'),
            'focus should still be on first group header');
        assert.containsNone(list, '.o-data-row', "first group should now be folded");
        ...
        list.$('.o-group-header .o-group-buttons button:first').focus();
        ...
        await testUtils.fields.triggerKeydown($(document.activeElement), 'enter');
        assert.containsN(list, '.o-data-row', 3, "first group should still be open");
    ...
    QUnit.test('add a new row in grouped editable="top" list', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-header:first')); // open group
        await testUtils.dom.click(list.$('.o-group-field-row-add a'));// add a new row
        assert.strictEqual(list.$('.o-selected-row .o-input[name=foo]')[0], document.activeElement,
            'The first input of the line should have the focus');
        assert.containsN(list, 'tbody:nth(1) .o-data-row', 4);

        await testUtils.dom.click(list.$buttons.find('.o-list-button-discard')); // discard new row
        await testUtils.dom.click(list.$('.o-group-header:eq(1)')); // open second group
        assert.containsOnce(list, 'tbody:nth(3) .o-data-row');

        await testUtils.dom.click(list.$('.o-group-field-row-add a:eq(1)')); // create row in second group
        assert.strictEqual(list.$('.o-group-name:eq(1)').text(), 'false (2)',
            "group should have correct name and count");
        assert.containsN(list, 'tbody:nth(3) .o-data-row', 2);
        assert.hasClass(list.$('.o-data-row:nth(3)'), 'o-selected-row');

        await testUtils.fields.editAndTrigger(list.$('tr.o-selected-row input[name="foo"]'), 'pla', 'input');
        await testUtils.dom.click(list.$buttons.find('.o-list-button-save'));
        assert.containsN(list, 'tbody:nth(3) .o-data-row', 2);
    ...
    QUnit.test('add and discard a line through keyboard navigation without crashing', async function (assert) {
        ...
        await testUtils.fields.triggerKeydown(list.$('.o-group-field-row-add'), 'enter');
        assert.containsN(list, 'tbody:nth(1) .o-data-row', 4, "new data row should be created");
        await testUtils.dom.click(list.$buttons.find('.o-list-button-discard'));
        // At this point, a crash manager should appear if no proper link targetting
        assert.containsN(list, 'tbody:nth(1) .o-data-row', 3,"new data row should be discarded.");
    ...
    QUnit.test('editable grouped list with create="0"', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-header:first')); // open group
        assert.containsNone(list, '.o-group-field-row-add a',
    ...
    QUnit.test('add a new row in (selection) grouped editable list', async function (assert) {
        ...
        assert.strictEqual(list.$('.o-data-row .o-data-cell:eq(1)').text(), 'Low',
            "should have a column name with a value from the groupby");

        await testUtils.dom.click(list.$('.o-group-header:eq(1)')); // open second group
        await testUtils.dom.click(list.$('.o-group-field-row-add a:eq(1)')); // create row in second group
        await testUtils.dom.click($('body')); // unselect row
        assert.strictEqual(list.$('.o-data-row:nth(5) .o-data-cell:eq(1)').text(), 'Medium',
            "should have a column name with a value from the groupby");
    ...
    QUnit.test('add a new row in (m2o) grouped editable list', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-field-row-add a'));
        await testUtils.dom.click($('body')); // unselect row
        assert.strictEqual(list.$('tbody:eq(1) .o-data-row:first .o-data-cell:eq(1)').text(), 'Value 1',
            "should have a column name with a value from the groupby");
        assert.verifySteps(['1']);
        ...
        await testUtils.dom.click(list.$('.o-group-field-row-add a:eq(1)')); // create row in second group
        await testUtils.dom.click($('body')); // unselect row
        assert.strictEqual(list.$('tbody:eq(3) .o-data-row:first .o-data-cell:eq(1)').text(), 'Value 2',
            "should have a column name with a value from the groupby");
        assert.verifySteps(['2']);
    ...
    QUnit.test('list view with optional fields rendering', async function (assert) {
        ...
        assert.ok(optionalFieldsToggler.classList.contains('o-optional-columns-dropdown-toggle'),
            'The optional fields toggler is the second last element');
        const optionalFieldsDropdown = list.el.querySelector('.o-list-view').lastElementChild;
        assert.ok(optionalFieldsDropdown.classList.contains('o-optional-columns'),
            'The optional fields dropdown is the last element');

        assert.ok(list.$('.o-optional-columns .dropdown-menu').hasClass('dropdown-menu-right'),
            'In LTR, the dropdown should be anchored to the right and expand to the left');

        // optional fields
        await testUtils.dom.click(list.$('table .o-optional-columns-dropdown-toggle'));
        assert.containsN(list, 'div.o-optional-columns div.dropdown-item', 2,
        ...
        await testUtils.dom.click(list.$('table .o-optional-columns-dropdown-toggle'));
        assert.strictEqual(list.$('div.o-optional-columns div.dropdown-item:first input:checked')[0],
            list.$('div.o-optional-columns div.dropdown-item [name="m2o"]')[0],
        ...
        await testUtils.dom.click(list.$('table .o-optional-columns-dropdown-toggle'));
        assert.notOk(list.$('div.o-optional-columns div.dropdown-item [name="m2o"]').is(":checked"));
    ...
    QUnit.test('list view with optional fields rendering in RTL mode', async function (assert) {
        ...
        assert.ok(list.$('.o-optional-columns .dropdown-menu').hasClass('dropdown-menu-left'),
    ...
    QUnit.test('optional fields do not disappear even after listview reload', async function (assert) {
        ...
        await testUtils.dom.click(list.$('table .o-optional-columns-dropdown-toggle'));
        assert.ok(list.$('div.o-optional-columns div.dropdown-item [name="m2o"]').is(":checked"));
    ...
    QUnit.test('selection is kept when optional fields are toggled', async function (assert) {
        ...
        // select a record
        await testUtils.dom.click(list.$('.o-data-row .o-list-record-selector input:first'));

        assert.containsOnce(list, '.o-list-record-selector input:checked');
    ...
    QUnit.test('list view with optional fields and async rendering', async function (assert) {
        ...
        assert.containsN(list, 'th', 2);
        assert.isNotVisible(list.$('.o-optional-columns-dropdown'));

        // add an optional field (we click on the label on purpose, as it will trigger
        // a second event on the input)
        await testUtils.dom.click(list.$('table .o-optional-columns-dropdown-toggle'));
        assert.isVisible(list.$('.o-optional-columns-dropdown'));
        assert.containsNone(list.$('.o-optional-columns-dropdown'), 'input:checked');
        await testUtils.dom.click(list.$('div.o-optional-columns div.dropdown-item:first label'));
        ...
        assert.containsN(list, 'th', 3);
        assert.isVisible(list.$('.o-optional-columns-dropdown'));
        assert.containsOnce(list.$('.o-optional-columns-dropdown'), 'input:checked');
    ...
    QUnit.test('open list optional fields dropdown position to right place', async function (assert) {
        ...
        const listWidth = form.el.querySelector('.o-list-view').offsetWidth;

        await testUtils.dom.click(form.el.querySelector('.o-optional-columns-dropdown-toggle'));
        assert.strictEqual(form.el.querySelector('.o-optional-columns').offsetLeft, listWidth,
    ...
    QUnit.test('change the viewType of the current action', async function (assert) {
        ...
        // enable optional field
        await testUtils.dom.click($(webClient.el).find('table .o-optional-columns-dropdown-toggle'));
        assert.notOk($(webClient.el).find('div.o-optional-columns div.dropdown-item [name="m2o"]').is(":checked"));
        assert.ok($(webClient.el).find('div.o-optional-columns div.dropdown-item [name="o2m"]').is(":checked"));
        await testUtils.dom.click($(webClient.el).find('div.o-optional-columns div.dropdown-item:first'));
        ...
        // disable optional field
        await testUtils.dom.click($(webClient.el).find('table .o-optional-columns-dropdown-toggle'));
        assert.ok($(webClient.el).find('div.o-optional-columns div.dropdown-item [name="m2o"]').is(":checked"));
        assert.ok($(webClient.el).find('div.o-optional-columns div.dropdown-item [name="o2m"]').is(":checked"));
        await testUtils.dom.click($(webClient.el).find('div.o-optional-columns div.dropdown-item:last input'));
        assert.ok($(webClient.el).find('th:contains(M2O field)').is(':visible'),
            "should have a visible m2o field"); //m2o field
    ...
    QUnit.test('list view with optional fields rendering and local storage mock', async function (assert) {
        ...
        var localStorageKey = 'optionalFields,foo,list,42,foo,m2o,reference';
        ...
        // enable optional field
        await testUtils.dom.click(list.$('div.o-optional-columns div.dropdown-item:eq(1) input'));
    ...
    QUnit.test("quickcreate in a many2one in a list", async function (assert) {
        ...
        const $input = list.$('.o-data-row:first .o-data-cell:first input');
        ...
        assert.strictEqual(list.el.getElementsByClassName('o-data-cell')[0].innerHTML, "aaa",
    ...
    QUnit.test('float field render with digits attribute on listview', async function (assert) {
        ...
        assert.strictEqual(list.$('td.o-list-number:eq(0)').text(), "0.400000", "should contain 6 digits decimal precision");
        list.destroy();
    });
    // TODO: write test on:
    ...
    QUnit.test('editable list: resize column headers', async function (assert) {
        ...
        const optionalDropdown = list.el.getElementsByClassName('o-optional-columns')[0];
        const optionalInitialX = optionalDropdown.getBoundingClientRect().x;
        const resizeHandle = th.getElementsByClassName('o-resize')[0];
    ...
    QUnit.test('editable list: resize column headers with max-width', async function (assert) {
        ...
        const resizeHandle = th.getElementsByClassName('o-resize')[0];
        const nextResizeHandle = thNext.getElementsByClassName('o-resize')[0];
    ...
    QUnit.test('resize column with several x2many lists in form group', async function (assert) {
        ...
        const resizeHandle = th.getElementsByClassName('o-resize')[0];
        const firstTableInitialWidth = form.el.querySelectorAll('.o-field-x2many_list table')[0].offsetWidth;
        const secondTableInititalWidth = form.el.querySelectorAll('.o-field-x2many_list table')[1].offsetWidth;
    ...
    QUnit.test('resize column with x2many list with several fields in form notebook', async function (assert) {
        ...
        const th = form.el.getElementsByTagName('th')[0];
        const resizeHandle = th.getElementsByClassName('o-resize')[0];
        const listInitialWidth = form.el.querySelector('.o-list-view').offsetWidth;

        await testUtils.dom.dragAndDrop(resizeHandle, form.el.getElementsByTagName('th')[1], { position: "right" });

        assert.strictEqual(form.el.querySelector('.o-list-view').offsetWidth, listInitialWidth,
    ...
    QUnit.test('enter edition in editable list with <widget>', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-row:first .o-data-cell:nth(1)'));
        assert.strictEqual(document.activeElement.name, "int_field");
    ...
    QUnit.test('enter edition in editable list with multiEdit = 0', async function (assert) {
        assert.expect(1);
        ...
        var list = await createView({
            ...
            model: 'foo',
            data: this.data,
            arch: '<tree editable="top" multiEdit="0">' +
                '<field name="int_field"/>' +
                '</tree>',
        });

        // click on int_field cell of first row
        await testUtils.dom.click(list.$('.o-data-row:first .o-data-cell:nth(0)'));
        assert.strictEqual(document.activeElement.name, "int_field");

        list.destroy();
    });

    QUnit.test('enter edition in editable list with multiEdit = 1', async function (assert) {
        assert.expect(1);
        ...
        var list = await createView({
            ...
            model: 'foo',
            data: this.data,
            arch: '<tree editable="top" multiEdit="1">' +
                '<field name="int_field"/>' +
                '</tree>',
        });
    ...
    QUnit.test('editable list view: multi edition of owl field component', async function (assert) {
        ...
        const list = await createView({
            arch: '<tree multiEdit="1"><field name="bar"/></tree>',
            data: this.data,
            model: 'foo',
            View: ListView,
        });
        ...
        assert.containsN(list, '.o-data-row', 4);
        assert.containsN(list, '.o-data-cell .custom-checkbox input:checked', 3);

        // select all records and edit the boolean field
        await testUtils.dom.click(list.$('thead .o-list-record-selector input'));
        assert.containsN(list, '.o-data-row .o-list-record-selector input:checked', 4);
        await testUtils.dom.click(list.$('.o-data-cell:first'));
        await testUtils.dom.click(list.$('.o-data-cell .o-field-boolean input'));

        assert.containsOnce(document.body, '.modal');
        await testUtils.dom.click($('.modal .modal-footer .btn-primary'));

        assert.containsNone(list, '.o-data-cell .custom-checkbox input:checked');
    ...
    QUnit.test("Date in evaluation context works with date field", async function (assert) {
        ...
        testUtils.mock.patch(BasicModel, {
            _getEvalContext() {
                ...
                assert.strictEqual(evalContext.currentDate, evalContext.today);
                return evalContext;
            },
        });
        ...
        assert.containsOnce(list, ".o-data-row .text-danger");
    ...
    QUnit.test("Auto save: add a record and leave action", async function (assert) {
        ...
        assert.strictEqual($(webClient.el).find('.o-field-cell[name="foo"]').text(), "yopblipgnapblip");
        assert.containsN(webClient, '.o-data-row', 4);

        await testUtils.dom.click($(webClient.el).find('.o-list-button-add'));
        await testUtils.fields.editInput($(webClient.el).find('.o-field-widget[name="foo"]'), "test");

        // change action and come back
        await doAction(webClient, 2);
        await doAction(webClient, 1, { clearBreadcrumbs: true });

        assert.strictEqual($(webClient.el).find('.o-field-cell[name="foo"]').text(), "yopblipgnapbliptest");
    ...
    QUnit.test("Auto save: modify a record and leave action", async function (assert) {
        ...
        await testUtils.dom.click($(webClient.el).find('.o-field-cell[name="foo"]:first'));
        await testUtils.fields.editInput($(webClient.el).find('.o-field-widget[name="foo"]'), "test");
        ...
        assert.strictEqual($(webClient.el).find('.o-field-cell[name="foo"]').text(), "testblipgnapblip");
    });
    ...
    QUnit.test("Auto save: modify a record and leave action (reject)", async function (assert) {
        ...
        await testUtils.dom.click($(webClient.el).find('.o-field-cell[name="foo"]:first'));
        await testUtils.fields.editInput($(webClient.el).find('.o-field-widget[name="foo"]'), "");

        await assert.rejects(doAction(webClient, 2));

        assert.strictEqual($(webClient.el).find('.o-field-cell[name="foo"]').text(), "blipgnapblip");
        assert.hasClass($(webClient.el).find('.o-field-widget[name="foo"]:first'), 'o-field-invalid');
        assert.containsN(webClient, '.o-data-row', 4);
    });
    ...
    QUnit.test("Auto save: add a record and change page", async function (assert) {
        ...
        assert.strictEqual(list.$('.o-field-cell[name="foo"]').text(), "yopblipgnap");

        await testUtils.dom.click(list.$('.o-list-button-add'));
        await testUtils.fields.editInput(list.$('.o-field-widget[name="foo"]'), "test");
        await testUtils.controlPanel.pagerNext(list);

        assert.strictEqual(list.$('.o-field-cell[name="foo"]').text(), "bliptest");
    ...
    QUnit.test("Auto save: modify a record and change page", async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-field-cell[name="foo"]:first'));
        await testUtils.fields.editInput(list.$('.o-field-widget[name="foo"]'), "test");
        await testUtils.controlPanel.pagerNext(list);

        assert.strictEqual(list.$('.o-field-cell[name="foo"]').text(), "blip");

        await testUtils.controlPanel.pagerPrevious(list);

        assert.strictEqual(list.$('.o-field-cell[name="foo"]').text(), "testblipgnap");
    ...
    QUnit.test("Auto save: modify a record and change page (reject)", async function (assert) {
        ...
        assert.hasClass(list.$('.o-field-widget[name="foo"]:first'), 'o-field-invalid');
        assert.strictEqual(list.$('.o-field-cell[name="foo"]').text(), "blipgnap");
    ...
    QUnit.test('Auto save: save on closing tab/browser (onchanges)', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-field-cell[name="int_field"]:first'));
        await testUtils.fields.editInput(list.$('.o-field-widget[name="int_field"]'), '2021');
    ...
    QUnit.test('edition, then navigation with tab (with a readonly re-evaluated field and onchange)', async function (assert) {
        ...
        assert.strictEqual(document.activeElement, form.$('tr.o-selected-row input[name="foo"]')[0],
            "focus should be on an input with name = foo");
        testUtils.fields.editInput(form.$('tr.o-selected-row input[name="foo"]'), 'new value');
        var tabEvent = $.Event("keydown", { which: $.ui.keyCode.TAB });
        await testUtils.dom.triggerEvents(form.$('tr.o-selected-row input[name="foo"]'), [tabEvent]);
        assert.strictEqual(document.activeElement, form.$('tr.o-selected-row input[name="int_field"]')[0],
            "focus should be on an input with name = int_field");
    ...
    QUnit.test('selecting a row after another one containing a table within an html field should be the correct one', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-data-cell:eq(1)'))
        assert.ok($('table.o-list-table > tbody > tr:eq(1)')[0].classList.contains('o-selected-row'), "The second row should be selected")
    ...
    QUnit.test('FieldColorPicker: dont overflow color picker in list', async function (assert) {
        ...
        await testUtils.dom.click(list.el.querySelector('.o-list-button-add'))
        const date_column_width = list.el.querySelector('.o-list-table thead th[data-name="date"]').style.width.replace('px', '');
        const int_field_column_width = list.el.querySelector('.o-list-table thead th[data-name="int_field"]').style.width.replace('px', '');
