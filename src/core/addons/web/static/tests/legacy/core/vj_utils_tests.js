verp.define('web.vjUtilsTests', function(require) {
"use strict";
...
QUnit.module('core', function () {
    ...
    QUnit.test('simple arithmetic', function(assert) {
        ...
        var result = vjUtils.vjEval("1 + 2");
        assert.strictEqual(result, 3, "should properly evaluate sum");
        result = vjUtils.vjEval("42 % 5");
        assert.strictEqual(result, 2, "should properly evaluate modulo operator");
        result = vjUtils.vjEval("2 ** 3");
    ...
    QUnit.test('currentDate', function (assert) {
        assert.expect(1);

        var currentDate = time.dateToStr(new Date());
        var result = vjUtils.eval('domains',
            [[],{"__ref":"domain","__debug":"[('label','>=',currentDate),('label','<=',currentDate)]","__id":"5dedcfc96648"}],
            vjUtils.context());
        assert.deepEqual(result, [
            ['label', '>=', currentDate],
            ['label', '<=', currentDate]
    ...
    QUnit.module('vjUtils (_formatAST)');
    ...
    QUnit.module('vj_utils (_normalizeDomain)');
    ...
    QUnit.module('vj_utils (assembleDomains)');
