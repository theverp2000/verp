verp.define('web.mixins', function (require) {
"use strict";
...
function VerpEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
VerpEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};

VerpEvent.prototype.isStopped = function () {
    return this.stopped;
};
...
var Events = Class.extend({
    ...
    trigger : function (events) {
        ...
        while ((event = events.shift())) {
            ...
            // if (!(node = calls[event])) origin
            //     continue;
            // Tony changed to match all event-name, event_name, eventName
            node = calls[event] || calls[event.replace(/-/g, '_')] || calls[jQuery.camelCase(event)];
            if (!node)
                continue;
            events.push({
                next : node.next,
...
var EventDispatcherMixin = _.extend({}, ParentedMixin, {
     ...
     * VERP Web where developers may want to replace existing callbacks with
     * theirs.
     *
    ...
    _delegateCustomEvents: function () {
        if (_.isEmpty(this.customEvents)) { return; }
        for (var key in this.customEvents) {
            if (!this.customEvents.hasOwnProperty(key)) { continue; }

            var method = this.proxy(this.customEvents[key]);
    ...
    triggerUp: function (name, info) {
        var event = new VerpEvent(this, name, info);
        //console.info('event: ', name, info);
        this._triggerUp(event);
        return event;
    },
    _triggerUp: function (event) {
        var parent;
        this.__edispatcherEvents.trigger(event.name, event);
        if (!event.isStopped() && (parent = this.getParent())) {
    ...
    set: function (arg1, arg2, arg3) {
        ...
        _.each(map, function (val, key) {
            ...
            if (key === 'value' && self.field && self.field.type === 'float' && tmp && val){
                ...
                if (_.isArray(digits)) {
                    if (utils.floatIsZero(tmp - val, digits[1])) {
                        return;
                    }
                }
            }
