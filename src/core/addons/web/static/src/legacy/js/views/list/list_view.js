/** @verp-module alias=web.ListView **/
...
import vjUtils from 'web.vjUtils';

var _lt = core._lt;

var ListView = BasicView.extend({
    accesskey: "l",
    displayName: _lt('List'),
    icon: 'fa-list-ul',
    ...
    init: function (viewInfo, params) {
        ...
        var pyevalContext = vj.dict.fromJSON(_.pick(params.context, function(value, key, object) {return !_.isUndefined(value)}) || {});
        var expandGroups = !!JSON.parse(vjUtils.vjEval(this.arch.attrs.expand || "0", {'context': pyevalContext}));
        ...
        this.controllerParams.activeActions.exportXlsx = this.arch.attrs.exportXlsx ? !!JSON.parse(this.arch.attrs.exportXlsx): true;
        ...
        this.rendererParams.isMultiEditable = this.arch.attrs.multiEdit && !!JSON.parse(this.arch.attrs.multiEdit);
        ...
        var groupsLimit = parseInt(this.arch.attrs.groupsLimit, 10);
    ...
    _updateMVCParams: function () {
        this._super.apply(this, arguments);
        this.controllerParams.noLeaf = !!this.loadParams.context.groupbyNoLeaf;
    },
});
