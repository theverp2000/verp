verp.define('web.BasicModel', function (require) {
"use strict";
 ...
 * javascript models and the web client. More specifically, its job is to give a
 ...
 *
 * Notes:
 * - id: is totally unrelated to resId.  id is a web client local concept
 * - resId: if set to a number or a virtual id (a virtual id is a character
 ...
 * - resIds: if set, it represent the context in which the data point is actually
 *     used.  For example, a given record in a form view (opened from a list view)
 *     might have a resId = 2 and resIds = [1,2,3]
 ...
 *     resIds, because we might have a very large number of records, or a
 *     domain, and the resIds would be the current page, not the full set.
 * - model is the actual name of a (verp) model, such as 'res.partner'
 ...
 *     data point.
 * - fieldNames: list of some relevant field names (string).  Usually, it
...
const vjUtils = require('web.vjUtils');
...
var BasicModel = AbstractModel.extend({
     ...
     *
     * 1. flagged as 'no abandon' (i.e. during a `defaultGet`, including any
     *    `onchange` from a `defaultGet`)
     ...
     *       --> resId doesn't correspond to the model and we don't have the
    ...
    deleteRecords: function (recordIds, modelName) {
        ...
        var context = _.extend(records[0].getContext(), session.userContext);
        return this._rpc({
                model: modelName,
                method: 'unlink',
                args: [_.pluck(records, 'resId')],
                context: context,
            })
            .then(function () {
                _.each(records, function (record) {
                    ...
                    if (parent && parent.type === 'list') {
                        ...
                        record.resIds.splice(record.offset, 1);
                        record.offset = Math.min(record.offset, record.resIds.length - 1);
                        record.resId = record.resIds[record.offset];
                        record.count--;
                    }
                });
    ...
    duplicateRecord: function (recordId) {
        ...
        return this._rpc({
            ...
            .then(function (resId) {
                var index = record.resIds.indexOf(record.resId);
                record.resIds.splice(index + 1, 0, resId);
                return self.load({
                    fieldsInfo: record.fieldsInfo,
                    fields: record.fields,
                    modelName: record.model,
                    resId: resId,
                    resIds: record.resIds.slice(0),
     ...
     * 'save' will bypass the mutex. This is useful when we are leaving Verp
     * (closing tab/browser), and we want to quickly save pending changes (in
     * an 'onbeforeunload' handler, which is mostly sync).
     *
     * This function should never be called except when we are leaving Verp.
     *
    ...
    freezeOrder: function (listID) {
        ...
        this._sortList(list);
        this.localData[listID].orderedResIDs = list.resIds;
    },
    /**
     ...
     * @param {any} options
     * @param {boolean} [options.env=false] if true, will only  return resId
     *   (if record) or resIds (if list)
    ...
    __get: function (id, options) {
        ...
        if (options.env) {
            var env = {
                ids: element.resIds ? element.resIds.slice(0) : [],
            };
            if (element.type === 'record') {
                env.currentId = this.isNew(element.id) ? undefined : element.resId;
            }
            return env;
        }
            ...
            for (var fieldName in data) {
                ...
                if (field.type === 'many2one') {
                    if (options.raw) {
                        ...
                        data[fieldName] = relDataPoint ? relDataPoint.resId : false;
                    } else {
                    ...
                    if (options.raw) {
                        ...
                        data[fieldName] = relDataPoint ?
                            relDataPoint.model + ',' + relDataPoint.resId :
                            false;
                    } else {
                        data[fieldName] = this.__get(data[fieldName]) || false;
                    }
                    ...
                    if (options.raw) {
                        if (typeof data[fieldName] === 'string') {
                            ...
                            data[fieldName] = relDataPoint.resIds;
                        } else {
    ...
    /**
     * Returns the current displayName for the record.
     *
    ...
    getName: function (id) {
        var record = this.localData[id];
        var returnValue = '';
        if (record._changes && 'displayName' in record._changes) {
            returnValue = record._changes.displayName;
        }
        else if ('displayName' in record.data) {
            returnValue = record.data.displayName;
        }
    ...
    isNew: function (id) {
        ...
        if (typeof resId === 'number') {
            return false;
        } else if (typeof resId === 'string' && /^[0-9]+-/.test(resId)) {
            return false;
        }
        return true;
    },
    ...
    __load: async function (params) {
        ...
        params.type = params.type || (params.resId !== undefined ? 'record' : 'list');
        // FIXME: the following seems only to be used by the basicModelTests
        ...
        if (params.type === 'record' && params.resId === undefined) {
    ...
    /**
     * Returns the list of resIds for a given list of local ids.
     *
     * @param {string[]} localIds
     * @returns {integer[]}
     */
    localIdsToResIds: function (localIds) {
        return localIds.map(localId => this.localData[localId].resId);
    },
    /**
    ...
    makeRecord: function (model, fields, fieldInfo) {
        ...
        _.each(fields, function (field) {
            recordFields[field.name] = _.pick(field, 'type', 'relation', 'domain', 'selection');
        });
        fieldInfo = fieldInfo || {};
        var fieldsInfo = {};
        fieldsInfo.default = {};
        ...
        _.each(fields, function (field) {
            ...
            if (field.type === 'many2one') {
                if (field.value) {
                    ...
                    var displayName = _.isArray(field.value) ? field.value[1] : undefined;
                ...
                if (field.value && field.value.length) {
                    if (_.isObject(field.value[0])) {
                        dpParams.resIds = _.pluck(field.value, 'id');
                        ...
                        _.each(field.value, function (data) {
                            ...
                            dataPoint._cache[recordDP.resId] = recordDP.id;
                        });
                    } else {
                        dpParams.resIds = field.value;
                        dataPoint = self._makeDataPoint(dpParams);
                        needLoad = true;
                    }
                } else {
     ...
     */
    notifyChanges: function (recordId, changes, options) {
        const notifyChanges = () => this._applyChange(recordId, changes, options);
        if (this.bypassMutex) {
            ...
            .then(function (wasResequenced) {
                ...
                return self._rpc({
                    ...
                    if (data.data.length) {
                        ...
                        if (dataType === 'list') {
                            data.data = _.sortBy(data.data, function (d) {
                                return _.indexOf(resIds, self.localData[d].resId)
                            });
                        }
                    }
                    data.resIds = [];
                    _.each(data.data, function (d) {
                        ...
                        if (dataPoint.type === 'record') {
                            data.resIds.push(dataPoint.resId);
                        } else {
                            data.resIds = data.resIds.concat(dataPoint.resIds);
                        }
                    });
            ...
            var prom = new Promise(function (resolve, reject) {
                ...
                if (method === 'create' || changedFields.length) {
                    ...
                    self._rpc({
                            ...
                            if (method === 'create') {
                                record.resId = id;  // creating returns an id, writing returns a boolean
                                record.data.id = id;
                                record.offset = record.resIds.length;
     ...
     *
     * @param {string} listID id for the list resource
     * @param {string} fieldName valid field name
     * @returns {Promise}
     */
    setSort: function (listID, fieldName) {
        var list = this.localData[listID];
        if (list.type === 'record') {
            return;
     ...
     *
     * Since the ORM can support both `active` and `xActive` fields for
     ...
     *   ('active', 'xActive') or undefined if no such field is present
     */
    getActiveField: function (record) {
        ...
        const hasXActive = fields.includes('xActive');
        return hasXActive?'xActive':undefined
    },
    /**
    ...
    toggleActive: function (recordIDs, parentId) {
        ...
        var resIds = _.map(recordIDs, function (recordId) {
            return self.localData[recordId].resId;
        });
        return this._rpc({
                model: parent.model,
                method: 'toggleActive',
     ...
     *   calling the defaultGet (eg. to set default values).
    ...
    _addX2ManyDefaultRecord: function (list, options) {
        ...
        if (additionalContexts){
            _.each(additionalContexts, function (context) {
                params.context = self._getContext(list, {additionalContext: context, sanitizeDefaultValues: true});
                makeDefaultRecords.push(self._makeDefaultRecord(list.model, params));
            });
        } else {
            params.context = self._getContext(list, {sanitizeDefaultValues: true});
            makeDefaultRecords.push(self._makeDefaultRecord(list.model, params));
        }
        ...
        return Promise.all(makeDefaultRecords).then(function (resultIds){
            ...
            _.each(resultIds, function (id){
                ...
                if (list.orderedResIDs) {
                    ...
                    list.orderedResIDs.splice(index, 0, record.resId);
    ...
    _applyChange: function (recordId, changes, options) {
        ...
        return Promise.all(defs).then(function () {
            var onchangeFields = []; // the fields that have changed and that have an onchange
            for (var fieldName in changes) {
                field = record.fields[fieldName];
     ...
     * to do a /nameGet to fetch a displayName.
     *
     * Moreover, for the many2one case, a new value can sometimes be set (i.e.
     * a displayName is given, but no id). When this happens, we first do a
     * nameCreate.
     *
     * @param {Object} record
    ...
    _applyX2OneChange: async function (record, fieldName, data, options) {
        ...
        if (allowedTypes.includes(field.type) && !data.id && data.displayName) {
            // only displayName given -> do a nameCreate
            ...
            // _recName cannot create record based on nameCreate.
            if (!result) {
                record._changes[fieldName] = false;
                return Promise.resolve();
            }
            data = {id: result[0], displayName: result[1]};
        }

        // here, we check that the many2one really changed. If the resId is the
        ...
        if (relatedRecord && (data.id === this.localData[relatedID].resId)) {
            return Promise.resolve();
        }
        var relData = _.pick(data, 'id', 'displayName');
        ...
        var def;
        if (relData.displayName === undefined || fieldOptions.alwaysReload) {
            ...
            def = this._rpc({
                ...
                .then(function (result) {
                    relData.displayName = result[0][1];
                });
        }
        return Promise.resolve(def).then(function () {
    ...
    _applyOnChange: function (values, record, options = {}) {
        ...
        for (let name in (values || {})) {
            ...
            if (!field) {
                ...
                // It is impossible to distinguish between values returned by the defaultGet
            ...
            if (field.type === 'many2one') {
                ...
                if (val) {
                    ...
                    // an array [id, displayName] or just an id.
                    var data = _.isArray(val) ?
                        {id: val[0], displayName: val[1]} :
                        {id: val};
                    if (!oldValue || (self.localData[oldValue].resId !== data.id)) {
                ...
                if (val) {
                    ...
                    if (!oldValue || self.localData[oldValue].resId !== resID ||
                ...
                _.each(val, function (command) {
                    ...
                    if (command[0] === 0 || command[0] === 1) {
                        ...
                        if (!rec) {
                            ...
                            if (command[0] === 1) {
                                params.resId = command[1];
                            }
                            rec = self._makeDataPoint(params);
                            list._cache[rec.resId] = rec.id;
                            if (options.firstOnChange) {
                        ...
                        // from `defaultGet`. The list has a savepoint only
                        // after having fully executed `defaultGet`.
        ...
        // inner function that adds a record (based on its resId) to a list
    ...
    _applyX2ManyChange: async function (record, fieldName, command, options) {
        ...
        switch (command.operation) {
            case 'ADD':
                ...
                list._cache[newRecord.resId] = newRecord.id;
            ...
            case 'ADD_M2M':
                ...
                // nameCreate records for which there is no id (typically, could
                // be the case of a quickCreate in a many2manyTags, so data.length
                // is 1)
                for (const r of data) {
                ...
                if (data.length + list.resIds.length > list.limit) {
                    list.limit = data.length + list.resIds.length;
                }
                ...
                _.each(data, function (d) {
                    ...
                    listRecords[d.id] = rec;
                    list._cache[rec.resId] = rec.id;
                    list._changes.push({operation: 'ADD', id: rec.id});
                });
                // read list's records as we only have their ids and optionally their displayName
                ...
                if (fieldNames.length) {
                    def = this._rpc({
                        ...
                        _.each(records, function (record) {
                            listRecords[record.id].data = record;
            ...
            case 'CREATE':
                ...
                def = this._addX2ManyDefaultRecord(list, createOptions).then(function (ids) {
                    _.each(ids, function(id){
                        ...
                        // FIXME: hack for lunch widget, which does useless defaultGet and onchange
                        if (command.data) {
            ...
            case 'REPLACE_WITH':
                ...
                list._changes = [];
                var newIds = _.difference(command.ids, list.resIds);
                var removedIds = _.difference(list.resIds, command.ids);
                ...
                if (removedIds.length) {
                    ...
                    removedDef = this._applyX2ManyChange(record, fieldName, {
                        ...
                        ids: _.map(removedIds, function (resID) {
                            ...
                            return _.findWhere(listData, {resId: resID}).id;
                        }),
                    }, options);
                }
     ...
     * 'count', 'data' and 'resIds' keys have been updated.
     *
     * @private
    ...
    _applyX2ManyOperations: function (list, options) {
        ...
        var self = this;
        list = _.extend({}, list);
        list.resIds = list.resIds.slice(0);
        ...
        _.each(changes, function (change) {
            ...
            switch (change.operation) {
                case 'ADD':
                    list.count++;
                    var resID = relRecord ? relRecord.resId : change.resID;
                    if (change.position === 'top' && (options ? options.position !== 'bottom' : true)) {
                        list.resIds.unshift(resID);
                    } else {
                        list.resIds.push(resID);
                    }
                    break;
                case 'FORGET':
                case 'DELETE':
                    ...
                    var deletedResID = relRecord ? relRecord.resId : change.id;
                    list.resIds = _.without(list.resIds, deletedResID);
                    break;
                case 'REMOVE_ALL':
                    list.count = 0;
                    list.resIds = [];
    ...
    _evalModifiers: function (element, modifiers) {
        ...
        const evaluated = {};
        for (const k of ['invisible', 'columnInvisible', 'readonly', 'required']) {
    ...
    /**
     * Fetch the type of model (e.g. 'res.partner') contained in the modelField
     * @private
    ...
    _fetchModelFieldReference: async function (record, fieldName, fieldInfo) {
        const modelField = fieldInfo.options.modelField;
        ...
        if (value) {
            if (record.fields[modelField].type !== "many2one"
                ...
                throw new Error(`The modelField of the reference field
                                 ${fieldName} must be a many2one('ir.model').`);
            }
            const modelId = this.localData[value].resId;
    ...
    /**
     * Fetch nameGet for a record datapoint.
     *
    ...
    _fetchNameGet: function (dataPoint) {
        return this._rpc({
            ...
            method: 'nameGet',
            args: [dataPoint.resId],
            context: dataPoint.getContext(),
        }).then(function (result) {
            dataPoint.data.displayName = result[0][1];
        });
    },
    /**
     * Fetch nameGet for a field of type Many2one or Reference
     *
     * @private
    ...
    _fetchNameGets: function (list, fieldName) {
        ...
        return this._rpc({
            ...
            .then(function (nameGets) {
                _.each(records, function (record) {
                    var nameGet = _.find(nameGets, function (nameGet) {
                        return nameGet[0] === record.data.id;
                    });
                    record.data.displayName = nameGet[1];
                });
            });
    },
    /**
     ...
     *   not given, fetch all the fields in record.fieldNames (+ displayName)
    ...
    _fetchRecord: function (record, options) {
        ...
        fieldNames = _.uniq(fieldNames.concat(['displayName']));
        return this._rpc({
                model: record.model,
                method: 'read',
                args: [[record.resId], fieldNames],
                context: _.extend({binSize: true}, record.getContext()),
            })
            .then(function (result) {
                if (result.length === 0) {
    ...
    /**
     * Fetch the `nameGet` for a reference field.
     *
     * @private
     * @param {Object} record
     * @param {string} fieldName
    ...
    /**
     * Fetch the extra data (`nameGet`) for the reference fields of the record
     * model.
     *
     * @private
     * @param {Object} record
     ...
     * Called by _readGroup to make only one 'nameGet' rpc by fieldName.
     *
     * @param {Object} list a valid resource object
     * @returns {Promise}
     */
     ...
     * field relation. For example, if fieldName is "tagIds" and referred to
     * project.tags, it will fetch project.tags' related fields where its id is
     * contained in toFetch.ids array.
     *
     * @param {Object} list a valid resource object
     * @param {Object} toFetch a list of records and resIds,
     *   @see _getDataToFetch
     * @param {string} fieldName
     * @returns {Promise}
     */
    _fetchRelatedData: function (list, toFetch, fieldName) {
        ...
        if (!ids.length || fieldInfo.__noFetch) {
            return Promise.resolve();
        }
                ...
                return self._rpc({
                    ...
                    .then(function (nameGets) {
                        _.each(records, function (rec) {
                            var nameGet = _.find(nameGets, function (n) {
                                return n[0] === rec.id;
                            });
                            rec.displayName = nameGet[1];
                        });
                        return records;
                    });
            });
    ...
    /**
     * Fetches the `nameGet` associated to the reference widget if the field is
    ...
    _fetchSpecialReference: function (record, fieldName, fieldInfo) {
        var field = record.fields[fieldName];
        if (field.type === 'char') {
            // if the widget reference is set on a char field, the nameGet
            // needs to be fetched a posteriori
            return Promise.resolve(this._fetchReference(record, fieldName));
        } else if (fieldInfo.options.modelField) {
     ...
     * _fetchSpecialMany2ones is that the field given by options.foldField is
     * also fetched.
     *
     * @param {Object} record - an element from the localData
     * @param {Object} fieldName - the name of the field
    ...
    _fetchSpecialStatus: function (record, fieldName, fieldInfo) {
        var foldField = fieldInfo.options.foldField;
        ...
        if (list.static) {
            def = this._readUngroupedList(list).then(function () {
                if (list.parentId && self.isNew(list.parentId)) {
                    // list from a defaultGet, so fetch displayName for many2one fields
     ...
     * record (i.e. maps to the new value for basic fields, to the resId for
    ...
    _generateChanges: function (record, options) {
        ...
        for (var fieldName in record.fields) {
            ...
            if (type === 'one2many' || type === 'many2many') {
                ...
                changes[fieldName] = value ? this.localData[value].resId : false;
            } else if (type === 'reference' && fieldName in changes) {
                value = changes[fieldName];
                changes[fieldName] = value ?
                    this.localData[value].model + ',' + this.localData[value].resId :
                    false;
    ...
    _generateOnChangeData: function (record, options) {
        ...
        if (!options.firstOnChange) {
            ...
            // 'displayName' is automatically added to the list of fields to fetch,
            ...
            var hasDisplayName = _.some(record.fieldsInfo, function (fieldsInfo) {
                return 'displayName' in fieldsInfo;
            });
            if (!hasDisplayName) {
    ...
    _generateX2ManyCommands: function (record, options) {
        ...
        for (var fieldName in fields) {
            ...
            if (type === 'many2many' || type === 'one2many') {
                ...
                var oldResIDs = list.resIds.slice(0);
                var relRecordAdded = [];
                ...
                if (type === 'many2many' || list._forceM2MLink) {
                    var relRecordCreated = _.filter(relRecordAdded, function (rec) {
                        return typeof rec.resId === 'string';
                    });
                    var realIDs = _.difference(list.resIds, _.pluck(relRecordCreated, 'resId'));
                    ...
                    commands[fieldName].push(x2ManyCommands.replaceWith(realIDs));
                    ...
                    _.each(relRecordUpdated, function (relRecord) {
                        ...
                        if (!_.isEmpty(changes)) {
                            var command = x2ManyCommands.update(relRecord.resId, changes);
                            commands[fieldName].push(command);
                        }
                    });
                } else if (type === 'one2many') {
                    var removedIds = _.difference(oldResIDs, list.resIds);
                    var addedIds = _.difference(list.resIds, oldResIDs);
                    var keptIds = _.intersection(oldResIDs, list.resIds);

                    // the didChange variable keeps track of the fact that at
                    // least one id was updated
                    var didChange = false;
                    var changes, command, relRecord;
                    for (var i = 0; i < list.resIds.length; i++) {
                        if (_.contains(keptIds, list.resIds[i])) {
                            // this is an id that already existed
                            relRecord = _.findWhere(relRecordUpdated, {resId: list.resIds[i]});
                            changes = relRecord ? this._generateChanges(relRecord, options) : {};
                            if (!_.isEmpty(changes)) {
                                command = x2ManyCommands.update(relRecord.resId, changes);
                                didChange = true;
                            } else {
                                command = x2ManyCommands.linkTo(list.resIds[i]);
                            }
                            commands[fieldName].push(command);
                        } else if (_.contains(addedIds, list.resIds[i])) {
                            // this is a new id (maybe existing in DB, but new in JS)
                            relRecord = _.findWhere(relRecordAdded, {resId: list.resIds[i]});
                            if (!relRecord) {
                                commands[fieldName].push(x2ManyCommands.linkTo(list.resIds[i]));
                                continue;
                            }
                            changes = this._generateChanges(relRecord, options);
                            if (!this.isNew(relRecord.id)) {
                                // the subrecord already exists in db
                                commands[fieldName].push(x2ManyCommands.linkTo(relRecord.resId));
                                if (changesOnly ? Object.keys(changes).length : this.isDirty(relRecord.id)) {
                                    delete changes.id;
                                    commands[fieldName].push(x2ManyCommands.update(relRecord.resId, changes));
                                }
                            } else {
                                ...
                                // fieldsGet of models of related fields. We save those values
    ...
    _getContext: function (element, options) {
        options = options || {};
        var context = new Context(session.userContext);
        context.setEvalContext(this._getEvalContext(element));

        if (options.full || !(options.fieldName || options.additionalContext)) {
            var contextToAdd = options.sanitizeDefaultValues ?
        ...
        if (element.rawContext) {
            ...
            rawContext.setEvalContext(evalContext);
     ...
     *   this list
     * @param {string} fieldName
     * @return {Object} a list of records and resIds
     */
     ...
     * where each key represent an id (resId) containing an array of
     * webclient id (referred to a datapoint, so not a resId).
     */
     ...
     *
     * It is actually quite subtle.  We need to add some magic keys: activeId
     * and activeIds.  Also, the session user context is added in the mix to be
    ...
    _getEvalContext: function (element, forDomain) {
        ...
        // Uses "currentCompanyId" because "companyId" would conflict with all the companyId fields
        // in general, the actual "companyId" field of the form should be used for m2o domains, not this fallback
        let currentCompanyId;
        if (session.userContext.allowedCompanyIds) {
            currentCompanyId = session.userContext.allowedCompanyIds[0];
        } else {
            currentCompanyId = session.userCompanies ?
                session.userCompanies.currentCompany :
                false;
        }
        return Object.assign(
            {
                activeId: evalContext.id || false,
                activeIds: evalContext.id ? [evalContext.id] : [],
                activeModel: element.model,
                currentCompanyId,
     ...
     * This permits to a webReadGroup to keep some of the properties
    ...
    _getGroupedListPropsToKeep(list, options) {
        ...
        if (options.onlyGroup || isOpen && options.hasSubgroups) {
            ...
            if (options.onlyGroup) {
                // keep count and resIds as in this case the group
                // won't be searchRead again. This situation happens
                // when using kanban quickCreate where the record is manually
                // added to the datapoint before getting here.
                propsToKeep.resIds = list.resIds;
     ...
     * fetch their names in the case of a defaultGet.
     *
     * @private
    ...
    _getRecordEvalContext: function (record, forDomain) {
        ...
        for (var fieldName in context) {
            ...
            if (field.type === 'many2one') {
                ...
                context[fieldName] = relDataPoint ? relDataPoint.resId : false;
                continue;
            }
            if (field.type === 'one2many' || field.type === 'many2many') {
                var ids;
                if (!context[fieldName] || _.isArray(context[fieldName])) { // no dataPoint created yet
                    ...
                    ids = relDataPoint.resIds.slice(0);
                }
                if (!forDomain) {
    ...
    _invalidateCache: function (dataPoint) {
        ...
        if (dataPoint.model === 'res.company' && !localStorage.getItem('runningTour')) {
     ...
     * readonly modifier unless there is a forceSave modifier (checking first
    ...
    _isFieldProtected: function (record, fieldName, viewType) {
        ...
        if (fieldInfo) {
            ...
            return modifiers.readonly && !fieldInfo.forceSave;
        } else {
            return false;
        }
    },
     ...
     *   present in the readGroup anymore (empty groups) will stay in the
     ...
     * @param {integer|null} [params.resId] actual id of record in the server
     * @param {integer[]} [params.resIds] context in which the data point is used, from a list of resId
    ...
    _makeDataPoint: function (params) {
        ...
        var dataPoint = {
            ...
            offset: params.offset || (type === 'record' ? _.indexOf(resIds, resId) : 0),
     ...
     * needs to be done:
     * - call the /defaultGet route to get default values
    ...
    async _makeDefaultRecord(modelName, params) {
        ...
        // -> This is a rare case where the defaulGet from the server
        ...
        // fetch additional data (special data and many2one namegets for "alwaysReload" fields)
    ...
    _parseServerData: function (fieldNames, element, data) {
        ...
        _.each(fieldNames, function (fieldName) {
            ...
            if (field.type === 'many2one') {
                ...
                if (val !== false) {
                    // the many2one value is of the form [id, displayName]
    ...
    async _performOnChange(record, fields, options = {}) {
        ...
        if (result.warning) {
            this.triggerUp('warning', result.warning);
            ...
            if (options.alwaysReload) {
                if (record.fields[name].type === 'many2one') {
                    ...
                    defs.push(self._rpc({
                        ...
                        .then(function (result) {
                            relRecord.data.displayName = result[0][1];
                        }));
                }
            }
        });
    ...
    _processX2ManyCommands: function (record, fieldName, commands, options) {
        ...
        _.each(commands, function (value) {
            ...
            if (value[0] === 0) {
                ...
                x2manyList._cache[r.resId] = r.id;
            ...
            if (value[0] === 6) {
                // REPLACE_WITH
                _.each(value[2], function (resId) {
                    x2manyList._changes.push({operation: 'ADD', resID: resId});
                });
     ...
     * performing a /readGroup. It also tries to read open subgroups if they
     * were open before.
     *
    ...
    _readGroup: function (list, options) {
        ...
        var orderedBy = _.filter(list.orderedBy, function (order) {
            return order.name === rawGroupBy || list.fields[order.name].groupOperator !== undefined;
        });
        ...
        return this._rpc({
                ...
                lazy: true,
                expand: expand,
                expandLimit: expand ? list.limit : null,
                expandOrderby: expand ? list.orderedBy : null,
            })
            .then(function (result) {
                ...
                _.each(groups, function (group) {
                    ...
                    // When groupbyNoLeaf key is present FIELD_ID_count doesn't exist
                    // we have to get the count from `__count` instead
                    // see _readGroupRaw in models.js
                    ...
                    var oldGroup = _.find(previousGroups, function (g) {
                        return g.resId === newGroup.resId && g.value === newGroup.value;
                    });
                    if (oldGroup) {
                    ...
                    if (newGroup.isOpen && newGroup.count > 0) {
                        openGroupCount++;
                        if (group.__data) {
                            // bypass the searchRead when the group's records have been obtained
                            // by the call to 'webReadGroup' (see @_searchReadUngroupedList)
                ...
                return Promise.all(defs).then(function (groups) {
                    if (!options.onlyGroups) {
                        // generate the resIds of the main list, being the concatenation
                        // of the fetched resIds in each group
                        list.resIds = _.flatten(_.map(groups, function (group) {
                            return group ? group.resIds : [];
                        }));
                    }
                    return list;
                }).then(function () {
    ...
    _readUngroupedList: function (list) {
        ...
        // generate the current count and resIds list by applying the changes
        list = this._applyX2ManyOperations(list);

        // for multi-pages list datapoints, we might need to read the
        // order field first to apply the order on all pages
        if (list.resIds.length > list.limit && list.orderedBy.length) {
            if (!list.orderedResIDs) {
                var fieldNames = _.pluck(list.orderedBy, 'label');
                def = this._readMissingFields(list, _.filter(list.resIds, _.isNumber), fieldNames);
            }
            def.then(function () {
                self._sortList(list);
            });
        ...
        return def.then(function () {
            var resIds = [];
            var currentResIDs = list.resIds;
            // if new records have been added to the list, their virtual ids have
            // been pushed at the end of resIds (or at the beginning, depending
            ...
            // resIds, which correspond to records that have been removed from
            // the relation (this information being taken into account in resIds
            // but not in orderedResIDs)
            if (list.orderedResIDs) {
                currentResIDs = list.orderedResIDs.filter(function (resID) {
                    return list.resIds.indexOf(resID) >= 0;
                });
            }
            ...
            return self._readMissingFields(list, resIds, fieldNames).then(function () {
                if (list.resIds.length <= list.limit) {
                    self._sortList(list);
                } else {
    ...
    _reload: function (id, options) {
        ...
        if (options.currentId !== undefined) {
            element.resId = options.currentId;
        }
        if (options.ids !== undefined) {
            element.resIds = options.ids;
            element.count = element.resIds.length;
        }
        if (element.type === 'record') {
            element.offset = _.indexOf(element.resIds, element.resId);
        }
     ...
     * In this case, we use the result of the webReadGroup rpc to tweak the
    ...
    async _rpc(params) {
        ...
        if (this.sampleModel && params.method === 'webReadGroup' && result.length) {
    ...
    /**
     * Do a /searchRead to get data for a list resource.  This does a
     * /searchRead because the data may not be static (for ex, a list view).
     *
     * @param {Object} list
     * @returns {Promise}
     */
    _searchReadUngroupedList: function (list) {
        ...
        if (list.__data) {
            ...
            // call to 'webReadGroup'), so we can bypass the searchRead
            // But the webReadGroup returns the rawGroupBy field's value, which may not be present
            ...
            prom = this._rpc({
                ...
                model: list.model,
                fields: fieldNames,
                context: _.extend({}, list.getContext(), {binSize: true}),
        ...
        return prom.then(function (result) {
            ...
            if (list.loadMoreOffset) {
                ...
                list.resIds = list.resIds.concat(ids);
            } else {
                list.data = data;
                list.resIds = ids;
            }
    ...
    _setDataInRange: function (list) {
        var idsInRange;
        if (list.limit) {
            idsInRange = list.resIds.slice(list.offset, list.offset + list.limit);
        } else {
            idsInRange = list.resIds;
        }
        list.data = [];
        ...
        if (list.limit) {
            for (var i = list.offset + list.limit; i < list.resIds.length; i++) {
                var id = list.resIds[i];
    ...
    _setOffset: function (elementId, offset) {
        ...
        if (element.type === 'record' && element.resIds.length) {
            element.resId = element.resIds[offset];
        }
    },
    /**
    ...
    _sortList: function (list) {
        ...
        if (list.orderedResIDs) {
            ...
            utils.stableSort(list.resIds, function compareResIdIndexes (resId1, resId2) {
            ...
            // sort records according to orderedBy[0]
            var compareRecords = function (resId1, resId2, level) {
                ...
                // If the field is a relation, sort on the displayName of those records
                if (list.fields[order.name].type === 'many2one') {
                    orderData1 = orderData1 ? self.localData[orderData1].data.displayName : "";
                    orderData2 = orderData2 ? self.localData[orderData2].data.displayName : "";
                }
            ...
            utils.stableSort(list.resIds, compareRecords);
        }
        this._setDataInRange(list);
    },
    /**
     * Updates the resIds of the parent of a given element of type list.
     *
     * After some operations (e.g. loading more records, folding/unfolding a
     * group), the resIds list of an element may be updated. When this happens,
     * the resIds of its ancestors need to be updated as well. This is the
    ...
    _updateParentResIDs: function (element) {
        ...
        if (element.parentId) {
            var parent = this.localData[element.parentId];
            parent.resIds =  _.flatten(_.map(parent.data, function (dataPointID) {
                return self.localData[dataPointID].resIds;
            }));
    ...
    _updateRecordsData: function (records, fieldName, values) {
        ...
        _.each(records, function (record) {
            ...
            x2mList.data = [];
            _.each(x2mList.resIds, function (resId) {
                var dataPoint = self._makeDataPoint({
                    modelName: field.relation,
                    data: id2Values.get(resId),
                    fields: fields,
                    fieldsInfo: fieldsInfo,
                    parentId: x2mList.id,
                    viewType: viewType,
                ...
                x2mList._cache[resId] = dataPoint.id;
            });
        });
    },
    /**
     * Used to apply a updateFn to other records with same resId, that are
     * under the same topmost list groupedBy many2many.
     *
     * When a list datapoint is grouped by many2many, same resId record could
     ...
     *  representing the same resId record
     */
    _updateDuplicateRecords(recordId, updateFn) {
        const { model, resId } = this.localData[recordId];
        ...
        utils.traverseRecords(topmostList, (r) => {
            if (r.resId === resId && r.id !== recordId) {
                updateFn(r.id);
            }
        });
    },
