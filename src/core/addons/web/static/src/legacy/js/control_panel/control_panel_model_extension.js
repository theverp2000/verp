verp.define("web/static/src/js/control_panel/control_panel_model_extension.js", function (require) {
    "use strict";
    ...
    const DISABLE_FAVORITE = "searchDisableCustomFilters";
    ...
    class ControlPanelModelExtension extends ActionModel.Extension {
        ...
        constructor() {
            ...
            for (const key in this.actionContext) {
                const match = /^searchDefault_(.*)$/.exec(key);
                if (match) {
        ...
        getIrFilterValues(preFilter) {
            ...
            const userContext = this.env.session.userContext;
        ...
        async deleteFavorite(filterId) {
            ...
            await this.env.dataManager.deleteFilter(serverSideId);
        ...
        getDomain() {
            ...
            let domain;
            try {
                domain = this._getDomain(groups);
                return Domain.prototype.stringToArray(domain, userContext);
            } catch (err) {
                throw new Error(
                    `${this.env._t("Control panel model extension failed to evaluate domain=")} '${domain}', context='${JSON.stringify(userContext)}'.\n${err}`
                );
            }
        }
        ...
        toggleFilterWithOptions(filterId, optionId) {
            ...
            if (index >= 0) {
                ...
                if (filter.type === 'filter' && !yearSelected(this._getSelectedOptionIds(filterId))) {
                    // Here we add 'thisYear' as options if no option of type
        ...
        _evalArchChild(child) {
            if (child.attrs.context) {
                try {
                    const context = vjUtils.eval('context', child.attrs.context);
                    child.attrs.context = context;
                    if (context.groupby) {
                        // let us extract basic data since we just evaluated context
                        // and use a correct tag!
                        child.attrs.fieldName = context.groupby.split(':')[0];
                        child.attrs.defaultInterval = context.groupby.split(':')[1];
                        child.tag = 'groupby';
                    }
                } catch (e) { }
            }
        ...
        _extractAttributes(filter, attrs) {
            ...
            switch (filter.type) {
                ...
                case 'field': {
                    ...
                    if (attrs.filterDomain) {
                        filter.filterDomain = attrs.filterDomain;
        ...
        /**
         * Returns an object irFilter serving to create an irFilte in db
        ...
        _favoriteToIrFilter(favorite) {
            ...
            if ('isDefault' in favorite) {
                irFilter.isDefault = favorite.isDefault;
            }
        ...
        _getAutoCompletionFilterDomain(filter, filterQueryElements) {
            ...
            return vjUtils.assembleDomains(domains, 'OR');
        }
        ...
        _getContext(groups) {
            ...
            const evaluationContext = this.env.session.userContext;
            try {
                return vjUtils.eval('contexts', contexts, evaluationContext);
            } catch (err) {
                throw new Error(
        ...
        _getDomain(groups) {
            ...
            return vjUtils.assembleDomains(domains, 'AND');
        }
        ...
        */
        _getFilterContext(filter, filterQueryElements) {
            ...
            if (filter.type === 'field' && filter.context) {
                context = vjUtils.eval('context',
                    filter.context,
                    { self: filterQueryElements.map(({ value }) => value) },
                );
            }
            // the following code aims to remodel this:
            // https://github.com/verp/verp/blob/12.0/addons/web/static/src/js/views/search/search_inputs.js#L498
        ...
        _getGroupBy(groups) {
            ...
            const groupby = groupBys.length ? groupBys : (this.actionContext.groupby || []);
        ...
        /**
         * Returns a filter of type 'favorite' starting from an irFilter comming from db.
         * @private
         * @param {Object} irFilter
         * @returns {Object}
         */
        _irFilterToFavorite(irFilter) {
            let userId = irFilter.userId || false;
            if (Array.isArray(userId)) {
                userId = userId[0];
            }
            const groupNumber = userId ? FAVORITE_PRIVATE_GROUP : FAVORITE_SHARED_GROUP;
            const context = vjUtils.eval('context', irFilter.context, this.env.session.userContext);
            let groupBys = [];
            if (context.groupby) {
                groupBys = context.groupby;
                delete context.groupby;
            }
            let comparison;
            ...
            if (irFilter.isDefault) {
                favorite.isDefault = irFilter.isDefault;
            }
            if (comparison) {
        ...
        /**
         * Compute the search Query and save it as an irFilter in db.
        ...
        async _saveQuery(preFilter) {
            ...
            const serverSideId = await this.env.dataManager.createFilter(irFilter);
