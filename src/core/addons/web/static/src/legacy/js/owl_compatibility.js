verp.define('web.OwlCompatibility', function () {
    "use strict";

    /**
     * This file defines the necessary tools for the transition phase where Verp
    ...
    const widgetSymbol = verp.widgetSymbol;
     ...
     * adapter for Owl components that embed Verp legacy widgets (or dynamically
     * both Owl components and Verp legacy widgets), e.g.:
     *
     *                           Owl Component
     *                                 |
    ...
    class ComponentAdapter extends Component {
         ...
         */
        mounted() {
            if (this.widget && this.widget.onAttachCallback) {
                this.widget.onAttachCallback();
            }
        }
         ...
         */
        willUnmount() {
            if (this.widget && this.widget.onDetachCallback) {
                this.widget.onDetachCallback();
            }
        }
        ...
        /**
         * Mocks _triggerUp to redirect Verp legacy events to OWL events.
         *
         * @private
         * @param {VerpEvent} ev
         */
        _triggerUp(ev) {
            ...
            } else if (evType === 'getSession') {
                if (payload.callback) {
                    payload.callback(this.env.session);
                }
            } else if (evType === 'loadViews') {
                const params = {
                    model: payload.modelName,
                    context: payload.context,
                    viewsDescr: payload.views,
                };
                this.env.dataManager
                    .loadViews(params, payload.options || {})
                    .then(payload.onSuccess);
            } else if (evType === 'loadFilters') {
     ...
     * together when an Verp legacy widget needs to instantiate Owl components.
     ...
     * custom events (triggerUp) on the parent legacy widget.
    ...
    const WidgetAdapterMixin = {
        /**
         * Calls onAttachCallback on each child ComponentWrapper, which will
         * call __callMounted on each sub component (recursively), to mark them
         * as mounted.
         */
        onAttachCallback() {
            for (const component of children.get(this) || []) {
                component.onAttachCallback();
            }
        },
        /**
         * Calls onDetachCallback on each child ComponentWrapper, which will
         * call __callWillUnmount to mark itself and its children as no longer
         * mounted.
         */
        onDetachCallback() {
            for (const component of children.get(this) || []) {
                component.onDetachCallback();
            }
        },
        /**
    ...
    class ComponentWrapper extends Component {
         ...
         * custom (yet DOM) events to legacy custom events (triggerUp).
         *
         * @override
         ...
         * Verp legacy event. This function is called just before the event is
         * actually triggered.
         *
         * @private
         * @param {string} evType
            ...
            if (this.parentWidget && !this._handledEvents.has(evType)) {
                ...
                this.el.addEventListener(evType, ev => {
                    ...
                    if (!ev.cancelBubble) {
                        ...
                        // this.parentWidget.triggerUp(ev.type.replace(/-/g, '_'), detail); // origin: change event-name => event_name
                        // this.parentWidget.triggerUp(jQuery.camelCase(ev.type), detail); // Tony changed event-name => eventName
                        this.parentWidget.triggerUp(ev.type, detail); // Tony 2 no changed
                    }
                });
            }
        }
