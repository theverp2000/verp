...
export class SearchModel extends EventBus {
    ...
    async load(config) {
        ...
        for (const key in this.globalContext) {
            ...
            const searchDefaultMatch = /^searchDefault_(.*)$/.exec(key);
            ...
            const searchpanelDefaultMatch = /^searchpanelDefault_(.*)$/.exec(key);
    ...
    async createNewFavorite(params) {
        ...
        const serverSideId = await this.orm.call("ir.filters", "createOrReplace", [irFilter]);
    ...
    _createCategoryTree(sectionId, result) {
        ...
        let { errorMsg, parentField: parentField, values } = result;
        if (errorMsg) {
            category.errorMsg = errorMsg;
            values = [];
        }
            ...
            for (const value of values) {
                ...
                if (!groups.has(groupId)) {
                    ...
                    groups.set(groupId, {
                        ...
                        values: new Map(),
                        tooltip: value.groupTooltip,
                        sequence: value.groupSequence,
                        hexColor: value.groupHexColor,
                    });
        ...
        await Promise.all(
            categories.map(async (category) => {
                const result = await this.orm.call(
                    ...
                    [category.fieldName],
                    {
                        categoryDomain: this._getCategoryDomain(category.id),
                        enableCounters: category.enableCounters,
    ...
    async _fetchFilters(filters) {
        ...
        await Promise.all(
            filters.map(async (filter) => {
                const result = await this.orm.call(
                    this.resModel,
                    "searchPanelSelectMultiRange",
                    [filter.fieldName],
                    {
                        categoryDomain: categoryDomain,
                        comodelDomain: new Domain(filter.domain).toList(evalContext),
                        enableCounters: filter.enableCounters,
                        filterDomain: this._getFilterDomain(filter.id),
                        expand: filter.expand,
                        groupby: filter.groupby || false,
                        groupDomain: this._getGroupDomain(filter),
                        limit: filter.limit,
                        searchDomain: searchDomain,
                    }
                );
    ...
    _getCategoryDomain(excludedCategoryId) {
        ...
        for (const category of this.categories) {
            ...
            const operator = field.type === "many2one" && category.parentField ? "childOf" : "=";
    ...
    _getFieldDomain(field, autocompleteValues) {
        const domains = autocompleteValues.map(({ label, value, operator }) => {
            ...
            if (field.filterDomain) {
                domain = new Domain(field.filterDomain).toList({ self: label, rawValue: value });
            } else {
    ...
    _irFilterToFavorite(irFilter) {
        let userId = false;
        if (Array.isArray(irFilter.userId)) {
            userId = irFilter.userId[0];
        }
