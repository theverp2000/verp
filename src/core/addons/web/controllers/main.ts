  ...
  async load(req, res, opts: { actionId?: any, additionalContext?: any }) {
    ...
    if (isNaN(actionId)) {
      try {
        ...
        actionId = (await action.actionId).id;
      } catch (e) {
        actionId = 0;   // force failed read
      }
    }
    let result: any = false;
    const baseAction = await (await env.items('ir.actions.actions').browse([actionId]).sudo()).readOne(['type']);
    if (bool(baseAction)) {
      const ctx = Object.assign({}, req.context);
      const type = baseAction['type'];
      if (type === 'ir.actions.report') {
        setOptions(ctx, { 'binSize': true });
      }
      if (opts.additionalContext) {
        setOptions(ctx, opts.additionalContext);
      }
      req.context = ctx;
      result = (await (await env.items(type).sudo()).searchRead([['actionId', '=', actionId]]))[0];
      result['id'] = actionId; // change actwindow.id to action.id
...
class ReportController extends http.Controller {
  ...
  async reportDownload(req: WebRequest, res, opts: { data?: any, context?: any } = {}) {
    ...
    try {
      if (['qweb-pdf', 'qweb-text'].includes(type)) {
        ...
        if (docids) {
          ...
          if (await report.printReportName && !(len(obj) > 1)) {
            const reportName = await unsafeAsync(await report.printReportName, { 'object': obj, 'time': Date });
      ...
      console.error("Error while generating report", reportname, '\n', e);
