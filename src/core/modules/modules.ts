...
export function getModulePath(modul: string, downloaded?: boolean, displayWarning?: boolean): string {
  let p: string;
  for (const adp of addons.paths) {
    const files = MANIFEST_NAMES.map((manifest) =>path.join(adp, modul, manifest)).concat(path.join(adp, modul, '.zip'));
    for (const f of files) {
      if (existsSync(f)) {
        p = path.join(adp, modul);
        break;
      }
    }
  }
  if (!p && downloaded) {
    return path.join(config.addonsDataDir, modul);
  }
  if (!p && displayWarning) {
    console.log('module %s: module not found', modul);
  }
  return p;
}
...
export function getResourcePath(modul: string, ...args: string[]): string {
  const modPath = getModulePath(modul);
  if (!modPath) {
    return '';
  }
