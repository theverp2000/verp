    ...
    for (const rec of list) {
      if (typeof rec === 'string') { // SQL string
        ...
        const sql = _format(
          ...
          table: cr._gen.quote(table),
          fields: rec[1].map(x => cr._gen.quote(x))
        });
        current = rec;
        ...
        for (const [table, values] of Object.entries<{}>(rec)) {
          ...
          const sql = _format(
            ...
            table: cr._gen.quote(table),
            fields: Object.keys(values).map(x => cr._gen.quote(x))
          });
...
export async function createCategories(cr: Cursor, categories: string[]): Promise<number> {
  ...
  while (categories.length) {
    ...
    const gen = cr._gen;
    let res = await cr.execute(`SELECT ${gen.quoteIdentifiers("resId")} FROM ${gen.quoteIdentifiers("irModelData")} WHERE label=$1 AND module=$2 AND model=$3`,
      { bind: [xmlid, 'base', 'ir.module.category'] });
    let cId;
    if (res.length && res[0]['resId']) {
      cId = res[0]['resId'];
    } else {
      res = await cr.execute(`INSERT INTO ${gen.quoteIdentifiers("irModuleCategory")} (label, ${gen.quoteIdentifiers("parentId")}) VALUES ($1, $2) RETURNING id`, { bind: [categories[0], pId] });
      cId = res[0]['id'];
      await cr.execute(`INSERT INTO ${gen.quoteIdentifiers("irModelData")} (module, label, ${gen.quoteIdentifiers("resId")}, model, noupdate) VALUES ($1, $2, $3, $4, $5)`, { bind: ['base', xmlid, cId, 'ir.module.category', true] });
    }
    pId = cId;
