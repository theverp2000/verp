...
function originIds(ids) {
  return List.from(map(filter(ids ?? [], (id) => bool(id) || id.origin), (id) => bool(id) ? id : id.origin));
}
...
class RecordCache {
  ...
  *[Symbol.iterator]() {
  ...
  get(fname, value) {
    ...
    return this._record.env.cache.get(this._record, field, value);
  }
...
export class BaseModel extends Function {
  ...
  async checkAccessRights(operation, raiseException = true) {
    ...
    return model.check(this._name, operation, raiseException)
  }
  ...
  async checkAccessRule(operation) {
    ...
    console.info('Failed operation on deleted record(s): %s, uid: %s, model: %s', operation, this._uid, this._name);
    throw new MissingError(
      await this._t('One of the documents you are trying to access has been deleted, please try again after refreshing.') + format('\n\n(%s %s, %s %s, %s %s, %s %s)', await this._t('Document type:'), this._name, await this._t('Operations:'), operation, await this._t('Records:'), invalid.ids.slice(0, 6), await this._t('User:'), this._uid)
    );
  }
  ...
  async _filterAccessRulesSystem(operation) {
    const dom = await this.env.items('ir.rule')._computeDomain(this._name, operation);
  ...
  async checkFieldAccessRights(operation, fields) {
    ...
    if (!bool(fields)) {
      ...
      if (invalidFields.length) {
        console.log('Access Denied by ACLs for operation: %s, uid: %s, model: %s, fields: %s', operation, this._uid, this._name, invalidFields.join(','));
        const description = await (await this.env.items('ir.model')._get(this._name)).label;
        ...
        throw new AccessError(_format(await this._t(`The requested operation can not be completed due to security restrictions.
          ...
          {
            'documentModel': this._name,
            'documentKind': description || this._name,
            'operation': operation,
            'user': this._uid,
  ...
  async copyTranslations(newObj, excluded = {}) {
    ...
    if (!('__copyTranslationsSeen' in old._context)) {
      old = await old.withContext({ __copyTranslationsSeen: new DefaultDict(() => new Set()) });
    }
    const seenMap = old._context['__copyTranslationsSeen'];
    if (seenMap[old._name].has(old.id)) {
      return;
    }
    seenMap[old._name].add(old.id);
  ...
  async _readGroupFillTemporal(data, groupby, aggregatedFields, annotatedGroupbys,
    ...
    const groupedData = new DefaultDict(() => []);
    for (const d of data) {
  ...
  async slug() {
    return tools.slug([this.id, await this.seoName || await this.displayName]);
  }

  @api.model()
  async nameSearch(options = {}) { // todo will change labelSearch
  ...
  async _addMissingDefaultValues(values) {
    ...
    for (const [name, value] of defaults) {
      const type = this._fields[name].type;
      if (type === 'many2many' && bool(value) && typeof value[0] === 'number') {
        // convert a list of ids into a list of commands
        defaults[name] = [Command.set(value)];
      }
      else if (type === 'one2many' && bool(value) && typeof value[0] === 'object' && !tools.isIterable(value[0])) {
        // convert a list of objects/dicts into a list of commands
  ...
  async _convertToWrite(values) {
    ...
    const result = {};
    for (let [name, value] of tools.getEntries(values)) {
      if (name in fields) {
      ...
      if (key === null) {
        ...
        if (!found) {
          ...
          if (newRecords.ok) {
            ...
            records.or(await cacheRecords.filtered(async (r) => intersection((await r[key.name])._ids, this._ids).length));
          }
        }
    ...
    for (const orderPart of orderSpec.split(',')) {
      ...
      if (orderField === 'id') {
        ...
        if (field.store && field.type === 'many2one') {
          ...
          console.warn("Model %s cannot be sorted on field %s (not a column)", this._name, orderField);
  ...
  async _generateM2oOrderBy(alias, orderField, query, reverseDirection, seen) {
    ...
    if (!field.store) {
      console.debug("Many2one function/related fields must be stored to be used as ordering fields! Ignoring sorting for %s.%s", this._name, orderField);
      return [];
    }
      ...
      for (const fname of arg[0].split('.')) {
        ...
        if (field.relatedField) {
          ...
          for (const f of field.related.split('.')) {
            ...
            if (rfield) {
              toFlush[model._name] = toFlush[model._name] ?? new Set();
              toFlush[model._name].add(f);
  ...
  repr() {
    return `${this._name}${this._ids.length == 0 ? '()' :
      this._ids.length == 1 ? '(' + this._ids[0] + ',)' : '(' + this._ids[0] + ',...)'}`;
  }
  ...
  *[Symbol.iterator]() {
  ...
  async _updateCache(values = {}, validate = true) {
    ...
    const fields = this._fields;
    const fieldValues = [];
    for (const [name, value] of tools.getEntries(values)) {
      if (name in fields) {
  ...
  async _checkRemovedColums(log = false) {
    ...
    for (const row of res) {
      if (log) {
        console.debug(`column ${row['attname']} is in the table ${this.cls._table} but not in the corresponding object ${this._name}`);
      }
      if (row['attnotnull']) {
  ...
  async _createParentColumns() {
    const cls = this.constructor;
    const cr = this._cr;
    await cr._obj.getQueryInterface().addColumn(cls._table, 'parentPath', 'string', { transaction: cr.objTransaction });
  ...
  async _addSqlConstraints() {
    ...
    for (const [key, definition, message] of cls._sqlConstraints) {
      ...
      if (conname.length > 63 && !currentDefinition) {
        console.info("Constraint name %s has more than 63 characters", conname);
      }
      if (currentDefinition && currentDefinition['definition'] === definition) {
        continue;
      }
