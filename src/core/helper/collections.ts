...
import { iterMultiItems } from './datastructures';
...
export class MultiDict<T = any> extends Function {
   ...
   * @param newList An iterable with the new values for the key.  Old values
                         are removed first.
   */
...
export class MapKey<K = any, V = any> extends Map<K, V> {
  ...
  private static _hash(item) {
    if (item != null && typeof item['_hash'] === 'function') {
      return item._hash();
    }
    else {
      return item;
...
export class DefaultMapKey<K, V> extends Map<K, any> {
...
class _Map2<K, V> extends Map<K, V> {
  get(key: K, value?: V): any {
...
/**
 * extends Set with MapKey
 */
export class OrderedSet extends Set {
  private _maps: MapKey<any, null>;

  constructor(elems: any[] = []) {
    super();
    this._maps = new MapKey()
    elems.map((e) => this._maps.setdefault(e, null));
  }
