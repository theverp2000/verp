...
export class Cursor extends BaseCursor {
  ...
  constructor(pool: DatabasePool, dbname: string, dsn: sequelize.ConnectionInfo, serialized?: boolean) {
    ...
    this.dbService = this._obj.dialect;
    this._query = this.dbService.queryInterface;
    this._gen = this.dbService.queryGenerator;
    if (this.sqlLog) {
...
export class Connection {
  ...
  get dsn() {
    const dsn = Object.assign({}, this._options);
    pop(dsn, 'password', undefined);
    return dsn;
  }
}
