...
/**
 * Like the function `partial` of the given arguments but vals will be inserted firstly.
 * @param func 
 * @param args 
 * @returns 
 */
export function partial2(func: Function, ...args: any[]): Function {
  if (!isCallable(func)) {
    throw new TypeError("the first argument must be callable");
  }
  return (...vals: any[]) => func(...vals, ...args);
}
...
export function isInstance(obj: any, ...classes: any[]) {
  ...
  return (obj instanceof classes[0]) || classes.slice(1).some((c) => obj instanceof c);
}
...
export function equal(a, b, m=0) {
  ...
  if (a && typeof a.eq === 'function') return a.eq(b); // ModelRecords or NewId
