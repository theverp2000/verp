...
import { DATE_LENGTH, setOptions } from "./misc";
...
export function getMonth(date: Date): [Date, Date] {
  ...
  const dateTo = new Date(year, month, DateTime.local(year, month + 1).daysInMonth);
...
function formatDuration(duration: Duration, opts: { granularity?: string, threshold?: number, addDirection?: boolean, format?: string, locale?: string } = {}) {
  setOptions(opts, { granularity: 'second', threshold: 0.85, addDirection: false, format: 'long', locale: SERVER_LOCALE });
...
export function toDate(value?: string | Date, JSDate: boolean = true): DateTime | Date | null {
  ...
  value = new Date(value.toISOString().slice(0, DATE_LENGTH));
  if (!JSDate) {
...
function modf(float: number) {
  return [Number((float % 1).toFixed(3).substring(0, 3)), Math.floor(float)];
}

/**
* Convert a number of hours into a time object [hours, minutes].
* @param hours 
*/
export function floatToTime(hours: number): [number, number] {
  const date = new Date();
  if (hours == 24.0) {
    setTimeMax(date);
  ...
  return [integral, tools.parseInt(floatRound(60 * fractional, { precisionDigits: 0 }))];
}
