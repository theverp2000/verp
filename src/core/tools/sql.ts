 ...
 * @returns bool - if field has been incremented or not 
 */
export async function incrementFieldSkiplock(record, field) {
  if (!bool(record)) {
    return false;
  }
  ...
  const query = _f(`
      UPDATE {table} SET {field} = {field} + 1 WHERE id IN (
          SELECT id from {table} WHERE id in ({ids}) FOR UPDATE SKIP LOCKED
      ) RETURNING id
  `, { table: `"${record.cls._table}"`, field: `"${field}"`, ids: String(record.ids) || 'NULL' });
  return bool(await cr.execute(query));
}