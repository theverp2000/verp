...
export class IterableGenerator {
  ...
  *[Symbol.iterator]() {
    for (const res of this.func(...this.args)) {
      yield res;
    }
  }
}
...
export function remove(list: any[], elem: any) {
  ...
  if (index > -1) {         // only splice array when item is found
    return list.splice(index, 1);  // 2nd parameter means remove one item only
  }
  return list;
}
...
export function sorted(list: Iterable<any>, func?: any, reverse = false) {
  if (typeof func === 'boolean') {
    reverse = func;
    func = undefined;
  }
  let comparableArray = Array.from(list);
  if (typeof func === 'function') {
    comparableArray = comparableArray.map(x => [func(x), x]);
    if (reverse) {
    ...
    if (reverse) {
      return comparableArray.sort((a, b) => +(a < b) || -(a > b));
    }
    else {
      return comparableArray.sort((a, b) => +(a > b) || -(a < b));
    }
