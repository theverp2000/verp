...
import { IDbService } from "./dialects/abstract";
...
export class DbFactory {
  static create(dialect: string): IDbService {
    ...
    switch (dialect) {
      case 'postgres':
        dbFactory = require('./dialects/postgres').DbPostgres;
        break;
      case 'mssql':
        dbFactory = require('./dialects/mssql').DbMssql;
        break;
      case 'mysql':
        dbFactory = require('./dialects/mysql').DbMysql;
        break;
      case 'mariadb':
        dbFactory = require('./dialects/mariadb').DbMaria;
        break;
      case 'sqlite':
        dbFactory = require('./dialects/sqlite').DbSqlite;
        break;
      case 'db2':
      case 'snowflake':
      default:
        throw new Error(`The dialect ${dialect} is not supported. Supported dialects: mssql, mariadb, mysql, postgres, db2 and sqlite.`);
    }

    return dbFactory.prototype;
  }
  ...
  */
  ...
  static async expDuplicateDatabase(req, dbOriginalName, dbName) {
    ...
    await cr.execute(`CREATE DATABASE %s ENCODING 'unicode' TEMPLATE %s"`,
      [cr._gen.quote(dbName), cr._gen.quote(dbOriginalName)]
    )
    await cr.close();
