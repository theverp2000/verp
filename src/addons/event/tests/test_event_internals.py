...
from verp.addons.event.tests.common import TestEventCommon
from verp import exceptions
from verp.fields import Datetime as FieldsDatetime, Date as FieldsDate
from verp.tests.common import users, Form
...
class TestEventData(TestEventCommon):
    ...
    def setUpClass(cls):
        ...
        cls.patcher = patch('verp.addons.event.models.event_event.fields.Datetime', wraps=FieldsDatetime)
        ...
        cls.event_0.write({
            'dateBegin': datetime(2020, 2, 1, 8, 30, 0),
            'dateEnd': datetime(2020, 2, 4, 18, 45, 0),
        })
    ...
    def test_event_date_computation(self):
        ...
        event.write({
            'registration_ids': [(0, 0, {'partnerId': self.event_customer.id, 'name': 'test_reg'})],
            'dateBegin': datetime(2020, 1, 31, 15, 0, 0),
            'dateEnd': datetime(2020, 4, 5, 18, 0, 0),
        })
        registration = event.registration_ids[0]
        self.assertEqual(registration.get_date_range_str(), u'today')

        event.dateBegin = datetime(2020, 2, 1, 15, 0, 0)
        self.assertEqual(registration.get_date_range_str(), u'tomorrow')

        event.dateBegin = datetime(2020, 2, 2, 6, 0, 0)
        self.assertEqual(registration.get_date_range_str(), u'in 2 days')

        event.dateBegin = datetime(2020, 2, 20, 17, 0, 0)
        self.assertEqual(registration.get_date_range_str(), u'next month')

        event.dateBegin = datetime(2020, 3, 1, 10, 0, 0)
        self.assertEqual(registration.get_date_range_str(), u'on Mar 1, 2020, 11:00:00 AM')

        # Is actually 8:30 to 20:00 in Mexico
        event.write({
            'dateBegin': datetime(2020, 1, 31, 14, 30, 0),
            'dateEnd': datetime(2020, 2, 1, 2, 0, 0),
            'dateTz': 'Mexico/General'
        })
        self.assertTrue(event.isOneDay)
    ...
    @users('user_eventmanager')
    @mute_logger('verp.models.unlink')
    def test_event_configuration_from_type(self):
        ...
        event = self.env['event.event'].create({
            ...
            'dateBegin': FieldsDatetime.to_string(datetime.today() + timedelta(days=1)),
            'dateEnd': FieldsDatetime.to_string(datetime.today() + timedelta(days=15)),
            'event_mail_ids': False,
        })
        self.assertEqual(event.dateTz, self.env.user.tz)
        self.assertFalse(event.seatsLimited)
        ...
        event_type.write({
            'event_type_mail_ids': [(5, 0), (0, 0, {
                ...
                'templateRef': 'mail.template,%i' % self.env['ir.model.data']._xmlid_to_res_id('event.event_reminder')})
            ],
            'event_type_ticket_ids': [(5, 0), (0, 0, {'name': 'TestRegistration'})],
        })
        event.write({'eventTypeId': event_type.id})
        self.assertEqual(event.dateTz, 'Europe/Paris')
        self.assertTrue(event.seatsLimited)
        self.assertEqual(event.seatsMax, event_type.seatsMax)
        ...
        self.assertEqual(event.event_mail_ids.templateRef, self.env.ref('event.event_reminder'))
        ...
        self.assertEqual(event_type.seatsMax, 0)
        self.assertTrue(event.seatsLimited)
        self.assertEqual(event.seatsMax, 30)  # original template value
        event.write({'eventTypeId': False})
        self.assertEqual(event.eventTypeId, self.env["event.type"])

        # set template back -> update event
        event.write({'eventTypeId': event_type.id})
        self.assertFalse(event.seatsLimited)
        self.assertEqual(event.seatsMax, 0)
        self.assertEqual(len(event.event_ticket_ids), 1)
    ...
    def test_event_configuration_mails_from_type(self):
        ...
        event = self.env['event.event'].create({
            ...
            'dateBegin': FieldsDatetime.to_string(datetime.today() + timedelta(days=1)),
            'dateEnd': FieldsDatetime.to_string(datetime.today() + timedelta(days=15)),
            'eventTypeId': event_type_default.id
        })
        event.write({
            'event_mail_ids': [
                Command.clear(),
                Command.create({
                    ...
                    'templateRef': 'mail.template,%i' % self.env['ir.model.data']._xmlid_to_res_id('event.event_subscription'),
                })
            ]
        })
        ...
        event_form.eventTypeId = event_type_mails
        ...
        event_form.eventTypeId = event_type_default
    ...
    def test_event_configuration_tickets_from_type(self):
        ...
        registration.write({'eventTicketId': ticket.id})
        # start test scenario
        event_form = Form(event)
        ...
        event_form.eventTypeId = event_type_tickets
    ...
    def test_event_mail_default_config(self):
        ...
        self.assertEqual(event.event_mail_ids[0].templateRef, self.env.ref('event.event_subscription'))
        ...
        self.assertEqual(event.event_mail_ids[1].templateRef, self.env.ref('event.event_reminder'))
        ...
        self.assertEqual(event.event_mail_ids[2].templateRef, self.env.ref('event.event_reminder'))
    ...
    def test_event_mail_filter_template_on_event(self):
        ...
        self.env['mail.template'].create({'modelId': self.env['ir.model']._get('event.registration').id, 'name': 'test template'})
        self.env['mail.template'].create({'modelId': self.env['ir.model']._get('res.partner').id, 'name': 'test template'})
        templates = self.env['mail.template'].withContext(filter_template_on_event=True).name_search('test template')
    ...
    def test_event_registrable(self):
        ...
        event = self.event_0.with_user(self.env.user)
        event.write({
            'dateBegin': datetime(2020, 1, 30, 8, 0, 0),
            'dateEnd': datetime(2020, 1, 31, 8, 0, 0),
        })
        self.assertFalse(event.event_registrations_open)
        event.write({
            'dateEnd': datetime(2020, 2, 4, 8, 0, 0),
        })
        ...
        event.write({
            'dateBegin': datetime(2020, 1, 28, 15, 0, 0),
            'dateEnd': datetime(2020, 1, 30, 15, 0, 0),
        })
        self.assertFalse(event.event_registrations_open)
        ...
        event.write({
            'dateEnd': datetime(2020, 2, 1, 15, 0, 0),
            'seatsMax': 1,
            'seatsLimited': True,
        })
    ...
    def test_event_ongoing(self):
        event_1 = self.env['event.event'].create({
            'name': 'Test Event 1',
            'dateBegin': datetime(2020, 1, 25, 8, 0, 0),
            'dateEnd': datetime(2020, 2, 1, 18, 0, 0),
        })
        ...
        event_1.update({'dateBegin': datetime(2020, 2, 1, 9, 0, 0)})
        ...
        event_2 = self.env['event.event'].create({
            'name': 'Test Event 2',
            'dateBegin': datetime(2020, 1, 25, 8, 0, 0),
            'dateEnd': datetime(2020, 1, 28, 8, 0, 0),
        })
        ...
        event_2.update({'dateEnd': datetime(2020, 2, 2, 8, 0, 1)})
    ...
    def test_event_seats(self):
        ...
        self.assertEqual(event.addressId, self.env.user.companyId.partnerId)
        # seats: coming from event type configuration
        self.assertTrue(event.seatsLimited)
        self.assertEqual(event.seats_available, event.eventTypeId.seatsMax)
        ...
        self.assertEqual(event.seats_available, event.eventTypeId.seatsMax - 6)
...
class TestEventRegistrationData(TestEventCommon):
    ...
    def test_registration_partner_sync(self):
        ...
        self.assertEqual(new_reg.partnerId, customer)
        ...
        new_reg.write({'partnerId': customer.id})
        self.assertEqual(new_reg.partnerId, customer)
        self.assertEqual(new_reg.name, 'Nibbler In Space')
        self.assertEqual(new_reg.email, customer.email)
        self.assertEqual(new_reg.phone, test_phone)
...
class TestEventTicketData(TestEventCommon):
    ...
    def setUp(self):
        ...
        self.ticket_date_patcher = patch('verp.addons.event.models.event_ticket.fields.Date', wraps=FieldsDate)
        ...
        self.ticket_datetime_patcher = patch('verp.addons.event.models.event_ticket.fields.Datetime', wraps=FieldsDatetime)
    ...
    def test_event_ticket_fields(self):
        ...
        self.assertTrue(first_ticket.seatsLimited)
        self.assertTrue(first_ticket.sale_available)
        self.assertFalse(first_ticket.is_expired)

        self.assertFalse(second_ticket.seatsLimited)
...
class TestEventTypeData(TestEventCommon):
    ...
    def test_event_type_fields(self):
        ...
        self.assertEqual(event_type.seatsMax, 30)
