...
class TestEventUI(TestEventCommon):
    ...
    def test_event_registration_partner_sync(self):
        """ Ensure onchange on partnerId is kept for interface, not for computed
        fields. """
        registration_form = Form(self.env['event.registration'].withContext(
            default_name='WrongName',
            default_event_id=self.event_0.id
        ))
        self.assertEqual(registration_form.eventId, self.event_0)
        ...
        # trigger onchange
        registration_form.partnerId = self.event_customer
        ...
        self.assertEqual(registration.partnerId, self.event_customer)
        ...
        registration.write({'partnerId': False})
        self.assertEqual(registration.partnerId, self.env['res.partner'])
        self.assertEqual(registration.name, self.event_customer.name)
        self.assertEqual(registration.email, self.event_customer.email)
        self.assertFalse(registration.phone)
        self.assertFalse(registration.mobile)
        ...
        registration.write({'partnerId': self.event_customer2.id})
        self.assertEqual(registration.partnerId, self.event_customer2)
        self.assertEqual(registration.name, self.event_customer.name)
        self.assertEqual(registration.email, self.event_customer.email)
...
class TestEventFlow(TestEventCommon):
    ...
    def test_event_auto_confirm(self):
        ...
        test_event = self.env['event.event'].with_user(self.user_eventmanager).create({
            'name': 'TestEvent',
            'auto_confirm': True,
            'dateBegin': datetime.datetime.now() + relativedelta(days=-1),
            'dateEnd': datetime.datetime.now() + relativedelta(days=1),
            'seatsMax': 2,
            'seatsLimited': True,
        })
