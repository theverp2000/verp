...
from verp.tools import formataddr, mute_logger


class TestMailSchedule(TestEventCommon, MockEmail):

    @mute_logger('verp.addons.base.models.ir_model', 'verp.models')
    def test_event_mail_schedule(self):
        ...
        with freeze_time(now):
            ...
            test_event = self.env['event.event'].create({
                'name': 'TestEventMail',
                'create_date': now,
                'userId': self.user_eventmanager.id,
                'auto_confirm': True,
                'dateBegin': event_date_begin,
                'dateEnd': event_date_end,
                'event_mail_ids': [
        ...
        after_sub_scheduler = self.env['event.mail'].search([('eventId', '=', test_event.id), ('interval_type', '=', 'after_sub'), ('interval_unit', '=', 'now')])
        ...
        after_sub_scheduler_2 = self.env['event.mail'].search([('eventId', '=', test_event.id), ('interval_type', '=', 'after_sub'), ('interval_unit', '=', 'hours')])
        ...
        event_prev_scheduler = self.env['event.mail'].search([('eventId', '=', test_event.id), ('interval_type', '=', 'before_event')])
        ...
        event_next_scheduler = self.env['event.mail'].search([('eventId', '=', test_event.id), ('interval_type', '=', 'after_event')])
        ...
        self.assertMailMailWEmails(
            ...
            fields_values={'subject': 'Your registration at %s' % test_event.name,
                           'emailFrom': self.user_eventmanager.companyId.emailFormatted,
                          })
        ...
        for mail in self._new_mails:
            self.assertEqual(mail.emailFrom, self.user_eventmanager.companyId.emailFormatted)
            self.assertEqual(mail.subject, 'Your registration at %s' % test_event.name)
            self.assertEqual(mail.state, 'outgoing')
            self.assertEqual(mail.emailTo, formataddr((reg3.name, reg3.email)))
    ...
    def test_archived_event_mail_schedule(self):
        ...
        scheduler = self.env['event.mail'].search([('eventId', '=', test_event.id)])
