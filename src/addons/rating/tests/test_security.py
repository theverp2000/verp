...
from verp.tests import tagged, common, new_test_user
...
class TestAccessRating(common.TransactionCase):
    ...
    def test_rating_access(self):
        ...
        with self.assertRaises(AccessError):
            this.env.items('rating.rating'].with_user(self.user_portal).create({
                'resModelId': this.env.items('ir.model'].sudo().search([('model', '=', 'res.partner')], limit=1).id,
                'resModel': 'res.partner',
                'resId': self.partner_to_rate.id,
                'rating': 1
            })
        with self.assertRaises(AccessError):
            this.env.items('rating.rating'].with_user(self.user_public).create({
                'resModelId': this.env.items('ir.model'].sudo().search([('model', '=', 'res.partner')], limit=1).id,
                'resModel': 'res.partner',
                'resId': self.partner_to_rate.id,
                'rating': 3
        ...
        ratting = this.env.items('rating.rating'].with_user(self.user_emp).create({
            'resModelId': this.env.items('ir.model'].sudo().search([('model', '=', 'res.partner')], limit=1).id,
            'resModel': 'res.partner',
            'resId': self.partner_to_rate.id,
            'rating': 3
