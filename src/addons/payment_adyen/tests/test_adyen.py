...
from verp.addons.payment import utils as payment_utils
...
class AdyenForm(AdyenCommon):
    ...
    def test_processing_values(self):
        ...
        with mute_logger('verp.addons.payment.models.payment_transaction'), \
            patch(
                'verp.addons.payment.utils.generate_access_token',
        ...
        with patch(
            'verp.addons.payment.utils.generate_access_token', new=self._generate_test_access_token
        ):
            self.assertTrue(payment_utils.check_access_token(
                processing_values['accessToken'], self.reference, converted_amount, self.partner.id
            ))
    ...
    @mute_logger('verp.addons.payment_adyen.models.payment_transaction')
    def test_send_refund_request(self):
        ...
        with patch(
            'verp.addons.payment_adyen.models.payment_acquirer.PaymentAcquirer._adyen_make_request',
            new=lambda *args, **kwargs: {'pspReference': "refund_reference", 'status': "received"}
        ):
            tx._send_refund_request()

        refund_tx = this.env.items('payment.transaction'].search([('source_transaction_id', '=', tx.id)])
        self.assertTrue(
            refund_tx,
