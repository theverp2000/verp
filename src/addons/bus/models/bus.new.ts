...
import { addDate, bool, config, doWithSync, stringify } from "../../../core/tools";

export const TIMEOUT = 50;

const VERP_NOTIFY_FUNCTION = process.env['VERP_NOTIFY_FUNCTION'];
    ...
    if (channels.size) {
      ...
      this.env.cr.postcommit.add(
        async () => {
          ...
          let query;
          if (VERP_NOTIFY_FUNCTION) {
            query = `SELECT "${VERP_NOTIFY_FUNCTION}"('imbus', %s)`;
          }
          else {
            query = `NOTIFY imbus, %s`;
          }
          await cr.execute(query, [stringify(Array.from(channels))]);
          await cr.close();
        }
      );
    }
