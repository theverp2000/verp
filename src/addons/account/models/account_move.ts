...
class AccountMove extends Model {
   ...
   * Get correct accounting date for previous periods, taking tax lock date into account.
    ...
    @param hasTax (bool) Iff any taxes are involved in the lines of the invoice
    @returns (Date)
   */
   ...
   * @param hasTax If any taxes are involved in the lines of the invoice
   * @returns a list of tuples containing the lock dates affecting this move, ordered chronologically.
   */
  ...
  async _onchangePartnerId() {
    ...
    if (partnerId.ok) {
      ...
      if (!bool(recAccount) && !bool(payAccount)) {
        ...
        const msg = await self._t('Cannot find a chart of accounts for this company, You should configure it. \nPlease go to Account Configuration.');
        throw new RedirectWarning(msg, action.id, await self._t('Go to the configuration panel'));
      }
      ...
      if (invoiceWarn && invoiceWarn !== 'no-message') {
        ...
        warning = {
          'title': await self._t("Warning for %s", await partner.label),
  ...
  /**
    * This method is triggered by the tax group widget. It allows modifying the right move lines depending on the tax group whose amount got edited.
    */
  ...
  /**
    ...
    * @returns A dictionary containing all fields on which the tax will be grouped.
    */
  @api.model()
  ...
  async _recomputeTaxLines(recomputeTaxBaseAmount: boolean = false, taxRepLinesToRecompute?: any[]) {
     ...
     * Compute taxes amounts both in company currency / foreign currency as the ratio between
        ...
        @returns The result of the computeAll method.
      */
  ...
  async _recomputeCashRoundingLines() {
    this.ensureOne();
    const self = this;
    let inDraftMode = !self.eq(self._origin);
    const [lineIds, invoiceCashRoundingId, partnerId, currencyId, companyId, date] = await self('lineIds', 'invoiceCashRoundingId', 'partnerId', 'currencyId', 'companyId', 'date');
    /**
    ...
    async function _applyCashRounding(diffBalance, diffAmountCurrency, cashRoundingLine) {
      ...
      if (strategy === 'biggestTax') {
        ...
        update(roundingLineVals, {
          'label': await self._t('%s (rounding)', label),
          'accountId': accountId.id,
          'taxRepartitionLineId': taxRepartitionLineId.id,
          'excludeFromInvoiceTab': true,
        })
    ...
    if (!invoiceCashRoundingId.ok) {
      await self.set('lineIds', lineIds.sub(existingCashRoundingLine));
      return;
    }
    ...
    if (invoiceCashRoundingId.ok && existingCashRoundingLine.ok) {
      ...
      if (strategy !== oldStrategy) {
        await self.set('lineIds', lineIds.sub(existingCashRoundingLine));
        existingCashRoundingLine = self.env.items('account.move.line');
      }
    }
   ...
   * Recompute all lines that depend of others.
    ...
    * @param recomputeAllTaxes Force the computation of taxes. If set to false, the computation will be done
                                  or not depending of the field 'recomputeTaxLine' in lines.
    * @param recomputeTaxBaseAmount 
    */
    ...
    for (const invoice of this) {
      ...
      for (const line of await invoice.lineIds) {
        ...
        if (taxRepartitionLineId.ok) {
          currentTaxRepLines.push(taxRepartitionLineId._origin);
        }
        else if (taxIds.ok) {
          hasTaxes = true;
          let isRefund;
          ...
          for (const taxRepLine of taxRepLines) {
            expectedTaxRepLines.push(taxRepLine);
          }
        }
      }

      const deltaTaxRepLines = _.difference(expectedTaxRepLines, currentTaxRepLines);
  ...
  async _computeName() {
    ...
    const grouped = new DefaultDict(  // key: journalId, moveType
      () => new DefaultDict(  // key: first adjacent (date.year, date.month)
        () => {
          return {
   ...
   * @returns A list of dict in the format described in _getTaxTotals's taxLinesData's docstring.
   */
   ...
   * @param objectLines A list of records corresponding to the sub-objects generating the tax totals
                            (sale.order.line or purchase.order.line, for example)
   * @param taxResultsFunction A function to be called to get the results of the tax computation for a
   ...
   * @returns A list of dict in the format described in _get_tax_totals's tax_lines_data's docstring.
   */
  @api.model()
  ...
  async _getTaxTotals(partner, taxLinesData, amountTotal, amountUntaxed, currency) {
    ...
    const groupedTaxes = new DefaultDict(() => new MapKey<any, any>());
    ...
    for (const lineData of taxLinesData) {
      ...
      // Update tax data
      const taxGroupVals = groupedTaxes[subtotalTitle].setdefault(taxGroup, { 'baseAmount': 0.0, 'taxAmount': 0.0, 'baseLineKeys': new Set() });
  ...
  async _moveAutocompleteInvoiceLinesValues() {
    this.ensureOne();
    const invoiceLines = await (await this['lineIds']).filtered(async (x) => ! await x.excludeFromInvoiceTab && ! await x.displayType && !bool(x._origin));
    const cachedValsList: any[] = await invoiceLines.map(x => Dict.from(getEntries(x._cache)));

    for (const [cachedVals, line] of _.zip(cachedValsList, [...invoiceLines])) {
      ...
      if (!bool(line._cache.get('accountId', false)) && !bool(line._origin)) {
        const account = await line._getComputedAccount();
        await line.set('accountId', bool(account) ? account : await (await this['journalId']).defaultAccountId);
      }
      if (bool(await line.productId) && !line._cache.get('label', false)) {
      ...
      const commercialPartnerId = await (await this['partnerId']).commercialPartnerId
      if (!(await line.partnerId).eq(commercialPartnerId)) {
        await line.set('partnerId', commercialPartnerId);
      }
      await line.set('date', (await this['date']));
      await line.set('recomputeTaxLine', true);
      await line.set('currencyId', (await this['currencyId']));

      for (const [k, v] of cachedVals.items()) {
        const field = line._fields[k];
    ...
    await (await this['lineIds'])._onchangePriceSubtotal();
   ...
   * @param valsList The list of values passed to the 'create' method.
   * @returns Modified list of values.
   */
  @api.model()
  ...
  async write(vals) {
    for (const move of this) {
      ...
      if (restrictModeHashTable && state === "posted" && _.intersection(Object.keys(vals), move._getIntegrityHashFields()).length) {
        throw new UserError(await this._t("You cannot edit the following fields due to restrict mode being activated on the journal: %s.", move._getIntegrityHashFields().join(', ')));
      }
      ...
      if (state == "posted" && (
              ('label' in vals && label != vals['label'])
              || ('date' in vals && date != vals['date'])
      )) {
    ...
    if ('date' in vals || 'state' in vals) {
      const postedMove = await this.filtered(async (m) => await m.state === 'posted');
      await postedMove._checkFiscalyearLockDate();
      await (await postedMove.lineIds)._checkTaxLockDate();
    }
   ...
   * Moves with a sequence number can only be deleted if they are the last element of a chain of sequence.
      If they are not, deleting them would create a gap. If the user really wants to do this, he still can explicitly empty the 'label' field of the move; but we discourage that practice.
   */
  @api.ondelete(false)
   ...
   * @returns A res.partner record's id representing the delivery address.
   */
   ...
   * @returns A list of tuple [partial, amount, invoiceLine].
   */
   ...
   * @param defaultValues A copyDate of the original journal entry.
   * @param cancel A flag indicating the reverse is made to cancel the original journal entry.
   * @returns The updated defaultValues.
   */
  async _reverseMoveVals(defaultValues, cancel: boolean = true) {
     ...
     * @param moveVals The newly created invoice as a dictionary to be passed to the 'create' method.
     * @returns  A map invoiceRepartitionLine => refundRepartitionLine.
     */
    async function computeTaxRepartitionLinesMapping(moveVals) {
      // invoiceRepartitionLine => refundRepartitionLine
   ...
   * @param defaultValuesList A list of default values to consider per move. 
                              ('type' & 'reversedEntryId' are computed in the method).
   * @param cancel 
   * @returns An account.move recordset, reverse of the current this.
   */
  async _reverseMoves(defaultValuesList?: any, cancel: boolean = false) {
    ...
    for (const [move, reverseMove] of _.zip([...this], [...await reverseMoves.withContext({ checkMoveValidity: false })])) {
      // Update amountCurrency if the date has changed.
   ...
   * @returns true if the invoice is ready, false otherwise.
   */
  _isReadyToBeSent() {
    this.ensureOne();
    return true;
   ...
   * @returns Model<account.move> the documents that have been posted
   */
  async _post(soft = true) {
    ...
    // `userHasGroup` won't be bypassed by `sudo()` since it doesn't change the user anymore.
    ...
    for (const move of toPost) {
      ...
      // /!\ 'checkMoveValidity' must be there since the dynamic lines will be recomputed outside the 'onchange'
      // environment.
    ...
    for (const move of toPost) {
      ...
      // [p.id for p in [move.partnerId] if p not in move.sudo().messagePartnerIds])
   ...
   * @returns true if the move should get a auto-computed reference else false
   */
  ...
  async actionPost() {
    const movesWithPayments = await this.filtered('paymentId');
    const otherMoves = this.sub(movesWithPayments);
    if (movesWithPayments.ok) {
      await (await movesWithPayments.paymentId).actionPost();
    }
    if (otherMoves.ok) {
      await otherMoves._post(false);
    }
    return false;
  }
   ...
   * @param partialId The id of an existing partial reconciled with the current invoice.
   * @returns 
   */
  ...
  /**
   * @returns the correct mail template based on the current move type
   */
   ...
   * @returns An action opening the account.payment.register wizard.
   */
