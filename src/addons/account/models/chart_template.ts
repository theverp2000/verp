...
async function updateTaxesFromTemplates(cr: Cursor, chartTemplateXmlid: string) {
  ...
  const newTaxTemplateByCompany = new DefaultDict(() => env.items('account.tax.template'))  // only contains completely new taxes (not previous taxe had the xmlid)
...
class AccountChartTemplate extends Model {
  ...
  async tryLoading(opts: {req?: any, company?: any, installDemo?: boolean}={}) {
    ...
    if (!(await company.chartTemplateId).ok && ! await this.existingAccounting(company)) {
      ...
      if (installDemo && await (await this.env.ref('base.module_account')).demo) {
  ...
  async _createDemoData() {
    try {
      await this.env.cr.savepoint(async () => {
        const vals = (this as any)._getDemoData();
        for await (const [model, data] of vals) {
