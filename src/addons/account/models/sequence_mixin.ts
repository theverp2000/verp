...
class SequenceMixin extends AbstractModel {
  ...
  async _constrainsDateSequence() {
    ...
    for (const record of this) {
      ...
      if (bool(sequence) && date && date > constraintDate) {
        const formatValues = (await record._getSequenceFormatParam(sequence))[1];
        if (
          ...
          throw new ValidationError(_f(await this._t(
            ...
            "\tIn doing so, you might want to resequence your entries in order to maintain a continuous date-based sequence."].join('\n')), {
   ...
   * @param label the sequence that is used as a reference to detect the resetting
   ...
   * @param relaxed see _getLastSequence.
   * @returns tuple(whereString, whereParams) with
              whereString: the entire SQL WHERE clause as a string.
              whereParams: a dictionary containing the parameters to substitute
   ...
   * @returns string to use as the default sequence to increment
   */
   ...
   * Retrieve the previous sequence.
      ...
      _getLastSequenceDomain is [], i.e: a new year).
   ...
   * @param withPrefix The sequence prefix to restrict the search on, if any.
   * @returns the string of the previous sequence or None if there wasn't any.
   */
   ...
   * Get the format and format values for the sequence.
    ...
    @return [format, formatValues]:
        ...
        formatValues is the dict of values to format the `format` string
   ...
   * @returns true if it is the last element of the chain.
   */
  ...
  async _isEndOfSeqChain() {
    ...
    for (const values of batched.values()) {
      ...
      // lastRec must have the highest number in the database
