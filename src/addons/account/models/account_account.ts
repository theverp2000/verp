...
class AccountAccount extends Model {
  ...
  async _constrainsAllowedJournalIds() {
    ...
    const ids = await this._cr.execute(`
            SELECT aml.id
            FROM ${this._cr._gen.quote("accountMoveLine")} aml
            WHERE ${this._cr._gen.quote("aml.accountId")} in (%s)
            AND EXISTS (SELECT 1 FROM ${this._cr._gen.quote("accountAccountAccountJournalRel")} WHERE ${this._cr._gen.quote("accountAccountId")} = ${this._cr._gen.quote("aml.accountId")})
            AND NOT EXISTS (SELECT 1 FROM ${this._cr._gen.quote("accountAccountAccountJournalRel")} WHERE ${this._cr._gen.quote("accountAccountId")} = ${this._cr._gen.quote("aml.accountId")} AND ${this._cr._gen.quote("accountJournalId")} = ${this._cr._gen.quote("aml.journalId")})
