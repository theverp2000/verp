...
class AccountJournal extends Model {
  ...
  async _computeSuspenseAccountId() {
    for (const journal of this) {
      ...
      const accountJournalSuspenseAccount = await companyId.accountJournalSuspenseAccountId;
      let _suspenseAccountId;
      if (!['bank', 'cash'].includes(type)) {
        _suspenseAccountId = false;
      }
      else if (bool(suspenseAccountId)) {
        _suspenseAccountId = suspenseAccountId;
      }
      else if (bool(accountJournalSuspenseAccount)) {
        _suspenseAccountId = accountJournalSuspenseAccount;
      }
      else {
        _suspenseAccountId = false;
      }
  ...
  async getNextBankCashDefaultCode(journalType, company) {
    ...
    for (const num of range(1, 100)) {
      // journalCode has a maximal size of 5, hence we can enforce the boundary num < 100
