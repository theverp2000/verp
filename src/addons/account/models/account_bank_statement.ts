...
class AccountBankStatement extends Model {
  ...
  async write(values) {
    ...
    if (values['date'] || values['journal']) {
      // If we are changing the date or journal of a bank statement, we have to change its previousStatementId. This is done
      // automatically using the compute function, but we also have to change the previousStatementId of records that were
      // previously pointing toward us and records that were pointing towards our new previousStatementId. This is done here
      // by marking those record as needing to be recomputed.
      // Note that marking the field is not enough as we also have to recompute all its other fields that are depending on 'previousStatementId'
  ...
  async create(values) {
    ...
    // In that case, we have to search for statement that are pointing to the same previousStatementId as ourselve in order to
    // change their previousStatementId to us. This is done by marking the field 'previousStatementId' to be recomputed for such records.
    // Note that marking the field is not enough as we also have to recompute all its other fields that are depending on 'previousStatementId'
    // hence the need to call modified afterwards.
  ...
  async unlink() {
    for (const statement of this) {
      ...
      // Some other bank statements might be link to this one, so in that case we have to switch the previousStatementId
...
class AccountBankStatementLine extends Model {
   ...
   * Helper used to dispatch the journal items between:
    ...
    * @returns [liquidityLines, suspenseLines, otherLines]
   */
  async _seekForLines() {
    const env = this.env;
    ...
    const [journal, move] = await this('journalId', 'moveId');
    const [defaultAccount, suspenseAccount] = await journal('defaultAccountId', 'suspenseAccountId');
    for (const line of await move.lineIds) {
      const account = await line.accountId;
      if (account.eq(defaultAccount)) {
        liquidityLines = liquidityLines.add(line);
      }
      else if (account.eq(suspenseAccount)) {
  ...
  /**
   * Prepare values to create a new account.move.line moveLine.
      By default, without specified 'counterpart_vals' or 'moveLine', the counterpart line is
      ...
      and 'moveLine' will be used to create the counterpart of an existing account.move.line to which
      ...
      @param counterpartVals:    A dictionary containing:
          'balance':                  Optional amount to consider during the reconciliation. If a foreign currency is set on the
                                      ...
                                      This value must be provided if moveLine is not.
          'amountResidual':          The residual amount to reconcile expressed in the company's currency.
                                      /!\ This value should be equivalent to moveLine.amount_residual except we want
          ...
          'amountResidualCurrency': The residual amount to reconcile expressed in the foreign's currency.
                                      ...
                                      /!\ This value should be equivalent to moveLine.amount_residual_currency except
      ...
      @param moveLine:           An optional account.move.line moveLine representing the counterpart line to reconcile.
      @return:                    The values to create a new account.move.line moveLine.
  ...
  async _prepareCounterpartMoveLineVals(counterpartVals, moveLine?: any) {
    ...
    const [statement, amount, date, foreignCurrencyId, moveId, partnerId] = await this('statementId', 'amount', 'date', 'foreignCurrencyId', 'moveId', 'partnerId');
    const journal = await statement.journalId;
    const companyCurrency = await (await journal.companyId).currencyId;
    let [journalCurrency, journalCompany] = await journal('currencyId', 'companyId');
    journalCurrency = journalCurrency.ok ? journalCurrency : companyCurrency;
    ...
    return {
      ...counterpartVals,
      'label': counterpartVals['label'] ?? (bool(moveLine) ? await moveLine.label : ''),
      'moveId': moveId.id,
      'partnerId': bool(partnerId.id)
        ? partnerId.id
        : counterpartVals['partnerId'] ?? (bool(moveLine) ? (await moveLine.partnerId).id : false),
      'currencyId': currencyId,
      'accountId': counterpartVals['accountId'] ?? (bool(moveLine) ? (await moveLine.accountId).id : false),
  ...
  async _prepareMoveLineDefaultVals(counterpartAccountId?: any) {
    ...
    const counterpartVals = {
      'label': await this['paymentRef'],
      'accountId': counterpartAccountId,
      'amountResidual': liquidityLineVals['debit'] - liquidityLineVals['credit'],
    }

    const foreignCurrency = await this['foreignCurrencyId'];
    if (bool(foreignCurrency) && !foreignCurrency.eq(await this['companyCurrencyId'])) {
  ...
  async create(valsList) {
    ...
    for (const vals of valsList) {
      const statement = this.env.items('account.bank.statement').browse(vals['statementId']);
    ...
    for (const [i, stLine] of enumerate(stLines)) {
      const move = await stLine.moveId;
      ...
      if (!('lineIds' in valsList[i])) {
        toWrite['lineIds'] = (await stLine._prepareMoveLineDefaultVals(counterpartAccountId)).map(lineVals => [0, 0, lineVals]);
      }
      await move.write(toWrite);

      // Otherwise field narration will be recomputed silently (at next flush) when writing on partnerId
      this.env.removeToCompute(move._fields['narration'], move);
    }
    return stLines;
  }
  ...
  async _synchronizeFromMoves(changedFields) {
    ...
    for (const stLine of await this.withContext({ skipAccountMoveSynchronization: true })) {
      ...
      if (changedFields.includes('lineIds')) {
        const [liquidityLines, suspenseLines, otherLines] = await stLine._seekForLines();
        const journal = await stLine.journalId;
        const companyCurrency = await (await journal.companyId).currencyId;
        ...
        if (len(liquidityLines) != 1) {
          throw new UserError(await this._t(['',
            ...
            'To be consistent, the journal entry must always have exactly one journal item involving the bank/cash account.'].join('\n'), await (await stLine.moveId).displayName));
        }
