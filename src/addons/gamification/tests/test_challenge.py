...
from verp.addons.base.tests.common import TransactionCaseWithUserDemo
...
class TestGamificationCommon(TransactionCaseWithUserDemo):
    ...
    def setUp(self):
        ...
        employees_group = self.env.ref('base.groupUser')
        ...
        self.robot = self.env['res.users'].withContext(no_reset_password=True).create({
            'name': 'R2D2',
            'login': 'r2d2@openerp.com',
            'email': 'r2d2@openerp.com',
            'groupsId': [(6, 0, [employees_group.id])]
        })
...
class test_challenge(TestGamificationCommon):
    ...
    def test_10_reach_challenge(self):
        ...
        goal_ids = Goals.search([('userId', '=', demo.id), ('definition_id', '=', self.env.ref('gamification.definition_base_timezone').id)])
        ...
        badge_ids = self.env['gamification.badge.user'].search([('badge_id', '=', badge_id), ('userId', '=', demo.id)])
    ...
    def test_20_update_all_goals_filter(self):
        ...
        (
        ...
        ) = all_test_users = self.env['res.users'].create([
            {
                ...
                'login': f'{kind}_{age}',
                'email': f'{kind}_{age}',
                'groupsId': [(6, 0, groupsId)],
            }
            for kind, groupsId in (
                ('Portal', []),
                ('Internal', [self.env.ref('base.groupUser').id]),
            )
        ...
        self.env['res.users.log'].search([('createdUid', 'in', challenge.user_ids.ids)]).unlink()
        now = datetime.datetime.now()

        # Create "old" log in records
        self.env['res.users.log'].create([
            {"createdUid": internal_login_before_update.id, 'createdAt': now - datetime.timedelta(minutes=3)},
            {"createdUid": portal_login_before_update.id, 'createdAt': now - datetime.timedelta(minutes=3)},
        ])
        ...
        goal_ids = self.env['gamification.goal'].search(
            [('challenge_id', '=', challenge.id), ('state', '!=', 'draft'), ('userId', 'in', challenge.user_ids.ids)]
        )
        ...
        self.env['res.users.log'].create([
            {"createdUid": internal_login_after_update.id, 'createdAt': now + datetime.timedelta(minutes=3)},
            {"createdUid": portal_login_after_update.id, 'createdAt': now + datetime.timedelta(minutes=3)},
        ])
