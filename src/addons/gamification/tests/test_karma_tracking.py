...
from verp import exceptions, fields
...
class TestKarmaTrackingCommon(common.TransactionCase):
    ...
    def test_computation_gain(self):
        ...
        self.assertEqual(results[0]['userId'], self.test_user_2.id)
        self.assertEqual(results[0]['karma_gain_total'], 200)
        self.assertEqual(results[0]['karma_position'], 1)
        self.assertEqual(results[1]['userId'], self.test_user.id)
        ...
        self.assertEqual(results[0]['userId'], self.test_user.id)
        self.assertEqual(results[0]['karma_gain_total'], 20)
        self.assertEqual(results[0]['karma_position'], 1)
        self.assertEqual(results[1]['userId'], self.test_user_2.id)
    ...
    def test_consolidation_cron(self):
        self.patcher = patch('verp.addons.gamification.models.gamification_karma_tracking.fields.Date', wraps=fields.Date)
        ...
        consolidated = self.env['gamification.karma.tracking'].search([
            ('userId', 'in', (self.test_user | self.test_user_2).ids),
            ('consolidated', '=', True),
    ...
    def test_user_as_erp_manager(self):
        ...
        trackings = self.env['gamification.karma.tracking'].sudo().search([('userId', '=', user.id)])
...
class TestComputeRankCommon(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        patch_email = patch('verp.addons.mail.models.mail_template.MailTemplate.send_mail', _patched_send_mail)
        patch_email.start()
    ...
    def test_02_update_rank_without_switch(self):
        ...
        patch_bulk = patch('verp.addons.gamification.models.res_users.Users._recompute_rank', _patched_recompute_rank)
        patch_bulk.start()
    ...
    def test_03_test_bulk_call(self):
        ...
        patch_bulk = patch('verp.addons.gamification.models.res_users.Users._recompute_rank_bulk', _patched_check_in_bulk)
        patch_bulk.start()
