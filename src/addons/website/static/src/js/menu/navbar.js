/** @verp-module alias=website.navbar */
...
var WebsiteNavbar = publicWidget.RootWidget.extend({
    ...
    events: _.extend({}, publicWidget.RootWidget.prototype.events || {}, {
        ...
        'click .o-mobile-menu-toggle': '_onMobileMenuToggleClick',
        'mouseenter #oeApplications:not(:has(.dropdown-item))': '_onOeApplicationsHovered',
        'show.bs.dropdown #oeApplications:not(:has(.dropdown-item))': '_onOeApplicationsShow',
    }),
    customEvents: _.extend({}, publicWidget.RootWidget.prototype.customEvents || {}, {
        'actionDemand': '_onActionDemand',
        'editMode': '_onEditMode',
        'readonlyMode': '_onReadonlyMode',
        'readyToSave': '_onSave',
    }),
    ...
    start: function () {
        var self = this;
        initAutoMoreMenu(this.el.querySelector('.o-menu-sections'), {
            maxWidth: function () {
                ...
                return self.$el.width()
                    - (self.$('.o-menu-systray').outerWidth(true) || 0)
                    - (self.$('#oeApplications').outerWidth(true) || 0)
                    - (self.$('.o-menu-toggle').outerWidth(true) || 0)
                    - (self.$('.o-menu-brand').outerWidth(true) || 0);
            },
        });
    ...
    _getRegistry: function () {
        return registry.category("websiteNavbarWidgets");
    },
    /**
    ...
    _onMobileMenuToggleClick: function () {
        this.$el.parent().toggleClass('o-mobile-menu-opened');
    },
    /**
     * Called in response to edit mode activation -> hides the navbar.
     *
     ...
     */
    _onSave: function (ev) {
        ev.data.defs.push(this._handleAction('onSave'));
    },
});
...
registry.category("publicRootWidgets").add("WebsiteNavbar", {
    Widget: WebsiteNavbar,
    selector: '#oeMainMenuNavbar',
});
