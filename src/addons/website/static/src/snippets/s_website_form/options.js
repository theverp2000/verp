verp.define('website.formEditor', function (require) {
'use strict';

const core = require('web.core');
const FormEditorRegistry = require('website.formEditorRegistry');
...
const FormEditor = options.Class.extend({
    ...
    _getCustomField: function (type, name) {
        return {
            ...
            records: [{
                id: _t('Option 1'),
                displayName: _t('Option 1'),
            }, {
                id: _t('Option 2'),
                displayName: _t('Option 2'),
            }, {
                id: _t('Option 3'),
                displayName: _t('Option 3'),
            }],
        };
    },
    /**
    ...
    _getDefaultFormat: function () {
        return {
            labelWidth: this.$target[0].querySelector('.s-website-form-label').style.width,
            labelPosition: 'left',
    ...
    _isOptionalMark: function () {
        return this.$target[0].classList.contains('o-mark-optional');
    },
    /**
     * @private
     * @returns {boolean}
    ...
    _isRequiredMark: function () {
        return this.$target[0].classList.contains('o-mark-required');
    },
    /**
     * @private
     * @param {Object} field
    ...
    _renderField: function (field, resetId = false) {
        ...
        template.innerHTML = qweb.render("website.formField" + _.upperFirst(field.type), {field: field}).trim();
        if (field.description && field.description !== true) {
            $(template.content.querySelector('.s-website-form-field-description')).replaceWith(field.description);
        }
...
const FieldEditor = FormEditor.extend({
    ...
    _getActiveField: function (noRecords) {
        let field;
        const labelText = this.$target.find('.s-website-form-label-content').text();
    ...
    _getFieldFormat: function () {
        ...
        const mark = this.$target[0].querySelector('.s-website-form-mark');
        if (mark) {
    ...
    _getFieldName: function () {
        const multipleName = this.$target[0].querySelector('.s-website-form-multiple');
        return multipleName ? multipleName.dataset.name : this.$target[0].querySelector('.s-website-form-input').name;
    },
    /**
    ...
    _getLabelPosition: function () {
        const label = this.$target[0].querySelector('.s-website-form-label');
        if (this.$target[0].querySelector('.row:not(.s-website-form-multiple)')) {
    ...
    _getMultipleInputs: function () {
        return this.$target[0].querySelector('.s-website-form-multiple');
    },
    /**
    ...
    _isFieldCustom: function () {
        return !!this.$target[0].classList.contains('s-website-form-custom');
    },
    /**
    ...
    _isFieldRequired: function () {
        ...
        return classList.contains('s-website-form-required') || classList.contains('s-website-form-model-required');
    },
    /**
    ...
    _setActiveProperties(field) {
        ...
        const description = this.$target[0].querySelector('.s-website-form-field-description');
        ...
        field.required = classList.contains('s-website-form-required');
        field.modelRequired = classList.contains('s-website-form-model-required');
        field.hidden = classList.contains('s-website-form-field-hidden');
...
options.registry.WebsiteFormEditor = FormEditor.extend({
    ...
    willStart: async function () {
        ...
        // Get list of websiteForm compatible models.
        ...
        const targetModelName = this.$target[0].dataset.modelName || 'mail.mail';
        ...
        this.models.forEach(el => {
            ...
            option.textContent = el.websiteFormLabel;
    ...
    start: function () {
        ...
        this.$target.find('.s-website-form-send, .s-website-form-field-description, .s-website-form-recaptcha').attr('contentEditable', true);
        // Get potential message
        this.$message = this.$target.parent().find('.s-website-form-end-message');
        this.showEndMessage = false;
        // If the form has no model it means a new snippet has been dropped.
        // Apply the default model selected in willStart on it.
        if (!this.$target[0].dataset.modelName) {
    ...
    cleanForSave: function () {
        const model = this.$target[0].dataset.modelName;
        ...
        if (model) {
            ...
            // really matter.
            const fields = [...this.$target[0].querySelectorAll('.s-website-form-field:not(.s-website-form-custom) .s-website-form-input')].map(el => el.name);
            if (fields.length) {
                // ideally we'd only do this if saving the form
                // succeeds... but no idea how to do that
                this._rpc({
                    model: 'ir.model.fields',
                    method: 'formbuilderWhitelist',
    ...
    onSuccess: function (previewMode, value, params) {
        ...
        if (value === 'message') {
            if (!this.$message.length) {
                this.$message = $(qweb.render('website.sWebsiteFormEndMessage'));
            }
    ...
    selectClass: function (previewMode, value, params) {
        this._super(...arguments);
        if (params.name === 'fieldMarkSelect') {
            this._setLabelsMark();
        }
    },
    /**
    ...
    toggleRecaptchaLegal: function (previewMode, value, params) {
        const recaptchaLegalEl = this.$target[0].querySelector('.s-website-form-recaptcha');
        if (recaptchaLegalEl) {
            recaptchaLegalEl.remove();
        } else {
            const template = document.createElement('template');
            const labelWidth = this.$target[0].querySelector('.s-website-form-label').style.width;
            $(template).html(qweb.render("website.sWebsiteFormRecaptchaLegal", {labelWidth: labelWidth}));
            const legal = template.content.firstElementChild;
            legal.setAttribute('contentEditable', true);
            this.$target.find('.s-website-form-submit').before(legal);
        }
    },
    ...
    _computeWidgetState: function (methodName, params) {
        switch (methodName) {
            ...
            case 'addActionField': {
                const value = this.$target.find(`.s-website-form-dnone input[name="${params.fieldName}"]`).val();
                if (value) {
                    return value;
                } else {
            ...
            case 'toggleRecaptchaLegal':
                return !this.$target[0].querySelector('.s-website-form-recaptcha') || '';
        }
    ...
    _renderCustomXML: function (uiFragment) {
        ...
        const formKey = this.activeForm.websiteFormKey;
        ...
        return Promise.all(proms).then(() => {
            formInfo.fields.forEach(field => {
                ...
                if (field.required) {
                    ...
                    const currentValue = this.$target.find(`.s-website-form-dnone input[name="${field.name}"]`).val();
                    ...
                    // 'emailTo' value to a dummy value on contact us form just
                    // by clicking on it.
    ...
    _addHiddenField: function (value, fieldName) {
        this.$target.find(`.s-website-form-dnone:has(input[name="${fieldName}"])`).remove();
        if (value) {
            const hiddenField = qweb.render('website.formFieldHidden', {
                field: {
                    name: fieldName,
                    value: value,
                },
            });
            this.$target.find('.s-website-form-submit').before(hiddenField);
        }
    },
    /**
    ...
    _buildInput: function (field) {
        ...
        inputEl.classList.add('o-we-large');
        return inputEl;
    },
    /**
    ...
    _buildSelect: function (field) {
        ...
        field.records.forEach(el => {
            ...
            button.textContent = el.displayName;
    ...
    _applyFormModel: async function (modelId) {
        let oldFormInfo;
        if (modelId) {
            const oldFormKey = this.activeForm.websiteFormKey;
            if (oldFormKey) {
                oldFormInfo = FormEditorRegistry.get(oldFormKey);
            }
            this.$target.find('.s-website-form-field').remove();
        ...
        // Model name
        this.$target[0].dataset.modelName = this.activeForm.model;
        // Load template
        if (formInfo) {
            ...
            await formInfo.formFields.forEach(async field => {
                ...
                this.$target.find('.s-website-form-submit, .s-website-form-recaptcha').first().before(this._renderField(field));
            });
        }
    },
    /**
    ...
    _setLabelsMark: function () {
        this.$target[0].querySelectorAll('.s-website-form-mark').forEach(el => el.remove());
        const mark = this._getMark();
        if (!mark) {
            return;
        }
        let fieldsToMark = [];
        const requiredSelector = '.s-website-form-model-required, .s-website-form-required';
        const fields = Array.from(this.$target[0].querySelectorAll('.s-website-form-field'));
        ...
        fieldsToMark.forEach(field => {
            ...
            span.classList.add('s-website-form-mark');
            span.textContent = ` ${mark}`;
            field.querySelector('.s-website-form-label').appendChild(span);
        });
    },
    /**
...
options.registry.WebsiteFieldEditor = FieldEditor.extend({
    ...
    willStart: async function () {
        ...
        const model = this.formEl.dataset.modelName;
        let getFields;
     ...
     */
    cleanForSave: function () {
        this.$target[0].querySelectorAll('#editableSelect').forEach(el => el.remove());
        const select = this._getSelect();
        if (select) {
     ...
     */
    onRemove() {
        const fieldName = this.$target[0].querySelector('.s-website-form-input').name;
        const isMultipleField = this.formEl.querySelectorAll(`.s-website-form-input[name="${fieldName}"]`).length > 1;
        if (isMultipleField) {
            return;
        }
    ...
    customField: async function (previewMode, value, params) {
        ...
        const oldLabelText = this.$target[0].querySelector('.s-website-form-label-content').textContent;
    ...
    setLabelText: function (previewMode, value, params) {
        this.$target.find('.s-website-form-label-content').text(value);
        if (this._isFieldCustom()) {
            const multiple = this.$target[0].querySelector('.s-website-form-multiple');
            if (multiple) {
                multiple.dataset.name = value;
            }
            const inputEls = this.$target[0].querySelectorAll('.s-website-form-input');
            ...
            const dependentEls = this.formEl.querySelectorAll(`.s-website-form-field[data-visibility-dependency="${previousInputName}"]`);
    ...
    renderListItems: async function (previewMode, value, params) {
        ...
        const inputEls = this.$target[0].querySelectorAll('.s-website-form-input, option');
        const inputName = this.$target[0].querySelector('.s-website-form-input').name;
    ...
    setVisibility(previewMode, widgetValue, params) {
        if (widgetValue === 'conditional') {
            const widget = this.findWidget('hiddenConditionOpt');
    ...
    _computeWidgetState: function (methodName, params) {
        switch (methodName) {
            ...
            case 'setLabelText':
                return this.$target.find('.s-website-form-label-content').text();
    ...
    _computeWidgetVisibility: function (widgetName, params) {
        ...
        switch (widgetName) {
            case 'hiddenConditionTimeComparatorsOpt':
                return dependencyEl && dependencyEl.dataset.target;
            case 'hiddenConditionDateBetween':
            ...
            case 'hiddenConditionDatetimeBetween':
            ...
            case 'hiddenConditionAdditionalDatetime':
                return dependencyEl && dependencyEl.dataset.target && dependencyEl.dataset.target.includes('#datetimepicker')
                && !['set', '!set'].includes(this.$target[0].dataset.visibilityComparator);
            case 'hiddenConditionAdditionalDate':
                return dependencyEl && dependencyEl.dataset.target && dependencyEl.dataset.target.includes('#datepicker')
                && !['set', '!set'].includes(this.$target[0].dataset.visibilityComparator);
            case 'hiddenConditionAdditionalText':
                if (!this.$target[0].classList.contains('s-website-form-field-hidden-if') ||
            ...
            case 'hiddenConditionNo-text-opt':
                return dependencyEl && (dependencyEl.type === 'checkbox' || dependencyEl.type === 'radio' || dependencyEl.nodeName === 'SELECT');
            case 'hiddenConditionNumOpt':
                return dependencyEl && dependencyEl.type === 'number';
            case 'hiddenConditionTextOpt':
                if (!this.$target[0].classList.contains('s-website-form-field-hidden-if') || (dependencyEl &&
            ...
            case 'hiddenConditionDateOpt':
                return dependencyEl && dependencyEl.dataset.target && dependencyEl.dataset.target.includes('#datepicker');
            case 'hiddenConditionDatetimeOpt':
                return dependencyEl && dependencyEl.dataset.target && dependencyEl.dataset.target.includes('#datetimepicker');
            case 'hiddenConditionFileOpt':
                return dependencyEl && dependencyEl.type === 'file';
            case 'hiddenConditionOpt':
                return this.$target[0].classList.contains('s-website-form-field-hidden-if');
            case 'charInputTypeOpt':
                return !this.$target[0].classList.contains('s-website-form-custom') && ['char', 'email', 'tel', 'url'].includes(this.$target[0].dataset.type);
            case 'multiCheckDisplayOpt':
                return !!this._getMultipleInputs();
            case 'requiredOpt':
            case 'hiddenOpt':
            case 'typeOpt':
                return !this.$target[0].classList.contains('s-website-form-model-required');
        }
        return this._super(...arguments);
    },
    /**
     ...
     */
     _deleteConditionalVisibility(fieldEl) {
        ...
        fieldEl.classList.remove('s-website-form-field-hidden-if', 'd-none');
    },
    /**
     * @param {HTMLElement} [fieldEl]
    ...
    _getDependencyEl(fieldEl = this.$target[0]) {
        ...
        return this.formEl.querySelector(`.s-website-form-input[name="${dependencyName}"]`);
    },
    /**
     * @override
     */
    _renderCustomXML: async function (uiFragment) {
        ...
        uiFragment.querySelector('we-row[data-name="hiddenConditionOpt"]').append($(`
            <we-select data-name="hiddenConditionFileOpt" data-attribute-name="visibilityComparator" data-no-preview="true">
        ...
        const recursiveFindCircular = (el) => {
            ...
            return recursiveFindCircular(dependencyInputEl.closest('.s-website-form-field'));
        };

        // Update available visibility dependencies
        const selectDependencyEl = uiFragment.querySelector('we-select[data-name="hiddenConditionOpt"]');
        if (selectDependencyEl) {
            const existingDependencyNames = [];
            for (const el of this.formEl.querySelectorAll('.s-website-form-field:not(.s-website-form-dnone)')) {
                const inputEl = el.querySelector('.s-website-form-input');
                if (el.querySelector('.s-website-form-label-content') && inputEl && inputEl.name
                        && inputEl.name !== this.$target[0].querySelector('.s-website-form-input').name
                        && !existingDependencyNames.includes(inputEl.name) && !recursiveFindCircular(el)) {
                    const button = document.createElement('we-button');
                    button.textContent = el.querySelector('.s-website-form-label-content').textContent;
            ...
            if (dependencyEl) {
                if ((['radio', 'checkbox'].includes(dependencyEl.type) || dependencyEl.nodeName === 'SELECT')) {
                    ...
                    const selectOptEl = uiFragment.querySelectorAll('we-select[data-name="hiddenConditionNo-text-opt"]')[1];
                    ...
                    if (dependencyEl.nodeName === 'SELECT') {
                        ...
                        const dependencyContainerEl = dependencyEl.closest('.s-website-form-field');
                        const inputsInDependencyContainer = dependencyContainerEl.querySelectorAll('.s-website-form-input');
        ...
        const selectEl = uiFragment.querySelector('we-select[data-name="typeOpt"]');
        const currentFieldName = this._getFieldName();
        const fieldsInForm = Array.from(this.formEl.querySelectorAll('.s-website-form-field:not(.s-website-form-custom) .s-website-form-input')).map(el => el.name).filter(el => el !== currentFieldName);
    ...
    _replaceFieldElement(fieldEl) {
        ...
        const hasConditionalVisibility = this.$target[0].classList.contains('s-website-form-field-hidden-if');
        const previousName = this.$target[0].querySelector('.s-website-form-input').name;
        ...
        if (hasConditionalVisibility) {
            this.$target[0].classList.add('s-website-form-field-hidden-if', 'd-none');
        }
        const dependentFieldEls = this.formEl.querySelectorAll(`.s-website-form-field[data-visibility-dependency="${previousName}"]`);
        const newName = this.$target[0].querySelector('.s-website-form-input').name;
     ...
     */
     _setVisibilityDependency(value) {
        ...
        if (this.formEl.querySelector(`.s-website-form-input[name="${value}"]`).type !== (previousDependency && previousDependency.type)) {
            this.rerender = true;
        }
...
options.registry.AddFieldForm = FormEditor.extend({
    ...
    addField: async function (previewMode, value, params) {
        ...
        const fieldEl = this._renderField(field);
        this.$target.find('.s-website-form-submit, .s-website-form-recaptcha').first().before(fieldEl);
        this.triggerUp('activateSnippet', {
            $snippet: $(fieldEl),
        });
    },
...
const DisableOverlayButtonOption = options.Class.extend({
    ...
    disableButton: function (buttonName, message) {
        ...
        const className = 'oe-snippet-' + buttonName.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
...
options.registry.MobileVisibility.include({
    ...
    async _computeVisibility() {
        ...
        return await this._super(...arguments)
            && !this.$target.hasClass('s-website-form-field-hidden');
    },
});
});
