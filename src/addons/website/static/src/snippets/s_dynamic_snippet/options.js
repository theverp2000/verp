verp.define('website.sDynamicSnippetOptions', function (require) {
'use strict';
...
const dynamicSnippetOptions = options.Class.extend({
    ...
    /**
     * See from updateUI in sWebsiteForm
     * 
     * @override
     */
    async updateUI() {
    ...
    async updateUIVisibility() {
        ...
        const groupingMessage = this.el.querySelector('.o-grouping-message');
    ...
    _getTemplateClass: function (templateKey) {
        return templateKey.replace(/.*\.dynamicFilterTemplate/, "s");
    },
    ...
    _computeWidgetVisibility: function (widgetName, params) {
        ...
        if (widgetName === 'numberOfElementsOpt') {
            const template = this._getCurrentTemplate();
            return template && !template.numOfEl;
        }

        if (widgetName === 'numberOfElementsSmallDevicesOpt') {
            const template = this._getCurrentTemplate();
            return template && !template.numOfElSm;
        }

        if (widgetName === 'numberOfRecordsOpt') {
            const template = this._getCurrentTemplate();
    ...
    _refreshPublicWidgets: function () {
        return this._super.apply(this, arguments).then(() => {
            const template = this._getCurrentTemplate();
            this.$target.find('.missing-option-warning').toggleClass(
                'd-none',
                !!template
            );
        });
    ...
    _fetchDynamicFilters: function () {
        return this._rpc({route: '/website/snippet/optionsFilters', params: {
            modelName: this.modelNameFilter,
            searchDomain: this.contextualFilterDomain,
        }});
    },
    /**
    ...
    _fetchDynamicFilterTemplates: function () {
        ...
        return filter ? this._rpc({route: '/website/snippet/filterTemplates', params: {
            filterName: filter.modelName.replaceAll('.', '_'),
        }}) : [];
    },
    /**
     *
    ...
    _renderDynamicFiltersSelector: async function (uiFragment) {
        ...
        const filtersSelectorEl = uiFragment.querySelector('[data-name="filterOpt"]');
    ...
    _renderDynamicFilterTemplatesSelector: async function (uiFragment) {
        ...
        const templatesSelectorEl = uiFragment.querySelector('[data-name="templateOpt"]');
    ...
    _setOptionsDefaultValues: function () {
        ...
        this.options.wysiwyg.verpEditor.observerUnactive();
        ...
        const filter = this.dynamicFilters[this.$target.get(0).dataset['filterId']];
        this._filterUpdated(filter);
        this.options.wysiwyg.verpEditor.observerActive();
    },
    /**
    ...
    _filterUpdated: function (filter) {
        if (filter && this.currentModelName !== filter.modelName) {
            this.currentModelName = filter.modelName;
            this.rerender = true;
        }
    },
    /**
...
options.registry.dynamicSnippet = dynamicSnippetOptions;
