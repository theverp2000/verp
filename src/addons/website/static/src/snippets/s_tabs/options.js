verp.define('website.sTabsOptions', function (require) {
'use strict';
...
options.registry.NavTabs = options.registry.MultipleItems.extend({
    ...
    _generateUniqueIDs: function () {
        for (var i = 0; i < this.$navLinks.length; i++) {
            ...
            var idLink = 'navTabsLink_' + id;
            var idContent = 'navTabsContent_' + id;
...
options.registry.NavTabsStyle = options.Class.extend({
    ...
    setStyle: function (previewMode, widgetValue, params) {
        const $nav = this.$target.find('.s-tabs-nav:first .nav');
        const isPills = widgetValue === 'pills';
        $nav.toggleClass('nav-tabs card-header-tabs', !isPills);
        $nav.toggleClass('nav-pills', isPills);
        this.$target.find('.s-tabs-nav:first').toggleClass('card-header', !isPills).toggleClass('mb-3', isPills);
        this.$target.toggleClass('card', !isPills);
        this.$target.find('.s-tabs-content:first').toggleClass('card-body', !isPills);
    },
    /**
    ...
    setDirection: function (previewMode, widgetValue, params) {
        ...
        this.$target.find('.s-tabs-content:first').toggleClass('col-md-9', isVertical);
    },
    ...
    _computeWidgetState: function (methodName, params) {
        switch (methodName) {
            case 'setStyle':
                return this.$target.find('.s-tabs-nav:first .nav').hasClass('nav-pills') ? 'pills' : 'tabs';
            case 'setDirection':
                return this.$target.find('.s-tabs-nav:first .nav').hasClass('flex-column') ? 'vertical' : 'horizontal';
        }
        return this._super(...arguments);
    },
});
