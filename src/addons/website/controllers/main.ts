...
export async function queryURL(path: string = '', pathArgs?: any, kw?: {}) {
    return (new QueryURL(path, pathArgs, kw))();
}
...
export class Website extends Home {
    ...
    async websiteConfigurator(req, res, opts: { step?: any }) {
        ...
        const website = await env.items('website').getCurrentWebsite();
        if (await website.configuratorDone) {
            return req.redirect(res, '/');
        }
        if ((await req.getEnv()).lang !== await (await website.defaultLangId).code) {
            return req.redirect(res, f('/%s%s', await (await website.defaultLangId).urlCode, req.httpRequest.pathname));
        }
    ...
    async hybridList(req, res, opts: { page?: any, search?: any, searchType?: any } = {}) {
        setOptions(opts, { page: 1, search: '', searchType: 'all' });
        if (!opts.search) {
        ...
        return req.render(res, "website.listHybrid", values);
    }
    ...
    async pagesManagement(req, res, opts: { page?: any, sortby?: any, search?: any } = {}) {
        setOptions(opts, { page: 1, sortby: 'url', search: '' });
    ...
    async pagenew(req, res, opts: { path?: any, noredirect?: any, addMenu?: any, template?: any } = {}) {
        setOptions(opts, { path: "", noredirect: false, addMenu: false, template: false });
    ...
    async resetTemplate(req, res, opts: { viewId?: any, mode?: any, redirect?: any } = {}) {
        setOptions(opts, { mode: 'soft', redirect: '/' });
