...
class WebsiteCoverPropertiesMixin extends AbstractModel {
  ...
  static coverProperties = Fields.Text('Cover Properties', { default: self => stringify(self._defaultCoverProperties()) });

  _defaultCoverProperties() {
    return {
  ...
  async write(vals) {
    ...
    // If we do not receive a valid resizeClass via the coverProperties, we
    // keep the original one (prevents updates on list displays from
    // destroying resizeClass).
