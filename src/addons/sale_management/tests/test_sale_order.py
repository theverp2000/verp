...
class TestSaleOrder(TestSaleCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        SaleOrderTemplateLine.create({
            ...
            'productId': cls.product_1.id,
            'productUomId': cls.product_1.uomId.id
        })

        SaleOrderTemplateOption.create({
            'name': 'Optional product 1',
            'sale_order_template_id': cls.quotation_template_no_discount.id,
            'productId': cls.optional_product.id,
            'uomId': cls.optional_product.uomId.id
        })
        ...
        cls.discount_included_price_list = Pricelist.create({
            ...
            'item_ids': [
                (0, 0, {
                    ...
                    'productTemplateId': cls.product_1.productTemplateId.id,
                    'computePrice': 'fixed',
                ...
                (0, 0, {
                    ...
                    'applied_on': '1_product',
                    'productTemplateId': cls.optional_product.productTemplateId.id,
                    'computePrice': 'fixed',
    ...
    def test_01_template_without_pricelist(self):
        ...
        self.assertEqual(
            self.sale_order_no_price_list.order_line[0].productId.id,
            self.product_1.id,
            "The sale order shall contains the same products as the"
            "quotation template.")

        self.assertEqual(
            self.sale_order_no_price_list.order_line[0].priceUnit,
        ...
        self.assertEqual(
            self.sale_order_no_price_list.sale_order_option_ids[0].productId.id,
            self.optional_product.id,
            "The sale order shall contains the same optional products as the"
            "quotation template.")

        self.assertEqual(
            self.sale_order_no_price_list.sale_order_option_ids[0].priceUnit,
        ...
        self.assertEqual(
            self.sale_order_no_price_list.order_line[1].productId.id,
            self.optional_product.id,
            "The sale order shall contains the same products as the"
            "quotation template.")

        self.assertEqual(
            self.sale_order_no_price_list.order_line[1].priceUnit,
            self.pub_option_price,
            "Without any price list and discount, the public price of"
            "the optional product shall be used.")
    ...
    def test_02_template_with_discount_included_pricelist(self):
        ...
        self.sale_order.write({
            'pricelistId': self.discount_included_price_list.id,
            'sale_order_template_id': self.quotation_template_no_discount.id
        })
        self.sale_order.onchange_sale_order_template_id()

        self.assertEqual(
            self.sale_order.order_line[0].priceUnit,
            self.pl_product_price,
            "If a pricelist is set, the product price shall be computed"
            "according to it.")

        self.assertEqual(
            self.sale_order.sale_order_option_ids[0].priceUnit,
        ...
        self.assertEqual(
            self.sale_order.order_line[1].priceUnit,
            self.pl_option_price,
            "If a pricelist is set, the optional product price shall"
            "be computed according to it.")
    ...
    def test_03_template_with_discount_excluded_pricelist(self):
        ...
        self.sale_order.write({
            'pricelistId': self.discount_excluded_price_list.id,
            'sale_order_template_id': self.quotation_template_no_discount.id
        })
        self.sale_order.onchange_sale_order_template_id()
    ...
    def test_04_update_pricelist_option_line(self):
        ...
        self.env.user.write({
            'groupsId': [(4, self.env.ref('product.group_discount_per_so_line').id)],
        })
    ...
    def test_option_creation(self):
        ...
        with order_form.sale_order_option_ids.new() as option:
            option.productId = self.product_1
            self.assertTrue(bool(option.uomId))
