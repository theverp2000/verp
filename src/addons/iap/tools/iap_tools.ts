...
export async function iapJsonrpc(env: Environment, url: string, opts: {method?: string, params?: any, timeout?: number}={}) {
    setOptions(opts, {method: 'call', params: {}, timeout: 15});
    const payload = {
        'jsonrpc': '2.0',
        'method': opts.method,
        'params': opts.params,
    ...
    // except (ValueError, requests.exceptions.ConnectionError, requests.exceptions.MissingSchema, requests.exceptions.Timeout, requests.exceptions.HTTPError) as e:
        throw new AccessError(
            await _t('The url that this service requested returned an error. Please contact the author of the app. The url it tried to contact was %s', url)
        );
    }
}
