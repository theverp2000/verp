verp.define("board.dashboard_tests", function (require) {
    "use strict";
    ...
    var vjUtils = require("web.vj_utils");
    ...
    QUnit.test("basic functionality, with one sub action", async function (assert) {
        ...
        var form = await createView({
            ...
            arch:
                ...
                '<board style="2-1">' +
                "<column>" +
                '<action context="{&quot;orderedBy&quot;: [{&quot;name&quot;: &quot;foo&quot;, &quot;asc&quot;: True}]}" viewMode="list" string="ABC" name="51" domain="[[\'foo\', \'!=\', \'False\']]"></action>' +
                "</column>" +
                "</board>" +
                "</form>",
    ...
    QUnit.test("views in the dashboard do not have a control panel", async function (assert) {
        ...
        var form = await createView({
            ...
            arch:
                ...
                '<board style="2-1">' +
                "<column>" +
                '<action context="{}" viewMode="list" string="ABC" name="51" domain="[]"></action>' +
                "</column>" +
                "</board>" +
                "</form>",
    ...
    QUnit.test("can render an action without viewMode attribute", async function (assert) {
        // The viewMode attribute is automatically set to the 'action' nodes when
    ...
    QUnit.test("can open record using action form view", async function (assert) {
        ...
        var form = await createView({
            ...
            archs: {
                "partner,4,list": '<tree string="Partner"><field name="foo"/></tree>',
                "partner,5,form": '<form string="Partner"><field name="displayName"/></form>',
            },
            intercepts: {
                doAction: function (event) {
                    assert.deepEqual(
    ...
    QUnit.test("twice the same action in a dashboard", async function (assert) {
        ...
        var form = await createView({
            ...
            arch:
                ...
                "<column>" +
                '<action context="{}" viewMode="list" string="ABC" name="51" domain="[]"></action>' +
                '<action context="{}" viewMode="kanban" string="DEF" name="51" domain="[]"></action>' +
                "</column>" +
                "</board>" +
                "</form>",
            mockRPC: function (route) {
        ...
        assert.strictEqual(
            $secondAction.find(".o-kanban-view").length,
            1,
    ...
    QUnit.test("non-existing action in a dashboard", async function (assert) {
        ...
        var form = await createView({
            ...
            arch:
                ...
                '<board style="2-1">' +
                "<column>" +
                '<action context="{}" viewMode="kanban" string="ABC" name="51" domain="[]"></action>' +
                "</column>" +
                "</board>" +
                "</form>",
            intercepts: {
    ...
    QUnit.test("clicking on a kanban's button should trigger the action", async function (assert) {
        ...
        var form = await createView({
            ...
            arch:
                ...
                '<board style="2-1">' +
                "<column>" +
                '<action name="149" string="Partner" viewMode="kanban" id="action_0_1"></action>' +
                "</column>" +
                "</board>" +
                "</form>",
            archs: {
    ...
    QUnit.test("subviews are aware of attach in or detach from the DOM", async function (assert) {
        ...
        // patch list renderer `onAttachCallback` for the test only
        testUtils.mock.patch(ListRenderer, {
            onAttachCallback: function () {
                assert.step("subview onAttachCallback");
            },
        });
        ...
        assert.verifySteps(["subview onAttachCallback"]);

        // restore onAttachCallback of ListRenderer
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            var board = await createView({
                ...
                mockRPC: function (route) {
                    if (route === "/web/action/load") {
                        return Promise.resolve({ resModel: "partner", views: [[false, "list"]] });
                    }
                    return this._super.apply(this, arguments);
                },
                archs: {
                ...
                intercepts: {
                    updateFilters: assert.step.bind(assert, "updateFilters"),
                },
            });
    ...
    QUnit.test("save actions to dashboard", async function (assert) {
        ...
        const mockRPC = (route, args) => {
            if (route === "/board/add_to_dashboard") {
                ...
                assert.strictEqual(args.viewMode, "list", "should save the correct view type");
                return Promise.resolve(true);
            }
        };
    ...
    QUnit.test("save a action domain to dashboard", async function (assert) {
        ...
        patchWithCleanup(browser, { setTimeout: (fn) => fn() });
        var view_domain = ["displayName", "ilike", "a"];
        var filterDomain = ["displayName", "ilike", "b"];

        // The filter domain already contains the view domain, but is always added by dashboard..,
        var expected_domain = ["&", view_domain, "&", view_domain, filterDomain];
    ...
    QUnit.test("Views should be loaded in the user's language", async function (assert) {
        ...
        var form = await createView({
            ...
            arch:
                ...
                '<board style="2-1">' +
                "<column>" +
                '<action context="{\'lang\': \'en_US\'}" viewMode="list" string="ABC" name="51" domain="[]"></action>' +
                "</column>" +
                "</board>" +
                "</form>",
            mockRPC: function (route, args) {
                if (args.method === "load_views") {
                    assert.deepEqual(
                        vjUtils.eval("context", args.kwargs.context),
                        { lang: "fr_FR" },
    ...
    QUnit.test("click on a cell of pivot view inside dashboard", async function (assert) {
        ...
        var form = await createView({
            ...
            arch:
                ...
                '<board style="2-1">' +
                "<column>" +
                '<action viewMode="pivot" string="ABC" name="51"></action>' +
                "</column>" +
                "</board>" +
                "</form>",
            mockRPC: function (route) {
    ...
    QUnit.test("Add a view with dynamic domain", async function (assert) {
        ...
        serverData.views = {
            ...
            "partner,false,search": `
                <search>
                    <filter name="filter" domain="[('userId','=',uid)]"/>
                </search>
            `,
        };
        ...
        const mockRPC = (route, args) => {
            if (route === "/board/add_to_dashboard") {
                assert.deepEqual(args.domain, 	["&", ["int_field", "<=", 3], ["userId", "=", 7]]);
                return Promise.resolve(true);
            }
        };
