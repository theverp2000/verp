...
verp_mailgate: "|/path/to/verp-mailgate.js --host=localhost -u {uid} -p PASSWORD -d {dbname}`, conf))
    ...
    async buttonConfirmLogin() {
        for (const server of this) {
            ...
            //     raise UserError(await this._t("Invalid server name !\n %s", tools.ustr(e)))
            // except (gaierror, timeout, IMAP4.abort) as e:
            //     raise UserError(await this._t("No response received. Check server information.\n %s", tools.ustr(e)))
            // except (IMAP4.error, poplib.error_proto) as err:
            //     raise UserError(await this._t("Server replied with following exception:\n %s", tools.ustr(err)))
            // except SSLError as e:
            //     raise UserError(await this._t("An SSL exception occurred. Check SSL/TLS configuration on server port.\n %s", tools.ustr(e)))
