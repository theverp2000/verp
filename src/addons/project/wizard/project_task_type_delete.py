...
from verp import api, fields, models, _
...
class ProjectTaskTypeDelete(models.TransientModel):
    ...
    def _compute_tasks_count(self):
        for wizard in self:
            wizard.tasks_count = self.withContext(active_test=False).env['project.task'].search_count([('stageId', 'in', wizard.stage_ids.ids)])
    ...
    def action_confirm(self):
        tasks = self.withContext(active_test=False).env['project.task'].search([('stageId', 'in', self.stage_ids.ids)])
    ...
    def _get_action(self):
        projectId = self.env.context.get('default_project_id')
