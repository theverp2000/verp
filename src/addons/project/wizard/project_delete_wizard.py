...
class ProjectDelete(models.TransientModel):
    ...
    def _compute_projects_archived(self):
        for wizard in self.withContext(active_test=False):
            wizard.projects_archived = all(not p.active for p in wizard.project_ids)

    def _compute_task_count(self):
        for wizard in self:
            wizard.task_count = sum(wizard.withContext(active_test=False).project_ids.mapped('task_count'))

    def action_archive(self):
        self.project_ids.write({'active': False})

    def confirm_delete(self):
        self.withContext(active_test=False).project_ids.unlink()
