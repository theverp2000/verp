...
from verp import fields, models, tools


class ReportProjectTaskUser(models.Model):
    _name = "report.project.task.user"
    _description = "Tasks Analysis"
    _order = 'name desc, projectId'
    _auto = False
    ...
    dateEnd = fields.Datetime(string='Ending Date', readonly=True)
    date_deadline = fields.Date(string='Deadline', readonly=True)
    date_last_stage_update = fields.Datetime(string='Last Stage Update', readonly=True)
    projectId = fields.Many2one('project.project', string='Project', readonly=True)
    ...
    companyId = fields.Many2one('res.company', string='Company', readonly=True)
    partnerId = fields.Many2one('res.partner', string='Customer', readonly=True)
    stageId = fields.Many2one('project.task.type', string='Stage', readonly=True)
    taskId = fields.Many2one('project.task', string='Tasks', readonly=True)
    ...
    def init(self):
        ...
        self._cr.execute("""
            CREATE view %s as
              %s
              FROM project_task t
              LEFT JOIN project_task_user_rel tu on t.id=tu.taskId
                WHERE t.active = 'true'
