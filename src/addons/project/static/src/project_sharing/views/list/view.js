...
export default ListView.extend({
    ...
    getRenderer (parent, state) {
        ...
        for (const child of this.arch.children) {
            if (child.attrs && child.attrs.modifiers && child.attrs.modifiers.columnInvisible) {
                columnInvisibleFields[child.attrs.name] = child.attrs.modifiers.columnInvisible;
            }
        }
        this.rendererParams.columnInvisibleFields = Object.entries(columnInvisibleFields).reduce((acc, entry) => {
            ...
            if (domains instanceof Array && state.data.length) {
                ...
                if (Object.keys(data).length) {
                    ...
                    acc[fieldName] = this.model._evalModifiers({ ...record, data }, { columnInvisible: domains }).columnInvisible;
                }
            } else {
