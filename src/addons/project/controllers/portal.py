...
from verp import conf, http, _
from verp.exceptions import AccessError, MissingError
from verp.http import request
from verp.addons.portal.controllers.portal import CustomerPortal, pager as portal_pager
from verp.tools import groupby as groupbyelem

from verp.osv.expression import OR, AND

from verp.addons.web.controllers.main import HomeStaticTemplateHelpers
...
class ProjectCustomerPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'task_count' in counters:
            values['task_count'] = request.env['project.task'].search_count([('projectId', '!=', False)]) \
    ...
    # My Project
    # ------------------------------------------------------------
    def _project_get_page_view_values(self, project, access_token, page=1, dateBegin=None, dateEnd=None, sortby=None, search=None, search_in='content', groupby=None, **kwargs):
        ...
        if dateBegin and dateEnd:
            domain += [('create_date', '>', dateBegin), ('create_date', '<=', dateEnd)]
        ...
        pager = portal_pager(
            url=url,
            url_args={'dateBegin': dateBegin, 'dateEnd': dateEnd, 'sortby': sortby, 'groupby': groupby, 'search_in': search_in, 'search': search, 'access_token': access_token},
            total=task_count,
            page=page,
    ...
    def portal_my_projects(self, page=1, dateBegin=None, dateEnd=None, sortby=None, **kw):
        values = self._prepare_portal_layout_values()
        ...
        pager = portal_pager(
            url="/my/projects",
            url_args={'dateBegin': dateBegin, 'dateEnd': dateEnd, 'sortby': sortby},
            total=project_count,
            page=page,
            step=self._items_per_page
        )
    ...
    def portal_my_project(self, projectId=None, access_token=None, page=1, dateBegin=None, dateEnd=None, sortby=None, search=None, search_in='content', groupby=None, **kw):
        try:
            project_sudo = self._document_check_access('project.project', projectId, access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')
        if project_sudo.collaborator_count and project_sudo.with_user(request.env.user)._check_project_sharing_access():
            return request.render("project.project_sharing_portal", {'projectId': projectId})
        project_sudo = project_sudo if access_token else project_sudo.with_user(request.env.user)
        values = self._project_get_page_view_values(project_sudo, access_token, page, dateBegin, dateEnd, sortby, search, search_in, groupby, **kw)
        values['task_url'] = 'project/%s/task' % projectId
    ...
    def _prepare_project_sharing_session_info(self, project):
        ...
        project_company = project.companyId
        session_info.update(
    ...
    @http.route("/my/project/<int:projectId>/project_sharing", type="http", auth="user", methods=['GET'])
    def render_project_backend_view(self, projectId):
        project = request.env['project.project'].sudo().browse(projectId)
    ...
    def portal_my_project_task(self, projectId=None, taskId=None, access_token=None, **kw):
        try:
            project_sudo = self._document_check_access('project.project', projectId, access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')
        ...
        if access_token:
            Task = Task.sudo()
        task_sudo = Task.search([('projectId', '=', projectId), ('id', '=', taskId)], limit=1).sudo()
    ...
    def _task_get_page_view_values(self, task, access_token, **kwargs):
        ...
        if project:
            ...
            try:
                project_accessible = bool(task.projectId.id and self._document_check_access('project.project', task.projectId.id))
            except (AccessError, MissingError):
        ...
        if project:
            ...
            task_url = f"{task.projectId.access_url}/task/%s?model=project.project&res_id={values['user'].id}&access_token={access_token}"
    ...
    def _task_get_searchbar_sortings(self):
        return {
            ...
            'project': {'label': _('Project'), 'order': 'projectId, stageId', 'sequence': 3},
            'users': {'label': _('Assignees'), 'order': 'user_ids', 'sequence': 4},
            'stage': {'label': _('Stage'), 'order': 'stageId, projectId', 'sequence': 5},
    ...
    def _task_get_search_domain(self, search_in, search):
        ...
        if search_in in ('customer', 'all'):
            search_domain.append([('partnerId', 'ilike', search)])
        if search_in in ('message', 'all'):
            search_domain.append([('message_ids.body', 'ilike', search)])
        if search_in in ('stage', 'all'):
            search_domain.append([('stageId', 'ilike', search)])
        if search_in in ('project', 'all'):
            search_domain.append([('projectId', 'ilike', search)])
    ...
    def portal_my_tasks(self, page=1, dateBegin=None, dateEnd=None, sortby=None, filterby=None, search=None, search_in='content', groupby=None, **kw):
        values = self._prepare_portal_layout_values()
        searchbar_sortings = self._task_get_searchbar_sortings()
        ...
        searchbar_filters = {
            'all': {'label': _('All'), 'domain': [('projectId', '!=', False)]},
        }
        ...
        for project in projects:
            searchbar_filters.update({
                str(project.id): {'label': project.name, 'domain': [('projectId', '=', project.id)]}
            })

        # extends filterby criteria with project (criteria name is the project id)
        # Note: portal users can't view projects they don't follow
        project_groups = request.env['project.task'].read_group([('projectId', 'not in', projects.ids)],
                                                                ['projectId'], ['projectId'])
        for group in project_groups:
            proj_id = group['projectId'][0] if group['projectId'] else False
            proj_name = group['projectId'][1] if group['projectId'] else _('Others')
            searchbar_filters.update({
                str(proj_id): {'label': proj_name, 'domain': [('projectId', '=', proj_id)]}
            })
        ...
        pager = portal_pager(
            url="/my/tasks",
            url_args={'dateBegin': dateBegin, 'dateEnd': dateEnd, 'sortby': sortby, 'filterby': filterby, 'groupby': groupby, 'search_in': search_in, 'search': search},
            total=task_count,
            page=page,
            step=self._items_per_page
        )
    ...
    def portal_my_task(self, taskId, access_token=None, **kw):
        try:
            task_sudo = self._document_check_access('project.task', taskId, access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')
