...
from verp.http import request, route

from verp.addons.portal.controllers.mail import PortalChatter
from .portal import ProjectCustomerPortal


class ProjectSharingChatter(PortalChatter):
    def _check_project_access_and_get_token(self, projectId, resModel, res_id, token):
        ...
        """
        project_sudo = ProjectCustomerPortal._document_check_access(self, 'project.project', projectId, token)
        can_access = project_sudo and resModel == 'project.task' and project_sudo.with_user(request.env.user)._check_project_sharing_access()
        task = None
        if can_access:
            task = request.env['project.task'].sudo().search([('id', '=', res_id), ('projectId', '=', project_sudo.id)])
    ...
    @route()
    def portal_chatter_init(self, resModel, res_id, domain=False, limit=False, **kwargs):
        ...
        if project_sharing_id:
            ...
            token = self._check_project_access_and_get_token(project_sharing_id, resModel, res_id, kwargs.get('token'))
            if token:
                del kwargs['project_sharing_id']
                kwargs['token'] = token
        return super().portal_chatter_init(resModel, res_id, domain=domain, limit=limit, **kwargs)

    @route()
    def portal_chatter_post(self, resModel, res_id, message, attachment_ids=None, attachment_tokens=None, **kw):
        project_sharing_id = kw.get('project_sharing_id')
        if project_sharing_id:
            token = self._check_project_access_and_get_token(project_sharing_id, resModel, res_id, kw.get('token'))
            if token:
                del kw['project_sharing_id']
                kw['token'] = token
        return super().portal_chatter_post(resModel, res_id, message, attachment_ids=attachment_ids, attachment_tokens=attachment_tokens, **kw)

    @route()
    def portal_message_fetch(self, resModel, res_id, domain=False, limit=10, offset=0, **kw):
        project_sharing_id = kw.get('project_sharing_id')
        if project_sharing_id:
            token = self._check_project_access_and_get_token(project_sharing_id, resModel, res_id, kw.get('token'))
            if token:
                kw['token'] = token
        return super().portal_message_fetch(resModel, res_id, domain=domain, limit=limit, offset=offset, **kw)
