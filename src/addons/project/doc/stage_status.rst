...
field has been totally removed and replaced by stages, using ``stageId``. The
following models are impacted:
 ...
 - ``project.task`` now use only stages. However a convention still exists about
   ...
   - ``stageId and stageId.sequence = 1``
...
datetime field. This field is updated everytime ``stageId`` is updated.
...
The ``date_open`` field meaning has been updated. It is now set when the ``userId``
