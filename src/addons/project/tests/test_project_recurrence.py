...
class TestProjectrecurrence(TransactionCase):
    ...
    def set_task_create_date(self, taskId, create_date):
        self.env.cr.execute("UPDATE project_task SET create_date=%s WHERE id=%s", (create_date, taskId))

    def test_recurrence_simple(self):
        with freeze_time("2020-02-01"):
            with Form(self.env['project.task']) as form:
                form.name = 'test recurring task'
                form.projectId = self.project_recurring

                form.recurringTask = True
                form.repeat_interval = 5
                form.repeatUnit = 'month'
                form.repeatType = 'after'
                form.repeatNumber = 10
                form.repeatOnMonth = 'date'
                form.repeatDay = '31'
            task = form.save()
            self.assertTrue(bool(task.recurrenceId), 'should create a recurrence')

            task.write(dict(repeat_interval=2, repeatNumber=11))
            self.assertEqual(task.recurrenceId.repeat_interval, 2, 'recurrence should be updated')
            self.assertEqual(task.recurrenceId.repeatNumber, 11, 'recurrence should be updated')
            self.assertEqual(task.recurrenceId.recurrence_left, 11)
            self.assertEqual(task.recurrenceId.next_recurrence_date, date(2020, 2, 29))

            task.recurringTask = False
            self.assertFalse(bool(task.recurrenceId), 'the recurrence should be deleted')

    def test_recurrence_cron_repeat_after(self):
        domain = [('projectId', '=', self.project_recurring.id)]
        with freeze_time("2020-01-01"):
            ...
            self.assertEqual(task.recurrenceId.next_recurrence_date, date(2020, 1, 15))
    ...
    def test_recurrence_cron_repeat_until(self):
        ...
        with freeze_time("2020-01-01"):
            ...
            form.repeat_interval = 1
            form.repeatUnit = 'month'
            form.repeatType = 'until'
            form.repeatUntil = date(2020, 2, 20)
            form.repeatOnMonth = 'date'
            form.repeatDay = '15'
            task = form.save()
            task.plannedHours = 2
    ...
    def test_recurrence_update_task(self):
        with freeze_time("2020-01-01"):
            task = self.env['project.task'].create({
                    ...
                    'projectId': self.project_recurring.id,
                    'recurringTask': True,
                    'repeat_interval': 1,
                    'repeatUnit': 'week',
                    'repeatType': 'after',
        ...
        task_c, task_b, task_a = self.env['project.task'].search([('projectId', '=', self.project_recurring.id)])
    ...
    def test_recurrence_fields_visibility(self):
        ...
        self.assertTrue(form.repeatShowDow)
        self.assertFalse(form.repeatShowDay)
        self.assertFalse(form.repeatShowWeek)
        self.assertFalse(form.repeatShowMonth)

        form.repeatUnit = 'month'
        form.repeatOnMonth = 'date'
        self.assertFalse(form.repeatShowDow)
        self.assertTrue(form.repeatShowDay)
        self.assertFalse(form.repeatShowWeek)
        self.assertFalse(form.repeatShowMonth)

        form.repeatUnit = 'month'
        form.repeatOnMonth = 'day'
        self.assertFalse(form.repeatShowDow)
        self.assertFalse(form.repeatShowDay)
        self.assertTrue(form.repeatShowWeek)
        self.assertFalse(form.repeatShowMonth)

        form.repeatUnit = 'year'
        form.repeatOnYear = 'date'
        self.assertFalse(form.repeatShowDow)
        self.assertTrue(form.repeatShowDay)
        self.assertFalse(form.repeatShowWeek)
        self.assertTrue(form.repeatShowMonth)

        form.repeatUnit = 'year'
        form.repeatOnYear = 'day'
        self.assertFalse(form.repeatShowDow)
        self.assertFalse(form.repeatShowDay)
        self.assertTrue(form.repeatShowWeek)
        self.assertTrue(form.repeatShowMonth)

        form.recurringTask = False
        self.assertFalse(form.repeatShowDow)
        self.assertFalse(form.repeatShowDay)
        self.assertFalse(form.repeatShowWeek)
        self.assertFalse(form.repeatShowMonth)
    ...
    def test_recurrence_next_dates_month(self):
        dates = self.env['project.task.recurrence']._get_next_recurring_dates(
            dateStart=date(2020, 1, 15),
            repeat_interval=1,
            repeatUnit='month',
            repeatType=False, # Forever
            repeatUntil=False,
            repeatOnMonth='date',
            repeatOnYear=False,
            weekdays=False,
        ...
        dates = self.env['project.task.recurrence']._get_next_recurring_dates(
            dateStart=date(2020, 1, 10),
            repeat_interval=1,
            repeatUnit='month',
            repeatType='until',
            repeatUntil=datetime(2020, 5, 31),
            repeatOnMonth='day',
            repeatOnYear=False,
        ...
        dates = self.env['project.task.recurrence']._get_next_recurring_dates(
            dateStart=datetime(2020, 1, 10),
            repeat_interval=6, # twice a year
            repeatUnit='month',
            repeatType='until',
            repeatUntil=datetime(2021, 1, 11),
            repeatOnMonth='date',
            repeatOnYear=False,
            weekdays=[TH(+1)],
            repeatDay='3', # the 3rd of the month
            repeatWeek=False,
            repeatMonth=False,
            count=1)
        ...
        dates = self.env['project.task.recurrence']._get_next_recurring_dates(
            dateStart=date(2022, 2, 26),
            repeat_interval=1,
            repeatUnit='month',
            repeatType='until',
            repeatUntil=date(2022, 2, 28),
            repeatOnMonth='date',
            repeatOnYear=False,
            weekdays=False,
            repeatDay=31,
        ...
        dates = self.env['project.task.recurrence']._get_next_recurring_dates(
            dateStart=date(2022, 11, 26),
            repeat_interval=3,
            repeatUnit='month',
            repeatType='until',
            repeatUntil=date(2024, 2, 29),
            repeatOnMonth='date',
            repeatOnYear=False,
            weekdays=False,
            repeatDay=25,
        ...
        # Use the exact same parameters than the previous test but with a repeatDay that is not passed yet
    ...
    def test_recurrence_next_dates_year(self):
        dates = self.env['project.task.recurrence']._get_next_recurring_dates(
            dateStart=date(2020, 12, 1),
            repeat_interval=1,
            repeatUnit='year',
            repeatType='until',
            repeatUntil=datetime(2026, 1, 1),
            repeatOnMonth=False,
            repeatOnYear='date',
            weekdays=False,
            repeatDay=31,
    ...
    def test_recurrence_cron_repeat_after_subtasks(self):
        ...
        with freeze_time("2020-01-01"):
            with Form(parent_task.child_ids.with_context({'tracking_disable': True})) as form:
                form.description = 'my super recurring task bla bla bla'
                form.date_deadline = datetime(2020, 2, 1)
                form.display_project_id = parent_task.projectId
                ...
                form.repeatDay = '15'
            subtask = form.save()
            subtask.plannedHours = 2

            self.assertEqual(subtask.recurrenceId.next_recurrence_date, date(2020, 1, 15))
            ...
            self.assertEqual(subtask.recurrenceId.recurrence_left, 2)
            for task in project_tasks:
                self.assertEqual(task.display_project_id, parent_task.projectId, "All tasks should have a display project id set")
        ...
        with freeze_time("2020-01-15"):
            ...
            self.assertEqual(subtask.recurrenceId.recurrence_left, 1)
            for task in project_tasks:
                self.assertEqual(task.display_project_id, parent_task.projectId, "All tasks should have a display project id set")
        ...
        with freeze_time("2020-02-15"):
            ...
            self.assertEqual(subtask.recurrenceId.recurrence_left, 0)
            for task in project_tasks:
                self.assertEqual(task.display_project_id, parent_task.projectId, "All tasks should have a display project id set")
            self.env['project.task.recurrence']._cron_create_recurring_tasks()
    ...
    def test_disabling_recurrence(self):
        ...
        task_b.recurringTask = False

        self.assertFalse(any((task_a + task_b + task_c).mapped('recurringTask')),
