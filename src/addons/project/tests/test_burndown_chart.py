...
class TestBurndownChart(TransactionCase):
    def set_create_date(self, table, resId, create_date):
        self.env.cr.execute("UPDATE {} SET create_date=%s WHERE id=%s".format(table), (create_date, resId))
    ...
    def test_burndown_chart(self):
        ...
        project = self.env['project.project'].create({
            ...
            'aliasName': 'project+burndown_chart',
            'typeIds': [Command.link(stageId) for stageId in stages.ids],
        })
        ...
        with freeze_time('%s-02-10' % (current_year - 1)):
            (task_a + task_b).write({'stageId': in_progress_stage.id})
            self.env.cr.flush()

        with freeze_time('%s-02-20' % (current_year - 1)):
            task_c.write({'stageId': in_progress_stage.id})
            self.env.cr.flush()

        with freeze_time('%s-03-15' % (current_year - 1)):
            (task_d + task_e).write({'stageId': in_progress_stage.id})
            self.env.cr.flush()

        with freeze_time('%s-04-10' % (current_year - 1)):
            (task_a + task_b).write({'stageId': testing_stage.id})
            self.env.cr.flush()

        with freeze_time('%s-05-12' % (current_year - 1)):
            task_c.write({'stageId': testing_stage.id})
            self.env.cr.flush()

        with freeze_time('%s-06-25' % (current_year - 1)):
            task_d.write({'stageId': testing_stage.id})
            self.env.cr.flush()

        with freeze_time('%s-07-25' % (current_year - 1)):
            task_e.write({'stageId': testing_stage.id})
            self.env.cr.flush()

        with freeze_time('%s-08-01' % (current_year - 1)):
            task_a.write({'stageId': done_stage.id})
            self.env.cr.flush()

        with freeze_time('%s-09-10' % (current_year - 1)):
            task_b.write({'stageId': done_stage.id})
            self.env.cr.flush()

        with freeze_time('%s-10-05' % (current_year - 1)):
            task_c.write({'stageId': done_stage.id})
            self.env.cr.flush()

        with freeze_time('%s-11-25' % (current_year - 1)):
            task_d.write({'stageId': done_stage.id})
            self.env.cr.flush()

        with freeze_time('%s-12-12' % (current_year - 1)):
            task_e.write({'stageId': done_stage.id})
            self.env.cr.flush()

        read_group_result = self.env['project.task.burndown.chart.report'].with_context(fill_temporal=True).read_group([('projectId', '=', project.id), ('display_project_id', '!=', False)], ['date', 'stageId', 'nb_tasks'], ['date:month', 'stageId'], lazy=False)
        read_group_result_dict = {(res['date:month'], res['stageId'][0]): res['nb_tasks'] for res in read_group_result}
        ...
        for (month, stageId), nb_tasks in read_group_result_dict.items():
            ...
            expected_nb_tasks = expected_dict.get((month, stageId), 5 if stageId != todo_stage.id else 1)
            self.assertEqual(
                nb_tasks,
                expected_nb_tasks,
                'In %s, the number of tasks should be equal to %s in %s stage.' % (month, expected_nb_tasks, stages_dict.get(stageId, 'Unknown'))
            )
