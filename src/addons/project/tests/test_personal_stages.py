...
from verp.tests import tagged, HttpCase
...
class TestPersonalStages(TestProjectCommon):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.user_stages = cls.env['project.task.type'].search([('userId', '=', cls.user_projectuser.id)])
        cls.manager_stages = cls.env['project.task.type'].search([('userId', '=', cls.user_projectmanager.id)])
