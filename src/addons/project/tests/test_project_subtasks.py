...
class TestProjectSubtasks(TestProjectCommon):
    def test_task_display_project_with_default_form(self):
        ...
        with Form(self.env['project.task'].with_context({'tracking_disable': True})) as task_form:
            ...
            task_form.projectId = self.project_pigs
        task = task_form.save()

        self.assertEqual(task.projectId, self.project_pigs, "The project should be assigned.")
        self.assertEqual(task.display_project_id, task.projectId, "The display project of a first layer task should be assigned to projectId.")
        ...
        self.assertEqual(task.projectId, self.project_pigs, "The project should be assigned from the default project.")
        self.assertEqual(task.display_project_id, task.projectId, "The display project of a first layer task should be assigned to projectId.")
    ...
    def test_task_display_project_with_any_task_form(self):
        ...
        for form_view in form_views:
            ...
            self.assertEqual(task.projectId, self.project_pigs, "The project should be assigned from the default project, form_view name : %s." % form_view.name)
            self.assertEqual(task.display_project_id, task.projectId, "The display project of a first layer task should be assigned to projectId, form_view name : %s." % form_view.name)
    ...
    def test_subtask_display_project(self):
        ...
        self.assertEqual(self.task_1.child_ids.projectId, self.project_pigs, "The project should be assigned from the default project.")
        self.assertFalse(self.task_1.child_ids.display_project_id, "The display project of a sub task should be false to projectId.")
        ...
        self.assertEqual(self.task_1.projectId, self.project_pigs, "Changing the project of a subtask should not change parent project")
        ...
        self.assertEqual(self.task_1.child_ids.projectId, self.project_goats, "Changing display project id on a subtask should change project id")
        ...
        self.assertEqual(self.task_1.child_ids.projectId, self.project_pigs, "Resetting display project to False on a subtask should change project id to parent project id")

        # 4)
        with Form(self.task_1.with_context({'tracking_disable': True})) as task_form:
            task_form.projectId = self.project_goats

        self.assertEqual(self.task_1.projectId, self.project_goats, "Parent project should change.")
        self.assertFalse(self.task_1.child_ids.display_project_id, "Display Project of the task should be False")
        self.assertEqual(self.task_1.child_ids.projectId, self.project_goats, "Resetting display project to False on a subtask should follow project of its parent")
        ...
        self.assertEqual(self.task_1.projectId, self.project_pigs, "Parent project should change back.")
        self.assertEqual(self.task_1.child_ids.display_project_id, self.project_goats, "Display Project of the task should be well assigned")
        self.assertEqual(self.task_1.child_ids.projectId, self.project_goats, "Changing display project id on a subtask should change project id")
        ...
        self.assertEqual(orphan_subtask.projectId, self.project_goats, "Changing display project id on a subtask should change project id")
        ...
        self.assertEqual(orphan_subtask.projectId, self.project_pigs, "Removing parent should not change project")
    ...
    def test_subtask_stage(self):
        ...
        self.project_pigs.typeIds |= stage_a
        self.project_pigs.typeIds |= stage_b

        with Form(self.task_1.with_context({'tracking_disable': True})) as task_form:
            with task_form.child_ids.new() as subtask_form:
                subtask_form.name = 'Test Subtask 1'

        self.assertEqual(self.task_1.child_ids.stageId, stage_a, "The stage of the child task should be the default one of the project.")

        with Form(self.task_1.with_context({'tracking_disable': True})) as task_form:
            task_form.stageId = stage_b

        self.assertEqual(self.task_1.child_ids.stageId, stage_a, "The stage of the child task should remain the same while changing parent task stage.")
        ...
        self.assertEqual(self.task_1.child_ids.stageId, stage_a, "The stage of the child task should be the default one of the project even if parent stage id is different.")

        with Form(self.task_1.with_context({'tracking_disable': True})) as task_form:
            with task_form.child_ids.edit(0) as subtask_form:
                subtask_form.display_project_id = self.project_goats

        self.assertEqual(self.task_1.child_ids.stageId.name, "New", "The stage of the child task should be the default one of the display project id, once set.")
    ...
    def test_copy_project_with_subtasks(self):
        ...
        self.env['project.task'].with_context({'mail_create_nolog': True}).create({
            ...
            'projectId': self.project_goats.id,
