    ...
    def test_generate_analytic_account(self):
        ...
        with self.sudo('manager-b'):
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                ...
                self.assertEqual(self.project_company_a.company_id, self.project_company_a.analyticAccountId.company_id, "The analytic account created from a project should be in the same company")
    ...
    def test_create_task(self):
        with self.sudo('employee-a'):
            ...
            with Form(self.env['project.task'].with_context({'tracking_disable': True})) as task_form:
                ...
                task_form.projectId = self.project_company_a
            task = task_form.save()
        ...
        with self.sudo('employee-a'):
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                with Form(self.task_1) as task_form:
                    task_form.projectId = self.project_company_b
                task = task_form.save()
    ...
    def test_create_subtask(self):
        with self.sudo('employee-a'):
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                ...
                self.task_2.write({'projectId': False})
                ...
                self.assertEqual(task.company_id, task.projectId.company_id, "The company of the orphan subtask should be the one from its project.")
    ...
    def test_cross_subtask_project(self):
        ...
        with self.sudo('employee-a'):
            with self.allow_companies([self.company_a.id, self.company_b.id]):
                ...
                self.assertEqual(task.projectId, self.task_1.projectId, "The default project of a subtask should be the default subtask project of the project from the mother task")
                self.assertEqual(task.company_id, task.projectId.company_id, "The company of the orphan subtask should be the one from its project.")
