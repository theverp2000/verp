...
from verp.addons.mail.tests.common import MockEmail
...
To: {to}
cc: {cc}
Received: by mail1.verp.com (Postfix, from userid 10002)
...
class TestProjectFlow(TestProjectCommon, MockEmail):
    ...
    @mute_logger('verp.addons.mail.mail_thread')
    def test_task_process_without_stage(self):
        ...
        # Test: messages
        self.assertEqual(len(task.messageIds), 1,
                         'project: message_process: newly created task should have 1 message: email')
        self.assertEqual(task.messageIds[0].subtypeId, self.env.ref('project.mt_task_new'),
                         'project: message_process: first message of new task should have Task Created subtype')
        self.assertEqual(task.messageIds[0].authorId, self.user_projectuser.partner_id,
                         'project: message_process: second message should be the one from Agrolait (partner failed)')
        self.assertEqual(task.messageIds[0].subject, 'Frogs',
        ...
        self.assertEqual(task.projectId.id, self.project_pigs.id, 'project_task: incorrect project')
        self.assertEqual(task.stageId.sequence, False, "project_task: shouldn't have a stage, i.e. sequence=False")
    ...
    def test_task_process_with_stages(self):
        ...
        self.assertEqual(task.messageIds[0].subject, 'Cats',
        ...
        self.assertEqual(task.projectId.id, self.project_goats.id, 'project_task: incorrect project')
        self.assertEqual(task.stageId.sequence, 1, "project_task: should have a stage with sequence=1")
    ...
    def test_rating(self):
        ...
        # Our One2Many is linked to a resId (int) for which the orm doesn't create an inverse
        ...
        first_task.projectId = self.project_goats.id
    ...
    def test_send_rating_review(self):
        project_settings = self.env["res.config.settings"].create({'groupProjectRating': True})
        project_settings.execute()
        self.assertTrue(self.project_goats.ratingActive, 'The customer ratings should be enabled in this project.')

        won_stage = self.project_goats.typeIds[-1]
        rating_request_mail_template = self.env.ref('project.rating_project_request_email_template')
        won_stage.write({'ratingTemplateId': rating_request_mail_template.id})
        ...
        with self.mock_mail_gateway():
            tasks.with_user(self.user_projectmanager).write({'stageId': won_stage.id})
        ...
        for task in tasks:
            ...
            rating_request_message = task.messageIds[:1]
            if not task.user_ids or len(task.user_ids) > 1:
                self.assertFalse(task.rating_ids.ratedPartnerId, 'This rating should have no assigned user if the task related have no assignees or more than one assignee.')
                ...
                self.assertEqual(task.rating_ids.ratedPartnerId, task.user_ids.partner_id, 'The rating should have an assigned user if the task has only one assignee.')
