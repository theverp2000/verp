...
class TestTaskDependencies(TestProjectCommon):
    ...
    def test_task_dependencies(self):
        """ Test the task dependencies feature
            ...
            2) Checks if the task1 has the task in dependOnIds field.
        """
        self.assertEqual(len(self.task_1.dependOnIds), 0, "The task 1 should not have any dependency.")
        self.task_1.write({
            'dependOnIds': [Command.link(self.task_2.id)],
        })
        self.assertEqual(len(self.task_1.dependOnIds), 1, "The task 1 should have a dependency.")
        self.task_1.write({
            'dependOnIds': [Command.link(self.task_3.id)],
        })
        self.assertEqual(len(self.task_1.dependOnIds), 2, "The task 1 should have two dependencies.")
    ...
    def test_cyclic_dependencies(self):
        ...
        # 1) Check initial setting on three tasks
        self.assertTrue(
            len(self.task_1.dependOnIds) == len(self.task_2.dependOnIds) == len(self.task_3.dependOnIds) == 0,
            "The three tasks should depend on no tasks.")
        self.assertTrue(self.task_1.allowTaskDependencies, 'The task dependencies feature should be enable.')
        self.assertTrue(self.task_2.allowTaskDependencies, 'The task dependencies feature should be enable.')
        self.assertTrue(self.task_3.allowTaskDependencies, 'The task dependencies feature should be enable.')

        # 2) Add task2 as dependency in task1
        self.task_1.write({
            'dependOnIds': [Command.link(self.task_2.id)],
        })
        self.assertEqual(len(self.task_1.dependOnIds), 1, 'The task 1 should have one dependency.')

        # 3) Add task3 as dependency in task2
        self.task_2.write({
            'dependOnIds': [Command.link(self.task_3.id)],
        })
        self.assertEqual(len(self.task_2.dependOnIds), 1, "The task 2 should have one dependency.")

        # 4) Add task1 as dependency in task3 and check a validation error is raised
        with self.assertRaises(ValidationError), self.cr.savepoint():
            self.task_3.write({
                'dependOnIds': [Command.link(self.task_1.id)],
            })
        self.assertEqual(len(self.task_3.dependOnIds), 0, "The dependency should not be added in the task 3 because of a cyclic dependency.")
        ...
        self.assertEqual(len(self.task_2.dependOnIds), 1, "The number of dependencies should no change in the task 2 because of a cyclic dependency.")
    ...
    def test_tracking_dependencies(self):
        ...
        self.flush_tracking()
        self.assertEqual(len(self.task_1.messageIds), 1,
        ...
        self.flush_tracking()
        self.assertEqual(len(self.task_2.messageIds), 1,
        ...
        self.flush_tracking()
        self.assertEqual(len(self.task_1.messageIds), 2,
        ...
        self.flush_tracking()
        self.assertEqual(len(self.task_1.messageIds), 3,
    ...
    def test_task_dependencies_settings_change(self):
        ...
        self.assertTrue(self.project_pigs.allowTaskDependencies, "Projects allowTaskDependencies should follow group_project_task_dependencies setting changes")

        self.project_chickens = self.env['project.project'].create({
            'name': 'My Chicken Project'
        })
        self.assertTrue(self.project_chickens.allowTaskDependencies, "New Projects allowTaskDependencies should default to group_project_task_dependencies")

        set_task_dependencies_setting(False)
        self.assertFalse(self.project_pigs.allowTaskDependencies, "Projects allowTaskDependencies should follow group_project_task_dependencies setting changes")

        self.project_ducks = self.env['project.project'].create({
            'name': 'My Ducks Project'
        })
        self.assertFalse(self.project_ducks.allowTaskDependencies, "New Projects allowTaskDependencies should default to group_project_task_dependencies")

    def test_duplicate_project_with_task_dependencies(self):

        self.project_pigs.allowTaskDependencies = True
        self.task_1.dependOnIds = self.task_2
