 ...
 * @returns 
 */
export function makeAware(dt: string | Date): any[] {
    // return [new Date(dt), (val: Date) => { val.setTime(val.getTime() + new Date(dt).getTimezoneOffset() * 60000); return val }];
    if (typeof dt === 'string') {
        console.error('call makeAware with a string');
    }
    return [typeof dt === 'string' ? new Date(dt): dt, (val) => typeof dt === 'string' ? new Date(val) : val];
}
...
export class Intervals {
    ...
    constructor(intervals?: Iterable<any>) {
        this._items = [];
        if (intervals) {
            // normalize the representation of intervals
            const starts = [];
            const recses = [];
            const listBoundaries = boundaries(intervals, 'start', 'stop');
            const sortedintervals = sorted(listBoundaries, (item: any) => item[0].toISOString()+item[1]);
            let start;
            for (const [value, flag, recs] of sortedintervals) {
                if (flag === 'start') {
                    starts.push(value);
                    recses.push(recs);
                }
    ...
    *[Symbol.iterator]() {
     ...
     */
    or(other) {
        return new Intervals(chain(this._items, other._items));
    }
     ...
     */
    and(other) {
        return this._merge(other, false)
    }
     ...
     * @param difference 
     */
    _merge(other, difference) {
        const result = new Intervals();
        const push = result._items.push.bind(result._items);

        // using 'self' and 'other' below forces normalization
        const bounds1 = boundaries(this, 'start', 'stop');
        const bounds2 = boundaries(other, 'switch', 'switch');

        let start = null,                   // set by start/stop
            recs1 = null,                       // set by start
            enabled = difference;               // changed by switch
        const sortedintervals = sorted(chain(bounds1, bounds2), (item: any) => item[0].toISOString()+item[1]);
        for (const [value, flag, recs] of sortedintervals) {
            if (flag == 'start') {
                start = value;
                recs1 = recs;
                ...
                if (enabled && start != null && start < value) {
                    push([start, value, recs1]);
                }
                enabled = !enabled;
            }
...
class ResourceCalendar extends Model {
    ...
    async defaultGet(fields) {
        ...
        if (fields.includes('attendanceIds') && !res['attendanceIds']) {
    ...
    static attendanceIds = Fields.One2many('resource.calendar.attendance', 'calendarId', {
        string: 'Working Time', compute: '_computeAttendanceIds', store: true, readonly: false, copy: true
    });
    static leaveIds = Fields.One2many('resource.calendar.leaves', 'calendarId', { string: 'Time Off' });
    static globalLeaveIds = Fields.One2many(
        'resource.calendar.leaves', 'calendarId', {
    ...
    async _computeAttendanceIds() {
        for (const calendar of await this.filtered(async (c) => !bool(c._origin) || !(await c._origin.companyId).eq(await c.companyId))) {
            ...
            for (const attendance of await companyCalendar.attendanceIds) {
                if (!bool(await attendance.resourceId)) {
    ...
    async _checkAttendanceIds() {
        for (const resource of this) {
            ...
            if (await resource.twoWeeksCalendar &&
                bool(await attendanceIds.filtered(async (a) => await a.displayType === 'lineSection')) &&
    ...
    async _getGlobalAttendances() {
        return (await this['attendanceIds']).filtered(async (attendance) =>
            ...
            && !bool(await attendance.resourceId) && ! await attendance.displayType)
    }
    ...
    */
    async _attendanceIntervalsBatch(startDt?: Date, endDt?: Date, resources?: any, domain?: any[], tz?: string) {
        this.ensureOne();
        resources = bool(resources) ? resources : this.env.items('resource.resource');
        const requiredTz = tz;
        ...
        // const cacheDates = new DefaultDict(() => new Dict());
        const cacheDeltas = new DefaultDict(() => new Dict())
        const result = {};
        const attendances = await this.env.items('resource.calendar.attendance').search(domain);
        for (const attendance of attendances) {
            for (const resource of resourcesList) {
                // express all dates and times in specified tz or in the resource's timezone
                tz = requiredTz ? requiredTz : timezone(await (resource.ok ? resource : this as any).tz);
                let start = startDt, end = endDt;
                /*
                let keyTz = String([tz, startDt.toISOString()]);
                if (keyTz in cacheDates) {
                    start = cacheDates[keyTz];
                }
                else {
                    start = startDt;//dateSetTz(startDt, tz);
                    cacheDates[keyTz] = start;
                }
                keyTz = String([tz, endDt.toISOString()]);
                if (keyTz in cacheDates) {
                    end = cacheDates[keyTz];
                }
                else {
                    end = endDt; //dateSetTz(endDt, tz);
                ...
                */
                ...
                for (const day of days.all()) {
                    // We need to exclude incorrect days according to re-defined start previously with weeks=-1 (Note: until is correctly handled)
                    if (await this['twoWeeksCalendar'] && await attendance.dateFrom && await attendance.dateFrom > day) {
                        continue;
                    }
                    // attendance hours are interpreted in the resource's timezone
                    const hourFrom = await attendance.hourFrom;
                    let keyTz = String([tz, day.toISOString(), hourFrom]);
                    let dt0, dt1;
                    if (keyTz in cacheDeltas) {
                        dt0 = cacheDeltas[keyTz];
                    }
                    else {
                        const [h, m] = floatToTime(hourFrom);
                        dt0 = combine(day, h.toString().padStart(2,'0')+':'+m.toString().padStart(2,'0'));
                        cacheDeltas[keyTz] = dt0; //dateSetTz
                    }

                    const hourTo = await attendance.hourTo;
                    keyTz = String([tz, day.toISOString(), hourTo]);
                    if (keyTz in cacheDeltas) {
                        dt1 = cacheDeltas[keyTz];
                    }
                    else {
                        const [h, m] = floatToTime(hourTo);
                        dt1 = combine(day, h.toString().padStart(2,'0')+':'+m.toString().padStart(2,'0'));
                        cacheDeltas[keyTz] = dt1;// dateSetTz
                    }
                    start = startDt;//cacheDates[String([tz, startDt.toISOString()])];
                    end = endDt;//cacheDates[String([tz, endDt.toISOString()])];
                    result[resource.id] = result[resource.id] ?? []
    ...
    */
    async _leaveIntervalsBatch(startDt?: Date, endDt?: Date, resources?: any, domain?: any[], tz?: string) {
        ...
        for (const leave of await this.env.items('resource.calendar.leaves').search(domain)) {
            for (const resource of resourcesList) {
                ...
                // else {
                //     start = startDt;//dateSetTz(startDt, tz);
                //     keyTz = String([tz, startDt.toISOString()]);
                //     tzDates[keyTz] = start;
                // }
                // keyTz = String([tz, endDt.toISOString()]);
                // if (keyTz in tzDates) {
                ...
                //     tzDates[keyTz] = end;
                // }
                const dt0 = await leave.dateFrom;//dateSetTz(await leave.dateFrom, tz);
                const dt1 = await leave.dateTo;//dateSetTz(await leave.dateTo, tz);
                result[resource.id] = result[resource.id] ?? [];
    ...
    async _workIntervalsBatch(startDt?: Date, endDt?: Date, resources?: any, domain?: any[], tz?: string) {
        ...
        const attendanceIntervals = await this._attendanceIntervalsBatch(startDt, endDt, resources, [], tz);
        const leaveIntervals = await this._leaveIntervalsBatch(startDt, endDt, resources, domain, tz);
        return Object.fromEntries(resourcesList.map(r => [r.id, attendanceIntervals[r.id].sub(leaveIntervals[r.id])]));
    }
    ...
    async _getResourcesDayTotal(fromDatetime: Date, toDatetime: Date, resources?: any) {
        ...
        const result = new Dict<Dict<Number>>();
        for (const resource of resourcesList) {
     ...
     * @param matchEnd wether to search for the begining of an interval or the end.
     * @param searchRange time interval considered. Defaults to the entire day of `dt`
     * @param resources 
     * @returns 
     */
    async _getClosestWorkTime(dt, opts: { matchEnd?: boolean, searchRange?: any, resources?: any } = {}) {
     ...
     * `compute_leaves` controls whether or not this method is taking into
            ...
            None means default value ('timeType', '=', 'leave')
...
class ResourceCalendarLeaves extends Model {
    ...
    async defaultGet(fieldsList) {
        ...
        if (fieldsList.includes('dateFrom') && fieldsList.includes('dateTo') && !res['dateFrom'] && !res['dateTo']) {
            ...
            const userTz = timezone(await (await this.env.user()).tz || this._context['tz'] || await (await (await this['companyId']).resourceCalendarId).tz || 'UTC');
            let dateFrom = dateSetTz(combine(today, 'min'), userTz);
            let dateTo = dateSetTz(combine(today, 'max'), userTz);
            const intervals = (await (await (await this.env.company()).resourceCalendarId)._workIntervalsBatch(dateSetTz(dateFrom, 'UTC'), dateSetTz(dateTo, 'UTC')))['false'];
            if (bool(intervals)) {  // Then we stop and return the dates given in parameter
                const listIntervals = intervals.map(([start, stop]) => [start, stop]);  // Convert intervals in interval list
                dateFrom = listIntervals[0][0];  // We take the first date in the interval list
                dateTo = listIntervals[listIntervals.length - 1][1];  // We take the last date in the interval list
            }
            update(res, {
                dateFrom: dateWithoutTz(dateSetTz(dateFrom, 'UTC')),
                dateTo: dateWithoutTz(dateSetTz(dateTo, 'UTC'))
            });
        }
        return res;
    }
    ...
    async checkDates() {
        if ((await this.filtered(async (leave) => await leave.dateFrom > await leave.dateTo)).ok) {
            ...
            console.error(this.id, await this._t('The start date of the time off must be earlier than the end date.'));
        }
    }
