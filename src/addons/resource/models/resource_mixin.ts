...
class ResourceMixin extends AbstractModel {
   ...
   * By default the resource calendar is used, but it can be changed using the `calendar` argument.
      ...
      `domain` is used in order to recognise the leaves to take, None means default value ('timeType', '=', 'leave')
  ...
  async _getWorkDaysDataBatch(fromDatetime, toDatetime, computeLeaves: boolean = true, calendar?: any, domain?: any) {
    ...
    // fromDatetime = timezoneDatetime(fromDatetime)
    // toDatetime = timezoneDatetime(toDatetime)

    const mappedResources = new DefaultMapKey(() => this.env.items('resource.resource'));
    for (const record of this) {
      const key = bool(calendar) ? calendar : await record.resourceCalendarId;
      mappedResources.set(key, mappedResources.get(key).or(await record.resourceId));
    }
    for (const [calendar, calendarResources] of mappedResources) {
      ...
      let intervals;
      if (computeLeaves) {
        intervals = await calendar._workIntervalsBatch(fromDatetime, toDatetime, calendarResources, domain);
      }
      else {
        intervals = await calendar._attendanceIntervalsBatch(fromDatetime, toDatetime, calendarResources);
      }

      for (const calendarResource of calendarResources) {
        result[calendarResource.id] = await calendar._getDaysData(intervals[calendarResource.id], dayTotal[calendarResource.id]);
      }
    }

    // convert "resource: result" into "employee: result"
    return Object.fromEntries(await resources.map(r => [mappedEmployees[r.id], result[r.id]]));
  }
