verp.define('partner_autocomplete.fieldchar', function (require) {
'use strict';
...
var FieldAutocomplete = FieldChar.extend(AutocompleteMixin, {
    className: 'o-field-partner_autocomplete',
    ...
    _selectCompany: function (company) {
        ...
        this._getCreateData(company).then(function (data) {
            if (data.logo) {
                var logoField = self.model === 'res.partner' ? 'image1920' : 'logo';
                data.company[logoField] = data.logo;
            }

            // Some fields are unnecessary in res.company
            if (self.model === 'res.company') {
                var fields = 'comment,childIds,bankIds,additionalInfo'.split(',');
                fields.forEach(function (field) {
                    delete data.company[field];
                });
            }

            self._setOne2ManyField('bankIds', data.company.bankIds);
            delete data.company.bankIds;

            self.triggerUp('fieldChanged', {
    ...
    _showDropdown: function () {
        ...
        if (this.suggestions.length > 0) {
            ...
            this.$el.removeClass('o-text-overflow');
            this.$input.addClass('o-text-overflow');
        }
    },
...
fieldRegistry.add('fieldPartnerAutocomplete', FieldAutocomplete);
