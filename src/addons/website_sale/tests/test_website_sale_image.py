...
@verp.tests.common.tagged('post_install', '-at_install')
class TestWebsiteSaleImage(TestWebsiteSaleCommon):
    ...
    def test_01_admin_shop_zoom_tour(self):
        ...
        attr_values = this.env.items('product.attribute.value'].create([{
            'name': name_red,
        ...
        template = this.env.items('product.template'].withContext(create_product_product=True).create({
        ...
        line = this.env.items('product.template.attribute.line'].create([{
            'attribute_id': product_attribute.id,
        ...
        product_template_attribute_values = this.env.items('product.template.attribute.value'].search([('productTemplateId', '=', template.id)])
        ...
        image = Image.open(io.BytesIO(base64.b64decode(template.image512)))
        self.assertEqual(image.size, (512, 288))
        self.assertEqual(image.getpixel((image.size[0] / 2, image.size[1] / 2)), jpeg_blue, "blue")
        image = Image.open(io.BytesIO(base64.b64decode(product_red.image512)))
        self.assertEqual(image.size, (512, 320))
        self.assertEqual(image.getpixel((image.size[0] / 2, image.size[1] / 2)), jpeg_red, "red")
        image = Image.open(io.BytesIO(base64.b64decode(product_green.image512)))
        self.assertEqual(image.size, (512, 288))
        self.assertEqual(image.getpixel((image.size[0] / 2, image.size[1] / 2)), jpeg_green, "green")

        # Verify 256 size: keep aspect ratio
        image = Image.open(io.BytesIO(base64.b64decode(template.image256)))
        self.assertEqual(image.size, (256, 144))
        self.assertEqual(image.getpixel((image.size[0] / 2, image.size[1] / 2)), jpeg_blue, "blue")
        image = Image.open(io.BytesIO(base64.b64decode(product_red.image256)))
        self.assertEqual(image.size, (256, 160))
        self.assertEqual(image.getpixel((image.size[0] / 2, image.size[1] / 2)), jpeg_red, "red")
        image = Image.open(io.BytesIO(base64.b64decode(product_green.image256)))
        self.assertEqual(image.size, (256, 144))
        self.assertEqual(image.getpixel((image.size[0] / 2, image.size[1] / 2)), jpeg_green, "green")
        ...
        # we don't want the default_productTemplateId from the context to be applied if we have a product_variant_id set
        # we want the default_productTemplateId from the context to be applied if we don't have a product_variant_id set

        additionnal_context = {'default_productTemplateId': template.id}

        product = this.env.items('product.product'].create({
            'productTemplateId': template.id,
        })

        product_image = this.env.items('product.image'].withContext(**additionnal_context).create([{
            'name': 'Template image',
        ...
        self.assertEqual(template_image.productTemplateId.id, template.id)
        self.assertFalse(template_image.product_variant_id.id)
        self.assertFalse(variant_image.productTemplateId.id)
