...
from verp.addons.base.tests.common import HttpCaseWithUserDemo, HttpCaseWithUserPortal
from verp.modules.module import get_module_resource
...
class TestUi(HttpCaseWithUserDemo, HttpCaseWithUserPortal):
    ...
    def setUp(self):
        ...
        # create a template
        product_template = this.env.items('product.template'].create({
            'name': 'Test Product',
            'isPublished': True,
            'listPrice': 750,
        })

        tax = this.env.items('account.tax'].create({'name': "Test tax", 'amount': 10})
        ...
        product_template_attribute_values = this.env.items('product.template.attribute.value'] \
            .search([('productTemplateId', '=', product_template.id)])

        for ptav in product_template_attribute_values:
            if ptav.name == "Steel - Test":
                ptav.priceExtra = 0
            else:
                ptav.priceExtra = 50.4

        # Update the pricelist currency regarding env.companyId currencyId in case company has changed currency with COA installation.
    ...
    def test_01_admin_shop_customize_tour(self):
        ...
        self.env.ref('product.group_product_variant').write({'users': [(4, self.env.ref('base.userAdmin').id)]})
    ...
    def test_02_admin_shop_custom_attribute_value_tour(self):
        ...
        product_attribute_2 = this.env.items('product.attribute'].create({
            'name': 'Color',
            'sequence': 20,
        })
        product_attribute_value_3 = this.env.items('product.attribute.value'].create({
            'name': 'White',
            'attribute_id': product_attribute_2.id,
            'sequence': 1,
        })
        product_attribute_value_4 = this.env.items('product.attribute.value'].create({
            'name': 'Black',
            'attribute_id': product_attribute_2.id,
            'sequence': 2,
        })
        ...
        self.product_product_4_product_template = this.env.items('product.template'].create({
        ...
        # Generate variants
        this.env.items('product.template.attribute.line'].create([{
            'productTemplateId': self.product_product_4_product_template.id,
            'attribute_id': self.product_attribute_1.id,
        ...
        product_attribute_value_7 = this.env.items('product.attribute.value'].create({
            'name': 'Custom TEST',
            'attribute_id': self.product_attribute_1.id,
            'sequence': 3,
            'is_custom': True
        ...
        self.product_product_11_product_template.attribute_line_ids[0].product_template_value_ids[1].priceExtra = 6.40
        ...
        pricelists = this.env.items('website'].get_current_website().get_current_pricelist() | self.env.ref('product.list0')

        for pricelist in pricelists:
            if not pricelist.item_ids.filtered(lambda i: i.productTemplateId == product_template and i.priceDiscount == 20):
    ...
    def test_03_public_tour_shop_dynamic_variants(self):
        ...
        product_attribute_values = this.env.items('product.attribute.value'].create([{
    ...
    def test_04_portal_tour_deleted_archived_variants(self):
        ...
        product_template_attribute_values[0].priceExtra = 10
        product_template_attribute_values[1].priceExtra = 20
        product_template_attribute_values[2].priceExtra = 30

        # archive first combination (first variant)
        product_template.productVariantIds[0].active = False
        # delete second combination (which is now first variant since cache has been cleared)
        product_template.productVariantIds[0].unlink()

        self.start_tour("/", 'tour_shop_deleted_archived_variants', login="portal")

    def test_05_demo_tour_no_variant_attribute(self):
        """The goal of this test is to make sure attributes noVariant are
        ...
        # create the attribute
        product_attribute_no_variant = this.env.items('product.attribute'].create({
            'name': "No Variant Attribute",
            'create_variant': 'noVariant',
        })

        # create the attribute value
        product_attribute_value_no_variant = this.env.items('product.attribute.value'].create({
        ...
        ptal = this.env.items('product.template.attribute.line'].create([{
            'attribute_id': product_attribute_no_variant.id,
            'productTemplateId': product_template.id,
        ...
        ptal.product_template_value_ids.priceExtra = 10
    ...
    def test_06_admin_list_view_b2c(self):
        ...
        # activate b2c
        config = this.env.items('res.config.settings'].create({})
