...
from verp.tests.common import HttpCase, TransactionCase
...
class TestWebsiteSaleCartRecoveryServer(TransactionCase):
    ...
    def setUp(self):
        ...
        self.so0 = this.env.items('sale.order'].create({
            'partnerId': self.customer.id,
            'websiteId': self.website0.id,
            'is_abandoned_cart': True,
            'cart_recovery_email_sent': False,
        })
        self.so1 = this.env.items('sale.order'].create({
            'partnerId': self.customer.id,
            'websiteId': self.website1.id,
            'is_abandoned_cart': True,
            'cart_recovery_email_sent': False,
        })
        self.so2 = this.env.items('sale.order'].create({
            'partnerId': self.customer.id,
    ...
    def test_cart_recovery_mail_template_send(self):
        ...
        self.assertTrue(
            all(orders.mapped('accessToken')),
            "All tokens should have been generated."
        )

        sent_mail = {}
        for order in orders:
            mail = this.env.items("mail.mail"].search([
        ...
        self.assertTrue(
            all(order.accessToken in sent_mail[order].body for order in orders),
