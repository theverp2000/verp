...
from verp.models import Command
from verp.tests.common import tagged
...
class WebsiteSaleCartPayment(PaymentCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.tx = cls.env['payment.transaction'].create({
            ...
            'acquirerId': cls.acquirer.id,
            'reference': cls.reference,
            'operation': 'online_redirect',
            'partnerId': cls.partner.id,
        })
        cls.order.write({'transactionIds': [Command.set([cls.tx.id])]})
    ...
    def test_paid_orders_cannot_be_retrieved(self):
        ...
        self.tx.acquirerId.support_authorization = True
