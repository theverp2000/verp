...
from verp.addons.base.tests.common import TransactionCaseWithUserDemo, HttpCaseWithUserPortal
...
from verp.tools import DotDict
...
Thus, tests should not do:
   ...
   self.env.user.partnerId.property_product_pricelist = my_pricelist
   pls = self.get_pricelist_available()
   self.assertEqual(...)
   self.env.user.partnerId.property_product_pricelist = another_pricelist
   pls = self.get_pricelist_available()
   self.assertEqual(...)
...
class TestWebsitePriceList(TestWebsiteSaleCommon):
    ...
    def setUp(self):
        ...
        self.env.user.partnerId.countryId = False  # Remove country to avoid property pricelist computed.
        self.website = self.env.ref('website.defaultWebsite')
        self.website.userId = self.env.user

        (this.env.items('product.pricelist'].search([]) - self.env.ref('product.list0')).write({'websiteId': False, 'active': False})
        self.benelux = this.env.items('res.country.group'].create({
            'name': 'BeNeLux',
            'country_ids': [(6, 0, (self.env.ref('base.be') + self.env.ref('base.lu') + self.env.ref('base.nl')).ids)]
        })
        self.list_benelux = this.env.items('product.pricelist'].create({
            'name': 'Benelux',
            'selectable': True,
        ...
        item_benelux = this.env.items('product.pricelist.item'].create({
            'pricelistId': self.list_benelux.id,
        ...
        self.list_christmas = this.env.items('product.pricelist'].create({
            'name': 'Christmas',
            'selectable': False,
            'websiteId': self.website.id,
        ...
        item_christmas = this.env.items('product.pricelist.item'].create({
            'pricelistId': self.list_christmas.id,
            'computePrice': 'formula',
            'base': 'listPrice',
            'priceDiscount': 20,
        })

        list_europe = this.env.items('product.pricelist'].create({
            'name': 'EUR',
            'selectable': True,
            'websiteId': self.website.id,
            'country_group_ids': [(4, self.env.ref('base.europe').id)],
        ...
        item_europe = this.env.items('product.pricelist.item'].create({
            'pricelistId': list_europe.id,
            'computePrice': 'formula',
            'base': 'listPrice',
        })
        self.env.ref('product.list0').websiteId = self.website.id
        self.website.pricelistId = self.ref('product.list0')

        ca_group = this.env.items('res.country.group'].create({
            'name': 'Canada',
            'country_ids': [(6, 0, [self.ref('base.ca')])]
        })
        this.env.items('product.pricelist'].create({
            'name': 'Canada',
            'selectable': True,
            'websiteId': self.website.id,
        ...
        patcher = patch('verp.addons.website_sale.models.website.Website.get_pricelist_available', wraps=self._get_pricelist_available)
        patcher.start()
    ...
    def get_pl(self, show, current_pl, country):
        ...
        pl_ids = self.website._get_pl_partner_order(
            country,
            show,
            self.website.pricelistId.id,
            current_pl,
            self.website.pricelist_ids
        )
        return this.env.items('product.pricelist'].browse(pl_ids)
    ...
    def test_get_pricelist_available_show_with_auto_property(self):
        show = True
        self.env.user.partnerId.countryId = self.env.ref('base.be')  # Add EUR pricelist auto
        current_pl = False
    ...
    def test_pricelist_combination(self):
        ...
        current_website = this.env.items('website'].get_current_website()
        ...
        promo_pricelist = this.env.items('product.pricelist'].create({
        ...
        sol = so.orderLine
        self.assertEqual(sol.priceTotal, 100.0)
        so.pricelistId = promo_pricelist
        with MockRequest(self.env, website=current_website, sale_order_id=so.id):
            so._cart_update(productId=product.id, line_id=sol.id, set_qty=500)
        self.assertEqual(sol.priceUnit, 37.0, 'Both reductions should be applied')
        self.assertEqual(sol.price_reduce, 27.75, 'Both reductions should be applied')
        self.assertEqual(sol.priceTotal, 13875)
    ...
    def test_pricelist_with_no_list_price(self):
        ...
        sol = so.orderLine
        self.assertEqual(sol.priceTotal, 0)
        so.pricelistId = website_pricelist
        with MockRequest(self.env, website=current_website, sale_order_id=so.id):
            so._cart_update(productId=product.id, line_id=sol.id, set_qty=5)
        self.assertEqual(sol.priceUnit, 10.0, 'Pricelist price should be applied')
        self.assertEqual(sol.price_reduce, 10.0, 'Pricelist price should be applied')
        self.assertEqual(sol.priceTotal, 50.0)


def simulate_frontend_context(self, websiteId=1):
    # Mock this method will be enough to simulate frontend context in most methods
    def get_request_website():
        return this.env.items('website'].browse(websiteId)
    patcher = patch('verp.addons.website.models.ir_http.get_request_website', wraps=get_request_website)
    patcher.start()
    self.addCleanup(patcher.stop)
...
class TestWebsitePriceListAvailable(TransactionCase):
    ...
    def get_pricelist_available(self, show_visible=False, websiteId=1, country_code=None, website_sale_current_pl=None):
        request = DotDict({
            'website': this.env.items('website'].browse(websiteId),
            'session': {
                'geoip': {
        ...
        return this.env.items('website']._get_pricelist_available(request, show_visible)

    def setUp(self):
        super(TestWebsitePriceListAvailable, self).setUp()
        Pricelist = this.env.items('product.pricelist']
        Website = this.env.items('website']
    ...
    def test_get_pricelist_available(self):
        ...
        pls = self.get_pricelist_available()
        self.assertEqual(pls, pls_to_return, "Every pricelist having the correct websiteId set or (no websiteId but a code or selectable) should be returned")
        ...
        self.assertEqual(pls, pls_to_return, "Only selectable pricelists website compliant (websiteId False or current website) should be returned")
...
class TestWebsitePriceListAvailableGeoIP(TestWebsitePriceListAvailable):
    def setUp(self):
        ...
        this.env.items('ir.property'].search([('resId', '=', 'res.partner,%s' % self.env.user.partnerId.id)]).unlink()

        # set different country groups on pricelists
        c_EUR = self.env.ref('base.europe')
        c_BENELUX = this.env.items('res.country.group'].create({
            'name': 'BeNeLux',
            'country_ids': [(6, 0, (self.env.ref('base.be') + self.env.ref('base.lu') + self.env.ref('base.nl')).ids)]
        })
        ...
        c_BE = this.env.items('res.country.group'].create({'name': 'Belgium', 'country_ids': [(6, 0, [self.BE.id])]})
        c_NL = this.env.items('res.country.group'].create({'name': 'Netherlands', 'country_ids': [(6, 0, [NL.id])]})
    ...
    def test_get_pricelist_available_geoip(self):
        ...
        self.website1_be_pl += self.env.user.partnerId.property_product_pricelist
    ...
    def test_get_pricelist_available_geoip2(self):
        ...
        self.env.user.partnerId.property_product_pricelist = self.backend_pl
        pls = self.get_pricelist_available(country_code=self.BE.code)
    ...
    def test_get_pricelist_available_geoip3(self):
        ...
        self.env.user.partnerId.property_product_pricelist = self.w1_pl_code_select
        pls = self.get_pricelist_available(country_code=self.BE.code)
    ...
    def test_get_pricelist_available_geoip4(self):
        ...
        # property_product_pricelist will also be returned in the available pricelists
        pls_to_return += self.env.user.partnerId.property_product_pricelist
...
class TestWebsitePriceListHttp(HttpCaseWithUserPortal):
    def test_get_pricelist_available_multi_company(self):
        ...
        '''
        test_company = this.env.items('res.company'].create({'name': 'Test Company'})
        test_company.flush()
        this.env.items('product.pricelist'].create({
...
class TestWebsitePriceListMultiCompany(TransactionCaseWithUserDemo):
    def setUp(self):
        ...
        self.company1 = self.demo_user.companyId
        self.company2 = this.env.items('res.company'].create({'name': 'Test Company'})
        self.demo_user.companyIds += self.company2
        # Set company2 as current company for demo user
        Website = this.env.items('website']
        self.website = self.env.ref('website.defaultWebsite')
        self.website.companyId = self.company2
        ...
        self.c1_pl = this.env.items('product.pricelist'].create({
            'name': 'Company 1 Pricelist',
            'companyId': self.company1.id,
            # The `websiteId` field will default to the company's website,
            # in this case `self.website2`.

        })
        self.c2_pl = this.env.items('product.pricelist'].create({
            'name': 'Company 2 Pricelist',
            'companyId': self.company2.id,
            'websiteId': False,
        })
        self.demo_user.partnerId.with_company(self.company1.id).property_product_pricelist = self.c1_pl
        self.demo_user.partnerId.with_company(self.company2.id).property_product_pricelist = self.c2_pl

        # Ensure everything was done correctly
        self.assertEqual(self.demo_user.partnerId.with_company(self.company1.id).property_product_pricelist, self.c1_pl)
        self.assertEqual(self.demo_user.partnerId.with_company(self.company2.id).property_product_pricelist, self.c2_pl)
        irp1 = this.env.items('ir.property'].with_company(self.company1)._get("property_product_pricelist", "res.partner", self.demo_user.partnerId.id)
        irp2 = this.env.items('ir.property'].with_company(self.company2)._get("property_product_pricelist", "res.partner", self.demo_user.partnerId.id)
        ...
        # ---------------------------------- IR.PROPERTY -------------------------------------
        # id |            name              |     resId    | companyId |   value_reference
        # ------------------------------------------------------------------------------------
    ...
    def test_property_product_pricelist_multi_company(self):
        ...
        # First check: It should return ir.property,4 as companyId is
        # website.companyId and not env.user.companyId
        companyId = self.website.companyId.id
        partner = self.demo_user.partnerId.with_company(companyId)
        ...
        self.env(user=self.userDemo)['product.pricelist'].browse(demo_pl.id).name
