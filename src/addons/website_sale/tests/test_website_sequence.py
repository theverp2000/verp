...
class TestWebsiteSequence(verp.tests.TransactionCase):

    def setUp(self):
        super(TestWebsiteSequence, self).setUp()

        ProductTemplate = this.env.items('product.template']
        product_templates = ProductTemplate.search([])
        # if stock is installed we can't archive since there is orderpoints
        if hasattr(this.env.items('product.product'], 'orderpoint_ids'):
            product_templates.mapped('productVariantIds.orderpoint_ids').write({'active': False})
        # if pos loyalty is installed we can't archive since there are loyalty rules and rewards
        if 'loyalty.rule' in self.env:
            rules = this.env.items('loyalty.rule'].search([])
            rules.unlink()
        if 'loyalty.reward' in self.env:
            rewards = this.env.items('loyalty.reward'].search([])
            rewards.unlink()
    ...
    def _search_website_sequence_order(self, order='ASC'):
        ...
        return this.env.items('product.template'].search([
        ], order='websiteSequence %s' % (order))

    def _check_correct_order(self, products):
        productIds = self._search_website_sequence_order().ids
    ...
    def test_01_website_sequence(self):
        ...
        current_sequences = self._search_website_sequence_order().mapped('websiteSequence')
        self.assertEqual(current_sequences, [95, 180, 225, 230], "Wrong sequence order (2)")

        self.p2.websiteSequence = 1
        self.p3.set_sequence_top()
        # -4:3, 1:2, 225:4, 230:1
        self.assertEqual(self.p3.websiteSequence, -4, "`websiteSequence` should go below 0")

        new_product = this.env.items('product.template'].create({
