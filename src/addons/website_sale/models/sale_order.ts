...
class SaleOrder extends Model {
    ...
    async _computeAbandonedCart() {
        for (const order of this) {
            ...
            const website = await order.websiteId; 
            if (website.ok && await order.state === 'draft' && await order.dateOrder) {
                const publicPartnerId = await (await website.userId).partnerId;
                // by default the expiration date is 1 hour if not specified on the website configuration
                const abandonedDelay = await website.cartAbandonedDelay || 1.0;
