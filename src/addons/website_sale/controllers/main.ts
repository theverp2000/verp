...
class TableCompute {
    ...
    async _process(products, ppg=20, ppr=4) {
        ...
        let rows = sorted(Object.entries(this.table), item => item[0]);
        rows = rows.map(r => r[1]);
        for (const col of range(rows.length)) {
            const cols = sorted(Object.entries(rows[col]), item => item[0]);
            x += cols.length;
...
class WebsiteSale extends http.Controller {
    ...
    async shop(req, res, post: {page?: any, category?: any, search?: any, minPrice?: any, maxPrice?: any, ppg?: any, order?: any}={}) {
        ...
        if (bool(category)) {
            url = f("/shop/category/%s", await category.slug());
        }
    ...
    async product(req, res, opts: {product?: any, category?: any, search?: any}={}) {
        const {product, category='', search=''} = opts;
        return req.render(res, "website_sale.product", await this._prepareProductValues(req, product, category, search, opts));
    }
    ...
    async oldProduct(req, res, opts: {product?: any, category?: any, search?: any}={}) {
        ...
        return req.redirect(res, buildUrlWParams(f("/shop/%s", await opts.product.slug(), req.params), {code: 301}));
    }
    ...
    async _prepareProductValues(req, product, category, search, opts) {
        ...
        // }
        const attrib = req.params.getlist('attrib'),
    ...
    async _getShopPaymentValues(req, order, opts) {
        ...
        return {
            ...
            'order': order,
            'paymentActionId': (await (await env.ref('payment.actionPaymentAcquirer')).actionId).id,
