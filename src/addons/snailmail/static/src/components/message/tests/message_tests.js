...
QUnit.test('Format Error', async function (assert) {
    ...
    bus.on('do-action', null, payload => {
        ...
        assert.strictEqual(
            payload.options.additionalContext.messageId,
            10,
...
QUnit.test('Missing Required Fields', async function (assert) {
    ...
    this.data['mail.message'].records.push({
        ...
        messageType: 'snailmail',
        resId: 20, // non 0 id, necessary to fetch failure at init
    ...
    this.data['snailmail.letter'].records.push({
        ...
        messageId: 10, // id of related message
    });
    const bus = new Bus();
    bus.on('do-action', null, payload => {
        assert.step('doAction');
        ...
        assert.strictEqual(
            payload.options.additionalContext.default_letter_id,
            22,
