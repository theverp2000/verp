...
from verp import fields, http, _
from verp.http import request
from verp.tools import date_utils, groupby as groupbyelem
from verp.osv.expression import AND, OR
...
class TimesheetCustomerPortal(CustomerPortal):
    ...
    def _get_search_domain(self, search_in, search):
        ...
        if search_in in ('project', 'all'):
            search_domain = OR([search_domain, [('projectId', 'ilike', search)]])
        if search_in in ('name', 'all'):
            search_domain = OR([search_domain, [('name', 'ilike', search)]])
        if search_in in ('employee', 'all'):
            search_domain = OR([search_domain, [('employee_id', 'ilike', search)]])
        if search_in in ('task', 'all'):
            search_domain = OR([search_domain, [('taskId', 'ilike', search)]])
    ...
    def _get_searchbar_sortings(self):
        return {
            ...
            'project': {'label': _('Project'), 'order': 'projectId'},
            'task': {'label': _('Task'), 'order': 'taskId'},
