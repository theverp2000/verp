...
// # from verp.addons.project.controllers.portal import CustomerPortal

@http.define()
class ProjectCustomerPortal extends CustomerPortal {
    ...
    def _task_get_page_view_values(self, task, access_token, **kwargs):
        ...
        task_domain = expression.AND([domain, [('taskId', '=', task.id)]])
        subtask_domain = expression.AND([domain, [('taskId', 'in', task.child_ids.ids)]])
        ...
        for timesheet in subtasks_timesheets:
            timesheets_by_subtask[timesheet.taskId] |= timesheet
