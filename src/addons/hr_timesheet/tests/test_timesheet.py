...
from verp.tests.common import TransactionCase, Form
from verp.exceptions import AccessError, UserError
...
class TestCommonTimesheet(TransactionCase):
    ...
    def setUp(self):
        ...
        self.task1 = self.env['project.task'].create({
            ...
            'priority': '0',
            'kanban_state': 'normal',
            'projectId': self.project_customer.id,
            'partner_id': self.partner.id,
        })
...
class TestTimesheet(TestCommonTimesheet):
    ...
    def test_log_timesheet(self):
        ...
        timesheet_uom = self.project_customer.analyticAccountId.company_id.project_time_mode_id
        # employee 1 log some timesheet on task 1
        timesheet1 = Timesheet.with_user(self.user_employee).create({
            'projectId': self.project_customer.id,
            'taskId': self.task1.id,
            'name': 'my first timesheet',
            'unit_amount': 4,
        })
        self.assertEqual(timesheet1.account_id, self.project_customer.analyticAccountId, 'Analytic account should be the same as the project')
        ...
        self.assertEqual(timesheet3.userId, self.user_employee2, 'Timesheet user should be the one linked to the given employee')
    ...
    def test_log_access_rights(self):
        ...
        self.assertEqual(timesheet1.userId, self.user_employee2, 'Changing timesheet employee should change the related user')
    ...
    def test_create_unlink_project(self):
        ...
        self.assertFalse(non_tracked_project.analyticAccountId, "A non time-tracked project shouldn't generate an analytic account")
        ...
        self.assertTrue(tracked_project.analyticAccountId, "A time-tracked project should generate an analytic account")
        self.assertTrue(tracked_project.analyticAccountId.active, "A time-tracked project should generate an active analytic account")
        self.assertEqual(tracked_project.partner_id, tracked_project.analyticAccountId.partner_id, "The generated AA should have the same partner as the project")
        self.assertEqual(tracked_project.name, tracked_project.analyticAccountId.name, "The generated AA should have the same name as the project")
        self.assertEqual(tracked_project.analyticAccountId.project_count, 1, "The generated AA should be linked to the project")
        ...
        analytic_project = self.env['project.project'].create({
            ...
            'allow_timesheets': True,
            'partner_id': self.partner.id,
            'analyticAccountId': tracked_project.analyticAccountId.id,
        })
        self.assertNotEqual(analytic_project.name, tracked_project.analyticAccountId.name, "The name of the associated AA can be different from the project")
        self.assertEqual(tracked_project.analyticAccountId.project_count, 2, "The AA should be linked to 2 project")

        # analytic linked to projects containing tasks can not be removed
        task = self.env['project.task'].create({
            'name': 'task in tracked project',
            'projectId': tracked_project.id,
        })
        with self.assertRaises(UserError):
            tracked_project.analyticAccountId.unlink()
    ...
    def test_transfert_project(self):
        ...
        Timesheet.create([{
            ...
            'projectId': self.project_customer.id,
            'taskId': task_child.id,
            'name': 'my second timesheet',
            'unit_amount': 4,
            'employee_id': self.empl_employee.id,
        }, {
            'projectId': self.project_customer.id,
            'taskId': task_grandchild.id,
            'name': 'my third timesheet',
            'unit_amount': 4,
            'employee_id': self.empl_employee.id,
        }])

        timesheet_count1 = Timesheet.search_count([('projectId', '=', self.project_customer.id)])
        timesheet_count2 = Timesheet.search_count([('projectId', '=', self.project_customer2.id)])
        ...
        with Form(self.task1) as task_form:
            task_form.projectId = self.project_customer2
    ...
    def test_recompute_amount_for_multiple_timesheets(self):
        ...
        with self.assertRaises(AccessError):
            ...
            timesheets.with_user(self.empl_employee.userId).write({
                'unit_amount': 2,
            })
    ...
    def test_task_with_timesheet_project_change(self):
        ...
        project_manager = self.env['res.users'].create({
            ...
            'groups_id': [(6, 0, [self.ref('project.groupProjectManager')])],
        })
        ...
        self.assertEqual(timesheet.projectId, second_project, 'The projectId of timesheet should be second_project')
    ...
    def test_create_timesheet_employee_not_in_company(self):
        ...
        project = self.env['project.project'].create({
            'name': 'Aa Project',
            'company_id': company_3.id,
            'analyticAccountId': analytic_account.id,
        })
        task = self.env['project.task'].create({
            'name': 'Aa Task',
            'projectId': project.id,
    ...
    def test_ensure_product_uom_set_in_timesheet(self):
        ...
        timesheet1, timesheet2 = self.env['account.analytic.line'].create([
            {'unit_amount': 1.0, 'projectId': self.project_customer.id},
            {'unit_amount': 3.0, 'projectId': self.project_customer.id, 'product_uom_id': False},
        ])
        self.assertEqual(
            timesheet1.product_uom_id,
            self.project_customer.analyticAccountId.company_id.timesheet_encode_uom_id,
    ...
    def test_check_timesheet_user(self):
        ...
        self.assertFalse(without_user_timesheet.userId, 'User is not set in timesheet.')

        with_user_timesheet = Timesheet.with_context(default_employee_id=self.empl_employee.id).create({
            'projectId': self.project_customer.id,
            'unit_amount': 8.0,
        })
        self.assertEqual(with_user_timesheet.userId, self.user_employee, 'User Employee is set in timesheet.')
    ...
    def test_create_timesheet_with_companyless_analytic_account(self):
        ...
        self.project_customer.analyticAccountId.company_id = False
    ...
    def test_percentage_of_planned_hours(self):
        ...
        self.task1.plannedHours = round(11/60, 2)
