...
from verp import models, fields, api, _, _lt
from verp.tools.float_utils import float_compare
from verp.exceptions import UserError, ValidationError, RedirectWarning
...
class Project(models.Model):
    ...
    def _compute_total_timesheet_time(self):
        timesheets_read_group = self.env['account.analytic.line'].read_group(
            [('projectId', 'in', self.ids)],
            ['projectId', 'unit_amount', 'product_uom_id'],
            ['projectId', 'product_uom_id'],
            lazy=False)
        ...
        for result in timesheets_read_group:
            ...
            if uom_id:
                uom_ids.add(uom_id)
            timesheet_time_dict[result['projectId'][0]].append((uom_id, result['unit_amount']))
    ...
    def _compute_timesheet_count(self):
        ...
        if self.env['account.analytic.line'].check_access_rights('read', raise_exception=False):
            ...
            timesheet_project_map = {project_info['projectId'][0]: project_info['project_id_count'] for project_info in timesheet_read_group}
        for project in self:
...
class Task(models.Model):
    ...
    allow_timesheets = fields.Boolean("Allow timesheets", related='projectId.allow_timesheets', help="Timesheets can be logged on this task.", readonly=True)
    ...
    timesheet_ids = fields.One2many('account.analytic.line', 'taskId', 'Timesheets')
    ...
    def _compute_effective_hours(self):
        if not any(self._ids):
            ...
            return
        timesheet_read_group = self.env['account.analytic.line'].read_group([('taskId', 'in', self.ids)], ['unit_amount', 'taskId'], ['taskId'])
        timesheets_per_task = {res['taskId'][0]: res['unit_amount'] for res in timesheet_read_group}
        for task in self:
    ...
    def action_view_subtask_timesheet(self):
        ...
        return {
            ...
            'domain': [('projectId', '!=', False), ('taskId', 'in', tasks.ids)],
        }
    ...
    def write(self, values):
        ...
        if 'projectId' in values and not values.get('projectId') and self._get_timesheet():
        ...
        if 'projectId' in values:
            project = self.env['project.project'].browse(values.get('projectId'))
            if project.allow_timesheets:
                # We write on all non yet invoiced timesheet the new projectId (if project allow timesheet)
                self._get_timesheet().write({'projectId': values.get('projectId')})
    ...
    def _unlink_except_contains_entries(self):
        ...
        task_with_timesheets_ids = [res['taskId'][0] for res in timesheet_data]
