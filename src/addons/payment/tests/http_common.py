...
from verp import http
from verp.tests import HttpCase
...
class PaymentHttpCommon(PaymentTestUtils, HttpCase):
    ...
    def _build_jsonrpc_payload(self, params):
        ...
        if not getattr(self, 'session', None):
            ...
            self.authenticate('', '')
        params['csrfToken'] = http.WebRequest.csrfToken(self)
        return {
            "jsonrpc": "2.0",
            "method": "call",
            "id": str(uuid4()),
    ...
    def _get_tx_context(self, response, form_name):
        ...
        checkoutForm = html_tree.xpath(f"//form[@name='{form_name}']")[0]
        values = {}
        for key, val in checkoutForm.items():
    ...
    def _prepare_pay_values(self, amount=0.0, currency=None, reference='', partner=None):
        ...
        return {
            ...
            'accessToken': self._generate_test_access_token(partner.id, amount, currency.id),
        }
    ...
    def portal_pay(self, **route_kwargs):
        ...
        Or an accessToken should be specified in route_kwargs
        """
        uri = '/payment/pay'
