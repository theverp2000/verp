...
class TestFlows(PaymentCommon, PaymentHttpCommon):
    ...
    def _test_flow(self, flow):
        ...
        self.assertEqual(tx_sudo.acquirerId.id, self.acquirer.id)
        self.assertEqual(tx_sudo.amount, self.amount)
        self.assertEqual(tx_sudo.currencyId.id, self.currency.id)
        self.assertEqual(tx_sudo.partnerId.id, self.partner.id)
        self.assertEqual(tx_sudo.reference, self.reference)

        # processing_values == given values
        self.assertEqual(processing_values['acquirerId'], self.acquirer.id)
        self.assertEqual(processing_values['amount'], self.amount)
        self.assertEqual(processing_values['currencyId'], self.currency.id)
        self.assertEqual(processing_values['partnerId'], self.partner.id)
        ...
        self.assertIn("accessToken=", tx_sudo.landing_route)
        ...
        if flow == 'redirect':
            ...
            self.assertEqual(
                redirect_form_info['inputs']['userId'],
                str(self.user.id))
            self.assertEqual(
                redirect_form_info['inputs']['viewId'],
    ...
    def test_10_direct_checkout_public(self):
        ...
        self.partner.companyId = self.env.company.id
    ...
    def _test_validation(self, flow):
        ...
        expected_values = {
            ...
            'accessToken': self._generate_test_access_token(self.partner.id, None, None),
        ...
        transaction_values = {
            'amount': None,
            'currencyId': None,
            'partnerId': tx_context['partnerId'],
            'accessToken': tx_context['accessToken'],
            'flow': flow,
            'payment_option_id': self.acquirer.id,
        ...
        self.assertEqual(tx_sudo.currencyId.id, validation_currency.id)
        self.assertEqual(tx_sudo.partnerId.id, self.partner.id)
        self.assertEqual(tx_sudo.reference, expected_reference)
        # processing_values == given values
        self.assertEqual(processing_values['acquirerId'], self.acquirer.id)
        self.assertEqual(processing_values['amount'], validation_amount)
        self.assertEqual(processing_values['currencyId'], validation_currency.id)
        self.assertEqual(processing_values['partnerId'], self.partner.id)
    ...
    def test_pay_redirect_if_no_partner_exist(self):
        ...
        self.authenticate(self.portal_user.login, self.portal_user.login)
        tx_context = self.get_tx_checkout_context(**route_values)
        self.assertEqual(tx_context['partnerId'], self.portal_partner.id)

    def test_pay_no_token(self):
        route_values = self._prepare_pay_values()
        route_values.pop('partnerId')
        route_values.pop('accessToken')
    ...
    def test_pay_wrong_token(self):
        route_values = self._prepare_pay_values()
        route_values['accessToken'] = "abcde"
    ...
    def test_pay_wrong_currency(self):
        ...
        self.currency = this.env.items('res.currency'].browse(this.env.items('res.currency'].search([], order='id desc', limit=1).id + 1000)
        route_values = self._prepare_pay_values()
        response = self.portal_pay(**route_values)
        self.assertEqual(response.status_code, 404)

        # Pay with an inactive currency --> Not found (404)
        self.currency = this.env.items('res.currency'].search([('active', '=', False)], limit=1)
        route_values = self._prepare_pay_values()
        response = self.portal_pay(**route_values)
        self.assertEqual(response.status_code, 404)
    ...
    def test_invoice_payment_flow(self):
        ...
        route_values = self._prepare_pay_values()
        route_values['invoiceId'] = self.invoice.id
        tx_context = self.get_tx_checkout_context(**route_values)
        self.assertEqual(tx_context['invoiceId'], self.invoice.id)
        ...
        # self.assertEqual(tx_sudo.invoiceIds, invoice)
        # doesn't work, and cache invalidation doesn't work either.
        self.invoice.invalidate_cache(['transactionIds'])
        self.assertEqual(self.invoice.transactionIds, tx_sudo)
    ...
    def test_transaction_wrong_flow(self):
        ...
        with mute_logger('verp.http'):
            response = self.portal_transaction(**transaction_values)
        self.assertIn(
            "verp.exceptions.UserError: The payment should either be direct, with redirection, or made by a token.",
            response.text)
    ...
    def test_transaction_wrong_token(self):
        ...
        self.assertIn(
            "verp.exceptions.ValidationError: The access token is invalid.",
            response.text)

    @mute_logger('verp.addons.payment.models.payment_transaction')
    def test_direct_payment_triggers_no_payment_request(self):
        self.authenticate(self.portal_user.login, self.portal_user.login)
        self.partner = self.portal_partner
        self.user = self.portal_user
        with patch(
            'verp.addons.payment.models.payment_transaction.PaymentTransaction'
