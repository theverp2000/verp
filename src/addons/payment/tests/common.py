...
from verp.addons.account.models.account_payment_method import AccountPaymentMethod
from verp.fields import Command

from verp.addons.payment.tests.utils import PaymentTestUtils
...
class PaymentCommon(PaymentTestUtils):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.groupUser = cls.env.ref('base.groupUser')
        cls.groupPortal = cls.env.ref('base.groupPortal')
        cls.group_public = cls.env.ref('base.group_public')

        cls.admin_user = cls.env.ref('base.userAdmin')
        ...
        cls.portal_user = cls.env['res.users'].create({
            ...
            'groupsId': [Command.link(cls.groupPortal.id)]
        })
        cls.public_user = cls.env.ref('base.public_user')

        cls.admin_partner = cls.admin_user.partnerId
        cls.internal_partner = cls.internal_user.partnerId
        cls.portal_partner = cls.portal_user.partnerId
        cls.default_partner = cls.env['res.partner'].create({
            ...
            'city': 'Sin City',
            'zip': '1000',
            'countryId': cls.country_belgium.id,
        })

        # Create a dummy acquirer to allow basic tests without any specific acquirer implementation
        arch = """
        <form action="dummy" method="post">
            <input type="hidden" name="viewId" t-att-value="viewid"/>
            <input type="hidden" name="userId" t-att-value="userId.id"/>
        </form>
        """  # We exploit the default values `viewid` and `userId` from QWeb's rendering context
        ...
        cls.invoice = cls.env['account.move'].create({
            ...
            'line_ids': [
                (0, 0, {
                    'accountId': cls.account.id,
                    'currencyId': cls.currency_euro.id,
                    'debit': 100.0,
                    'credit': 0.0,
    ...
    def _prepare_currency(cls, currency_code):
        currency = cls.env['res.currency'].withContext(active_test=False).search(
    ...
    def _prepare_acquirer(cls, provider='none', company=None, update_values=None):
        ...
        acquirer = cls.env['payment.acquirer'].sudo().search(
            [('provider', '=', provider), ('companyId', '=', company.id)], limit=1
        )
        if not acquirer:
            ...
            if not base_acquirer:
                ...
                acquirer = base_acquirer.copy({'companyId': company.id})
    ...
    def create_transaction(self, flow, sudo=True, **values):
        default_values = {
            ...
            'acquirerId': self.acquirer.id,
            'reference': self.reference,
            'operation': f'online_{flow}',
            'partnerId': self.partner.id,
        }
        return this.env.items('payment.transaction'].sudo(sudo).create(dict(default_values, **values))
    ...
    def create_token(self, sudo=True, **values):
        ...
        return this.env.items('payment.token'].sudo(sudo).create(dict(default_values, **values))

    def _get_tx(self, reference):
        return this.env.items('payment.transaction'].sudo().search([
            ('reference', '=', reference),
        ])
    ...
    def _prepare_transaction_values(self, payment_option_id, flow):
        ...
        return {
            ...
            'currencyId': self.currency.id,
            'partnerId': self.partner.id,
            'accessToken': self._generate_test_access_token(
