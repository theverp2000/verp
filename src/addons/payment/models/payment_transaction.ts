...
class PaymentTransaction extends Model {
    ...
    async create(valuesList) {
        for (const values of valuesList) {
            ...
            if (values['operation'] === 'validation') {
                ...
                const currency = await this.env.items('res.currency').browse(values['currencyId']).exists();
     ...
     * @returns The dict of acquirer-specific create values
        :rtype: dict
     * @param provider 
     * @param values 
     * @returns 
     ...
     * @param amountToRefund (float) The amount to be refunded
     * @returns null
     */
     ...
     * @param provider (string) The provider of the acquirer handling the transaction
     * @param opts {
     *      string prefix: The custom prefix used to compute the full reference
     *      string separator: The custom separator used to separate the prefix from the suffix, and
                              passed to `_compute_reference_prefix` if it is called
     *      dict kwargs: Optional values passed to `_compute_reference_prefix` if no custom
                            prefix is provided
     * }
     ...
     * @param provider (string) The provider of the acquirer handling the transaction
     * @param separator (string) The custom separator used to separate data references
     * @param values (dict) The transaction values used to compute the reference prefix. It should
     ...
     * @param callbackModelId (number) The model on which the callback method is defined, as a
                                      `res.model` id
     * @param callbackResId (number) The record on which the callback method must be called, as an id
                                    of the callback model
     * @param callbackMethod (string) The name of the callback method
     ...
     * @returns The dict of processing values
     */
    async _getProcessingValues() {
        ...
        if (['onlineRedirect', 'validation'].includes(await this['operation'])) {
            ...
            if (bool(redirectFormView)) {  // Some acquirer don't need a redirect form
                const renderingValues = await this._getSpecificRenderingValues(processingValues);
                console.info(
    ...
    * @param processingValues (dict) The generic processing values of the transaction
    * @returns The dict of acquirer-specific processing values
     */
     ...
     * @param processingValues - dict The processing values of the transaction
     * @returns The dict of acquirer-specific rendering values
     */
     ...
     * @param amountToRefund (float) The strictly positive amount to refund, in the same currency
     ...
     * @param provider (string) The provider of the acquirer that handled the transaction
     * @param data (dict) The feedback data sent by the provider
     * @returns recordset of `payment.transaction` - The transaction
     */
    @api.model()
     ...
     * @param provider (string) The provider of the acquirer that handled the transaction
     * @param data (dict) The feedback data sent by the acquirer
     * @returns recordset of `payment.transaction` - The transaction if found
     */
    @api.model()
     ...
     * @param stateMessage (string) The reason for which the transaction is set in 'authorized' state
     * @returns void
     */
     ...
     * @param stateMessage (string) The reason for which the transaction is set in 'cancel' state
     * @returns void
     */
     ...
     * @param stateMessage (string) The reason for which the transaction is set in 'error' state
     * @returns void
     */
     ...
     * @param allowedStates (string[]) The allowed source states for the target state
     * @param targetState (string) The target state
     * @param stateMessage (string) The message to set as `state_message`
     * @returns recordset of `payment.transaction` - The recordset of transactions whose state was correctly updated
     */
    async _updateState(allowedStates, targetState, stateMessage) {
         ...
         * @param transactions (recordset) The transactions to classify, as a `payment.transaction`
                                            recordset
         * @returns A 3-items tuple of recordsets of classified transactions, in this order:
     ...
     * @param extraCreateValues (dict) Optional extra create values
     * @returns recordset of `account.payment` - The created payment
     */
     ...
     * @param message (strign) The message to be logged
     * @returns void
     */
     ...
     * @returns string - The 'transaction sent' message
     */
     ...
     * @returns recordset of `payment.transaction` - The last transaction of the recordset, sorted by id
     */
    async _getLast() {
