...
class MailThread extends AbstractModel {
    ...
    if (rcptTosLocalparts.length) {
      ...
      if (bool(destAliases)) {
        ...
        for (const alias of destAliases) {
          ...
          if (bool(route)) {
            console.info(
              'Routing mail from %s to %s with Message-Id %s: direct alias match: %s',
  ...
  /*
    ...
    def _message_route_process(self, message, message_dict, routes):
        self = self.withContext(attachments_mime_plainxml=true) # import XML attachments as text
        ...
        for model, threadId, custom_values, userId, alias in routes or ():
            ...
            Model = this.env.items(model].withContext(mail_create_nosubscribe=true, mail_create_nolog=true)
            ...
            if thread._name == 'mail.thread':  # message with parentId not linked to record
                ...
                thread = thread.withContext(mail_create_nosubscribe=not partner_from_found)
