...
class Message extends Model {
  ...
  async _messageFormat(fnames, formatReply: boolean = true) {
    ...
    for (const vals of valsList) {
      ...
      const reactionsPerContent = new DefaultDict(() => this.env.items('mail.message.reaction'));
  ...
  */
  async _notifyMessageNotificationUpdate() {
    ...
    const messagesPerPartner = new DefaultDict(() => this.env.items('mail.message'));
