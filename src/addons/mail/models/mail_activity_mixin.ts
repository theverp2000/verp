...
class MailActivityMixin extends AbstractModel {
  ...
  async activitySchedule(options: { actTypeXmlid?: string, dateDeadline?: Date, summary?: string, note?: string, actValues?: any } = {}) {
    let { actTypeXmlid, dateDeadline, summary, note, actValues } = options;
    if (this.env.context['mailActivityAutomationSkip']) {
      return false;
    }

    if (!dateDeadline) {
      dateDeadline = await _Date.contextToday(this);
    }
    // if (isInstance(dateDeadline, Date)) {
    //   console.warn("Scheduled deadline should be a date (got %s)", dateDeadline);
    // }
    let activityType;
    if (actTypeXmlid) {
      activityType = await this.env.ref(actTypeXmlid, false) || await this._defaultActivityType();
    }
    else {
      const activityTypeId = actValues['activityTypeId'] ?? false;
    ...
    for (const record of this) {
      const createVals = {
        ...
        'summary': summary || await activityType.summary,
        'automated': true,
        'note': note || await activityType.defaultNote,
        'dateDeadline': dateDeadline,
        'resModelId': modelId,
        'resId': record.id,
      }
      Object.assign(createVals, actValues);
