...
function factory(dependencies) {
    ...
    class ChannelInvitationForm extends dependencies['mail.model'] {
        ...
        async onClickInvite(ev) {
            if (this.thread.channelType === 'chat') {
                const partnersTo = [...new Set([
                    this.messaging.currentPartner.id,
                    ...this.thread.members.map(member => member.id),
                    ...this.selectedPartners.map(partner => partner.id),
                ])];
                const channel = await this.messaging.models['mail.thread'].createGroupChat({ partnersTo });
                if (this.thread.rtc) {
                    /**
                ...
                await this.env.services.rpc(({
                    ...
                    args: [[this.thread.id]],
                    kwargs: {
                        partnerIds: this.selectedPartners.map(partner => partner.id),
                        inviteToRtcCall: !!this.thread.rtc,
                    },
                }));
            }
            this.update({
        ...
        async searchPartnersToInvite() {
            ...
            try {
                ...
                const { count, partners: partnersData } = await this.env.services.rpc(
                    {
                        model: 'res.partner',
                        method: 'searchForChannelInvite',
                        kwargs: {
                            channelId: channelId,
                            searchTerm: cleanSearchTerm(this.searchTerm),
                        },
                    },
                    { shadow: true }
                );
        ...
        _computeInviteButtonText() {
            ...
            switch (this.thread.channelType) {
                case 'chat':
    ...
    ChannelInvitationForm.fields = {
        chatWindow: one2one('mail.chatWindow', {
        ...
        popoverView: one2one('mail.popoverView', {
            inverse: 'channelInvitationForm',
            isCausal: true,
        }),
        /**
    ...
    ChannelInvitationForm.modelName = 'mail.channelInvitationForm';

    return ChannelInvitationForm;
}

registerNewModel('mail.channelInvitationForm', factory);
