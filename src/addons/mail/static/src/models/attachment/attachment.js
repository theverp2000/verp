...
function factory(dependencies) {
    ...
    class Attachment extends dependencies['mail.model'] {
        ...
        _computeDefaultSource() {
            ...
            if (this.isPdf) {
                ...
                if (!this.accessToken && this.originThread && this.originThread.model === 'mail.channel') {
                    return `${pdfLib}/mail/channel/${this.originThread.id}/attachment/${this.id}`;
                }
                const accessToken = this.accessToken ? `?accessToken%3D${this.accessToken}` : '';
                return `${pdfLib}/web/content/${this.id}${accessToken}`;
            }
            ...
            const accessToken = this.accessToken ? `?accessToken=${this.accessToken}` : '';
         ...
         */
        _computeDisplayName() {
            const displayName = this.label || this.filename;
            if (displayName) {
                return displayName;
            }
            return clear();
    ...
    Attachment.fields = {
         ...
         */
        attachmentCards: one2many('mail.attachmentCard', {
            inverse: 'attachment',
            isCausal: true,
        }),
        /**
         ...
         */
        attachmentImages: one2many('mail.attachmentImage', {
            inverse: 'attachment',
            isCausal: true,
        }),
        /**
         ...
         */
        attachmentLists: many2many('mail.attachmentList', {
            inverse: 'attachments',
        }),
        attachmentViewer: many2many('mail.attachmentViewer', {
            inverse: 'attachments',
        }),
        checksum: attr(),
        /**
