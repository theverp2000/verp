...
function factory(dependencies) {
    ...
    class WelcomeView extends dependencies['mail.model'] {
        ...
        async performRpcAddGuestAsMember() {
            await this.env.services.rpc({
                route: '/mail/channel/addGuestAsMember',
                params: {
        ...
        _computeMediaPreview() {
            return (this.channel && this.channel.defaultDisplayMode === 'videoFullScreen')
                ? insertAndReplace()
                : clear();
        }
    ...
    WelcomeView.fields = {
         ...
         */
        discussPublicView: one2one('mail.discussPublicView', {
            inverse: 'welcomeView',
            readonly: true,
            required: true,
        }),
         ...
         */
        mediaPreview: one2one('mail.mediaPreview', {
    ...
    WelcomeView.modelName = 'mail.welcome.view';

    return WelcomeView;
}

registerNewModel('mail.welcome.view', factory);
