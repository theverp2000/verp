...
function factory(dependencies) {
       ...
       /**
        ...
        onClickUserName(ev) {
            ...
            this.update({
                ...
                pendingGuestName: this.messaging.currentGuest.label,
            });
        }
        ...
        _applyGuestRename() {
            if (this.hasGuestNameChanged) {
                this.messaging.models['mail.guest'].performRpcGuestUpdateName({
                    ...
                    label: this.pendingGuestName.trim(),
                });
            }
            this._resetGuestNameInput();
        }
        ...
        _applyThreadRename() {
            ...
            if (this.thread.channelType === 'chat' && newName !== this.thread.customChannelName) {
                this.thread.setCustomName(newName);
            }
            if (newName && this.thread.channelType === 'channel' && newName !== this.thread.label) {
                this.thread.rename(newName);
            }
            if (this.thread.channelType === 'group' && newName !== this.thread.label) {
                this.thread.rename(newName);
            }
        }
        ...
        _computeAvatarUrl() {
            if (this.messaging.isCurrentUserGuest) {
                ...
                return `/mail/channel/${this.thread.id}/guest/${this.messaging.currentGuest.id}/avatar128?unique=${this.messaging.currentGuest.label}`;
            }
        ...
        _computeHasGuestNameChanged() {
            return Boolean(
                ...
                this.pendingGuestName !== this.messaging.currentGuest.label
            );
        }
    ...
    ThreadViewTopbar.fields = {
         ...
         * open in the topbar.
         */
        invitePopoverView: one2one('mail.popoverView', {
            isCausal: true,
    ...
    ThreadViewTopbar.modelName = 'mail.threadViewTopbar';

    return ThreadViewTopbar;
}

registerNewModel('mail.threadViewTopbar', factory);
