...
function factory(dependencies) {
    ...
    class Discuss extends dependencies['mail.model'] {
        ...
        _created() {
            super._created();
            // Bind necessary until OWL supports arrow function in handlers: https://github.com/verp/owl/issues/876
        ...
        async handleAddChannelAutocompleteSource(req, res) {
            ...
            const items = threads.map((thread) => {
                const escapedName = owl.utils.escape(thread.label);
                return {
                    id: thread.id,
                    label: escapedName,
                    value: escapedName,
            ...
            // renderToString https://github.com/verp/owl/issues/708
            items.push({
                label: _.str.sprintf(
        ...
        async onClickStartAMeetingButton(ev) {
            const meetingChannel = await this.messaging.models['mail.thread'].createGroupChat({
                defaultDisplayMode: 'videoFullScreen',
                partnersTo: [this.messaging.currentPartner.id],
            });
        ...
        openInitThread() {
            ...
            thread.open();
            if (this.messaging.device.isMobile && thread.channelType) {
                this.update({ activeMobileNavbarTabId: thread.channelType });
            }
        }
        ...
        async openThread(thread, { focus } = {}) {
            ...
            if (!this.isOpen) {
                this.env.bus.trigger('do-action', {
                    action: 'mail.actionDiscuss',
                    options: {
                        activeId: this.threadToActiveId(this),
                        clearBreadcrumbs: false,
                        onReverseBreadcrumb: () => this.close(), // this is useless, close is called by destroy anyway
                    },
                });
            }
        }
    ...
    Discuss.fields = {
         ...
         */
        categoryChannel: one2one('mail.discussSidebarCategory', {
         ...
         */
        categoryChat: one2one('mail.discussSidebarCategory', {
        ...
        /**
         * The menuId of discuss app, received on mail/initMessaging and
        ...
        /**
         * States the `mail.threadView` displaying `this.thread`.
         */
        threadView: one2one('mail.threadView', {
            related: 'threadViewer.threadView',
        }),
        /**
         * Determines the `mail.thread.viewer` managing the display of `this.thread`.
         */
