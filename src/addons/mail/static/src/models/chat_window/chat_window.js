...
function factory(dependencies) {
    ...
    class ChatWindow extends dependencies['mail.model'] {
        ...
        _computeHasCallButtons() {
            return Boolean(this.thread) && this.thread.rtcSessions.length === 0 && ['channel', 'chat', 'group'].includes(this.thread.channelType);
        }
         ...
         * @returns {mail.chatWindow|undefined}
         */
    ...
    ChatWindow.fields = {
         ...
         */
        channelInvitationForm: one2one('mail.channelInvitationForm', {
            inverse: 'chatWindow',
            isCausal: true,
        }),
        /**
        ...
        manager: many2one('mail.chatWindowManager', {
            inverse: 'chatWindows',
            readonly: true,
        }),
        managerAsNewMessage: one2one('mail.chatWindowManager', {
    ...
    ChatWindow.modelName = 'mail.chatWindow';

    return ChatWindow;
}

registerNewModel('mail.chatWindow', factory);
