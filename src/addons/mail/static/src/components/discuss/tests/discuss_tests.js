...
import { makeTestPromise, file } from 'web.testUtils';
...
QUnit.test('sidebar: inbox with counter', async function (assert) {
    ...
    // notification expected to be counted at initMessaging
    this.data['mail.notification'].records.push({ resPartnerId: this.data.currentPartnerId });
    await this.start();
    assert.strictEqual(
        document.querySelectorAll(`
            .o_DiscussSidebarMailbox[data-thread-local-id="${
...
QUnit.test('sidebar: public/private channel rendering', async function (assert) {
    ...
    this.data['mail.channel'].records.push(
        { id: 100, name: "channel1", isPublic: 'public', },
        { id: 101, name: "channel2", isPublic: 'private' }
    );
    await this.start();
    assert.strictEqual(
        document.querySelectorAll(`.o_DiscussSidebar_categoryChannel .o_DiscussSidebarCategory_item`).length,
...
QUnit.test('sidebar: chat imStatus rendering', async function (assert) {
    assert.expect(7);

    // expected correspondent, with a random unique id that will be used to link
    // partner to chat, and various imStatus values to assert
    this.data['res.partner'].records.push(
        { id: 101, imStatus: 'offline', name: "Partner1" },
        { id: 102, imStatus: 'online', name: "Partner2" },
        { id: 103, imStatus: 'away', name: "Partner3" }
    );
...
QUnit.test('open channel from activeId as channel id', async function (assert) {
    assert.expect(1);
    ...
    assert.containsOnce(
        ...
        `,
        "should have channel with ID 20 open in Discuss when providing activeId 20"
    );
});
...
QUnit.test('basic rendering of message', async function (assert) {
    ...
    assert.strictEqual(
        ...
        "/mail/channel/20/partner/11/avatar128",
...
QUnit.test('basic rendering of squashed message', async function (assert) {
    ...
    this.data['mail.message'].records.push(
        {
            authorId: 11, // must be same author as other message
            ...
            messageType: 'comment', // must be a squash-able type-
            model: 'mail.channel', // to link message to channel
            resId: 20, // id of related channel
        },
        {
            authorId: 11, // must be same author as other message
    ...
    this.data['mail.message'].records.push(
        {
            ...
            model: 'mail.channel', // to link message to channel
            needaction: true,
            needactionPartnerIds: [this.data.currentPartnerId], // for consistency
            resId: 20, // id of related channel
        },
        {
            authorId: 11, // must be same author as other message
...
QUnit.test('auto_scroll to bottom of thread', async function (assert) {
    // AKU TODO: thread specific test
    assert.expect(2);
...
QUnit.test('redirect to author (open chat)', async function (assert) {
    ...
    this.data['res.users'].records.push({ partnerId: 7 });
    this.data['mail.channel'].records.push(
        // channel expected to be found in the sidebar
        {
...
QUnit.test('starred: unstar all', async function (assert) {
    ...
    this.data['mail.message'].records.push(
        { body: "not empty", starredPartnerIds: [this.data.currentPartnerId] },
        { body: "not empty", starredPartnerIds: [this.data.currentPartnerId] }
    );
    await this.start({
        discuss: {
...
QUnit.test('toggleStar message', async function (assert) {
    assert.expect(16);
...
QUnit.test('post a simple message', async function (assert) {
    ...
    await this.start({
        ...
        async mockRPC(route, args) {
            ...
            if (route === '/mail/message/post') {
                ...
                assert.strictEqual(
                    args.post_data.messageType,
                    "comment",
                    "should set message type as 'comment'"
                );
                assert.strictEqual(
                    args.post_data.subtypeXmlid,
                    "mail.mtComment",
                    "should set subtypeXmlid as 'comment'"
                );
    ...
    assert.verifySteps(['messagePost']);
    assert.strictEqual(
        document.querySelector(`.o_ComposerTextInput_textarea`).value,
        "",
...
QUnit.test('rendering of inbox message', async function (assert) {
    ...
    this.data['mail.message'].records.push({
        ...
        needaction: true,
        needactionPartnerIds: [this.data.currentPartnerId], // for consistency
        recordName: 'Refactoring', // random name, will be asserted in the test
...
QUnit.test('receive new needaction messages', async function (assert) {
    ...
    await afterNextRender(() => {
        this.widget.call('busService', 'trigger', 'notification', [{
            type: 'mail.message/inbox',
            payload: {
                body: "not empty",
                id: 100,
                needactionPartnerIds: [3],
                model: 'res.partner',
                resId: 20,
            },
        }]);
    ...
    this.data['mail.message'].records.push(
        {
            ...
            id: 1,
            needaction: true,
            needactionPartnerIds: [this.data.currentPartnerId],
        },
        {
            body: "not empty",
            id: 2,
...
QUnit.test('receive new chat message: out of verp focus (notification, channel)', async function (assert) {
    assert.expect(4);

    // channel expected to be found in the sidebar
    // with a random unique id that will be referenced in the test
    this.data['mail.channel'].records.push({ id: 20, channelType: 'chat' });
    const bus = new Bus();
    bus.on('setTitlePart', null, payload => {
        assert.step('setTitlePart');
    ...
    // simulate receiving a new message with verp focused
    await afterNextRender(() => {
        this.widget.call('busService', 'trigger', 'notification', [{
    ...
    assert.verifySteps(['setTitlePart']);
});

QUnit.test('receive new chat message: out of verp focus (notification, chat)', async function (assert) {
    assert.expect(4);

    // chat expected to be found in the sidebar with the proper channelType
    // and a random unique id that will be referenced in the test
    this.data['mail.channel'].records.push({ channelType: "chat", id: 10 });
    const bus = new Bus();
    bus.on('setTitlePart', null, payload => {
        assert.step('setTitlePart');
        assert.strictEqual(payload.part, '_chat');
...
QUnit.test('receive new chat messages: out of verp focus (tab title)', async function (assert) {
    assert.expect(12);
    ...
    // with random unique id and name that will be referenced in the test
    this.data['mail.channel'].records.push(
        { channelType: 'chat', id: 20, isPublic: 'private' },
        { channelType: 'chat', id: 10, isPublic: 'private' },
    );
    const bus = new Bus();
    bus.on('setTitlePart', null, payload => {
        step++;
    ...
    // simulate receiving a new message in chat 20 with verp focused
    await afterNextRender(() => {
        this.widget.call('busService', 'trigger', 'notification', [{
            type: 'mail.channel/newMessage',
            payload: {
    ...
    // simulate receiving a new message in chat 10 with verp focused
    await afterNextRender(() => {
        this.widget.call('busService', 'trigger', 'notification', [{
            type: 'mail.channel/newMessage',
            payload: {
    ...
    // simulate receiving another new message in chat 10 with verp focused
    await afterNextRender(() => {
        this.widget.call('busService', 'trigger', 'notification', [{
            type: 'mail.channel/newMessage',
            payload: {
