...
import { makeTestPromise } from 'web.testUtils';
...
QUnit.test('no new message when discuss is open', async function (assert) {
    ...
    await afterNextRender(() => this.discussWidget.onDetachCallback());
    assert.strictEqual(
        document.querySelectorAll(`.o_MessagingMenu_newMessageButton`).length,
        1,
    ...
    await afterNextRender(() => this.discussWidget.onAttachCallback());
    assert.strictEqual(
        document.querySelectorAll(`.o_MessagingMenu_newMessageButton`).length,
        0,
...
QUnit.test('channel preview: basic rendering', async function (assert) {
    ...
    this.data['mail.message'].records.push({
        authorId: 7, // not current partner, will be asserted in the test
...
QUnit.test('filtered previews', async function (assert) {
    ...
    this.data['mail.channel'].records.push(
        { channelType: "chat", id: 10 },
        { id: 20 },
    );
    this.data['mail.message'].records.push(
        {
            model: 'mail.channel', // to link message to channel
            resId: 10, // id of related channel
        },
        {
            model: 'mail.channel', // to link message to channel
            resId: 20, // id of related channel
...
QUnit.test('rendering with VerpBot has a request (default)', async function (assert) {
    assert.expect(4);
    ...
    assert.ok(
        ...
        "should display a notification counter next to the messaging menu for VerpBot request"
    );
    assert.strictEqual(
        document.querySelector('.o_MessagingMenu_counter').textContent,
        "1",
    ...
    assert.strictEqual(
        ...
        'VerpBot has a request',
        "notification should display that VerpBot has a request"
    );
});

QUnit.test('rendering without VerpBot has a request (denied)', async function (assert) {
    assert.expect(2);
...
QUnit.test('rendering without VerpBot has a request (accepted)', async function (assert) {
    assert.expect(2);
