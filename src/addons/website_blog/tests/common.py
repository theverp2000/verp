...
class TestWebsiteBlogCommon(common.TransactionCase):
    def setUp(self):
        ...
        group_employee_id = self.ref('base.groupUser')
        group_public_id = self.ref('base.group_public')

        self.user_employee = Users.withContext({'no_reset_password': True}).create({
            'name': 'Armande Employee',
            'login': 'armande',
            'email': 'armande.employee@example.com',
            'notification_type': 'inbox',
            'groupsId': [(6, 0, [group_employee_id])]
        })
        self.user_blogmanager = Users.withContext({'no_reset_password': True}).create({
            'name': 'Bastien BlogManager',
            'login': 'bastien',
            'email': 'bastien.blogmanager@example.com',
            'notification_type': 'inbox',
            'groupsId': [(6, 0, [group_blog_manager_id, group_employee_id])]
        })
        self.user_public = Users.withContext({'no_reset_password': True}).create({
            'name': 'Cedric Public',
            'login': 'cedric',
            'email': 'cedric.public@example.com',
            'notification_type': 'inbox',
            'groupsId': [(6, 0, [group_public_id])]
        })
