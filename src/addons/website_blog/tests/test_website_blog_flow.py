...
from verp.addons.website_blog.tests.common import TestWebsiteBlogCommon
...
class TestWebsiteBlogFlow(TestWebsiteBlogCommon):
    def setUp(self):
        ...
        group_portal = self.env.ref('base.groupPortal')
        self.user_portal = self.env['res.users'].withContext({'no_reset_password': True}).create({
            'name': 'Dorian Portal',
            'login': 'portal_user',
            'email': 'portal_user@example.com',
            'notification_type': 'inbox',
            'groupsId': [(6, 0, [group_portal.id])]
        })
    ...
    def test_website_blog_followers(self):
        ...
        publish_message = next((m for m in self.test_blog_post.blog_id.messageIds if m.subtypeId.id == self.ref('website_blog.mt_blog_blog_published')), None)
        self.assertEqual(
        ...
        self.test_blog_post.sudo().message_post(
            ...
            messageType='comment',
            authorId=self.user_employee.partner_id.id,
