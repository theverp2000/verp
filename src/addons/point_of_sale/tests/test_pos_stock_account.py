...
class TestPoSStock(TestPoSCommon):
    ...
    def setUp() {
        ...
        # change cost(standardPrice) of anglo products
        # then set inventory from 10 -> 15
        self.product1.write({'standardPrice': 6.0})
        self.product2.write({'standardPrice': 6.0})
        self.adjust_inventory([self.product1, self.product2, self.product3], [15, 15, 15])

        # change cost(standardPrice) of anglo products
        # then set inventory from 15 -> 25
        self.product1.write({'standardPrice': 13.0})
        self.product2.write({'standardPrice': 13.0})
    ...
    def test_01_orders_no_invoiced() {
        """
        ...
        def _before_closing_cb():
            ...
            for order in self.posSession.orderIds:
                self.assertEqual(order.pickingIds[0].state, 'done', 'Picking should be in done state.')
                self.assertTrue(all(state == 'done' for state in order.pickingIds[0].move_lines.mapped('state')), 'Move Lines should be in done state.')
        ...
        self._run_test({
            ...
            'journal_entries_after_closing': {
                'session_journal_entry': {
                    'lineIds': [
                        {'accountId': self.sales_account.id, 'partnerId': False, 'debit': 0, 'credit': 1010.0, 'reconciled': False},
                        {'accountId': self.expense_account.id, 'partnerId': False, 'debit': 327, 'credit': 0, 'reconciled': False},
                        {'accountId': self.cash_pm1.receivable_account_id.id, 'partnerId': False, 'debit': 1010.0, 'credit': 0, 'reconciled': True},
                        {'accountId': self.output_account.id, 'partnerId': False, 'debit': 0, 'credit': 327, 'reconciled': True},
                    ],
                },
                'cash_statement': [
                    ((1010.0, ), {
                        'lineIds': [
                            {'accountId': self.cash_pm1.journalId.default_account_id.id, 'partnerId': False, 'debit': 1010.0, 'credit': 0, 'reconciled': False},
                            {'accountId': self.cash_pm1.receivable_account_id.id, 'partnerId': False, 'debit': 0, 'credit': 1010.0, 'reconciled': True},
                        ]
                    }),
                ],
                'bank_payments': [],
    ...
    def test_02_orders_with_invoice() {
        """
        ...
        self._run_test({
            ...
            'journal_entries_before_closing': {
                '00100-010-0003': {
                    'payments': [
                        ((self.cash_pm1, 360.0), {
                            'lineIds': [
                                {'accountId': self.c1_receivable.id, 'partnerId': self.customer.id, 'debit': 0, 'credit': 360.0, 'reconciled': True},
                                {'accountId': self.pos_receivable_account.id, 'partnerId': False, 'debit': 360.0, 'credit': 0, 'reconciled': False},
                            ]
                        }),
                    ],
                },
            ...
            'journal_entries_after_closing': {
                'session_journal_entry': {
                    'lineIds': [
                        {'accountId': self.sales_account.id, 'partnerId': False, 'debit': 0, 'credit': 650, 'reconciled': False},
                        {'accountId': self.expense_account.id, 'partnerId': False, 'debit': 206, 'credit': 0, 'reconciled': False},
                        {'accountId': self.cash_pm1.receivable_account_id.id, 'partnerId': False, 'debit': 1010.0, 'credit': 0, 'reconciled': True},
                        {'accountId': self.pos_receivable_account.id, 'partnerId': False, 'debit': 0, 'credit': 360, 'reconciled': True},
                        {'accountId': self.output_account.id, 'partnerId': False, 'debit': 0, 'credit': 206, 'reconciled': True},
                    ],
                },
                'cash_statement': [
                    ((1010.0, ), {
    ...
    def test_03_order_product_w_owner() {
        ...
        for order in self.posSession.orderIds:
            self.assertEqual(order.pickingIds[0].state, 'done', 'Picking should be in done state.')
            self.assertTrue(all(state == 'done' for state in order.pickingIds[0].move_lines.mapped('state')), 'Move Lines should be in done state.')
            self.assertTrue(self.partner_a == order.pickingIds[0].move_lines[0].moveLineIds[0].ownerId, 'Move Lines Owner should be taken into account.')

        # close the session
        self.posSession.action_pos_session_validate()

    def test_04_order_refund() {
        self.categ4 = this.env.items('product.category'].create({
            'name': 'Category 4',
        ...
        refund_action = this.env.items('pos.order'].browse(order[0]['id']).refund()
        refund = this.env.items('pos.order'].browse(refund_action['resId'])

        payment_context = {"activeIds": refund.ids, "activeId": refund.id}
        refund_payment = this.env.items('pos.make.payment'].withContext(**payment_context).create({
        ...
        self.posSession.action_pos_session_validate()
        expense_account_move_line = this.env.items('account.move.line'].search([('accountId', '=', self.expense_account.id)])
