...
# Recommends: antiword, graphviz, ghostscript, javascript-gevent, poppler-utils
...
# javascript-usb in wheezy is too old
...
wget 'https://nightly.theverp.com/master/iotbox/eftdvs' -P /usr/local/bin/
chmod +x /usr/local/bin/eftdvs
wget 'https://nightly.theverp.com/master/iotbox/eftapi.so' -P /usr/lib/

groupadd usbusers
usermod -a -G usbusers pi
usermod -a -G lp pi
usermod -a -G input lightdm
mkdir -v /var/log/verp
chown pi:pi /var/log/verp
chown pi:pi -R /home/pi/verp/
...
echo "* * * * * rm /var/run/verp/sessions/*" | crontab -
...
echo "addons/hw_drivers/iot_devices/" > /home/pi/verp/.git/info/exclude
