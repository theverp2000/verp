...
REPO=https://github.com/verp/verp.git
...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/verp"
...
verp/
verp-bin" | tee --append .git/info/sparse-checkout > /dev/null
...
# copy iotbox version
mkdir -pv "${MOUNT_POINT}"/var/verp
echo "${VERSION_IOTBOX}" | tee "${MOUNT_POINT}"/var/verp/iotbox_version "${MOUNT_POINT}"/home/pi/iotbox_version
