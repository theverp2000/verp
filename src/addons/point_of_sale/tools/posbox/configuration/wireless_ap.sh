...
FORCE_HOST_AP="${1}"
WIRED_IP=$(javascript -c "import netifaces as ni; print(ni.ifaddresses('eth0').get(ni.AF_INET) and ni.ifaddresses('eth0')[ni.AF_INET][0]['addr'] or '')")
...
if [ -z "${WIRED_IP}" ] ; then
	...
	if [ -f "${WIFI_NETWORK_FILE}" ] && [ -z "${FORCE_HOST_AP}" ] ; then
		...
		/home/pi/verp/addons/point_of_sale/tools/posbox/configuration/connect_to_wifi.sh &
	else
		...
		nginx -c /home/pi/verp/addons/point_of_sale/tools/posbox/configuration/nginx_ap.conf

		service verp restart
	fi
# wired
else
	killall nginx
