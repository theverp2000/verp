...
class PosConfig extends Model {
    ...
    async _checkGroupsImplied() {
        for (const posConfig of this) {
            for (const fieldName of posConfig.fieldsGetKeys().filter(f => f.startsWith('group'))) {
                ...
                if (['boolean', 'selection'].includes(field.type) && ('impliedGroup' in field)) {
    ...
    async assignPaymentJournals(company) {
        for (const posConfig of this) {
            ...
            const Journal = this.env.items('account.journal');
            const cashJournal = await Journal.search([['companyId', '=', company.id], ['type', '=', 'cash']], {limit: 1});
            const bankJournal = await Journal.search([['companyId', '=', company.id], ['type', '=', 'bank']], {limit: 1});
    ...
    async generatePosJournal(company) {
        for (const posConfig of this) {
            ...
            const Journal = this.env.items('account.journal');
            let posJournal = await Journal.search([['companyId', '=', company.id], ['code', '=', 'POSS']]);
            if (! bool(posJournal)) {
                posJournal = await Journal.create({
                    'type': 'general',
