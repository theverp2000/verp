...
class PosOrder extends Model {
    ...
    async _processOrder(order, draft, existingOrder) {
        ...
        const self = await this.withCompany(await posOrder.companyId);
        await self._processPaymentLines(order, posOrder, posSession, draft);
     ...
     * Create account.bank.statement.lines from the dictionary given to the parent function.
        ...
        If the paymentLine is an updated version of an existing one, the existing paymentLine will first be
        removed before making a new one.

     * @param posOrder dictionary representing the order.
     * @param order Order object the payment lines should belong to.
     * @param posSession PoS session the order was created in.
     * @param draft Indicate that the pos_order is not validated yet.
     */
     ...
     * @returns list of db-ids for the removed orders.
     */
    @api.model()
...
class PosOrderLine extends Model {
    ...
    async _orderLineFields(line, sessionId?: any) {
        if (bool(line) && !('label' in line[2])) {
            const session = sessionId ? await this.env.items('pos.session').browse(sessionId).exists() : null;
     ...
     * @param refundOrder the pre-created refund order
     * @param PosOrderLineLot the pre-created Pack operation Lot
     * @returns dictionary of data which is for creating a refund order line from the original line
     */
    ...
    async _launchStockRuleFromPosOrderLines() {
        ...
        for (let line of this) {
            ...
            procurements.push(this.env.items('procurement.group').Procurement(
     ...
     * @param stockMoves recordset of `stock.move`, used for fifo/avco lines
     */
...
class ReportSaleDetails extends AbstractModel {
     ...
     * @param dateStart string - The dateTime to start, default today 00:00:00.
     * @param dateStop string - The dateTime to stop, default dateStart + 23:59:59.
     * @param configIds number[] - Pos Config id's to include.
     * @param sessionIds number[] - Pos Config id's to include.
