    ...
    async _actionDone() {
        ...
        for (const rec of this) {
            ...
            if (await posOrder.toShip && ! await posOrder.toInvoice) {
                const costPerAccount = new DefaultMapKey(() => 0.0, (items) => String(items.map(i => i.id)));
...
class StockMove extends Model {
    ...
    async _prepareLinesDataDict(orderLines) {
        const linesData = new DefaultDict(() => new Dict());
                        ...
                        if (bool(existingLots)) {
                            ...
                            if (bool(existingLot)) {
                                ...
                                if (!floatIsZero(availableQuantity, {precisionRounding: await (await product.uomId).rounding})) {
                                    await move._updateReservedQuantity(qty, Math.min(qty, availableQuantity), await move.locationId, {lotId: existingLot});
                                    continue;
                                }
                            }
                        }
