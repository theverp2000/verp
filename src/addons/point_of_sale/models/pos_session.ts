...
class PosSession extends Model {
    ...
    async _accumulateAmounts(data) {
        ...
        combineReceivablesBank = new DefaultMapKey(amounts),
        combineReceivablesCash = new DefaultMapKey(amounts),
        combineReceivablesPayLater = new DefaultMapKey(amounts),
        combineInvoiceReceivables = new DefaultMapKey(amounts),
        splitInvoiceReceivables = new DefaultMapKey(amounts),
        sales = new DefaultMapKey(amounts, item => String(item)),
        stockExpense = new DefaultMapKey(amounts),
        stockReturn = new DefaultMapKey(amounts),
        stockOutput = new DefaultMapKey(amounts),
        taxes = new DefaultMapKey(taxAmounts, item => String(item)),
        ...
        combineInvPaymentReceivableLines = new DefaultMapKey(() => this.env.items('account.move.line')),
        splitInvPaymentReceivableLines = new DefaultMapKey(() => this.env.items('account.move.line')),
        ...
        for (const order of orderIds) {
            ...
            if (!orderIsInvoiced) {
                const orderTaxes = new DefaultMapKey(taxAmounts);
        ...
        if (await company.angloSaxonAccounting) {
            ...
            if (globalSessionPickings.ok) {
                const stockMoves = await (await this.env.items('stock.move').sudo()).search([
                    ['pickingId', 'in', globalSessionPickings.ids],
                    ['companyId.angloSaxonAccounting', '=', true],
                    ['productId.categId.propertyValuation', '=', 'auto'],
                ]);
                for (const move of stockMoves) {
    ...
    async _createCashStatementLinesAndCashMoveLines(data) {
        ...
        const splitCashStatementLineVals = new DefaultMapKey(() => []);
        const splitCashReceivableVals = new DefaultMapKey(() => []);
        ...
        const combineCashStatementLineVals = new DefaultMapKey(() => []);
        const combineCashReceivableVals = new DefaultMapKey(() => []);
    ...
    async _createInvoiceReceivableLines(data) {
        ...
        combineInvoiceReceivableVals = new DefaultMapKey(() => []),
        splitInvoiceReceivableVals = new DefaultMapKey(() => []),
    ...
    async _createStockOutputLines(data) {
        ...
        stockOutputVals = new DefaultMapKey(() => []),
    ...
    async tryCashInOut(type, amount, reason, extras) {
        ...
        await (await (await (await this.env.items('cash.box.out'))
            ...
            .create({'amount': sign * amount, 'label': reason}))
            .run();
