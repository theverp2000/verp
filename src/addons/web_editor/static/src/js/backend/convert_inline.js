/** @verp-module alias=web_editor.convertInline */
'use strict';

import FieldHtml from 'web_editor.field.html';
import { isBlock, rgbToHex } from '../../../lib/verp-editor/src/utils/utils';
...
function addTables($editable) {
    ...
    for (const snippet of editable.querySelectorAll('.o-mail-snippet-general, .o-layout')) {
...
function bootstrapToTable($editable) {
    ...
    for (const rowInColumn of [...editable.querySelectorAll('.row')].filter(row => RE_COL_MATCH.test(row.parentElement.className))) {
        _wrap(rowInColumn, 'div', 'o-fake-table');
    }
    ...
    for(const masonryTopInnerContainer of editable.querySelectorAll('.s-masonry-block > .container')) {
        masonryTopInnerContainer.style.setProperty('height', '100%');
    }
    for (const masonryGrid of editable.querySelectorAll('.o-masonry-grid-container')) {
        masonryGrid.style.setProperty('padding', 0);
        for (const fakeTable of [...masonryGrid.children].filter(c => c.classList.contains('o-fake-table'))) {
            fakeTable.style.setProperty('height', _getHeight(fakeTable));
        }
    }
    for (const masonryRow of editable.querySelectorAll('.o-masonry-grid-container > .o-fake-table > .row.h-100')) {
    ...
    for (const container of [...editable.querySelectorAll('.container, .container-fluid, .o-fake-table')].filter(n => [...n.children].some(c => c.classList.contains('row')))) {
        // TABLE
        const table = _createTable(container.attributes);
        for (const child of [...container.childNodes]) {
            table.append(child);
        }
        table.classList.remove('container', 'container-fluid', 'o-fake-table');
        if (!table.className) {
...
function classToStyle($editable, cssRules) {
    ...
    for (const node of nodeToRules.keys()) {
        ...
        if (node.nodeName === 'IMG' && node.classList.contains('s-media-list-img')) {
        ...
        if (node.nodeName === 'A' && node.classList.contains('btn') && !node.classList.contains('btn-link') && !node.children.length) {
            ...
            writes.push(() => { _wrap(node, 'p', 'o-outlook-hack', 'text-align:center;margin:0'); });
        }
    };
...
function toInline($editable, cssRules, $iframe) {
    $editable.removeClass('verp-editor-editable');
    const editable = $editable.get(0);
    ...
    $editable.addClass('verp-editor-editable');
}
/**
...
function fontToImg($editable) {
    const editable = $editable.get(0);
    const fonts = verp.__DEBUG__.services["wysiwyg.fonts"];
    ...
    for (const font of editable.querySelectorAll('.fa')) {
        ...
        if (content) {
            ...
            image.setAttribute('src', `/web_editor/fontToImg/${content.charCodeAt(0)}/${window.encodeURI(color)}/${window.encodeURI(bg)}/${Math.max(1, Math.round(intrinsicWidth))}x${Math.max(1, Math.round(intrinsicHeight))}`);
...
function formatTables($editable) {
    const editable = $editable.get(0);
    const writes = [];
    for (const table of editable.querySelectorAll('table.o-mail-snippet-general, .o-mail-snippet-general table')) {
                ...
                for (const selector of selectorText.split(RE_COMMAS_OUTSIDE_PARENTHESES)) {
                    if (selector && !SELECTORS_IGNORE.test(selector)) {
                        ...
                        if (selector === 'body') {
                            ...
                            // 'o-layout'. Give it the body's styles so they can
                            // trickle down.
                            cssRules.push({ selector: '.o-layout', rawRule: subRule, specificity: 1 });
                        }
                    }
                }
            }
...
function _createTable(attributes = []) {
    ...
    if (table.classList.contains('o-layout')) {
