verp.define('web_editor.field.html', function (require) {
'use strict';
 ...
 *  - codeview
 */
var FieldHtml = basicFields.DebouncedField.extend(TranslatableFieldMixin, {
    description: _lt("Html"),
    className: 'oe-form-field oe-form-field-html',
    ...
    customEvents: {
        wysiwygFocus: '_onWysiwygFocus',
        wysiwygBlur: '_onWysiwygBlur',
        wysiwygChange: '_onchange',
        wysiwygAttachment: '_onAttachmentChange',
    },
    /**
     * @override
     */
    willStart: async function () {
        ...
        this._onupdateIframeId = 'onLoad' + _.uniqueId('FieldHtml_');
        await this._super();
        ...
        if (this.nodeOptions.cssEdit || this.nodeOptions['style-inline']) {
            this.cssEdit = await ajax.loadAsset(this.nodeOptions.cssEdit || 'web_editor.assetsEditHtmlField');
        }
    },
    /**
     * @override
    ...
    _getWysiwygOptions: function () {
        return Object.assign({}, this.nodeOptions, {
            ...
            collaborationChannel: !!this.nodeOptions.collaborative && {
                ...
                collaborationResId: parseInt(this.resId),
            },
            ...
            value: this.value,
            allowCommandVideo: Boolean(this.nodeOptions.allowCommandVideo) && (!this.field.sanitize || !this.field.sanitizeTags),
            mediaModalParams: {
    ...
    /**
     * triggerUp 'fieldChanged' add record into the "ir.attachment" field found in the view.
    ...
    _renderReadonly: function () {
        ...
        if (this.nodeOptions.cssReadonly) {
            this.$iframe = $('<iframe class="o-readonly d-none"/>');
            ...
            window.top[this._onupdateIframeId] = function (_avoidDoubleLoad) {
                ...
                self.$content = $('#iframeTarget', self.$iframe[0].contentWindow.document.body);
                resolver();
                self.triggerUp('iframeUpdated', { $iframe: self.$iframe });
            };
                ...
                ajax.loadAsset(self.nodeOptions.cssReadonly).then(function (asset) {
                    ...
                    cwindow.document
                        ...
                        .write(
                            ...
                            '</head>\n' +
                            '<body class="o-in-iframe o-readonly" style="overflow: hidden;">\n' +
                                '<div id="iframeTarget">' + value + '</div>\n' +
            ...
            this.$content = $('<div class="o-readonly"/>').html(value);
        ...
        def.then(function () {
            if (!self.hasReadonlyModifier) {
                self.$content.on('click', 'ul.o-checklist > li', self._onReadonlyClickChecklist.bind(self));
            }
            if (self.$iframe) {
    ...
    _onchange: function (ev) {
        ...
        var $lis = this.$content.find('.note-editable ul.o-checklist > li:not(:has(> ul.o-checklist))');
        if (!$lis.length) {
            return;
        }
        var max = 0;
    ...
    _onReadonlyClickChecklist: function (ev) {
        ...
        ev.stopPropagation();
        ev.preventDefault();
        var checked = $(ev.target).hasClass('o-checked');
    ...
    _onLoadWysiwyg: function () {
        ...
        if (this.nodeOptions.cssEdit && this.wysiwyg) {
            $container = this.wysiwyg.$iframeBody.find('.email-designer-top-actions');
        } else {
            $container = this.$el;
            $button.css({
                'font-size': '15px',
                position: 'absolute',
                top: '5px',
                [_t.database.parameters.direction === 'rtl' ? 'left' : 'right']: verp.debug && this.nodeOptions.codeview ? '40px' : '5px',
            });
        }
        $container.append($button);
        if (verp.debug && this.nodeOptions.codeview) {
            const $codeviewButtonToolbar = $(`
                <div id="codeview-btn-group" class="btn-group">
                    <button class="o-codeview-btn btn btn-primary">
                        <i class="fa fa-code"></i>
                    </button>
                </div>
            `);
            this.$floatingCodeViewButton = $codeviewButtonToolbar.clone();
            this._$codeview = $('<textarea class="o-codeview d-none"/>');
