...
@MetaModel.define()
class ReportBankBook extends AbstractModel {
    static _module = module;
    static _name = 'report.account.daily.bankbook';
    static _description = 'Bank Book';

    /**
     * @param accounts the recordset of accounts
     * @param initBalance boolean value of initialBalance
     * @param sortby sorting by date or partner and journal
     * @param displayAccount type of account(receivable, payable and both)
     * @returns a dictionary of accounts with following key and value {
            ...
            'amountCurrency': sum of amountCurrency,
    ...
    async _getAccountMoveEntry(accounts, initBalance, sortby, displayAccount) {
        ...
        if (!bool(accounts)) {
            ...
            accounts = await this.env['account.account'].search([['id', 'in', accounts]]);
        }
        const sql = `SELECT l.id AS lid, l."accountId" AS "accountId", l.date AS ldate, j.code AS lcode, l."currencyId", l."amountCurrency", l.ref AS lref, l.label AS llabel, COALESCE(l.debit,0) AS debit, 
