...
class Lead extends Model {
    ...
    */
    ...
    async _updateAutomatedProbabilities() {
        ...
        const probabilityLeads = new DefaultDict(() => []);
        for (const [leadId, probability] of sorted(Object.entries(leadProbabilities), ([key]) => key)) {
    ...
    async _plsGetSafeStartDate() {
        const strDate = await (await this.env.items('ir.config.parameter').sudo()).getParam('crm.plsStartDate');
    ...
    async _plsGetLeadPlsValues(domain = []) {
        ...
        if (bool(domain)) {
            ...
            if (useTags) {
                ...
                args.push(this._cr._gen.quote('tagId'));
