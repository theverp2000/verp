    ...
    hooks.beforeEach(async () => {
        serverData = {
            ...
            views: {
                ...
                "foo,false,graph": `<graph jsClass="forecast_graph"/>`,
                "foo,false,search": `
                    <search>
                        <filter name="forecastFilter" string="Forecast Filter" context="{ 'forecastFilter': 1 }"/>
                        <filter name="group_by_bar" string="Bar" context="{ 'groupby': 'bar' }"/>
                        <filter name="group_by_date_field" string="Date Field" context="{ 'groupby': 'date_field' }"/>
                    </search>
                `,
            },
        };
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            const LegacyForecastView = AbstractView.extend({
                displayName: _lt("Legacy toy view"),
                icon: "fa fa-bars",
                multiRecord: true,
                viewType: "legacy_toy",
