...
from verp import exceptions, Command, fields
from verp.tests import Form
from verp.addons.mrp.tests.common import TestMrpCommon
from verp.tools import float_compare, float_round, float_repr
...
class TestBoM(TestMrpCommon):
    ...
    def test_10_variants(self):
        test_bom = self.env['mrp.bom'].create({
            ...
            'operationIds': [
                Command.create({
                    ...
                    'workcenterId': self.workcenter_1.id,
                    'timeCycle': 12,
                    'sequence': 1
                }),
                Command.create({
            ...
            'byproduct_ids': [
                Command.create({
                    ...
                    'productUomId': self.product_1.uomId.id,
                    'productQty': 1,
                }),
                Command.create({
                    'productId': self.product_2.id,
                    'productUomId': self.product_2.uomId.id,
                    'productQty': 1,
                    'bom_product_template_attribute_value_ids': [Command.link(self.product_7_attr1_v1.id)]
                }),
                Command.create({
                    'productId': self.product_3.id,
                    'productUomId': self.product_3.uomId.id,
                    'productQty': 1,
                    'bom_product_template_attribute_value_ids': [Command.link(self.product_7_attr1_v2.id)]
                }),
            ],
        ...
        mrp_order_form.productId = self.product_7_3
        mrp_order = mrp_order_form.save()
        self.assertEqual(mrp_order.bomId, test_bom)
        self.assertEqual(len(mrp_order.workorderIds), 1)
        self.assertEqual(mrp_order.workorderIds.operationId, test_bom.operationIds[0])
        self.assertEqual(len(mrp_order.move_byproduct_ids), 1)
        self.assertEqual(mrp_order.move_byproduct_ids.productId, self.product_1)

        mrp_order_form = Form(self.env['mrp.production'])
        mrp_order_form.productId = self.product_7_1
        mrp_order_form.productId = self.env['product.product']  # Check form
        mrp_order_form.productId = self.product_7_1
        mrp_order_form.bomId = self.env['mrp.bom']  # Check form
        mrp_order_form.bomId = test_bom
        mrp_order = mrp_order_form.save()
        self.assertEqual(mrp_order.bomId, test_bom)
        self.assertEqual(len(mrp_order.workorderIds), 2)
        self.assertEqual(mrp_order.workorderIds.operationId, test_bom.operationIds[:2])
        self.assertEqual(len(mrp_order.move_byproduct_ids), 2)
        self.assertEqual(mrp_order.move_byproduct_ids.productId, self.product_1 | self.product_2)

        mrp_order_form = Form(self.env['mrp.production'])
        mrp_order_form.productId = self.product_7_2
        mrp_order = mrp_order_form.save()
        self.assertEqual(mrp_order.bomId, test_bom)
        self.assertEqual(len(mrp_order.workorderIds), 2)
        self.assertEqual(mrp_order.workorderIds.operationId, test_bom.operationIds[0] | test_bom.operationIds[2])
        self.assertEqual(len(mrp_order.move_byproduct_ids), 2)
        self.assertEqual(mrp_order.move_byproduct_ids.productId, self.product_1 | self.product_3)
    ...
    def test_11_multi_level_variants(self):
        ...
        test_bom_1 = self.env['mrp.bom'].create({
            'productTemplateId': self.product_5.productTemplateId.id,
            'productUomId': self.product_5.uomId.id,
            'productQty': 1.0,
            'type': 'phantom'
        })
        test_bom_1.write({
        ...
        test_bom_2.write({
            'operationIds': [
                ...
                (0, 0, {'name': 'Weld Machine', 'workcenterId': self.workcenter_1.id, 'timeCycle': 18, 'sequence': 2}),
            ]
        })
        ...
        test_bom_1.write({'pickingTypeId': self.warehouse_1.manu_type_id.id})
        self.bom_2.write({'pickingTypeId': tmp_picking_type.id})
        test_bom_2.write({'pickingTypeId': tmp_picking_type.id})
        boms, lines = test_bom_2.explode(self.product_7_1, 4)
        self.assertEqual(set((test_bom_2 | self.bom_2).ids), set([b[0].id for b in boms]))
        self.assertEqual(set((test_bom_2_l1 | test_bom_2_l4 | self.bom_2.bom_line_ids).ids), set([l[0].id for l in lines]))
        ...
        test_bom_3 = self.env['mrp.bom'].create({
            'productId': self.product_9.id,
            'productTemplateId': self.product_9.productTemplateId.id,
            'productUomId': self.product_9.uomId.id,
            'productQty': 1.0,
            'consumption': 'flexible',
            'type': 'normal'
        })
        test_bom_4 = self.env['mrp.bom'].create({
            'productId': self.product_10.id,
            'productTemplateId': self.product_10.productTemplateId.id,
            'productUomId': self.product_10.uomId.id,
            'productQty': 1.0,
            'consumption': 'flexible',
            'type': 'phantom'
        })
    ...
    def test_12_multi_level_variants2(self):
        ...
        self.env['mrp.bom'].create({
            'productTemplateId': product_template.id,
            'productQty': 1.0,
            'type': 'normal',
            'bom_line_ids': [
                (0, 0, {
        ...
        for combination, consumed_products in dict_consumed_products.items():
            product = product_template.productVariantIds.filtered(lambda p: p.product_template_attribute_value_ids == combination)
            mrp_order_form = Form(self.env['mrp.production'])
            mrp_order_form.productId = product
            mrp_order = mrp_order_form.save()

            # Check consumed materials in production order.
            self.assertEqual(mrp_order.moveRawIds.productId, consumed_products)

    def test_13_bom_kit_qty(self):
        self.env['mrp.bom'].create({
            'productId': self.product_7_3.id,
            'productTemplateId': self.product_7_template.id,
            'productUomId': self.uom_unit.id,
            'productQty': 4.0,
            'type': 'phantom',
    ...
    def test_14_bom_kit_qty_multi_uom(self):
        ...
        uom_unit = self.env.ref('uom.productUomUnit')
        ...
        self.env['mrp.bom'].create({
            'productTemplateId': product_unit.productTemplateId.id,
            'productUomId': self.uom_unit.id,
            'productQty': 1.0,
            'type': 'phantom',
            'bom_line_ids': [
                (0, 0, {
                    'productId': product_dozens.id,
                    'productQty': 1,
                    'productUomId': uom_unit.id,
                })
            ]
        })
        location = self.env.ref('stock.stock_location_stock')
    ...
    def test_13_negative_on_hand_qty(self):
        ...
        _ = self.env['mrp.bom'].create({
            'productId': self.product_2.id,
            'productTemplateId': self.product_2.productTemplateId.id,
            'productUomId': uom_unit.id,
            'productQty': 1.00,
            'type': 'phantom',
            'bom_line_ids': [
    ...
    def test_20_bom_report(self):
        ...
        bom_form_crumble.productTemplateId = crumble.productTemplateId
        bom_form_crumble.productQty = 11
        bom_form_crumble.productUomId = uom_kg
        ...
        with Form(bom_crumble) as bom:
            with bom.bom_line_ids.new() as line:
                line.productId = butter
                line.productUomId = uom_kg
                line.productQty = 5
            with bom.bom_line_ids.new() as line:
                line.productId = biscuit
                line.productUomId = uom_kg
                line.productQty = 6
            with bom.operationIds.new() as operation:
                operation.workcenterId = workcenter
                operation.name = 'Prepare biscuits'
                operation.time_cycle_manual = 5
                operation.bomId = bom_crumble  # Can't handle by the testing env
            with bom.operationIds.new() as operation:
                operation.workcenterId = workcenter
                operation.name = 'Prepare butter'
                operation.time_cycle_manual = 3
                operation.bomId = bom_crumble
            with bom.operationIds.new() as operation:
                operation.workcenterId = workcenter
        ...
        report_values = self.env['report.mrp.bomstructure']._getReportData(bomId=bom_crumble.id, searchQty=11, searchVariant=False)
        ...
        report_values = self.env['report.mrp.bomstructure']._getReportData(bomId=bom_crumble.id, searchQty=1, searchVariant=False)
        ...
        report_values_12 = self.env['report.mrp.bomstructure']._getReportData(bomId=bom_crumble.id, searchQty=12, searchVariant=False)
        report_values_22 = self.env['report.mrp.bomstructure']._getReportData(bomId=bom_crumble.id, searchQty=22, searchVariant=False)
        ...
        cream = self.env['product.product'].create({
            ...
            'type': 'product',
            'uomId': uom_litre.id,
            'uomPoId': uom_litre.id,
            'standardPrice': 5.17,
        })
        bom_form_cheese_cake = Form(self.env['mrp.bom'])
        bom_form_cheese_cake.productTemplateId = cheese_cake.productTemplateId
        bom_form_cheese_cake.productQty = 60
        bom_form_cheese_cake.productUomId = self.uom_unit
        ...
        with Form(bom_cheese_cake) as bom:
            with bom.bom_line_ids.new() as line:
                line.productId = cream
                line.productUomId = uom_litre
                line.productQty = 3
            with bom.bom_line_ids.new() as line:
                line.productId = crumble
                line.productUomId = uom_kg
                line.productQty = 5.4
            with bom.operationIds.new() as operation:
                operation.workcenterId = workcenter
                ...
                operation.bomId = bom_cheese_cake
            with bom.operationIds.new() as operation:
                operation.workcenterId = workcenter_2
                operation.name = 'Cake mounting'
                operation.time_cycle_manual = 5
                operation.bomId = bom_cheese_cake

        # TEST CHEESE BOM STRUCTURE VALUE WITH BOM QUANTITY
        report_values = self.env['report.mrp.bomstructure']._getReportData(bomId=bom_cheese_cake.id, searchQty=60, searchVariant=False)
        ...
        for component_line in report_values['lines']['components']:
            ...
            if component_line['prod_id'] == crumble.id:
                ...
                crumble_cost = self.env['report.mrp.bomstructure']._getReportData(bomId=bom_crumble.id, searchQty=5.4, searchVariant=False)['lines']['total']
    ...
    def test_bom_report_dozens(self):
        ...
        bom_form_drawer.productTemplateId = drawer.productTemplateId
        bom_form_drawer.productQty = 11
        bom_form_drawer.productUomId = uom_dozen
        ...
        with Form(bom_drawer) as bom:
            with bom.bom_line_ids.new() as line:
                line.productId = screw
                line.productUomId = uom_unit
                line.productQty = 5
            with bom.operationIds.new() as operation:
                operation.workcenterId = workcenter
                operation.name = 'Screw drawer'
                operation.time_cycle_manual = 5
                operation.bomId = bom_drawer

        # TEST BOM STRUCTURE VALUE WITH BOM QUANTITY
        report_values = self.env['report.mrp.bomstructure']._getReportData(bomId=bom_drawer.id, searchQty=11, searchVariant=False)
        # 5 min 'Prepare biscuits' + 3 min 'Prepare butter' + 5 min 'Mix manually' = 13 minutes
    ...
    def test_21_bom_report_variant(self):
        ...
        self.paint_color_attribute_line = self.env['product.template.attribute.line'].create({
            'productTemplateId': self.paint.id,
            'attribute_id': self.color_attribute.id,
            'value_ids': [(6, 0, [self.color_red.id, self.color_blue.id])],
        })
        ...
        self.paint.productVariantIds.write({'standardPrice': 20})
        ...
        self.dashboard_gps_attribute_line = self.env['product.template.attribute.line'].create({
            'productTemplateId': self.dashboard.id,
            'attribute_id': self.gps_attribute.id,
            'value_ids': [(6, 0, [self.gps_yes.id, self.gps_no.id])],
        })
        ...
        bom_form_car.productTemplateId = self.car
        bom_form_car.productQty = 5
        with bom_form_car.bom_line_ids.new() as line:
            line.productId = self.paint._get_variant_for_combination(self.paint_color_red)
            line.productUomId = uom_litre
            line.productQty = 50
            line.bom_product_template_attribute_value_ids.add(self.car_color_red)
        with bom_form_car.bom_line_ids.new() as line:
            line.productId = self.paint._get_variant_for_combination(self.paint_color_blue)
            line.productUomId = uom_litre
            line.productQty = 50
            line.bom_product_template_attribute_value_ids.add(self.car_color_blue)
        with bom_form_car.bom_line_ids.new() as line:
            line.productId = self.dashboard._get_variant_for_combination(self.dashboard_gps_yes + self.dashboard_color_red)
            line.productQty = 5
            line.bom_product_template_attribute_value_ids.add(self.car_gps_yes)
            line.bom_product_template_attribute_value_ids.add(self.car_color_red)
        with bom_form_car.bom_line_ids.new() as line:
            line.productId = self.dashboard._get_variant_for_combination(self.dashboard_gps_yes + self.dashboard_color_blue)
            line.productQty = 5
            line.bom_product_template_attribute_value_ids.add(self.car_gps_yes)
            line.bom_product_template_attribute_value_ids.add(self.car_color_blue)
        with bom_form_car.bom_line_ids.new() as line:
            line.productId = self.dashboard._get_variant_for_combination(self.dashboard_gps_no + self.dashboard_color_red)
            line.productQty = 5
            line.bom_product_template_attribute_value_ids.add(self.car_gps_no)
            line.bom_product_template_attribute_value_ids.add(self.car_color_red)
        with bom_form_car.bom_line_ids.new() as line:
            line.productId = self.dashboard._get_variant_for_combination(self.dashboard_gps_no + self.dashboard_color_blue)
            line.productQty = 5
            line.bom_product_template_attribute_value_ids.add(self.car_gps_no)
            line.bom_product_template_attribute_value_ids.add(self.car_color_blue)
        ...
        bom_dashboard.productTemplateId = self.dashboard
        bom_dashboard.productQty = 2
        ...
        report_values = self.env['report.mrp.bomstructure']._getReportData(bomId=bom_car.id, searchQty=1, searchVariant=blue_car_with_gps.id)
        ...
        report_values_dashboad = self.env['report.mrp.bomstructure']._getBom(
        ...
        report_values = self.env['report.mrp.bomstructure']._getReportData(bomId=bom_car.id, searchQty=1, searchVariant=red_car_without_gps.id)
    ...
    def test_22_bom_report_recursive_bom(self):
        ...
        bom_finished.productTemplateId = finished.productTemplateId
        bom_finished.productQty = 100
        with bom_finished.bom_line_ids.new() as line:
            line.productId = semi_finished
            line.productUomId = uom_kg
            line.productQty = 5
        bom_finished = bom_finished.save()

        bom_semi_finished = Form(self.env['mrp.bom'])
        bom_semi_finished.productTemplateId = semi_finished.productTemplateId
        bom_semi_finished.productQty = 11
        with bom_semi_finished.bom_line_ids.new() as line:
            line.productId = assembly
            line.productUomId = uom_dozen
            line.productQty = 2
        bom_semi_finished = bom_semi_finished.save()

        bom_assembly = Form(self.env['mrp.bom'])
        bom_assembly.productTemplateId = assembly.productTemplateId
        bom_assembly.productQty = 5
        with bom_assembly.bom_line_ids.new() as line:
            line.productId = raw_material
            line.productUomId = uom_litre
            line.productQty = 4
        bom_assembly = bom_assembly.save()

        report_values = self.env['report.mrp.bomstructure']._getReportData(bomId=bom_finished.id, searchQty=80)
    ...
    def test_validate_no_bom_line_with_same_product_variant(self):
        ...
        uom_unit = self.env.ref('uom.productUomUnit')
        bom_finished = Form(self.env['mrp.bom'])
        bom_finished.productTemplateId = self.product_7_template
        bom_finished.productId = self.product_7_3
        bom_finished.productQty = 100
        with bom_finished.bom_line_ids.new() as line:
            line.productId = self.product_7_3
            line.productUomId = uom_unit
            line.productQty = 5
        with self.assertRaises(exceptions.ValidationError), self.cr.savepoint():
            bom_finished = bom_finished.save()
    ...
    def test_validate_bom_line_with_different_product_variant(self):
        ...
        bom_finished.productQty = 100
        with bom_finished.bom_line_ids.new() as line:
            line.productId = self.product_7_2
            line.productUomId = uom_unit
            line.productQty = 5
        bom_finished = bom_finished.save()
    ...
    def test_validate_bom_line_with_variant_of_bom_product(self):
        ...
        uom_unit = self.env.ref('uom.productUomUnit')
        bom_finished = Form(self.env['mrp.bom'])
        bom_finished.productTemplateId = self.product_6.productTemplateId
        # no productId
        bom_finished.productQty = 100
        with bom_finished.bom_line_ids.new() as line:
            line.productId = self.product_7_2
    ...
    def test_replenishment(self):
        ...
        self.env['mrp.bom'].create({
            'productId': product_gram.id,
            'productTemplateId': product_gram.productTemplateId.id,
            'productUomId': uom_kg.id,
            'productQty': 2.0,
            'type': 'normal',
        })
        # We create a delivery order of 2300 grams
        picking_form = Form(self.env['stock.picking'])
        picking_form.pickingTypeId = self.env.ref('stock.picking_type_out')
        with picking_form.move_ids_without_package.new() as move:
            move.productId = product_gram
            move.productUomQty = 2300.0
        ...
        orderpoint = self.env['stock.warehouse.orderpoint'].search([('productId', '=', product_gram.id)])
        manufacturing_route_id = self.ref('mrp.routeWarehouse0Manufacture')
        self.assertEqual(orderpoint.routeId.id, manufacturing_route_id)
    ...
    def test_cycle_on_line_creation(self):
        bom_1_finished_product = self.bom_1.productId
        bom_2_finished_product = self.bom_2.productId
        with self.assertRaises(exceptions.ValidationError):
            # finished product is one of the components:
            self.bom_1.bom_line_ids = [(0, 0, {'productId': bom_1_finished_product.id, 'productQty': 1.0},)]
        with self.assertRaises(exceptions.ValidationError):
            # cycle:
            self.bom_1.bom_line_ids = [(0, 0, {'productId': bom_2_finished_product.id, 'productQty': 1.0},)]

    def test_cycle_on_line_update(self):
        lines = self.bom_1.bom_line_ids
        bom_2_finished_product = self.bom_2.productId
        with self.assertRaises(exceptions.ValidationError):
            self.bom_1.bom_line_ids = [(1, lines[0].id, {'productId': bom_2_finished_product.id})]

    def test_cycle_on_bom_unarchive(self):
        finished_product = self.bom_1.productId
        component = self.bom_1.bom_line_ids.productId[0]
        self.bom_1.active = False
        self.env['mrp.bom'].create({
            'productId': component.id,
            'productTemplateId': component.productTemplateId.id,
            'productUomId': component.uomId.id,
            'productQty': 1.0,
            'type': 'normal',
            'bom_line_ids': [
                (0, 0, {'productId': finished_product.id, 'productQty': 1.0}),
            ],
        })
        with self.assertRaises(exceptions.ValidationError):
    ...
    def test_cycle_on_bom_creation(self):
        finished_product = self.bom_4.productId
    ...
    def test_indirect_cycle_on_bom_creation(self):
        ...
        self.env['mrp.bom'].create([{
            ...
            'productUomId': finished.uomId.id,
            'productQty': 1.0,
            'type': 'normal',
            'bom_line_ids': [
                (0, 0, {'productId': compo.id, 'productQty': 1.0}),
            ],
        ...
        with self.assertRaises(exceptions.ValidationError):
            self.env['mrp.bom'].create({
                'productId': product_C.id,
                'productTemplateId': product_C.productTemplateId.id,
                'productUomId': product_C.uomId.id,
                'productQty': 1.0,
                'type': 'normal',
                'bom_line_ids': [
                    (0, 0, {'productId': product_A.id, 'productQty': 1.0}),
                ],
            })
    ...
    def test_cycle_on_bom_sequencing(self):
        ...
        # (see verp/addons/web/controllers/main.py:1352)
    ...
    def test_cycle_on_legit_apply_variants(self):
        ...
        self.env['mrp.bom'].create({
            'productTemplateId': self.product_7_template.id,
            'productUomId': self.product_7_template.uomId.id,
            'productQty': 1.0,
            'type': 'normal',
            'bom_line_ids': [
                (0, 0, {
        ...
        self.env['mrp.bom'].create({
            ...
            'type': 'normal',
            'bom_line_ids': [
                (0, 0, {'productId': self.product_7_1.id, 'productQty': 1.0}),
            ],
        })
