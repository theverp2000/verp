...
from verp.fields import Datetime as Dt
...
class TestMrpCancelMO(TestMrpCommon):
    ...
    def test_cancel_mo_without_routing_1(self):
        ...
        self.assertEqual(manufacturing_order.moveRawIds[0].state, 'cancel',
            "Cancelled MO raw moves must be cancelled as well.")
        self.assertEqual(manufacturing_order.moveRawIds[1].state, 'cancel',
            "Cancelled MO raw moves must be cancelled as well.")
    ...
    def test_cancel_mo_without_routing_3(self):
        ...
        self.assertEqual(manufacturing_order.moveRawIds[0].state, 'done',
            "Due to 'post_inventory', some move raw must stay in done state")
        self.assertEqual(manufacturing_order.moveRawIds[1].state, 'done',
            "Due to 'post_inventory', some move raw must stay in done state")
        self.assertEqual(manufacturing_order.moveRawIds[2].state, 'cancel',
            "The other move raw are cancelled like their MO.")
        self.assertEqual(manufacturing_order.moveRawIds[3].state, 'cancel',
            "The other move raw are cancelled like their MO.")
