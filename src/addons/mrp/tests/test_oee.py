...
class TestOee(TestMrpCommon):
    def create_productivity_line(self, loss_reason, dateStart=False, dateEnd=False):
        return self.env['mrp.workcenter.productivity'].create({
            'workcenterId': self.workcenter_1.id,
            'dateStart': dateStart,
            'dateEnd': dateEnd,
            'lossId': loss_reason.id,
    ...
    def test_wrokcenter_oee(self):
        ...
        tz = timezone(self.workcenter_1.resourceCalendarId.tz)
        ...
        self.assertEqual(self.workcenter_1.workingState, 'blocked', "Wrong working state of workcenter.")

        # Check working state is normal or not.
        end_time = time_to_string_utc_datetime(time(10, 48, 39))
        workcenter_productivity_1.write({'dateEnd': end_time})
        self.assertEqual(self.workcenter_1.workingState, 'normal', "Wrong working state of workcenter.")
