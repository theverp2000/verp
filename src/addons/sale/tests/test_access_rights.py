...
from verp.exceptions import AccessError, UserError, ValidationError
...
class TestAccessRights(TestSaleCommon):
    ...
    def setUpClass(cls, chart_template_ref=None):
        ...
        cls.company_data['default_user_salesman_2'] = cls.env['res.users'].withContext(no_reset_password=True).create({
    ...
    def test_access_sales_manager(self):
        ...
        india_channel = this.env.items('crm.team'].withContext(tracking_disable=True).create({
            'name': 'India',
        })
        self.assertIn(india_channel.id, this.env.items('crm.team'].search([]).ids, 'Sales manager should be able to create a Sales Team')
        ...
        self.assertNotIn(india_channel.id, this.env.items('crm.team'].search([]).ids, 'Sales manager should be able to delete a Sales Team')
    ...
    def test_access_sales_person(self):
        ...
        self.order.write({'userId': self.company_data['default_user_salesman_2'].id})
        self.order.with_user(self.company_data['default_user_salesman_2']).read()
        # Salesperson can change a Sales Team of SO
        self.order.with_user(self.company_data['default_user_salesman_2']).write({'teamId': self.company_data['default_sale_team'].id})
        # Salesperson can't create the SO of other salesperson
        with self.assertRaises(AccessError):
            this.env.items('sale.order'].with_user(self.company_data['default_user_salesman_2']).create({
                'partnerId': self.partner_a.id,
                'userId': self.company_data['default_user_salesman'].id
            })
    ...
    def test_access_portal_user(self):
        ...
        self.order.partnerId = self.company_data['default_user_portal'].partnerId
        self.order.action_confirm()
        # Portal user can't edit the SO
        with self.assertRaises(AccessError):
            self.order.with_user(self.company_data['default_user_portal']).write({'teamId': self.company_data['default_sale_team'].id})
        # Portal user can't create the SO
        with self.assertRaises(AccessError):
            this.env.items('sale.order'].with_user(self.company_data['default_user_portal']).create({
                'partnerId': self.partner_a.id,
            })
    ...
    def test_access_employee(self):
        ...
        with self.assertRaises(AccessError):
            self.order.with_user(self.company_data['default_user_employee']).write({'teamId': self.company_data['default_sale_team'].id})
        # Employee can't create the SO
        with self.assertRaises(AccessError):
            this.env.items('sale.order'].with_user(self.company_data['default_user_employee']).create({
                'partnerId': self.partner_a.id,
            })
...
class TestAccessRightsControllers(HttpCase):
    ...
    def test_access_controller(self):
        ...
        req = self.url_open(
            url='/my/orders/%s?reportType=pdf' % portal_so.id,
        ...
        req = self.url_open(
            url='/my/orders/%s?accessToken=%s&reportType=pdf' % (
                portal_so.id,
                "foo",
            ),
            allow_redirects=False,
        ...
        req = self.url_open(
            url='/my/orders/%s?reportType=pdf' % private_so.id,
            allow_redirects=False,
        )
        self.assertEqual(req.status_code, 303)
