verp.define('web.singleton_list_tests', function (require) {
"use strict";
...
QUnit.module('Views', {
    beforeEach: function () {
        ...
        this.mockRPC = function (route, args) {
            if (route === '/web/dataset/callKw/person/create') {
    ...
    QUnit.test('Create new record correctly', async function (assert) {
        ...
        var list = await createView({
            ...
            model: 'person',
            data: this.data,
            arch: '<tree editable="top" jsClass="singleton_list">'+
                    '<field name="label"/>'+
                    '<field name="age"/>'+
                   '</tree>',
        ...
        assert.containsN(list, '.o-data-row', 3, "should have 3 records");
        ...
        assert.containsN(list, '.o-data-row', 4, "should now have 4 records");
        list.destroy();
    });
    ...
    QUnit.test('Don\'t duplicate record', async function (assert) {
        ...
        assert.containsN(list, '.o-data-row', 3, "should still have 3 records");
        // ... and verify modification was occured.
        var nameField = list.$('td[title="' + name + '"]');
        var ageField = nameField.parent().find('.o-list-number');
    ...
    QUnit.test('Don\'t raise error when trying to create duplicate line', async function (assert) {
        assert.expect(3);
       /* In some condition, a list editable with the `singletonlist` jsClass
       ...
       */
        ...
        assert.containsN(list, '.o-data-row', 1, "should have 1 records");
        ...
        assert.containsN(list, '.o-data-row', 1, "should now have 1 records");
        // ... and verify modification was occured.
        var nameField = list.$('td[title="' + name + '"]');
        var ageField = nameField.parent().find('.o-list-number');
        assert.strictEqual(ageField.text(), age, "The age field must be updated");
    ...
    QUnit.test('Refresh the list only when needed', async function (assert) {
        ...
        // Modify Record
        await testUtils.dom.click(list.$('.o-data-row:nth-child(2) > .o-list-number'));
        var $input = $('.o-selected-row input[name=age]');
    ...
    QUnit.test('Work in grouped list', async function (assert) {
        ...
        await testUtils.dom.click(list.$('.o-group-header:nth-child(2)'));
        ...
        await testUtils.fields.triggerKeydown($input, 'tab');
        $input = $('.o-selected-row input[name=job]');
        ...
        // Opens 'Soldier' group
        await testUtils.dom.click(list.$('.o-group-header:nth-child(1)').first());
        // Checks the record has been correctly updated
        var ageCell = $('tr.o-data-row td.o-list-number').first();
        assert.strictEqual(ageCell.text(), "55",
            "age of the record must be updated");
        // Edits the freshly created record...
        await testUtils.dom.click(list.$('tr.o-data-row td.o-list-number').eq(1));
        $input = $('.o-selected-row input[name=age]');
        ...
        ageCell = $('tr.o-data-row td.o-list-number').first();
