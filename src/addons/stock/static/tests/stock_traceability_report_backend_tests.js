verp.define('stock.stock_traceability_report_backend_tests', function (require) {
    "use strict";

    const ControlPanel = require('web.ControlPanel');
    const dom = require('web.dom');
    const StockReportGeneric = require('stock.stockReportGeneric');
            ...
            const report = await createStockReportAction({
                ...
                data: {
                    'stock.traceability.report': {
                        fields: {},
                        getHtml: () => ({ html: template }),
                    },
                },
            });

            // HTML content is nested in a div inside of the content
            assert.strictEqual(report.el.querySelector('.o-content > div').innerHTML, template,
        ...
        QUnit.test("mounted is called once when returning on 'Stock report' from breadcrumb", async assert => {
            ...
            const webClient = await createWebClient({
                ...
                mockRPC: function (route) {
                    if (route === '/web/dataset/callKw/stock.traceability.report/getHtml') {
