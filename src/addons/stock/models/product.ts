...
class Product extends Model {
  ...
  async _getDomainLocations() {
    ...
    const self = this;
    let Location = this.env.items('stock.location');
    let Warehouse = this.env.items('stock.warehouse');
    ...
    if (bool(warehouse)) {
      ...
      if (bool(location)) {
        ...
        const parents = await Location.browse(wIds).mapped("parentPath");
        for (const loc of Location.browse(lIds)) {
          if (await parents.some(async parent => (await loc.parentPath).startsWith(parent))) {
      ...
      if (location) {
        ...
        locationIds = Array.from((await (await Warehouse.search([])).mapped('viewLocationId')).ids);
      }
    }
  ...
  async _getDomainLocationsNew(locationIds, companyId?: any, computeChild: any = true) {
    ...
    for (const location of hierarchicalLocations) {
      ...
      locDomain.push(['locationId.parentPath', '=like', await location.parentPath + '%']);
      destLocDomain = destLocDomain.length ? ['|'].concat(destLocDomain) : destLocDomain;
      destLocDomain.push(['locationDestId.parentPath', '=like', await location.parentPath + '%']);
    }
    if (bool(otherLocations)) {
      locDomain = locDomain.length ? ['|'].concat(locDomain) : locDomain;
  ...
  async _searchQtyAvailable(operator, value) {
    ...
    // '_searchProductQuantity' method and call '_searchQtyAvailableNew' instead. This
  ...
  async _searchQtyAvailableNew(operator, value, lotId?: any, ownerId?: any, packageId?: any) {
    ...
    if (typeof value !== 'number') {
      throw new UserError(await this._t("Invalid domain right operand '%s'. It must be of type Integer/Float", value));
    }
  ...
  async getTheoreticalQuantity(productId, locationId, lotId?: any, packageId?: any, ownerId?: any, toUom?: any) {
    ...
    const quants = await this.env.items('stock.quant')._gather(productId, locationId, {lotId, packageId, ownerId, strict: true});
...
class ProductTemplate extends Model {
  ...
  @api.depends('type')
  async _computeTracking() {
    await (await this.filtered(
      async (t) => !await t.tracking || ['consu', 'service'].includes(await t.type) && await t.tracking != 'none'
    )).set('tracking', 'none');
  }
  ...
  async _computeProductTooltip() {
    ...
    for (const record of this) {
      if (await record.type === 'product') {
        await record.set('productTooltip', await record.productTooltip + await this._t(
  ...
  @api.onchange('tracking')
  async onchangeTracking() {
    return (await this.mapped('productVariantIds')).onchangeTracking();
  }
  ...
  async _onchangeType() {
    ...
    if (len(this.ids) && len(productVariantIds.ids) && await (await this.env.items('stock.move.line').sudo()).searchCount([
  ...
  async write(vals) {
    ...
    if ('companyId' in vals && vals['companyId']) {
      const productsChangingCompany = await this.filtered(async (product) => (await product.companyId).id != vals['companyId']);
      if (bool(productsChangingCompany)) {
        const move = await (await this.env.items('stock.move').sudo()).search([
          ['productId', 'in', (await productsChangingCompany.productVariantIds).ids],
          ['companyId', 'not in', [vals['companyId'], false]],
        ], { order: null, limit: 1 });
        if (bool(move)) {
          throw new UserError(await this._t("This product's company cannot be changed as long as there are stock moves of it belonging to another company."));
        }

        // Forbid changing a product's company when quant(s) exist in another company.
        const quant = await (await this.env.items('stock.quant').sudo()).search([
          ['productId', 'in', (await productsChangingCompany.productVariantIds).ids],
          ['companyId', 'not in', [vals['companyId'], false]],
          ['quantity', '!=', 0],
        ], { order: null, limit: 1 });
        if (bool(quant)) {
          throw new UserError(await this._t("This product's company cannot be changed as long as there are quantities of it belonging to another company."));
        }
      }
    }
  ...
  async copy(defaultValue?: any) {
    ...
    const newProductDict = new MapKey<any, any>();
    ...
    for (const storageCategoryCapacity of await (await this['productVariantIds']).storageCategoryCapacityIds) {
      ...
      storageCategoryCapacityVals.push((await storageCategoryCapacity.copyData({ 'productId': newProductDict.get(productAttributeValue) }))[0]);
    }
