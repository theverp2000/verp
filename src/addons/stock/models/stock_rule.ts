...
export class ProcurementException extends Error {
  ...
  constructor(procurementExceptions) {
    ...
    if (bool(this.procurementExceptions)) {
      this.message = this.procurementExceptions.map(([, msg]) => msg).join('\n');
      this.stack = 'ProcurementException\n' + parseStack(this).slice(1).join('\n');
      this.name = 'ProcurementException';
    }
  }
}
...
class StockRule extends Model {
   ...
   * Return the source, destination and pickingType applied on a stock
      ...
      _getMessageDict functions since it often requires those data.
   * @returns 
   */
  ...
  async _runPull(procurements: any[]) {
    const movesValuesByCompany = new DefaultDict(() => []);
    const mtsoProductsByLocations = new MapKey<any, any>();
    ...
    const forecastedQtiesByLoc = new MapKey();
    for (const [location, productIds] of mtsoProductsByLocations.items()) {
      ...
      forecastedQtiesByLoc.set(location, {});
      for (const product of products) {
        forecastedQtiesByLoc.get(location)[product.id] = await product.freeQty;
      }
    }

    // Prepare the move values, adapt the `procureMethod` if needed.
    ...
    for (const [procurement, rule] of procurements) {
      ...
      if (procureMethod === 'mtsElseMto') {
        const {productUom, productQty, productId} = procurement;
        ...
        if (floatCompare(qtyNeeded, 0, { precisionRounding: await uomId.rounding }) <= 0) {
          forecastedQtiesByLoc.get(locationSrcId)[productId.id] -= qtyNeeded;
          procureMethod = 'makeToOrder';
        }
        else if (floatCompare(qtyNeeded, forecastedQtiesByLoc.get(locationSrcId)[productId.id], { precisionRounding: await uomId.rounding }) > 0) {
          procureMethod = 'makeToOrder';
        }
        else {
          forecastedQtiesByLoc.get(locationSrcId)[productId.id] -= qtyNeeded;
      ...
      movesValuesByCompany.set(procurement.companyId.id, moveValues);
    }
    for (const [companyId, movesValues] of movesValuesByCompany.items()) {
      ...
      // Since actionConfirm launch following procurementGroup we should activate it.
  ...
  async _getStockMoveValues(procurement: any={}) {
    let {productId, productQty, productUom, locationId, label, origin, companyId, values} = procurement;
    ...
    const dateScheduled = subDate(_Datetime.toDatetime(values['datePlanned']) as Date, { days: await this['delay'] || 0 });
    const dateDeadline = values['dateDeadline'] && subDate(_Datetime.toDatetime(values['dateDeadline']) as Date, { days: await this['delay'] || 0 }) || false;
    ...
    const moveValues = {
      ...
      'pickingTypeId': (await this['pickingTypeId']).id,
      'groupId': groupId,
      'routeIds': await (values['routeIds'] ?? []).map(route => [4, route.id]),
   ...
   * @param product the product of the procurement (addons.product.models.product.ProductProduct)
   * @param values 
   * @returns the cumulative delay and cumulative delay's description ([number, [][str, str]])
   */
...
class ProcurementGroup extends Model {
   ...
   * Fulfil `procurements` with the help of stock rules.
      ...
      type of documents).
   * @param procurements the description of the procurement, list of `~verp.addons.stock.models.stock_rule.ProcurementGroup.Procurement`
   * @param raiseUserError will raise either an UserError or a ProcurementException
   * @throws UserError: if `raiseUserError` is true and a procurement isn't fulfillable
   * @throws ProcurementException: if `raiseUserError` is false and a procurement isn't fulfillable
   */
  @api.model()
  async run(procurements: any[], raiseUserError = true) {
    ...
    for (const procurement of procurements) {
      const {values, locationId, productId, productQty, productUom} = procurement;
      ...
      if (!bool(rule)) {
        const error = await this._t('No rule has been found to replenish "%s" in "%s". Verify the routes configuration on the product.', await productId.displayName, await locationId.displayName);
        procurementErrors.push([procurement, error]);
      }
      else {
        const action = await rule.action === 'pullPush' ? 'pull' : await rule.action;
        actionsToRun[action] = actionsToRun[action] ?? [];
    ...
    for (const [action, procurements] of Object.entries(actionsToRun)) {
      ...
      const func = Rule[`_run${_.upperFirst(action)}`];
      if (isCallable(func)) {
        try {
          await func.call(Rule, procurements);
        } catch (e) {
        ...
        console.error("The method _run %s doesn't exist on the procurement rules", action);
      }
    }
    if (procurementErrors.length) {
      raiseException(procurementErrors);
  ...
  async _getRule(productId, locationId, values) {
    ...
    while (!bool(result) && bool(location)) {
      const domain = this._getRuleDomain(location, values);
      result = await this._searchRule(values['routeIds'] ?? false, values['productPackagingId'] ?? false, productId, values['warehouseId'] ?? false, domain);
  ...
  @api.model()
  _getRuleDomain(location, values) {
  ...
  async _runSchedulerTasks(useNewCursor = false, companyId?: any) {
    ...
    domain = this._getMovesToAssignDomain(companyId);
   ...
   */
  @api.model()
  async runScheduler(useNewCursor: any = false, companyId: any = false) {
    if (typeof useNewCursor === 'object') { // none args, kwargs => useNewCursor = {}
      const options = useNewCursor;
      useNewCursor = options.useNewCursor;
      companyId = options.companyId;
    }
    let self = this;
    try {
      if (useNewCursor) {
        const cr = (await registry(self._cr.dbName)).cursor();
        self = self.withEnv(await self.env.change({ cr }));  // TDE FIXME
      }
      await self._runSchedulerTasks(useNewCursor, companyId);
    }
    catch(e) {
      console.error("Error during stock scheduler");
      throw e;
    }
    finally {
      if (useNewCursor) {
