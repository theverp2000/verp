...
export function Routing(fromLoc: any, destLoc: any, pickingType: any, action: any) {
  return {
    fromLoc: fromLoc,
    destLoc: destLoc,
...
class Warehouse extends Model {
  ...
  async _createOrUpdateGlobalRoutesRules() {
    const rules = await this._getGlobalRouteRulesValues();
    for (const [ruleField, ruleDetails] of Object.entries(rules)) {
   ...
   * Create or update the warehouse's routes.
    _getRoutesValues method return a dict with:
        - route field name (e.g: crossdockRouteId).
        - field that trigger an update on the route (key 'depends').
        - routingKey used in order to find rules contained in the route.
        - create values.
    ...
    use the getRulesDict that return a dict:
    ...
    The routing_key from _getRoutesValues is match with the getRulesDict
    key in order to create/update the rules in the route
    (_findExistingRuleOrCreate method is responsible for this part).
   */
  async _createOrUpdateRoute() {
    // Create routes and active/create their related rules.
    const routes = [];
    const rulesDict = await this.getRulesDict();
    for (const [routeField, routeData] of Object.entries<any>(await this._getRoutesValues())) {
      ...
      // Create/Active rules
      await this._findExistingRuleOrCreate(rulesList);
   ...
   * Return information in order to update warehouse routes.
    ...
    - This key contains a dict with route values:
        - routingKey: a key used in order to match rules from
        getRulesDict function. It would be usefull in order to generate
        the route's rules.
        - routeCreateValues: When the Many2one does not exist the route
        is created based on values contained in this dict.
        - routeUpdateValues: When a field contained in 'depends' key is
        modified and the Many2one exist on the warehouse, the route will be
        update with the values contained in this dict.
        - rulesValues: values added to the routing in order to create the
        route's rules.
   * @returns 
   */
  async _getRoutesValues(): Promise<any> {
    const [active, receptionSteps, deliverySteps, company] = await this('active', 'receptionSteps', 'deliverySteps', 'companyId');
    return {
      'receptionRouteId': {
        'routingKey': receptionSteps,
        'depends': ['receptionSteps'],
        'routeUpdateValues': {
          'label': await this._formatRoutename(null, receptionSteps),
          'active': active,
        },
        'routeCreateValues': {
      ...
      'deliveryRouteId': {
        ...
        'depends': ['deliverySteps'],
        'routeUpdateValues': {
          'label': await this._formatRoutename(null, deliverySteps),
          'active': active,
        },
        'routeCreateValues': {
          'productCategSelectable': true,
      ...
      'crossdockRouteId': {
        ...
        'routeUpdateValues': {
          'label': await this._formatRoutename(null, 'crossdock'),
  ...
  /**
   * Return receive route values with 'procureMethod': 'make_to_order' in order to update warehouse routes.

    This function has the same receive route values as _get_routes_values with the addition of
    'procureMethod': 'make_to_order' to the 'rules_values'. This is expected to be used by
    modules that extend stock and add actions that can trigger receive 'make_to_order' rules (i.e.
    we don't want any of the generated rules by get_rules_dict to default to 'makeToStock').
  ...
  async _createMissingLocations(vals) {
    for (const warehouse of this) {
      ...
      for (const [location, locationValues] of Object.entries(subLocations)) {
        if (!bool(await warehouse[location]) && !(location in vals)) {
  ...
  async _getRuleValues(routeValues, values?: any, nameSuffix = '') {
    ...
    for (const routing of routeValues) {
      // const {fromLoc, destLoc, action, pickingType} = routing;
      const routeRuleValues = {
        'label': await this._formatRulename(routing.fromLoc, routing.destLoc, nameSuffix),
        'locationSrcId': routing.fromLoc.id,
        'locationId': routing.destLoc.id,
        'action': routing.action,
        'auto': 'manual',
        'pickingTypeId': routing.pickingType.id,
    ...
    if (values && values['propagateCancel'] && rulesList.length) {
      ...
      // which means:
      // Input -> Quality check should have propagateCancel = true
      // Quality check -> Stock should have propagateCancel = false
