...
class StockWarehouseOrderpoint extends Model {
    ...
    async _computeRules() {
        for (const orderpoint of this) {
            const [product, location, route] = await orderpoint('productId', 'locationId', 'routeId');
            if (! bool(product) || !bool(location)) {
                await orderpoint.set('ruleIds', false);
                continue;
            }
            await orderpoint.set('ruleIds', await product._getRulesFromLocation(location, route));
        }
    }
    ...
    async _computeQty() {
        const orderpointsContexts = new DefaultDict(() => this.env.items('stock.warehouse.orderpoint'));
        for (const orderpoint of this) {
        ...
        for (const [orderpointContext, orderpointsByContext] of orderpointsContexts.items()) {
            const productsQty = {};
            for (const p of await (await (await orderpointsByContext.productId).withContext(jsonParse(orderpointContext))).read(['qtyAvailable', 'virtualAvailable'])) {
                productsQty[p['id']] = p;
            }
    ...
    async _computeQtyToOrder() {
        for (const orderpoint of this) {
            ...
            if (floatCompare(await orderpoint.qtyForecast, await orderpoint.productMinQty, {precisionRounding: rounding}) < 0) {
                qtyToOrder = Math.max(await orderpoint.productMinQty, await orderpoint.productMaxQty) - await orderpoint.qtyForecast;
    ...
    async _getReplenishmentOrderNotification() {
        ...
        if (this.env.context['writtenAfter']) {
            domain = expression.AND([domain, [['updatedAt', '>', this.env.context['writtenAfter']]]]);
        }
        const move = await this.env.items('stock.move').search(domain, {limit: 1});
        if (bool(await move.pickingId)) {
            return {
                'type': 'ir.actions.client',
                'tag': 'displayNotification',
                'params': {
                    'title': await this._t('The inter-warehouse transfers have been generated'),
                    'sticky': false,
                }
            }
        }
     ...
     * @param useNewCursor if set, use a dedicated cursor and auto-commit after processing 1000 orderpoints.
        ...
        for (const orderpointsBatchIds of splitEvery(1000, self.ids)) {
            ...
            while (orderpointsBatch.ok) {
                ...
                for (const orderpoint of orderpointsBatch) {
                    ...
                    if (floatCompare(qtyToOrder, 0.0, {precisionRounding: await productUom.rounding}) == 1) {
                        ...
                        const values = await orderpoint._prepareProcurementValues(date);
                        procurements.push(self.env.items('procurement.group').Procurement(productId, qtyToOrder, productUom, locationId, label, origin, companyId, values));
                    }
                }
                let err;
                try {
            ...
            for (const [orderpoint, errorMsg] of allOrderpointsExceptions) {
                ...
                const existingActivity = await self.env.items('mail.activity').search([
                    ...
                    ['resModelId', '=', (await self.env.ref('product.model_productTemplate')).id],
                    ['note', '=', errorMsg]]);
    ...
    async _getOrderpointProcurementDate() {
        return combine(await this['leadDaysDate'], 'min');
    }
}