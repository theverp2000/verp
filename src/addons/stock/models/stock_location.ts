...
class Location extends Model {
   ...
   * @returns 
   */
  async _getPutawayStrategy(product, opts: { quantity?: number, pack?: any, packaging?: any, additionalQty?: any } = {}) {
    let { quantity = 0, pack, packaging, additionalQty } = opts;
    let products = this.env.context['products'] ?? this.env.items('product.product');
    ...
    let categ = len(await products.categId) == 1 ? await products.categId : this.env.items('product.category');
    let categs = categ;
    while (bool(await categ.parentId)) {
      categ = await categ.parentId;
      categs = categs.or(categ);
    }

    let putawayRules = await (await this['putawayRuleIds']).filtered(async (rule) =>
      (!bool(await rule.productId) || products.includes(await rule.productId)) &&
      (!bool(await rule.categoryId) || categs.includes(await rule.categoryId)) &&
      (!bool(await rule.packageTypeIds) || (await rule.packageTypeIds).includes(packageType)));

    putawayRules = await putawayRules.sorted(async (rule) => (
      (await rule.packageTypeIds).ids,
      (await rule.productId).id,
      (await rule.categoryId).eq(categs.slice(0, 1)),  // same categ, not a parent
      (await rule.categoryId).id)
      , true);

    let putawayLocation;
    const locations = await this['childInternalLocationIds'];
    if (bool(putawayRules)) {
      // get current product qty (qty in current quants and future qty on assigned ml) of all child locations
      const qtyByLocation = new DefaultDict(() => 0);
      if (bool(await locations.storageCategoryId)) {
        if (bool(pack) && bool(await pack.packageTypeId)) {
          const moveLineData = await this.env.items('stock.move.line').readGroup([
            ['id', 'not in', this._context['excludeSmlIds'] ?? []],
            ['resultPackageId.packageTypeId', '=', packageType.id],
            ['state', 'not in', ['draft', 'cancel', 'done']],
          ], ['resultPackageId:count_distinct'], ['locationDestId'])
          const quantData = await this.env.items('stock.quant').readGroup([
            ['packageId.packageTypeId', '=', packageType.id],
            ['locationId', 'in', locations.ids],
          ], ['packageId:count_distinct'], ['locationId'])
          for (const values of moveLineData) {
            qtyByLocation[values['locationDestId'][0]] = values['resultPackageId'];
          }
          for (const values of quantData) {
            qtyByLocation[values['locationId'][0]] += values['packageId'];
          }
        }
        else {
          const moveLineData = await this.env.items('stock.move.line').readGroup([
          ...
          for (const values of moveLineData) {
            ...
            let qtyDone = 0;
            for (const [qtyReserved, qty, mlUom] of _.zip<any, any, any>(values['productQty'], values['qtyDone'], Array.from(uoms))) {
      ...
      if (additionalQty) {
        for (const [locationId, qty] of Object.entries(additionalQty)) {
          qtyByLocation[locationId] += qty;
        }
      }
      putawayLocation = await putawayRules._getPutawayLocation(product, quantity, pack, packaging, qtyByLocation);
    }
  ...
  async _checkCanBeUsed(product, quantity: number = 0, pack?: any, locationQty: number = 0) {
    ...
    if (bool(storageCategoryId)) {
      ...
      if (bool(pack) && bool(await pack.packageTypeId)) {
        // check weight
        const packageSmls = await this.env.items('stock.move.line').search([['resultPackageId', '=', pack.id]]);
        if (await storageCategoryId.maxWeight < await this['forecastWeight'] + sum(await packageSmls.mapped(async (sml) => await sml.productQty * await (await sml.productId).weight))) {
          return false;
        }
        // check if enough space
        ...
        if (bool(packageCapacity) && locationQty >= await packageCapacity.quantity) {
          return false;
        }
      }
      else {
      ...
      const positiveQuant = await (await this['quantIds']).filtered(async (q) => floatCompare(await q.quantity, 0, { precisionRounding: await (await (await q.productId).uomId).rounding }) > 0);
      // check if only allow new product when empty
      if (await storageCategoryId.allowNewProduct == "empty" && bool(positiveQuant)) {
        return false;
      }
      // check if only allow same product
      if (await storageCategoryId.allowNewProduct == "same") {
        // In case it's a package, `product` is not defined, so try to get
        // the package products from the context
        product = bool(product) ? product : this._context['products'];
        if ((bool(positiveQuant) && (await positiveQuant.productId).ne(product)) || len(product) > 1) {
          return false;
        }
        if (bool(await this.env.items('stock.move.line').search([
          ['productId', '!=', product.id],
          ['state', 'not in', ['done', 'cancel']],
          ['location_dest_id', '=', this.id],
        ], { limit: 1 }))) {
          return false;
        }
      }
