...
class StockMove extends Model {
  ...
  async _computeShowDetailsVisible() {
    ...
    for (const move of this) {
      if (!(await move.productId).ok) {
        ...
        await move.set('showDetailsVisible',
          (((consignmentEnabled && await move.pickingCode !== 'incoming') || showDetailsVisible || await move.hasTracking !== 'none') && await move._showDetailsInDraft() &&
  ...
  /**
   * This will return the move lines to consider when applying _quantityDoneCompute on a stock.move.
      In some context, such as MRP, it is necessary to compute quantityDone on filtered sock.move.line.
   * @returns 
   */
  async _getMoveLines() {
    this.ensureOne();
  ...
  /**
   * This field represents the sum of the move lines `qtyDone`. It allows the user to know
  ...
  async _quantityDoneSet() {
    const quantityDone = await this(0).quantityDone;  // any call to create will invalidate `move.quantityDone`
    for (const move of this) {
  ...
  async _computeForecastInformation() {
    ...
    const outgoingUnreservedMovesPerWarehouse = new MapKey();
    ...
    const prefetchVirtualAvailable = new Dict();
    const virtualAvailableDict = {};
    for (const move of productMoves) {
      if ((await this._consumingPickingTypes()).includes(await (await move.pickingTypeId).code) && await move.state === 'draft') {
    ...
    for (const move of productMoves) {
      ...
      if ((await this._consumingPickingTypes()).includes(await pickingTypeId.code)) {
        if (state === 'assigned') {
          ...
          // for move _consumingPickingTypes and in draft -> the forecastAvailability > 0 if in stock
    ...
    for (const [warehouse, movesIds] of outgoingUnreservedMovesPerWarehouse) {
      ...
      const forecastInfo: MapKey = await moves._getForecastAvailabilityOutgoing(warehouse);
  ...
  async write(vals) {
    ...
    if ('productUomQty' in vals) {
      let moveToUnreserve = this.env.items('stock.move');
      ...
      if (this.env.context['doNotUnreserve'] == null) {
        moveToUnreserve = await this.filtered(
   ...
   * @returns 
   */
  async _propagateProductPackaging(productPackageId) {
    const alreadyPropagatedIds = _.union(this.env.context['productPackagingPropagationIds'] ?? [], this.ids);
    let self = await this.withContext({ productPackagingPropagationIds: alreadyPropagatedIds });
    for (const move of self) {
      // propagate on destination move
      for (const moveDest of await move.moveDestIds) {
        if (!alreadyPropagatedIds.includes(moveDest.id) &&
          !['cancel', 'done'].includes(await moveDest.state) &&
          (await moveDest.productPackagingId).id != productPackageId &&
          (await moveDest.moveOrigIds).eq(move)) {  // checks that you are the only parent move of your destination
          await moveDest.set('productPackagingId', productPackageId);
        }
      }
      // propagate on origin move
      for (const moveOrig of await move.moveOrigIds) {
        if (!alreadyPropagatedIds.includes(moveOrig.id) &&
          !['cancel', 'done'].includes(await moveOrig.state) &&
          (await moveOrig.productPackagingId).id != productPackageId &&
          (await moveOrig.moveDestIds).eq(move)) {  // checks that you are the only child move of your origin
          await moveOrig.set('productPackagingId', productPackageId);
        }
      }
    }
  }
   ...
   * @returns a list of recordset of the documents linked to `self`
   */
  ...
  async actionShowDetails() {
    ...
    if (await productId.tracking === "serial" && state === "assigned") {
      await this.set('nextSerial', await this.env.items('stock.production.lot').getNextSerial(companyId, productId));
    }
  ...
  async actionProductForecastReport() {
    ...
    let warehouse;
    if ((await this._consumingPickingTypes()).includes(await (await this['pickingTypeId']).code)) {
  ...
  async _doUnreserve() {
    ...
    for (const move of this) {
      ...
      if (state === 'cancel' || (state === 'done' && await move.scrapped)) {
        // We may have cancelled move in an open picking in a "propagateCancel" scenario.
  ...
  @api.model()
  async _prepareMergeNegativeMovesExcludedDistinctFields(): Promise<string[]> {
    return ['descriptionPicking', 'priceUnit']
  }
  ...
  async _mergeMoves(mergeInto?: any) {
    const distinctFields = await this._prepareMergeMovesDistinctFields();
    ...
    const movesByNegKey = new DefaultDict(() => this.env.items('stock.move'));
    ...
    await negQtyMoves.set('pickingId', false);
    const excludedFields = await this._prepareMergeNegativeMovesExcludedDistinctFields();
    ...
    for (const negMove of negQtyMoves) {
      ...
      for (const posMove of (movesByNegKey[key] ?? [])) {
        const currencyPrec = await (await (await posMove.productId).currencyId).decimalPlaces;
        const rounding = Math.min(currencyPrec, priceUnitPrec);

        if (floatCompare(await posMove.priceUnit, await negMove.priceUnit, { precisionDigits: rounding }) == 0) {
          const newTotalValue = await posMove.productQty * await posMove.priceUnit + await negMove.productQty * await negMove.priceUnit;
          ...
          if (floatCompare(await posMove.productUomQty, Math.abs(await negMove.productUomQty), { precisionRounding: await (await posMove.productUom).rounding }) >= 0) {
            ...
            const moveDestIds = [];
            for (const m of await negMove.mapped('moveDestIds')) {
              if ((await m.locationId).eq(await posMove.locationDestId)) {
                moveDestIds.push(Command.link(m.id));
              }
            }
            const moveOrigIds = [];
            for (const m of await negMove.mapped('moveOrigIds')) {
              if ((await m.locationDestId).eq(await posMove.locationId)) {
                moveDestIds.push(Command.link(m.id));
              }
            }
            await posMove.write({
              'priceUnit': await posMove.productQty ? floatRound(newTotalValue / await posMove.productQty, { precisionDigits: priceUnitPrec }) : 0,
              'moveDestIds': moveDestIds,
              'moveOrigIds': moveOrigIds,
            });
          ...
          await negMove.sei('priceUnit', floatRound(newTotalValue / await negMove.productQty, { precisionDigits: priceUnitPrec }));
  ...
  async _getRelevantStateAmongMoves() {
    ...
    const pickingId = await movesTodo.slice(0, 1).pickingId;
    if (bool(pickingId) && await pickingId.moveType === 'one') {
    ...
    else if (await movesTodo.slice(0, 1).state !== 'assigned' && await movesTodo.some(async (move) => ['assigned', 'partiallyAvailable'].includes(await move.state))) {
  ...
  async _onchangeSuggestPackaging() {
    ...
    if (bool(productId) && productQty && bool(productUom)) {
  ...
  async _onchangeLotIds() {
    ...
    let quantityDone = 0;
    for (const ml of await moveLineIds.filtered(async (ml) => !bool(await ml.lotId) && await ml.lotName)) {
  ...
  async _onchangeMoveLineIds() {
    ...
    for (const moveLine of moveLines) {
      ...
      if (lotName.includes(breakingChar)) {
        ...
        await moveLine.set('lotName', splitLines.length ? splitLines[0] : '');
  ...
  async _keyAssignPicking() {
    this.ensureOne();
    let keys: any[] = await this('groupId', 'locationId', 'locationDestId', 'pickingTypeId');
    const partnerId = await this['partnerId'];
    if (bool(partnerId) && (await keys[1].usage === 'transit' || await keys[2].usage === 'transit')) {
      keys = keys.concat(partnerId);
    }
    return keys;
  }
  ...
  async _assignPicking() {
    ...
    for (let [, moves] of groupedMoves) {
      ...
      if (bool(picking)) {
        ...
        // `partnerId` and `ref` field will refer to multiple records. In this case, we chose to wipe them.
        const vals = {};
        if (await moves.some(async (m) => (await picking.partnerId).id != (await m.partnerId).id)) {
          vals['partnerId'] = false;
        }
        if (await moves.some(async (m) => (await picking.origin).ne(await m.origin))) {
          vals['origin'] = false;
        }
        if (bool(vals)) {
          await picking.write(vals);
        }
      }
      else {
  ...
  async _filterByProduct(prod) {
    return this.filtered(async (sm) => (await sm.product_id).eq(prod));
  }
   ...
   * Return a list of commands to update the move lines (write on
    ...
    * @param lotNames A list containing all serial number to assign.
    * @param originMoveLine A move line to duplicate the value from, default to None
    * @returns A list of commands to create/update :class:`stock.move.line`
   */
  async _generateSerialMoveLineCommands(lotNames, originMoveLine?: any) {
    ...
    let locDest = bool(originMoveLine) && await originMoveLine.locationDestId;
    ...
    for (const lotName of lotNames) {
      ...
      if (bool(moveLines)) {
        ...
        qtyByLocation[(await moveLines[0].locationDestId).id] += 1;
        moveLines = moveLines.slice(1);
      }
      // ... or create a new move line with the serial name.
      else {
        const loc = bool(locDest) ? locDest : await (await this['locationDestId'])._getPutawayStrategy(await this['productId'], { quantity: 1, packaging: await this['productPackagingId'], additionalQty: qtyByLocation });
        const moveLineCmd = Object.assign({}, moveLineVals, { lotName, locationDestId: loc.id });
        moveLinesCommands.push([0, 0, moveLineCmd]);
        qtyByLocation[loc.id] += 1;
      }
    }
  ...
  async _isInterWh() {
    this.ensureOne();
    const fromWh = await (await this['locationId']).warehouseId;
    const toWh = await (await this['locationDestId']).warehouseId;
    return bool(fromWh) && bool(toWh) && fromWh.ne(toWh);
  }
  ...
  async _actionConfirm(merge = true, mergeInto: any = false) {
    ...
    const toAssign = new Map2();//OrderedSet);
    for (const move of this) {
    ...
    // procureMethod sometimes changes with certain workflows so just in case, apply to all moves
    ...
    // call `_actionAssign` on every confirmed move which locationId bypasses the reservation + those expected to be auto-assigned
  ...
  async _prepareProcurementValues() {
    ...
    return {
      ...
      'groupId': groupId,
      'routeIds': routeIds,
      'warehouseId': bool(warehouseId) ? warehouseId : await pickingTypeId.warehouseId,
      'priority': priority,
  ...
  async _updateReservedQuantity(need, availableQuantity, locationId, options: { lotId?: any, packageId?: any, ownerId?: any, strict?: boolean } = {}) {
    this.ensureOne();
    let { lotId, packageId, ownerId, strict = true } = options;
    if (!bool(lotId)) {
    ...
    if (productPackagingId.ok && await (await (await productId.productTemplateId).categId).packagingReserveMethod === "full") {
    ...
    // `takenQuantity` is in the quants unit of measure. There's a possibility that the move's
    // unit of measure won't be respected if we blindly reserve this quantity, a common usecase
    // is if the move's unit of measure's rounding does not allow fractional reservation. We chose
    // to convert `takenQuantity` to the move's unit of measure with a down rounding method and
    // then get it back in the quants unit of measure with an half-up roundingMethod. This
    // way, we'll never reserve more than allowed. We do not apply this logic if
    // `availableQuantity` is brought by a chained move line. In this case, `_prepareMoveLineVals`
    ...
    try {
      const self = this;
      await self.env.cr.savepoint(async () => {
        if (!floatIsZero(takenQuantity, { precisionRounding: await uomId.rounding })) {
          quants = await self.env.items('stock.quant')._updateReservedQuantity(
            productId, locationId, takenQuantity, { lotId, packageId, ownerId, strict });
        }
      });
    } catch (e) {
  ...
  */
  ...
  async _actionAssign() {
    ...
    for (const move of await this.filtered(async (m) => ['confirmed', 'waiting', 'partiallyAvailable'].includes(await m.state))) {
      ...
      if (await move._shouldBypassReservation()) {
        ...
        if (missingReservedQuantity && await (await move.productId).tracking === 'serial' && (await (await move.pickingTypeId).useCreateLots || await (await move.pickingTypeId).useExistingLots)) {
        ...
        if (floatIsZero(await move.productUomQty, { precisionRounding: await (await move.productUom).rounding })) {
          ...
          // `qtyDone` is in `ml.productUomId` and, as we will later increase
  ...
  async _actionCancel() {
    ...
    // unlink all associated moveLineIds
  ...
  async _createExtraMove() {
    ...
    if (floatCompare(quantityDone, productUomQty, { precisionRounding: rounding }) > 0) {
      ...
      for (const field of await this._prepareMergeMovesDistinctFields()) {
  ...
  async _actionDone(cancelBackorder = false) {
    ...
    for (const move of movesTodo) {
      ...
      if (floatCompare(await move.quantityDone, await move.productUomQty, { precisionDigits: rounding }) < 0) {
        ...
        const qtySplit = await (await move.productUom)._computeQuantity(await move.productUomQty - await move.quantityDone, await (await move.productId).uomId, { roundingMethod: 'HALF-UP' });
    ...
    const moveDestsPerCompany = new DefaultDict(() => this.env.items('stock.move'));
  ...
  async _split(qty, restrictPartnerId = false) {
    ...
    // precision and not the move's UOM to handle case where the `quantityDone` is not
  ...
  async _recomputeState() {
    const movesStateToWrite = new DefaultDict(() => new Set());
    for (const move of this) {
  ...
  /**
   * Return the move's document, used by `report.stock.product.replenishment`
  ...
  async _setQuantityDonePrepareVals(qty: number) {
    ...
    for (const ml of moveLineIds) {
      ...
      // Assign qtyDone and explicitly round to make sure there is no inconsistency between
      // ml.qtyDone and qty.
  ...
  async _setQuantitiesToReservation() {
    for (const move of this) {
      ...
      const pickingType = await move.pickingTypeId;
      for (const moveLine of await move.moveLineIds) {
        if (await move.hasTracking == 'none' ||
          (await pickingType.useExistingLots && bool(await moveLine.lotId)) ||
          (await pickingType.useCreateLots && await moveLine.lotName) ||
          (! await pickingType.useExistingLots && ! await pickingType.useCreateLots)) {
  ...
  async _adjustProcureMethod() {
    ...
    const mtsoProductsByLocations = new Map2<any, any>();//(list)
    const mtsoNeededQtiesByLoc = new Map2(); //dict)
  ...
  async _triggerScheduler() {
    ...
    const orderpointsByCompany = new DefaultDict(() => this.env.items('stock.warehouse.orderpoint'));
    const orderpointsContextByCompany = new Map2();//(dict)
    for (const move of this) {
  ...
  /**
   * Check for and trigger actionAssign for confirmed/partiallyAvailable moves related to done moves.
  ...
  async _triggerAssign() {
    ...
    await movesToReserve._actionAssign();
  }
  ...
  async _rollupMoveDests(seen: Set<number>) {
    for (const dst of await this['moveDestIds']) {
      if (!seen.has(dst.id)) {
        seen.add(dst.id)
        await dst._rollupMoveDests(seen);
      }
    }
    return seen;
  }
  ...
  async _getForecastAvailabilityOutgoing(warehouse) {
    ...
    const [inDomain, outDomain] = await this.env.items('report.stock.product.replenishment')._moveConfirmedDomain(null, productIds.ids, whLocationQuery);
    ...
    const outsPerProduct = new DefaultDict(() => []);
    const reservedOutsPerProduct = new DefaultDict(() => []);
    const insPerProduct = new DefaultDict(() => []);
    ...
    const result = new MapKey();// DefaultMapKey(() => [0.0, false]);
