...
class Picking extends Model {
  ...
  async _computeHasTracking() {
    for (const picking of this) {
      await picking.set('hasTracking', await (await picking.moveLines).some(async (m) => await m.hasTracking !== 'none'));
    }
  }
  ...
  async _computeProductsAvailability() {
    ...
    for (const picking of pickings) {
      // In case of draft the behavior of forecastAvailability is different : if forecastAvailability < 0 then there is a issue else not.
  ...
  async _computeState() {
    const pickingMovesStateMap = new Map2();
    const pickingMoveLines = new Map2();
  ...
  /**
   * Call `_actionDone` on the `stock.move` of the `stock.picking` in `self`.
  ...
  async _actionDone() {
    ...
    await todoMoves._actionDone(this.env.context['cancelBackorder']);
  ...
  /**
   * This method checks that all product of the package (quant) are well present in the moveLineIds of the picking.
   * @param pack 
   * @returns 
   */
 ...
 */
   ...
   * Generic method to log activity. To use with
      ...
      eg: 'moveDestIds' if we use move as record (previous example)
  ...
  async _logActivityGetDocuments(origObjChanges: MapKey<any, any>, streamField, stream, sortedMethod?: Function, groupbyMethod?: Function) {
    ...
    const documents = {};
    for (let [key, moves] of groupedMoves) {
      const [parent, responsible] = key.split('@');
      if (!bool(parent)) {
        continue;
      }
      ...
      for (const move of moves) {
        for (const origObject of moveToOrigObjectRel[move]) {
          renderingContext.set(move, [origObject, origObjChanges.get(origObject)]);
        }
      }
   ...
   * Log a note for each documents, responsible pair in
    ...
    * @param renderMethod a static function that will generate
      the html note to log on the activity. The renderMethod should
      use the args:
          - renderingContext dict: value of the documents argument
      the renderMethod should return a string with an html format
    * @param documents A tuple (document, responsible) as key.
      An activity will be log by key. A renderingContext as value.
      If used with _logActivityGetDocuments. In 'DOWN' stream
      cases the rendering_context will be a dict with format:
      {'stream_object': ('origObject', newQty, oldQty)}
   ...
   * Log an activity on picking that follow moves. The note
    ...
    * @returns 
    */
  async _logLessQuantitiesThanExpected(moves: MapKey<any, any>) {
    const self = this;
  ...
  async actionPickingMoveTree() {
    ...
    action['domain'] = [['pickingId', 'in', this.ids]];
    return action;
  }
