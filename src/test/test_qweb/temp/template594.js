async function* template594(self, values, log={}) {
    ...
    async function* tCallContent(self, values, log) {
        ...
        for (const [index, item] of enumerate(t_foreach_0)) {          values["o_index"] = index;          if (hasValue_2) {            [values["o"], values["o_value"]] = item;          }          else {            values["o"] = item;            values["o_value"] = item;          }          values["o_first"] = values["o_index"] == 0;          if (size_1 != null) {            values["o_last"] = index + 1 === size_1;          }          values["o_odd"] = index % 2;          values["o_even"] = ! values["o_odd"];          values["o_parity"] = values["o_odd"] ? 'odd' : 'even';
            log["lastPathNode"] = "/t/t/t";
            ...
            if (await values['o']._getNameInvoiceReport() == 'account.reportInvoiceDocument') {
                ...
                tCallOptions = Object.assign({}, compileOptions);                Object.assign(tCallOptions, {'callerTemplate': "594", 'lastPathNode': "/t/t/t/t[2]" })
    ...
    tCallOptions = Object.assign({}, compileOptions);    Object.assign(tCallOptions, {'callerTemplate': "594", 'lastPathNode': "/t/t" })
    for await (const val of (await self._compile("web.htmlContainer", tCallOptions))(self, tCallValues)) {            yield val;          }
    yield `
        `;
    } catch(e) {
        _debug('Error in %s at %s: %s', 'template594', log["lastPathNode"], e);
        _debug(String(template594)); // detail code
        throw e;
    }
}