async function* template1147(self, values, log={}) {
    ...
    try {
        // _debug(String(template1147.name)); // detail code
    log["lastPathNode"] = "/t";
    ...
    async function* tCallContent(self, values, log) {
        ...
        for (const [index, item] of enumerate(t_foreach_0)) {          values["sortby_index"] = index;          if (hasValue_2) {            [values["sortby"], values["sortby_value"]] = item;          }          else {            values["sortby"] = item;            values["sortby_value"] = item;          }          values["sortby_first"] = values["sortby_index"] == 0;          if (size_1 != null) {            values["sortby_last"] = index + 1 === size_1;          }          values["sortby_odd"] = index % 2;          values["sortby_even"] = ! values["sortby_odd"];          values["sortby_parity"] = values["sortby_odd"] ? 'odd' : 'even';
            log["lastPathNode"] = "/t/t/div/div[1]/div/div[1]/div[0]/div[0]/div/t";
        ...
        if (values['bins']) {
            ...
            yield `>
                                    <colgroup>
                                        ...
                                        `;
            let size_4;            let t_foreach_3 = values['ppr'] ?? [];            if (typeof(t_foreach_3) === 'number') {              size_4 = t_foreach_3;              values["p_size"] = size_4;              t_foreach_3 = range(size_4);            }            else if ('_length' in t_foreach_3) {              size_4 = t_foreach_3._length;              values["p_size"] = size_4;            }            else if ('length' in t_foreach_3) {              size_4 = t_foreach_3.length;              values["p_size"] = size_4;            }            else if ('size' in t_foreach_3) {              size_4 = t_foreach_3.size;              values["p_size"] = size_4;            }            else {              size_4 = null;            }            let hasValue_5 = false;            if (t_foreach_3 instanceof Map || t_foreach_3 instanceof MapKey) {              t_foreach_3 = t_foreach_3.entries();              hasValue_5 = true;            }            if (typeof t_foreach_3 === 'object' && !isIterable(t_foreach_3)) {              t_foreach_3 = Object.entries(t_foreach_3);              hasValue_5 = true;            }
            for (const [index, item] of enumerate(t_foreach_3)) {              values["p_index"] = index;              if (hasValue_5) {                [values["p"], values["p_value"]] = item;              }              else {                values["p"] = item;                values["p_value"] = item;              }              values["p_first"] = values["p_index"] == 0;              if (size_4 != null) {                values["p_last"] = index + 1 === size_4;              }              values["p_odd"] = index % 2;              values["p_even"] = ! values["p_odd"];              values["p_parity"] = values["p_odd"] ? 'odd' : 'even';
                log["lastPathNode"] = "/t/t/div/div[1]/div/div[1]/div[2]/table/colgroup/col";
                yield `<col/>`;
            }
