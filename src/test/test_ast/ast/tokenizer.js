...
export const tokenize = function (input, current=0, exitOn='') {
	const verbose = true;
	let lineNumber = 0;
	let assigner = false;
	...
	function start(input, current, exitOn) {
		...
		while (current < input.length) {
			...
			const doubleQuotedString = stringConditional('"',char, input, current);
			if (doubleQuotedString.type) {
				tokens.push({type: doubleQuotedString['type'], value: doubleQuotedString['value']});
			...
			const singleQuotedString = stringConditional("'", char, input, current);
			if (singleQuotedString.type) {
				tokens.push({type: singleQuotedString['type'], value: singleQuotedString['value']});
			...
			const backTickString = maybeBackTickStringCheck(char, input, current);
			if (backTickString.type) {
				tokens.push({type: backTickString['type'], value: backTickString['value']});
