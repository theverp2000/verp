 ...
 *      time: vj.extras.time,
 *  };
 *  var result = vj.eval(some_python_expression, context);
 */
...
// Port from pypy/lib_pypy/datetime.js
...
datetime.datetime = vj.type('datetime', null, {
    __init__: function () {
        ...
        var args = vj.VJ_parseArgs(arguments, [
            ...
            ['microsecond', zero], ['tzinfo', vj.null]
        ]);
        for(var key in args) {
    ...
    replace: function () {
        var args = vj.VJ_parseArgs(arguments, [
            ['year', vj.null], ['month', vj.null], ['day', vj.null],
            ['hour', vj.null], ['minute', vj.null], ['second', vj.null],
            ['microsecond', vj.null] // FIXME: tzinfo, can't use None as valid input
        ]);
        var params = {};
        for(var key in args) {
            if (!args.hasOwnProperty(key)) { continue; }
            ...
            var arg = args[key];
            params[key] = (arg === vj.null ? this[key] : asJS(arg));
        }
    ...
    add: function() {
        var args = vj.VJ_parseArgs(arguments, [
            ['years', vj.null], ['months', vj.null], ['days', vj.null],
            ['hours', vj.null], ['minutes', vj.null], ['seconds', vj.null],
        ]);
    ...
    subtract: function() {
        ...
        var params = {};
        for (var key in args) {
            params[key] = (args[key] === vj.null ? args[key] : vj.float.fromJSON(-asJS(args[key])));
        }
